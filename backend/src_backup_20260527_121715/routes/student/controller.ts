...
export async function getLevel(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const profile = await queryOne<{
      ...
      `SELECT level_a, level_b, track, task_count, six_dim_scores, opc_label
    ...
    // 计算升级进度
    const nextLevelRequirements = calculateNextLevelRequirements(profile.level_a, profile.level_b);
    ...
    res.json({
      ...
      data: {
        currentLevel: {
          a: profile.level_a,
          b: profile.level_b,
          label: `${profile.track}${profile.level_a}.${profile.level_b}`,
        },
        track: profile.track,
        taskCount: profile.task_count,
        opcLabel: profile.opc_label,
        nextLevelRequirements,
        canUpgrade: profile.task_count >= nextLevelRequirements.requiredTasks,
      },
    });
  } catch (err) {
    next(err);
...
export async function checkLevelUpgrade(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const profile = await queryOne<{
      ...
      'SELECT level_a, level_b, task_count, six_dim_scores FROM student_profiles WHERE user_id = $1',
      [userId]
    );
    ...
    const requirements = calculateNextLevelRequirements(profile.level_a, profile.level_b);
    const canUpgrade = profile.task_count >= requirements.requiredTasks;

    res.json({
      success: true,
      data: {
        canUpgrade,
        currentTaskCount: profile.task_count,
...
export async function getNextLevel(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const profile = await queryOne<{
      ...
      'SELECT level_a, level_b, track, task_count FROM student_profiles WHERE user_id = $1',
      [userId]
    );
    ...
    res.json({
      ...
      data: {
        ...
        requirements: {
          ...
          currentProgress: profile.task_count,
          progressPercentage: Math.min(100, (profile.task_count / requirements.requiredTasks) * 100),
        },
...
export async function getTestResult(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const profile = await queryOne<{
      ...
      `SELECT six_dim_scores, opc_label, opc_label_secondary, track, level_a, level_b
       FROM student_profiles
       WHERE user_id = $1`,
      [userId]
    );
