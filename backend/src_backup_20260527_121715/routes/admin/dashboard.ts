...
router.get('/stats/today', async (req, res, next) => {
  try {
    ...
    // 今日对话总数
    const totalConversations = await queryOne<{ count: number }>(
    ...
    // 需求确认成功率
    const confirmationStats = await queryOne<{ total: number; confirmed: number }>(
      `SELECT 
         COUNT(*) as total,
    ...
    // AI审核通过率
    const reviewStats = await queryOne<{ total: number; passed: number }>(
      `SELECT 
         COUNT(*) as total,
...
router.get('/students', async (req, res, next) => {
  try {
    ...
    // 计算通过率
    const studentsWithRate = students.map((s: any) => ({
      ...s,
...
router.get('/students/:studentId', async (req, res, next) => {
  try {
    ...
    const tasks = await query<any>(
      `SELECT t.*, ta.status, ta.started_at, ta.completed_at
       FROM tasks t
