...
import * as matchingCtrl from './matchingController';
...
// ============================================
// 企业端匹配API
router.post('/:taskId/trigger-matching', authenticate, requireRole('company'), matchingCtrl.triggerMatching);
router.post('/:taskId/rematch', authenticate, requireRole('company'), matchingCtrl.rematchTask);
router.get('/:taskId/matched-students', authenticate, requireRole('company'), matchingCtrl.getMatchedStudents);
router.post('/:taskId/push-to-students', authenticate, requireRole('company'), matchingCtrl.pushToStudents);
router.get('/:taskId/matching-stats', authenticate, requireRole('company'), matchingCtrl.getMatchingStats);

// 学生端匹配API
router.get('/students/recommended-tasks', authenticate, requireRole('student'), matchingCtrl.getRecommendedTasks);
router.get('/:taskId/translation', authenticate, matchingCtrl.getTaskTranslation);
router.post('/:taskId/accept-recommendation', authenticate, requireRole('student'), matchingCtrl.acceptRecommendation);

// 公开/登录可见
router.get('/market', authenticate, studentCtrl.getMarketTasks);
router.get('/matched', authenticate, requireRole('student'), studentCtrl.getMatchedTasks);
...
router.post('/:id/progress', authenticate, requireRole('student'), studentCtrl.updateProgress);
