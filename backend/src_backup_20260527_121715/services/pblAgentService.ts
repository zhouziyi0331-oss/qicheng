...
// 苏格拉底式PBL导师Agent服务
export class PBLAgentService {
  // ============================================
  // 1. 项目初始化
  // ============================================

  async initializeProject(userId: string, initialProblem: string) {
    ...
    try {
      ...
      // 记录对话
      await client.query(
        `INSERT INTO pbl_socratic_dialogues (
          project_id, role, content, dialogue_type, socratic_technique
        ) VALUES ($1, 'agent', $2, 'question', 'clarifying')`,
        [project.id, openingQuestions]
      );
  ...
  // 分析用户的初始问题
  private async analyzeInitialProblem(problem: string) {
...
请以JSON格式返回：
{
  ...
  // 生成开场问题
  private async generateOpeningQuestions(project: any) {
...
请直接返回问题列表，每个问题一行。`;
  ...
  async conductSocraticDialogue(
    ...
    try {
      ...
      // 生成Agent响应
      const response = await this.generateSocraticResponse(
        project,
        userMessage,
        dialogueHistory,
        memory,
      ...
      await client.query(
        ...
        [projectId, response.content, response.type, response.technique]
      );
  ...
  // 检测用户是否卡壳
  private async detectIfStuck(userMessage: string, history: any[]) {
...
请根据对话历史和用户最新消息，生成你的响应。`;
  ...
  // 分析响应类型
  private analyzeResponseType(response: string, isStuck: boolean) {
  ...
  async guideTaskDecomposition(projectId: string, taskTitle: string) {
    const client = await pool.connect();
    try {
      // 创建任务
      const taskResult = await client.query(
        `INSERT INTO pbl_task_decompositions (
  ...
  // 生成任务拆解引导问题
  private async generateDecompositionQuestions(task: any) {
...
请直接返回问题列表。`;
  ...
  // 评估任务拆解质量
  async evaluateDecomposition(taskId: string, subtasks: string[]) {
    const client = await pool.connect();
    try {
      const task = await client.query(
        'SELECT * FROM pbl_task_decompositions WHERE id = $1',
        [taskId]
      );

      const evaluation = await this.analyzeDecompositionQuality(
        task.rows[0],
        subtasks
      );
      ...
      // 更新任务
      await client.query(
        `UPDATE pbl_task_decompositions
  ...
  // 分析拆解质量
  private async analyzeDecompositionQuality(task: any, subtasks: string[]) {
...
以JSON格式返回：
{
  ...
  async suggestMVPSolution(taskId: string, userContext: string) {
    const client = await pool.connect();
    try {
      const task = await client.query(
        `SELECT t.*, p.domain, p.initial_problem
  ...
  // 生成MVP方案
  private async generateMVPSolution(task: any, userContext: string) {
...
以JSON格式返回：
{
  ...
  async executeCode(
    ...
    try {
      ...
      await client.query(
        ...
        [projectId, taskId, language, code, result.status, result.output, result.execution_time]
      );
  ...
  async guideReflection(projectId: string, reflectionType: string) {
...
请直接返回问题列表。`;
  ...
  async saveReflectionLog(
    ...
    try {
      const result = await client.query(
        `INSERT INTO pbl_reflection_logs (
  ...
  private async getAgentMemory(userId: string, projectId?: string) {
    const client = await pool.connect();
    try {
  ...
  private extractMemories(observation: any) {
    const memories: any[] = [];
  ...
  private async getProject(projectId: string) {
    ...
    try {
      const result = await client.query(
        'SELECT * FROM pbl_projects WHERE id = $1',
        [projectId]
      );
      return result.rows[0];
    } finally {
  ...
  private async getDialogueHistory(projectId: string, limit: number = 20) {
    const client = await pool.connect();
    try {
      const result = await client.query(
        `SELECT * FROM pbl_socratic_dialogues
         WHERE project_id = $1
...
export const pblAgentService = new PBLAgentService();
