...
class TeacherMemoryService {
   ...
   * 应该定期运行（例如每天一次）
   */
  async consolidateMemory(studentId: string): Promise<void> {
    try {
      logger.info(`Starting memory consolidation for student ${studentId}`);

      // 获取未巩固的短期记忆
      const shortTermMemories = await query(
      ...
      if (shortTermMemories.length < 3) {
        logger.info(`Not enough memories to consolidate for student ${studentId}`);
        return;
      }
      ...
      // 使用AI分析并更新长期理解
      const updatedUnderstanding = await this.analyzeAndUpdateUnderstanding(
        shortTermMemories,
        longTermMemory,
        thinkingRecords,
        keyMoments
      ...
      // 更新长期记忆
      await query(
        `UPDATE teacher_long_term_memory
      ...
      // 标记短期记忆为已巩固
      await query(
        `UPDATE teacher_short_term_memory
   ...
   * 分析并更新长期理解
   */
  private async analyzeAndUpdateUnderstanding(
...
${shortTermMemories.slice(0, 10).map((m: any) =>
  `- ${m.student_state}\n  老师回复：${m.teacher_response}`
).join('\n')}

## 最近的深度思考
${thinkingRecords.slice(0, 3).map((t: any) =>
  `- 洞察：${t.insight?.understanding || '无'}`
).join('\n')}

## 关键时刻
${keyMoments.map((k: any) =>
  `- ${k.event_description}：${k.teacher_insight}`
).join('\n')}
...
只返回JSON，不要其他文字：`;
      ...
      if (!jsonMatch) {
        logger.warn('Failed to parse understanding JSON');
   ...
   * 默认理解（当AI调用失败时）
   */
  private getDefaultUnderstanding(currentMemory: any): any {
    return {
   ...
   * 获取学生的长期记忆
   */
  async getLongTermMemory(studentId: string): Promise<any> {
    try {
   ...
   * 批量巩固记忆（定时任务）
   */
  async consolidateAllMemories(): Promise<void> {
    try {
      // 获取有未巩固记忆的学生
      ...
      logger.info(`Found ${students.length} students with memories to consolidate`);
  ...
  async cleanupOldMemories(): Promise<void> {
    try {
      const result = await query(
        `DELETE FROM teacher_short_term_memory
...
export default new TeacherMemoryService();
