...
aiTaskQueue.process(async (job) => {
  ...
  logger.info(`Processing AI task: ${type}`, { jobId: job.id });

  try {
    switch (type) {
      case AITaskType.PROFILE_ANALYSIS:
        return await processProfileAnalysis(data as any);

      case AITaskType.PROJECT_CONDITION_ANALYSIS:
        return await processProjectConditionAnalysis(data as any);

      case AITaskType.MATCH_ANALYSIS:
        return await processMatchAnalysis(data as any);

      case AITaskType.MENTOR_GUIDANCE:
        return await processMentorGuidance(data as any);

      default:
        throw new Error(`Unknown task type: ${type}`);
    }
  } catch (error) {
    logger.error(`AI task failed: ${type}`, { error, jobId: job.id });
    throw error;
  }
});
