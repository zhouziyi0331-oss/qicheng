...
import personalizedExpressionService from './personalizedExpressionService';
import teacherMemoryService from './teacherMemoryService';
...
class DeepThinkingTeacherService {
   ...
   */
  async onTaskStart(studentId: string, taskId: string, taskDescription: string): Promise<string> {
    try {
      logger.info(`Deep thinking teacher: Task start for student ${studentId}`);
      ...
      // 深度思考并生成回复
      const response = await personalizedExpressionService.generateResponse({
        studentId,
  ...
  async onStudentStuck(
    ...
    try {
      logger.info(`Deep thinking teacher: Student stuck for student ${studentId}`);
  ...
  async onWorkRejected(
    ...
    try {
      logger.info(`Deep thinking teacher: Work rejected for student ${studentId}, attempt ${attemptNumber}`);
      ...
      // 记录企业反馈观察
      await teacherObservationService.recordCompanyFeedback({
        companyId: '', // 需要从任务获取
        studentId,
        taskId,
  ...
  async onMilestoneComplete(
    ...
    try {
      logger.info(`Deep thinking teacher: Milestone complete for student ${studentId}`);
      ...
      // 触发记忆巩固（异步）
      teacherMemoryService.consolidateMemory(studentId).catch(err => {
        logger.error('Failed to consolidate memory:', err);
      });

      // 生成鼓励
      const response = await personalizedExpressionService.quickResponse(
        studentId,
        `学生完成了${milestone}，已完成${tasksCompleted}个任务`,
        'encouragement'
      );
   ...
   */
  async getStudentValueDescription(studentId: string): Promise<string> {
    try {
      logger.info(`Deep thinking teacher: Get value description for student ${studentId}`);

      // 获取长期记忆
      const memory = await teacherMemoryService.getLongTermMemory(studentId);
   ...
   */
  async proactiveInsight(studentId: string): Promise<string | null> {
    try {
      // 获取最近的行为
      const recentBehaviors = await teacherObservationService.getRecentBehaviors(studentId, 10);
      ...
      // 如果求助频率突然升高（超过50%）
      if (helpRate > 0.5) {
        logger.info(`Proactive insight: High help rate for student ${studentId}`);
   ...
   * 获取学生的深度理解（用于调试和展示）
   */
  async getStudentUnderstanding(studentId: string): Promise<any> {
    try {
      const memory = await teacherMemoryService.getLongTermMemory(studentId);
      const keyMoments = await teacherObservationService.getKeyMoments(studentId, 5);
      const recentBehaviors = await teacherObservationService.getRecentBehaviors(studentId, 10);
...
export default new DeepThinkingTeacherService();
