...
class VectorGenerationService {
   ...
   * 生成任务向量
   */
  async generateTaskVectors(taskId: string): Promise<TaskVectors> {
    try {
      // 获取任务信息
      ...
      const combinedEmbedding = titleEmbedding.map((val, idx) =>
        val * 0.3 + descriptionEmbedding[idx] * 0.7
      );

      logger.info(`Generated vectors for task ${taskId}`);
      ...
      logger.error(`Failed to generate task vectors for ${taskId}:`, error);
      throw error;
    }
  }
  ...
  async generateStudentProfileSummary(studentId: string): Promise<string> {
    try {
      ...
      const opcResults = await queryOne<{
        ...
        `SELECT opc_openness as openness, opc_persistence as persistence,
      ...
      const projectData = await query<{
        ...
        `SELECT t.title, t.description, tr.feedback as client_feedback,
                tr.quality_score
...
请严格按照上述结构生成摘要（约200字），每个维度用一句话描述：`;
      ...
      logger.info(`Generated profile summary for student ${studentId}`);

      return summary;
    } catch (error) {
      logger.error(`Failed to generate profile summary for student ${studentId}:`, error);
      throw error;
    }
  }
   ...
   * 生成学生向量（旧方法，保留用于兼容）
   */
  async generateStudentVectors(studentId: string): Promise<StudentVectors> {
    try {
      // 获取学生能力数据
      ...
      const combinedVector = await this.generateEmbedding(
        `${student?.username || ''}\n${student?.bio || ''}\n${skillsText}\n${trajectoryText}\n${qualityText}\n${preferenceText}`,
        1536
      );

      logger.info(`Generated vectors for student ${studentId}`);
      ...
      logger.error(`Failed to generate student vectors for ${studentId}:`, error);
      throw error;
    }
  }
  ...
  async updateTaskEmbedding(taskId: string): Promise<void> {
    try {
      ...
      logger.info(`Updated task requirement vector for ${taskId} using structured summary`);
    } catch (error) {
      logger.error(`Failed to update task embedding for ${taskId}:`, error);
      throw error;
    }
  }
  ...
  async updateStudentEmbedding(studentId: string): Promise<void> {
    try {
      ...
      logger.info(`Updated student profile vector for ${studentId} using structured summary`);
    } catch (error) {
      logger.error(`Failed to update student embedding for ${studentId}:`, error);
      throw error;
    }
  }
   ...
   * 批量更新所有任务的embedding
   */
  async updateAllTaskEmbeddings(): Promise<void> {
    try {
      const tasks = await query<{ id: string }>(
        `SELECT id FROM tasks WHERE status != 'deleted' ORDER BY created_at DESC`
      );

      logger.info(`Starting to update embeddings for ${tasks.length} tasks`);
      ...
      logger.info(`Completed updating embeddings for ${tasks.length} tasks`);
    } catch (error) {
      logger.error('Failed to update all task embeddings:', error);
      throw error;
    }
  }
   ...
   * 批量更新所有学生的embedding
   */
  async updateAllStudentEmbeddings(): Promise<void> {
    try {
      const students = await query<{ student_id: string }>(
        `SELECT student_id FROM student_capabilities ORDER BY created_at DESC`
      );

      logger.info(`Starting to update embeddings for ${students.length} students`);
      ...
      for (const student of students) {
        try {
          ...
          logger.error(`Failed to update embedding for student ${student.student_id}:`, error);
          // 继续处理下一个学生
        }
      }

      logger.info(`Completed updating embeddings for ${students.length} students`);
    } catch (error) {
      logger.error('Failed to update all student embeddings:', error);
      throw error;
    }
  }
