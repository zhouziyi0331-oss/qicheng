...
class GrowthComparisonService {
   ...
   * 生成成长对比数据
   */
  async generateComparison(studentId: string): Promise<GrowthComparisonData> {
    ...
    try {
      ...
      logger.info('[GrowthComparison] 成长对比生成成功', {
        studentId,
        daysOnPlatform,
      ...
      logger.error('[GrowthComparison] 生成成长对比失败:', error);
      throw error;
    } finally {
      client.release();
    }
   ...
   * 获取能力对比数据（六维雷达图）
   */
  private async getAbilityComparison(studentId: string): Promise<AbilityComparison[]> {
    const client = await pool.connect();
    ...
    try {
      ...
      const comparison: AbilityComparison[] = dimensions.map(dim => {
      ...
      logger.error('[GrowthComparison] 获取能力对比失败:', error);
      return this.getDefaultAbilityComparison();
    } finally {
      client.release();
    }
   ...
   * 获取关键指标对比
   */
  private async getMetricsComparison(studentId: string): Promise<MetricComparison[]> {
    const client = await pool.connect();
    ...
    try {
      ...
      const ratingResult = await client.query(
        `SELECT COALESCE(AVG(final_score), 0) as avg_score,
                COUNT(*) as count
         FROM task_submissions
         WHERE student_id = $1 AND status = 'approved' AND final_score IS NOT NULL`,
        [studentId]
      );
      ...
      logger.error('[GrowthComparison] 获取指标对比失败:', error);
      return [];
    } finally {
      client.release();
    }
   ...
   * 默认能力对比数据
   */
  private getDefaultAbilityComparison(): AbilityComparison[] {
...
export default new GrowthComparisonService();
