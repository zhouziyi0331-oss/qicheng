...
class MentorRelationshipService {
   ...
   * 检查学生是否有资格成为引路人
   */
  async checkQualification(studentId: string): Promise<{
    qualified: boolean;
    reason: string;
    requirements: {
      completedTasks: { current: number; required: number; met: boolean };
    ...
    try {
      ...
      if (!qualified) {
        const missing = [];
        if (!requirements.completedTasks.met) missing.push('完成任务数不足');
        if (!requirements.level.met) missing.push('等级不足');
        if (!requirements.avgRating.met) missing.push('评分不足');
        reason = missing.join('，');
      }
  ...
  async applyToBeMentor(params: {
    ...
    specialties?: string[];
  }): Promise<{ success: boolean; applicationId?: number; message: string }> {
    const client = await pool.connect();
    try {
      // 检查资格
      ...
      logger.info('[MentorRelationship] 申请成为引路人', {
   ...
   * 匹配策略：相似OPC + 同赛道 + 可用名额
   */
  async findMentorForStudent(studentId: string): Promise<MentorMatch[]> {
    const client = await pool.connect();
    try {
      // 获取学生的OPC标签和兴趣赛道
      ...
      // 计算匹配分数
      const matches: MentorMatch[] = mentors.rows.map(mentor => {
        let matchScore = 0;
...
export default new MentorRelationshipService();
