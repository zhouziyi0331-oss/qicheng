...
class TaskInvitationService {
   ...
   * 获取学生的所有邀约
   */
  async getMyInvitations(studentId: string): Promise<MyInvitationsResponse> {
    try {
      // 先自动过期旧邀约
      ...
      const invitations = result.rows as InvitationWithTask[];
      ...
      logger.error('Failed to get my invitations', { error, studentId });
      throw error;
    }
  }
   ...
   * 接受邀约
   */
  async acceptInvitation(invitationId: string, studentId: string): Promise<TaskInvitation> {
    ...
    try {
      ...
      if (new Date(invitation.expires_at) < new Date()) {
        // 标记为过期
        await client.query(
          `UPDATE task_invitations SET status = 'expired' WHERE id = $1`,
          [invitationId]
        );
      ...
      await client.query(
        ...
        [invitation.task_id, invitationId]
      );

      // 7. 创建订单（关联到task_assignments或orders表）
      // TODO: 根据实际业务逻辑创建订单
      await client.query(
        `INSERT INTO task_assignments (task_id, student_id, status, assigned_at)
         VALUES ($1, $2, 'accepted', NOW())
         ON CONFLICT (task_id, student_id) DO NOTHING`,
        [invitation.task_id, studentId]
      );

      await client.query('COMMIT');

      logger.info('Invitation accepted', { invitationId, studentId, taskId: invitation.task_id });

      return updateResult.rows[0];
    } catch (error) {
      await client.query('ROLLBACK');
      logger.error('Failed to accept invitation', { error, invitationId, studentId });
      throw error;
    } finally {
      client.release();
    }
   ...
   * 拒绝邀约
   */
  async declineInvitation(invitationId: string, studentId: string): Promise<TaskInvitation> {
    try {
      // 检查邀约是否存在且属于该学生
      const checkQuery = `
        SELECT * FROM task_invitations
        WHERE id = $1 AND student_id = $2
      `;
      const checkResult = await pool.query(checkQuery, [invitationId, studentId]);
      ...
      const updateQuery = `
        ...
        RETURNING *
      `;
      const updateResult = await pool.query(updateQuery, [invitationId]);

      logger.info('Invitation declined', { invitationId, studentId });

      return updateResult.rows[0];
    } catch (error) {
      logger.error('Failed to decline invitation', { error, invitationId, studentId });
      throw error;
    }
  }
   ...
   * 获取单个邀约详情
   */
  async getInvitationDetail(invitationId: string, studentId: string): Promise<InvitationWithTask | null> {
    try {
      const query = `
        SELECT
          ti.*,
      ...
      const result = await pool.query(query, [invitationId, studentId]);

      if (result.rows.length === 0) {
        return null;
      }

      return result.rows[0] as InvitationWithTask;
    } catch (error) {
      logger.error('Failed to get invitation detail', { error, invitationId, studentId });
      throw error;
    }
  }
  ...
  async createInvitation(params: {
    ...
    try {
      ...
      logger.info('Invitation created', { taskId: params.taskId, studentId: params.studentId });

      return result.rows[0];
    } catch (error) {
      logger.error('Failed to create invitation', { error, params });
      throw error;
    }
  }
  ...
  async createBatchInvitations(
    ...
    try {
      ...
      const invitations: TaskInvitation[] = [];
      ...
      logger.info('Batch invitations created', { taskId, count: students.length });

      return invitations;
    } catch (error) {
      await client.query('ROLLBACK');
      logger.error('Failed to create batch invitations', { error, taskId });
      throw error;
    } finally {
      client.release();
    }
