...
class DesignatedMasterService {
  ...
  async getMasterList(filter: MasterFilter = {}): Promise<MasterInfo[]> {
    ...
    try {
      ...
      const masters: MasterInfo[] = result.rows.map(row => ({
   ...
   * 获取单个大师详情
   */
  async getMasterDetail(masterId: string): Promise<MasterInfo | null> {
    const client = await pool.connect();
    try {
  ...
  async sendInvitation(input: InvitationInput): Promise<InvitationResponse> {
    ...
    try {
      ...
      const masterCheck = await client.query(
        `SELECT master_accept_designated, master_min_order_price
         FROM users
  ...
  async respondToInvitation(
    ...
    try {
      ...
      const invitation = await client.query(
        `SELECT id, task_id, enterprise_id, master_id, enterprise_offer, status
   ...
   * 创建任务分配（大师接受邀请后）
   */
  private async createTaskAssignment(taskId: string, masterId: string, agreedPrice: number): Promise<void> {
    const client = await pool.connect();
    try {
      // 创建任务分配记录
      await client.query(
...
export default new DesignatedMasterService();
