...
class InstantGrowthSummaryService {
   ...
   * 收集生成总结所需的所有数据
   */
  private async collectData(orderId: string) {
    const client = await pool.connect();
    try {
      // 1. 获取订单信息
      const orderResult = await client.query<OrderData>(
        `SELECT id, title, student_price, time_spent_hours, client_rating, completed_at, student_id
      ...
      const initialProfileResult = await client.query<StudentProfile>(
      ...
      const currentProfileResult = await client.query<StudentProfile>(
        `SELECT * FROM user_ability_profiles
      ...
      // 4. 获取本次订单的导师对话记录
      const mentorSessionsResult = await client.query<MentorSession>(
        `SELECT session_summary, key_insights, stuck_points
         FROM mentor_sessions
         WHERE student_id = $1 AND order_id = $2
         ORDER BY created_at DESC`,
        [order.student_id, orderId]
      );
      const mentorSessions = mentorSessionsResult.rows;

      // 5. 获取本次订单的成长观察
      const growthObservationsResult = await client.query<GrowthObservation>(
        `SELECT observation_type, observation_content, skills_observed
      ...
      const historyResult = await client.query(
        `SELECT
         ...
         FROM orders
         WHERE student_id = $1 AND status = 'completed' AND id != $2`,
        [order.student_id, orderId]
      );
   ...
   * 检查是否已有缓存的总结
   */
  private async checkCache(orderId: string): Promise<InstantSummary | null> {
    const client = await pool.connect();
    try {
      const result = await client.query(
        `SELECT summary_json FROM growth_summary_cache WHERE order_id = $1`,
        [orderId]
      );
   ...
   * 调用AI生成总结（严格按照技术规格）
   */
  private async callAI(data: any, retryCount: number = 0): Promise<InstantSummary> {
...
`;
    ...
    try {
      await client.query(
        `INSERT INTO growth_summary_cache
         ...
         SET summary_json = $3, generated_at = NOW()`,
        [userId, orderId, JSON.stringify(summary), 'claude-3-5-sonnet-20241022', 0]
      );
    } finally {
      client.release();
    }
   ...
   */
  private async updateGrowthObservation(
    orderId: string,
    summary: InstantSummary
  ): Promise<void> {
    const client = await pool.connect();
    try {
      // 查找本次订单的最新成长观察记录
      const result = await client.query(
        `SELECT id FROM mentor_growth_observations
         WHERE order_id = $1
      ...
      if (result.rows.length > 0) {
        ...
        await client.query(
          `UPDATE mentor_growth_observations
   ...
   * 获取学生的即时成长总结列表
   */
  async getStudentSummaries(userId: string, limit: number = 10): Promise<any[]> {
    const client = await pool.connect();
    try {
      const result = await client.query(
        `SELECT
...
export default new InstantGrowthSummaryService();
