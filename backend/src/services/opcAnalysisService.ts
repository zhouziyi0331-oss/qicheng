...
interface WorkConditionProfile {
  ...
  informationReception: {
    preference: string; // 描述学生如何接收信息
    idealCondition: string; // 理想的项目信息提供方式
    unsuitableCondition: string; // 不适合的项目类型
    clientType: string; // 适合的需求方类型
  };

  creationDrive: {
    source: string; // 创作动力来源
    motivation: string; // 什么能激发他
    unsuitableTask: string; // 不适合的任务类型
    projectType: string; // 适合的项目类型
  };

  learningApproach: {
    style: string; // 学习切入方式
    idealStart: string; // 理想的项目起点
    unsuitableStart: string; // 不适合的项目起点
    mentorStyle: string; // 需要的导师风格
  };

  executionRhythm: {
    pattern: string; // 执行节奏模式
    idealCycle: string; // 理想的项目周期
    unsuitableCycle: string; // 不适合的项目周期
    clientExpectation: string; // 需求方需要接受的工作方式
  };

  autonomyNeed: {
    level: string; // 自主度需求
    idealCollaboration: string; // 理想的协作方式
    unsuitableCollaboration: string; // 不适合的协作方式
  };

  riskTolerance: {
    attitude: string; // 风险承受态度
    idealChallenge: string; // 理想的挑战程度
    unsuitableChallenge: string; // 不适合的挑战程度
  };
...
class OPCAnalysisService {
   ...
   * 从OPC测试结果生成学生的工作条件画像
   */
  async generateWorkConditionProfile(testResult: OPCTestResult): Promise<WorkConditionProfile> {
    logger.info(`Generating work condition profile for student ${testResult.studentId}`);
    ...
    // 8. 生成综合画像文本
    const profileText = this.generateProfileText(
      testResult.personalityTag,
      informationReception,
      creationDrive,
      learningApproach,
      executionRhythm,
   ...
   * 分析信息接收方式
   */
  private analyzeInformationReception(testResult: OPCTestResult): any {
   ...
   * 分析创作驱动来源
   */
  private analyzeCreationDrive(testResult: OPCTestResult): any {
   ...
   * 分析学习切入方式
   */
  private analyzeLearningApproach(testResult: OPCTestResult): any {
   ...
   * 分析执行节奏
   */
  private analyzeExecutionRhythm(testResult: OPCTestResult): any {
   ...
   * 分析自主度需求
   */
  private analyzeAutonomyNeed(testResult: OPCTestResult): any {
   ...
   * 分析风险承受度
   */
  private analyzeRiskTolerance(testResult: OPCTestResult): any {
   ...
   * 推导核心优势（最适合的项目类型）
   */
  private deriveCoreStrengths(testResult: OPCTestResult, conditions: any): string[] {
   ...
   * 保存工作条件画像到数据库
   */
  async saveWorkConditionProfile(profile: WorkConditionProfile): Promise<void> {
    try {
      // 生成向量
      const profileVector = await vectorEmbeddingService.generateStudentProfileVector(profile.profileText);
      ...
      logger.info(`Saved work condition profile for student ${profile.studentId}${profileVector ? ' with vector' : ''}`);

      // 触发增量匹配：将新学生匹配到所有开放任务
      try {
        const matchingScheduler = require('./matchingScheduler').default;
        // 异步执行，不阻塞主流程
        matchingScheduler.matchNewStudentToOpenTasks(profile.studentId).catch((err: Error) => {
