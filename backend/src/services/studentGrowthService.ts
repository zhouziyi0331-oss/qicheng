...
class StudentGrowthService {
  ...
  async getGrowthTimeline(
    ...
    let eventQuery = `
      ...
      WHERE student_id = $1
    `;
    const eventParams: any[] = [studentId];
    let paramIndex = 2;
    ...
    const [eventsResult, milestonesResult, skillsResult] = await Promise.all([
      pool.query(eventQuery, eventParams),
      pool.query(
      ...
      pool.query(
        `SELECT * FROM student_skill_evolution
         WHERE student_id = $1
         ORDER BY current_level DESC, current_proficiency DESC`,
        [studentId]
      ),
    ]);
   ...
   * 任务完成后自动记录成长事件
   */
  async recordTaskCompletionEvent(
    studentId: string,
    taskId: string,
    taskTitle: string,
    rating: number
   ...
   * 技能习得时记录事件
   */
  async recordSkillAcquiredEvent(
    studentId: string,
    skillName: string,
    proficiency: number
  ): Promise<void> {
  ...
  async updateSkillEvolution(
    ...
    if (existing.rows.length === 0) {
      // 新技能
      await pool.query(
        `INSERT INTO student_skill_evolution
      ...
      await pool.query(
        `UPDATE student_skill_evolution
         SET current_level = $1,
             ...
             updated_at = NOW()
         WHERE student_id = $6 AND skill_name = $7`,
        [level, proficiency, JSON.stringify(levelHistory), growthRate, trend, studentId, skillName]
      );
    }
  }
   ...
   * 检查并解锁里程碑
   */
  async checkAndUnlockMilestones(studentId: string): Promise<Milestone[]> {
    ...
    for (const rule of milestoneRules) {
      if (rule.condition) {
        const existing = await pool.query(
          `SELECT * FROM student_milestones
           WHERE student_id = $1 AND milestone_type = $2`,
          [studentId, rule.type]
        );
        ...
        if (existing.rows.length === 0) {
          ...
          const result = await pool.query(
            `INSERT INTO student_milestones
             ...
             RETURNING *`,
            [uuidv4(), studentId, rule.type, rule.title, rule.description, rule.icon, rule.color]
          );
   ...
   * 计算影响力分数
   */
  private calculateImpactScore(rating: number): number {
   ...
   * 计算成长趋势
   */
  private calculateGrowthTrend(events: GrowthEvent[]): string {
    ...
    const recentEvents = events.filter(
      (e) => new Date(e.event_date) >= thirtyDaysAgo
    );

    if (recentEvents.length === 0) return 'inactive';

    const avgImpact =
      recentEvents.reduce((sum, e) => sum + e.impact_score, 0) / recentEvents.length;

    if (avgImpact >= 0.7) return 'accelerating';
    if (avgImpact >= 0.5) return 'steady';
    return 'slowing';
  }
  ...
  private calculateSkillTrend(
    levelHistory: Array<{ date: string; level: number; proficiency: number }>
    ...
    for (let i = 1; i < recent.length; i++) {
      if (recent[i].proficiency > recent[i - 1].proficiency) upCount++;
      else if (recent[i].proficiency < recent[i - 1].proficiency) downCount++;
    }
  ...
  private calculateGrowthRate(
    ...
    if (levelHistory.length < 2) return 0;
...
export default new StudentGrowthService();
