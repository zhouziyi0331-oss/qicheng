...
class MentorMemoryService {
   ...
   * 订单完成后更新学生长期画像
   */
  async updateStudentProfile(studentId: string, orderId: string): Promise<void> {
    try {
      logger.info(`[MentorMemory] 开始更新学生画像: ${studentId}`);
   ...
   * 生成引导风格（基于六维画像）
   */
  private async generateGuidanceStyle(studentId: string, abilitySnapshot: any): Promise<GuidanceStyle> {
    ...
    let styleType: GuidanceStyle['style_type'];
    let analogyPreference: GuidanceStyle['analogy_preference'];
    let stepDetailLevel: GuidanceStyle['step_detail_level'];
    let tone: GuidanceStyle['tone'];
...
请生成摘要（严格控制在200字内）：
   ...
   */
  async buildSystemPromptForAI06(studentId: string, basePrompt: string): Promise<string> {
...
${styleSection}
   ...
   * 记录成长观察（供其他服务调用）
   */
  async recordGrowthObservation(
    studentId: string,
    orderId: string,
    observationType: 'stuck_point' | 'skill_improvement' | 'tool_mastery' | 'milestone',
    content: string,
    category?: string,
    ...
    logger.info(`[MentorMemory] 成长观察已记录: ${studentId} - ${observationType}`);
  }

  /**
   * 批量初始化学生画像（用于迁移现有学生）
   */
  async batchInitializeProfiles(studentIds?: string[]): Promise<void> {
    try {
      // 如果没有指定学生ID，获取所有学生
      let students: any[];
      ...
      logger.info(`[MentorMemory] 开始批量初始化 ${students.length} 个学生画像`);
      ...
      for (const student of students) {
        try {
          ...
          if (existing) {
            logger.info(`[MentorMemory] 学生 ${student.id} 已有画像，跳过`);
            continue;
          }
...
export default new MentorMemoryService();
