...
class DeliverableArchiveService {
   ...
   * 手动创建档案
   */
  async createArchive(data: DeliverableArchive): Promise<any> {
    const {
      task_id,
      company_id,
      student_id,
   ...
   * 获取档案列表
   */
  async getArchives(filter: ArchiveFilter): Promise<{ archives: any[]; total: number }> {
    const {
      companyId,
      category,
      customCategory,
   ...
   * 获取档案详情
   */
  async getArchiveById(archiveId: string, companyId: string): Promise<any> {
    const result = await pool.query(
      `SELECT da.*,
              u.username as student_name,
              u.avatar as student_avatar,
       ...
       LEFT JOIN users u ON da.student_id = u.id
       LEFT JOIN tasks t ON da.task_id = t.id
       WHERE da.id = $1 AND da.company_id = $2`,
      [archiveId, companyId]
    );
    ...
    const versionsResult = await pool.query(
      `SELECT * FROM deliverable_versions WHERE deliverable_id = $1 ORDER BY version_number DESC`,
      [archiveId]
    );
   ...
   * 更新档案
   */
  async updateArchive(archiveId: string, companyId: string, updates: any): Promise<any> {
    const allowedFields = [
      'title',
      'description',
      'category',
   ...
   * 删除档案
   */
  async deleteArchive(archiveId: string, companyId: string): Promise<void> {
    const result = await pool.query(
      `DELETE FROM deliverable_archives WHERE id = $1 AND company_id = $2`,
      [archiveId, companyId]
    );
  ...
  async recordDownload(
    ...
    await pool.query(
      `INSERT INTO deliverable_downloads (id, deliverable_id, downloaded_by, downloaded_files, download_method)
       VALUES ($1, $2, $3, $4, $5)`,
      [uuidv4(), archiveId, userId, downloadedFiles, method]
    );
  }
  ...
  async addVersion(
    ...
    const versionResult = await pool.query(
      `SELECT COALESCE(MAX(version_number), 0) as max_version FROM deliverable_versions WHERE deliverable_id = $1`,
      [archiveId]
    );
    ...
    const result = await pool.query(
      `INSERT INTO deliverable_versions (id, deliverable_id, version_number, files, change_notes, uploaded_by)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING *`,
      [uuidv4(), archiveId, nextVersion, JSON.stringify(files), changeNotes, uploadedBy]
    );
    ...
    await pool.query(
      `UPDATE deliverable_archives
       ...
       WHERE id = $4`,
      [JSON.stringify(files), totalSize, files.length, archiveId]
    );
  ...
  async createCategory(
    ...
    try {
      const result = await pool.query(
        `INSERT INTO deliverable_categories (id, company_id, category_name, description, color, icon)
         VALUES ($1, $2, $3, $4, $5, $6)
         RETURNING *`,
        [uuidv4(), companyId, name, description, color || '#1890ff', icon]
      );
  ...
  async getCategories(companyId: string): Promise<any[]> {
    const result = await pool.query(
      `SELECT * FROM deliverable_categories WHERE company_id = $1 ORDER BY created_at DESC`,
      [companyId]
    );
   ...
   * 更新分类
   */
  async updateCategory(categoryId: string, companyId: string, updates: any): Promise<any> {
  ...
  async deleteCategory(categoryId: string, companyId: string): Promise<void> {
    const result = await pool.query(
      `DELETE FROM deliverable_categories WHERE id = $1 AND company_id = $2`,
  ...
  async createShareLink(
    ...
    const shareCode = uuidv4().replace(/-/g, '').substring(0, 16);
   ...
   * 验证分享链接
   */
  async validateShareLink(shareCode: string, password?: string): Promise<any> {
    const result = await pool.query(
      `SELECT ds.*, da.title, da.files, da.company_id
       FROM deliverable_shares ds
       JOIN deliverable_archives da ON ds.deliverable_id = da.id
       WHERE ds.share_code = $1 AND ds.is_active = true`,
      [shareCode]
    );
   ...
   * 记录分享下载
   */
  async recordShareDownload(shareCode: string): Promise<void> {
    await pool.query(
  ...
  async getArchiveStats(companyId: string): Promise<any> {
    ...
    const recentDownloads = await pool.query(
      `SELECT da.title, dd.downloaded_at
...
export default new DeliverableArchiveService();
