...
class AIPricingService {
  ...
  // 技能加成系数
  private readonly SKILL_PREMIUMS: { [key: string]: number } = {
    'AI': 1.5,
   ...
   * 计算智能定价
   */
  async calculatePrice(taskFeatures: TaskFeatures): Promise<PricingResult> {
    try {
      logger.info('Calculating AI pricing', { task: taskFeatures.title });
   ...
   * 计算基础价格
   */
  private async calculateBasePrice(taskFeatures: TaskFeatures): Promise<{
    base: number;
    skill_premium: number;
    difficulty_premium: number;
    urgency_premium: number;
   ...
   * 获取市场数据
   */
  private async getMarketData(taskFeatures: TaskFeatures): Promise<{
    platform_average: number;
    similar_tasks_avg: number;
    sample_size: number;
  }> {
    ...
    try {
      // 获取平台平均价格
      const platformAvgResult = await client.query(
        `SELECT AVG(budget) as avg_price
         FROM tasks
      ...
      if (taskFeatures.task_type) {
        const similarResult = await client.query(
          `SELECT AVG(budget) as avg_price, COUNT(*) as count
           FROM tasks
           WHERE task_type = $1 AND budget > 0
           AND created_at > NOW() - INTERVAL '90 days'`,
          [taskFeatures.task_type]
        );
   ...
   * 使用AI进行定价分析
   */
  private async getAIPricingAnalysis(
    taskFeatures: TaskFeatures,
    basePrice: any,
    marketData: any
  ): Promise<{ adjustment_factor: number; reasoning: string; recommendations: string[] }> {
    try {
...
以JSON格式返回：
{
   ...
   * 降级定价（AI失败时）
   */
  private getFallbackPricing(taskFeatures: TaskFeatures): PricingResult {
   ...
   * 保存定价记录
   */
  async savePricingRecord(taskId: string, taskFeatures: TaskFeatures, result: PricingResult): Promise<void> {
    const client = await pool.connect();
    try {
      await client.query(
        `INSERT INTO ai_pricing_history (
        ...
        // 如果表不存在，忽略错误
        logger.warn('ai_pricing_history table may not exist');
      });
    } finally {
      client.release();
    }
