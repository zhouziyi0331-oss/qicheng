...
class EscrowService {
   ...
   * 获取用户托管账户
   */
  async getAccount(userId: number, userType: 'student' | 'company'): Promise<EscrowAccount | null> {
    const result = await pool.query(
      `SELECT id, user_id, user_type, total_balance, frozen_balance,
  ...
  async createQuote(
    ...
    const result = await pool.query(
      `INSERT INTO task_quotes
       ...
       RETURNING *`,
      [taskId, studentId, companyId, quotedAmount, platformFeeRate, platformFee, studentNetIncome]
    );
   ...
   * 学生接受报价
   */
  async acceptQuote(quoteId: number, studentId: number): Promise<void> {
    ...
    try {
      ...
      const quoteResult = await client.query(
        'SELECT * FROM task_quotes WHERE id = $1 AND student_id = $2 AND status = $3',
        [quoteId, studentId, 'pending']
      );
   ...
   * 企业支付并进入托管
   */
  async payAndEscrow(quoteId: number, companyId: number, paymentMethod: string): Promise<void> {
    const client = await pool.connect();
    try {
      await client.query('BEGIN');
      ...
      // 获取报价信息
      const quoteResult = await client.query(
        'SELECT * FROM task_quotes WHERE id = $1 AND company_id = $2 AND status = $3',
        [quoteId, companyId, 'accepted']
      );
      ...
      const orderResult = await client.query(
        `INSERT INTO payment_orders
         ...
         RETURNING id`,
        [companyId, quote.task_id, quoteId, quote.quoted_amount, paymentMethod]
      );
      ...
      await client.query(
        `UPDATE tasks
         SET status = 'in_progress',
             ...
             escrow_amount = $1
         WHERE id = $2`,
        [quote.student_net_income, quote.task_id]
      );
      ...
      await client.query(
        ...
        [quote.student_net_income, quote.student_id]
      );
   ...
   * 任务完成，进入待结算（7天后可提现）
   */
  async completeTaskAndSettle(taskId: number, quoteId: number): Promise<void> {
    const client = await pool.connect();
    try {
      await client.query('BEGIN');
      ...
      // 获取报价信息
      const quoteResult = await client.query(
        'SELECT * FROM task_quotes WHERE id = $1 AND task_id = $2 AND status = $3',
        [quoteId, taskId, 'paid']
      );
      ...
      const accountResult = await client.query(
        'SELECT total_balance FROM escrow_accounts WHERE user_id = $1 AND user_type = $2',
        [quote.student_id, 'student']
      );
   ...
   * 释放待结算资金到可提现（7天后自动执行）
   */
  async releaseSettlement(taskId: number, quoteId: number): Promise<void> {
    const client = await pool.connect();
    try {
      await client.query('BEGIN');
      ...
      const taskResult = await client.query(
        'SELECT settlement_date FROM tasks WHERE id = $1',
        [taskId]
      );
      ...
      // 记录交易流水
      const accountResult = await client.query(
        'SELECT available_balance FROM escrow_accounts WHERE user_id = $1 AND user_type = $2',
        [quote.student_id, 'student']
      );
   ...
   * 记录交易流水
   */
  private async logTransaction(client: any, log: TransactionLog): Promise<void> {
    await client.query(
