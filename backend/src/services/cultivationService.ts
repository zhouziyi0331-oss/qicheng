...
class CultivationService {
   ...
   * 创建培养计划
   */
  async createPlan(data: CultivationPlan): Promise<any> {
    const {
      company_id,
      student_id,
      plan_name,
  ...
  async respondToPlan(
    ...
    const result = await pool.query(
      `UPDATE cultivation_plans
       ...
       RETURNING *`,
      [accepted, response, planId, studentId]
    );
   ...
   * 获取企业的培养计划列表
   */
  async getCompanyPlans(companyId: string, status?: string): Promise<any[]> {
    let query = `
      SELECT cp.*,
   ...
   * 获取学生的培养计划列表
   */
  async getStudentPlans(studentId: string, status?: string): Promise<any[]> {
    let query = `
      SELECT cp.*,
             u.company_name,
  ...
  async getPlanById(planId: string): Promise<any> {
    ...
    const phases = await pool.query(
      `SELECT * FROM cultivation_phase_progress WHERE plan_id = $1 ORDER BY phase_number`,
      [planId]
    );
    ...
    const tasks = await pool.query(
      `SELECT ct.*, t.title, t.status, t.budget
    ...
    const skills = await pool.query(
      `SELECT * FROM skill_learning_records WHERE plan_id = $1 ORDER BY created_at DESC`,
      [planId]
    );
   ...
   * 关联任务到培养计划
   */
  async linkTask(planId: string, taskId: string, phaseNumber: number, purpose?: string): Promise<void> {
    await pool.query(
      `INSERT INTO cultivation_tasks (id, plan_id, task_id, phase_number, purpose)
       VALUES ($1, $2, $3, $4, $5)`,
      [uuidv4(), planId, taskId, phaseNumber, purpose]
    );
  }
  ...
  async recordSkillLearning(
    ...
    const result = await pool.query(
      `INSERT INTO skill_learning_records
       (id, plan_id, student_id, skill_name, skill_category, learning_started_at)
       VALUES ($1, $2, $3, $4, $5, NOW())
       RETURNING *`,
      [uuidv4(), planId, studentId, skillName, skillCategory]
    );
   ...
   * 完成技能学习
   */
  async completeSkillLearning(
    recordId: string,
    proficiencyLevel: number,
    verifiedByTaskId?: string
  ): Promise<any> {
    const result = await pool.query(
      `UPDATE skill_learning_records
       ...
       WHERE id = $3
       RETURNING *`,
      [proficiencyLevel, verifiedByTaskId, recordId]
    );
    ...
    await pool.query(
      `UPDATE cultivation_plans
       ...
       WHERE id = $2 AND NOT ($1 = ANY(skills_acquired))`,
      [record.skill_name, record.plan_id]
    );
  ...
  async addFeedback(
    ...
    const result = await pool.query(
      `INSERT INTO cultivation_feedback (id, plan_id, feedback_by, feedback_role, feedback_type, content)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING *`,
      [uuidv4(), planId, feedbackBy, feedbackRole, feedbackType, content]
    );
   ...
   * 获取反馈列表
   */
  async getFeedbacks(planId: string): Promise<any[]> {
    const result = await pool.query(
      `SELECT cf.*,
  ...
  async evaluatePlan(
    ...
    const result = await pool.query(
      `UPDATE cultivation_plans
       ...
       WHERE id = $3
       RETURNING *`,
      [evaluation, successScore, planId]
    );
   ...
   * 获取培养统计
   */
  async getCultivationStats(companyId: string): Promise<any> {
    const result = await pool.query(
      `SELECT
   ...
   * 获取推荐培养方案模板
   */
  async getRecommendedTemplate(targetRole: string): Promise<any> {
    // 预定义的培养方案模板
