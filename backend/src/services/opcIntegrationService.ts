...
import studentCapabilityService from './studentCapabilityService';
import vectorGenerationService from './vectorGenerationService';
import opcAnalysisService from './opcAnalysisService';
...
class OPCIntegrationService {
   ...
   */
  async handleOPCCompletion(assessmentId: string, studentId: string): Promise<void> {
    logger.info(`[OPC集成] 开始处理学生 ${studentId} 的OPC测试完成事件`);
   ...
   * 获取OPC v2测试结果
   */
  private async getOPCResult(assessmentId: string): Promise<any> {
   ...
   */
  private async syncToStudentCapabilities(studentId: string, opcResult: any): Promise<void> {
    logger.info(`[OPC集成] 同步OPC结果到student_capabilities`);
    ...
    try {
      ...
      if (existing.rows.length === 0) {
        ...
        logger.info(`[OPC集成] 创建了新的student_capabilities记录`);
      } else {
        // 更新现有记录
        await client.query(
        ...
        logger.info(`[OPC集成] 更新了现有的student_capabilities记录`);
      }
    } finally {
      client.release();
    }
   ...
   * 生成画像摘要文本
   */
  private generateProfileSummary(opcResult: any): string {
    const parts: string[] = [];
   ...
   * 生成工作条件画像
   */
  private async generateWorkConditionProfile(studentId: string, opcResult: any): Promise<void> {
    logger.info(`[OPC集成] 生成工作条件画像`);
    ...
    try {
      ...
      const workConditionProfile = await opcAnalysisService.generateWorkConditionProfile(testResult);
   ...
   * 触发向量生成
   */
  private async triggerVectorGeneration(studentId: string): Promise<void> {
    logger.info(`[OPC集成] 触发向量生成`);
   ...
   * 将新学生匹配到所有开放任务
   */
  private async triggerIncrementalMatching(studentId: string): Promise<void> {
    logger.info(`[OPC集成] 触发增量匹配`);
    ...
    try {
      ...
      if (matchingScheduler && matchingScheduler.matchNewStudentToOpenTasks) {
        // 异步执行，不阻塞主流程
        matchingScheduler.matchNewStudentToOpenTasks(studentId).catch((err: Error) => {
          logger.error(`[OPC集成] 增量匹配失败:`, err);
        });
        logger.info(`[OPC集成] 增量匹配已触发`);
      } else {
   ...
   * 用于修复历史数据
   */
  async syncAllCompletedOPC(): Promise<void> {
    logger.info(`[OPC集成] 开始批量同步所有已完成OPC测试的学生`);
    ...
    try {
      ...
      logger.info(`[OPC集成] 找到 ${result.rows.length} 个已完成的OPC测试`);

      for (const row of result.rows) {
        try {
          await this.handleOPCCompletion(row.assessment_id, row.student_id);
          logger.info(`[OPC集成] 已同步学生 ${row.student_id}`);
   ...
   * 验证OPC集成是否正常工作
   */
  async verifyIntegration(studentId: string): Promise<{
    opcResult: boolean;
    ...
    try {
      ...
      const opcResult = await client.query(
        `SELECT id FROM opc_v2_results WHERE student_id = $1`,
        [studentId]
      );

      // 检查能力画像
      const abilityProfile = await client.query(
        `SELECT id FROM user_ability_profiles WHERE user_id = $1 AND is_current = true`,
        [studentId]
      );

      // 检查学生能力
      const studentCapability = await client.query(
        `SELECT id, combined_vector FROM student_capabilities WHERE student_id = $1`,
        [studentId]
      );

      // 检查工作条件画像
      const workConditionProfile = await client.query(
        `SELECT id FROM student_work_condition_profiles WHERE student_id = $1`,
        [studentId]
      );
...
export default new OPCIntegrationService();
