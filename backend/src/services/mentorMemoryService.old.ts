...
class MentorMemoryService {
   ...
   * 创建新记忆
   */
  async createMemory(input: MemoryCreationInput): Promise<Memory> {
    try {
      // 自动计算重要性分数（如果未提供）
      ...
      this.linkRelatedMemories(memory.id, input.studentId, relevanceTags).catch(err => {
        logger.error('关联相关记忆失败', { error: err, memoryId: memory.id });
      });
   ...
   * 计算记忆重要性分数
   */
  private async calculateImportance(input: MemoryCreationInput): Promise<number> {
    // 基础分数
    let score = 0.5;
  ...
  private async linkRelatedMemories(
    ...
    try {
      ...
      const result = await pool.query(query, [studentId, memoryId, tags]);
      const relatedIds = result.rows.map(row => row.id);

      if (relatedIds.length > 0) {
        // 更新当前记忆的关联
        await pool.query(
          `UPDATE mentor_memory SET related_memories = $1 WHERE id = $2`,
          [relatedIds, memoryId]
        );
        ...
        for (const relatedId of relatedIds) {
          await pool.query(
            `UPDATE mentor_memory
             SET related_memories = array_append(
               COALESCE(related_memories, ARRAY[]::integer[]),
               $1
             )
             WHERE id = $2 AND NOT ($1 = ANY(COALESCE(related_memories, ARRAY[]::integer[])))`,
            [memoryId, relatedId]
          );
        }
      }
  ...
  async recallMemories(
    ...
    try {
      ...
      const memories: Memory[] = result.rows;
   ...
   * 增加召回次数
   */
  private async incrementRecallCount(memoryId: number): Promise<void> {
    await pool.query(
      `UPDATE mentor_memory
...
如果没有值得记住的内容，返回空数组 []`;
   ...
   * 更新记忆重要性
   */
  async updateImportance(memoryId: number, newScore: number): Promise<void> {
    await pool.query(
      `UPDATE mentor_memory SET importance_score = $1 WHERE id = $2`,
      [Math.max(0, Math.min(1, newScore)), memoryId]
    );
  }
   ...
   * 获取记忆统计
   */
  async getMemoryStats(studentId: number): Promise<{
    ...
    try {
      ...
      const totalResult = await pool.query(
        `SELECT COUNT(*) as count FROM mentor_memory WHERE student_id = $1`,
        [studentId]
      );
      ...
      const avgResult = await pool.query(
        `SELECT AVG(importance_score) as avg FROM mentor_memory WHERE student_id = $1`,
        [studentId]
      );

      // 最常召回的记忆
      const mostRecalledResult = await pool.query(
        `SELECT * FROM mentor_memory
         WHERE student_id = $1
