...
// OPC测评AI分析服务
// ============================================================
export async function analyzeOPCTest(
  userId: string,
  ...
  try {
    ...
    logger.info('OPC test analyzed by Claude', { userId, usage: message.usage });
    ...
    logger.error('OPC AI analysis failed', { userId, error: (error as Error).message });
    // 降级到基础评分
...
// 构建AI分析提示词
// ============================================================
function buildOPCAnalysisPrompt(answers: Record<string, any>): string {
...
// 降级方案：基础评分逻辑
// ============================================================
export function buildFallbackAnalysis(answers: Record<string, any>): any {
  // 简单的评分逻辑（基于答案数量和类型）
...
// 跳级测试AI评分
// ============================================================
export async function evaluateChallengeTest(
  questions: any[],
  answers: any[],
  currentLevel: number,
  targetLevel: number
...
\`\`\``;
    ...
    logger.info('Challenge test evaluated', { currentLevel, targetLevel, score: result.score });

    return result;
  } catch (error) {
    logger.error('Challenge evaluation failed', { error: (error as Error).message });
    // 降级评分
...
// 转包理由AI审核
// ============================================================
export async function evaluateSubcontractReason(
  reason: string,
  taskTitle: string,
...
\`\`\``;
    ...
    logger.info('Subcontract reason evaluated', { approved: result.approved });

    return result;
  } catch (error) {
    logger.error('Subcontract evaluation failed', { error: (error as Error).message });
    // 降级：基于关键词判断
