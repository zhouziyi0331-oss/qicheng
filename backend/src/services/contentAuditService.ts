...
只返回JSON，不要其他解释。`;
   ...
   * 解析AI审核响应
   */
  private parseAuditResponse(response: Anthropic.Message): ContentAuditResult {
    try {
   ...
   */
  async batchAudit(
    contents: Array<{ id: string; type: 'post' | 'comment'; text: string; title?: string }>
  ): Promise<Map<string, ContentAuditResult>> {
    const results = new Map<string, ContentAuditResult>();
  ...
  async checkUserRestriction(
    userId: string,
    restrictionType: 'comment_ban' | 'post_ban' | 'full_ban'
  ): Promise<{ restricted: boolean; reason?: string; expiresAt?: Date }> {
    try {
  ...
  async addUserRestriction(
    ...
    try {
      ...
      await query(
        `INSERT INTO community_user_restrictions (
          user_id, restriction_type, reason, expires_at, created_by
        ) VALUES ($1, $2, $3, NOW() + INTERVAL '${durationHours} hours', $4)`,
        [userId, restrictionType, reason, createdBy || null]
      );

      logger.info('User restriction added', {
        userId,
        restrictionType,
        durationHours,
      });
