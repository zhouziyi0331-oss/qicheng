...
class TaskAmendmentService {
   ...
   * 创建追加需求
   */
  async createAmendment(params: CreateAmendmentParams): Promise<TaskAmendment> {
    ...
    try {
      ...
      const taskResult = await client.query(
        `SELECT status, company_id, accepted_student_id
         FROM tasks
         WHERE id = $1`,
        [params.task_id]
      );
   ...
   * 学生响应追加需求
   */
  async studentRespond(params: StudentResponseParams): Promise<TaskAmendment> {
    const client = await pool.connect();
    ...
    try {
      ...
      const amendmentResult = await client.query(
        `SELECT * FROM task_amendments WHERE id = $1`,
        [params.amendment_id]
      );
      ...
      logger.info('Student responded to amendment', {
   ...
   * 企业最终决定（在协商后）
   */
  async companyFinalDecision(params: CompanyDecisionParams): Promise<TaskAmendment> {
    const client = await pool.connect();
    ...
    try {
      ...
      logger.info('Company made final decision', {
        amendmentId: params.amendment_id,
        decision: params.decision,
      });
   ...
   * 应用追加需求到任务（内部方法）
   */
  private async applyAmendmentToTask(client: any, amendment: TaskAmendment): Promise<void> {
    // 更新任务的价格和截止日期
    ...
    logger.info('Amendment applied to task', {
      amendmentId: amendment.id,
   ...
   * AI评估追加需求的合理性
   */
  async analyzeAmendmentFairness(amendmentId: string): Promise<AIFairnessAnalysis> {
    const client = await pool.connect();
...
fairness_score: 0-100分，80分以上表示非常合理，60-80合理，40-60需要协商，40以下不合理`;
      ...
      const analysis: AIFairnessAnalysis = JSON.parse(jsonMatch[0]);
      ...
      logger.info('Amendment fairness analyzed', {
        amendmentId,
   ...
   * 获取任务的所有追加需求
   */
  async getTaskAmendments(taskId: string, userId: string): Promise<TaskAmendment[]> {
    const client = await pool.connect();
   ...
   * 获取追加需求详情
   */
  async getAmendment(amendmentId: string, userId: string): Promise<TaskAmendment | null> {
    const client = await pool.connect();

    try {
      const result = await client.query(
        `SELECT * FROM task_amendments
         WHERE id = $1 AND (company_id = $2 OR student_id = $2)`,
        [amendmentId, userId]
      );
   ...
   * 取消追加需求（企业主动取消）
   */
  async cancelAmendment(amendmentId: string, companyId: string, reason?: string): Promise<void> {
    const client = await pool.connect();
    ...
    try {
      const result = await client.query(
        `UPDATE task_amendments
         ...
         RETURNING student_id, task_id`,
        [reason || '企业取消了追加需求', amendmentId, companyId]
      );
...
export const taskAmendmentService = new TaskAmendmentService();
