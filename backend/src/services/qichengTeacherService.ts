  ...
  async analyzeAndTranslateTask(taskId: string): Promise<TaskTranslation> {
    try {
      ...
      const task = await queryOne<{
        ...
        `SELECT id, title, description, acceptance_criteria, track, level_required,
                budget_gross, estimated_hours
...
标题: ${task.title}
描述: ${task.description}
验收标准: ${task.acceptance_criteria || '未指定'}
任务轨道: ${task.track}
难度等级: ${task.level_required}
预算: ${task.budget_gross}元
预计时长: ${task.estimated_hours || '未知'}小时
  ...
  async generateProjectRequirementSummary(taskId: string): Promise<string> {
    try {
      const task = await queryOne<{
        ...
        `SELECT title, description, required_skills, track, level_required,
                budget_gross, duration
         FROM tasks WHERE id = $1`,
        [taskId]
      );
