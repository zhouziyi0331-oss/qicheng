...
class ROIAnalyticsService {
  ...
  async getFinancialStats(
    ...
    const result = await pool.query(
      `SELECT * FROM company_financial_stats
   ...
   * 获取历史统计数据
   */
  async getHistoricalStats(companyId: string, months: number): Promise<FinancialStats[]> {
    const result = await pool.query(
      `SELECT * FROM company_financial_stats
       WHERE company_id = $1
   ...
   * 生成成本对比分析
   */
  async generateCostComparison(
    companyId: string,
    currentStats: FinancialStats
  ): Promise<any> {
    ...
    const skillsResult = await pool.query(
      ...
      [companyId, currentStats.year, currentStats.month]
    );

    // 估算全职雇佣成本
    const fulltimeCost = await this.estimateFulltimeCost(
      skillsResult.rows,
      currentStats.total_task_hours || 160
    );
    ...
    return {
      ...
      roi_vs_outsourcing:
        outsourcingCost > 0 ? (savingsVsOutsourcing / outsourcingCost) * 100 : 0,
    };
  }
   ...
   * 估算全职雇佣成本
   */
  private async estimateFulltimeCost(
    ...
    // 获取平均市场月薪
    const avgMonthlyCost = await pool.query(
      `SELECT AVG(fulltime_monthly_cost) as avg_cost
   ...
   * 计算效率指标
   */
  async calculateEfficiencyMetrics(companyId: string): Promise<any> {
    const result = await pool.query(
      `SELECT
    ...
    return {
      ...
      high_quality_rate:
        totalTasks > 0 ? ((stats.high_quality_tasks / totalTasks) * 100).toFixed(1) : 0,
  ...
  private generateRecommendations(
    ...
    if (costComparison.savings_vs_fulltime > 0) {
      recommendations.push(
        `相比雇佣全职员工，本月节省了¥${costComparison.savings_vs_fulltime.toFixed(0)}`
      );
    }
   ...
   * 创建成本对比分析报告
   */
  async createCostComparisonAnalysis(
    companyId: string,
    ...
    const scenarios = {
      ...
      outsourcing: {
        ...
        savings_percentage:
          outsourcingCost > 0 ? ((outsourcingCost - actualCost) / outsourcingCost) * 100 : 0,
      },
    };
    ...
    // 保存分析报告
    const result = await pool.query(
      `INSERT INTO cost_comparison_analyses
   ...
   * 获取市场价格基准
   */
  async getMarketBenchmarks(): Promise<any[]> {
    const result = await pool.query(
      `SELECT * FROM market_price_benchmarks
   ...
   * 更新财务统计（手动刷新）
   */
  async refreshFinancialStats(companyId: string, year: number, month: number): Promise<void> {
    // 统计该月的任务数据
    const tasksResult = await pool.query(
      `SELECT
    ...
    // 更新或插入统计
    await pool.query(
      `INSERT INTO company_financial_stats
...
export default new ROIAnalyticsService();
