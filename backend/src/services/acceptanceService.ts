...
class AcceptanceService {
   ...
   * E-29: 创建验收清单
   */
  async createChecklist(taskId: string, items: any[]): Promise<any> {
    ...
    const result = await pool.query(
      `INSERT INTO acceptance_checklists (id, task_id, checklist_items, total_items)
       VALUES ($1, $2, $3, $4)
       RETURNING *`,
      [uuidv4(), taskId, JSON.stringify(checklistItems), items.length]
    );
  ...
  async updateChecklistItem(
    ...
    const checklistResult = await pool.query(
      `SELECT * FROM acceptance_checklists WHERE id = $1`,
      [checklistId]
    );
    ...
    // 统计
    const checkedItems = items.filter((i: any) => i.status !== 'pending').length;
    const approvedItems = items.filter((i: any) => i.status === 'approved').length;
    const rejectedItems = items.filter((i: any) => i.status === 'rejected').length;
    ...
    const result = await pool.query(
      `UPDATE acceptance_checklists
       ...
       WHERE id = $6
       RETURNING *`,
      [JSON.stringify(items), checkedItems, approvedItems, rejectedItems, overallStatus, checklistId]
    );
   ...
   * 获取验收清单
   */
  async getChecklist(taskId: string): Promise<any> {
    const result = await pool.query(
      `SELECT * FROM acceptance_checklists WHERE task_id = $1`,
      [taskId]
    );
   ...
   * E-30: 获取修改意见模板列表
   */
  async getRevisionTemplates(category?: string): Promise<any[]> {
   ...
   * 使用模板生成修改意见
   */
  async applyRevisionTemplate(templateId: string, placeholderValues: any): Promise<string> {
    // 获取模板
    const templateResult = await pool.query(
      `SELECT * FROM revision_comment_templates WHERE id = $1`,
      [templateId]
    );
    ...
    // 更新使用次数
    await pool.query(
      `UPDATE revision_comment_templates SET usage_count = usage_count + 1 WHERE id = $1`,
      [templateId]
    );
  ...
  async createDimensionalScore(data: {
    ...
    const result = await pool.query(
      `INSERT INTO dimensional_acceptance_scores
   ...
   * 获取维度评分
   */
  async getDimensionalScore(taskId: string): Promise<any> {
    const result = await pool.query(
      `SELECT * FROM dimensional_acceptance_scores WHERE task_id = $1`,
      [taskId]
    );
   ...
   * 获取学生的评分统计
   */
  async getStudentScoreStats(studentId: string): Promise<any> {
    const result = await pool.query(
      `SELECT
  ...
  async recordCooperationWillingness(data: {
    ...
    if (role === 'company') {
      const result = await pool.query(
        `INSERT INTO cooperation_willingness
         ...
         RETURNING *`,
        [uuidv4(), task_id, company_id, student_id, willing, reason, tags || []]
      );
   ...
   * 获取合作意愿记录
   */
  async getCooperationWillingness(taskId: string): Promise<any> {
    const result = await pool.query(
      `SELECT * FROM cooperation_willingness WHERE task_id = $1`,
      [taskId]
    );
   ...
   * 获取双向愿意合作的记录
   */
  async getMutualCooperationPairs(userId: string, role: string): Promise<any[]> {
  ...
  async createIPDeclaration(data: {
    ...
    const result = await pool.query(
      `INSERT INTO intellectual_property_declarations
   ...
   * 确认知识产权声明
   */
  async confirmIPDeclaration(declarationId: string, role: 'company' | 'student'): Promise<any> {
    ...
    const result = await pool.query(
      `UPDATE intellectual_property_declarations
       SET ${field} = true,
           ${timestampField} = NOW(),
           updated_at = NOW()
       WHERE id = $1
   ...
   * 获取知识产权声明
   */
  async getIPDeclaration(taskId: string): Promise<any> {
    const result = await pool.query(
      `SELECT * FROM intellectual_property_declarations WHERE task_id = $1`,
      [taskId]
    );
  ...
  async createRefundRequest(data: {
    ...
    const result = await pool.query(
      `INSERT INTO refund_compensation_records
  ...
  async reviewRefundRequest(
    ...
    const result = await pool.query(
      `UPDATE refund_compensation_records
       SET status = $1,
           reviewed_by = $2,
           approved_amount = $3,
           review_comment = $4,
       ...
       WHERE id = $5
       RETURNING *`,
      [status, reviewedBy, approvedAmount, reviewComment, requestId]
    );
   ...
   * 处理退款
   */
  async processRefund(requestId: string, transactionId: string): Promise<any> {
    const result = await pool.query(
      `UPDATE refund_compensation_records
