...
interface RetrospectiveQuestions {
  question1: string; // 最大难点是什么？怎么解决的？
  question2: string; // 下次会哪里做得不一样？
  question3: string; // 用了哪些工具？哪个最顺手？
}
...
class MentorRetrospectiveService {
   ...
   * 订单完成后触发复盘（延迟60秒）
   */
  async triggerRetrospective(studentId: string, orderId: string): Promise<void> {
    try {
      ...
      if (existing.rows.length > 0) {
        logger.info(`[MentorRetrospective] 复盘已存在，跳过`);
        return;
      }
...
请以JSON格式返回：
{
   ...
   * 格式化复盘消息
   */
  private formatRetrospectiveMessage(questions: RetrospectiveQuestions): string {
...
不用写很多，每个问题一句话就行。写完后这份复盘会自动进入你的成长记录。`;
   ...
   * 判断是否应该标记为精华复盘
   */
  private shouldBeFeatured(retrospective: any): boolean {
    ...
    // 条件3：包含具体的工具或方法
    const hasSpecificTools = /工具|软件|平台|方法|步骤/.test(
...
export default new MentorRetrospectiveService();
