...
class GraduationReportService {
   ...
   * 检查用户等级
   */
  private async checkUserLevel(userId: string): Promise<number> {
   ...
   * 检查是否已有报告
   */
  private async getExistingReport(userId: string): Promise<any> {
    const client = await pool.connect();
    try {
      const result = await client.query(
   ...
   * 收集学生所有数据
   */
  private async collectStudentData(userId: string): Promise<StudentData> {
    const client = await pool.connect();
    try {
      // 1. 用户基本信息
      const userResult = await client.query(
        `SELECT id, username, current_level, created_at
      ...
      // 4. 能力画像版本历史
      const profileVersionsResult = await client.query(
      ...
      // 6. 导师对话记录
      const mentorSessionsResult = await client.query(
      ...
      // 7. 成长观察记录
      const growthObservationsResult = await client.query(
  ...
  private async generateFullReport(
    ...
    // 生成目录
    const tableOfContents = chapters.map(
  ...
  private async generateChapter(
    ...
    studentData: StudentData,
    retryCount: number = 0
  ): Promise<GraduationReportChapter> {
   ...
   * 获取章节配置（包含最低字数要求）
   */
  private getChapterConfig(chapterNumber: number): { title: string; targetWords: number; minWords: number; temperature: number } {
    const configs: Record<number, { title: string; targetWords: number; minWords: number; temperature: number }> = {
      1: {
        title: '你的成长轨迹',
        targetWords: 1800,
        minWords: 1500,
   ...
   */
  private buildChapterSystemPrompt(chapterNumber: number): string {
...
直接输出章节正文，不要包含章节标题。如果学生有组队经历，必须引用；如果没有，推荐具体的互补技能类型。`,
    ...
    if (chapterNumber === 1) {
      ...
      studentData.growthObservations.forEach((obs) => {
        if (obs.skills_observed && obs.skills_observed.skills) {
          obs.skills_observed.skills.forEach((skill: string) => {
    ...
    try {
      const result = await client.query(
        `INSERT INTO growth_reports
         ...
         RETURNING id`,
        [userId, report.preview_content, JSON.stringify(report)]
      );
   ...
   * 获取报告预览
   */
  async getReportPreview(userId: string): Promise<any> {
    const client = await pool.connect();
    try {
      const result = await client.query(
        `SELECT id, preview_content, created_at, is_paid
         FROM growth_reports
         WHERE user_id = $1 AND report_type = 'graduation'
         LIMIT 1`,
        [userId]
   ...
   * 获取完整报告（需要已付费）
   */
  async getFullReport(reportId: string): Promise<GraduationReport | null> {
    const client = await pool.connect();
    try {
      const result = await client.query(
        `SELECT full_content_json, is_paid
         FROM growth_reports
         WHERE id = $1`,
        [reportId]
      );
  ...
  async processPayment(
    ...
    try {
      ...
      await client.query(
        `INSERT INTO graduation_report_payments
         (user_id, report_id, amount, payment_method, transaction_id, points_used, points_value, status, paid_at)
         VALUES ($1, $2, $3, $4, $5, $6, $7, 'completed', NOW())`,
        [userId, reportId, amount, paymentMethod, transactionId, pointsUsed, pointsUsed * 0.01]
      );
   ...
   * 检查是否需要更新报告（每完成3个新项目）
   */
  async checkNeedUpdate(userId: string): Promise<boolean> {
    const client = await pool.connect();
    try {
      // 获取报告生成时间
      const reportResult = await client.query(
        `SELECT created_at, update_count
         FROM growth_reports
         WHERE user_id = $1 AND report_type = 'graduation'
         LIMIT 1`,
        [userId]
      ...
      const orderResult = await client.query(
        `SELECT COUNT(*) as new_orders
         FROM orders
         WHERE student_id = $1 AND status = 'completed' AND completed_at > $2`,
        [userId, report.created_at]
      );
...
export default new GraduationReportService();
