...
class ToolRecommendationService {
  ...
  async recommendTools(
    ...
    try {
      ...
      const result = await pool.query(query, [JSON.stringify(suitableFor)]);
  ...
  async trackRecommendation(
    ...
    try {
      const result = await pool.query(
        `INSERT INTO mentor_tool_usage_tracking
         (student_id, tool_id, task_id, recommendation_context)
         VALUES ($1, $2, $3, $4)
         RETURNING id`,
        [studentId, toolId, taskId, context]
      );
   ...
   * 获取工具使用统计
   */
  async getToolStatistics(toolId: number): Promise<any> {
    try {
      const query = `
        SELECT
   ...
   * 获取最受欢迎的工具
   */
  async getPopularTools(category?: string, limit: number = 5): Promise<ToolRecommendation[]> {
    try {
      let query = `
        SELECT
  ...
  async addTool(tool: {
    ...
    try {
      const result = await pool.query(
        `INSERT INTO mentor_tool_recommendations
   ...
   * 分析任务类型
   */
  private async analyzeTaskType(taskId: number): Promise<string> {
    try {
      const result = await pool.query(
   ...
   * 获取学生技能水平
   */
  private async getStudentSkillLevel(studentId: number, taskType: string): Promise<string> {
    try {
      const result = await pool.query(
        `SELECT technical_skills, total_tasks_completed
   ...
   * 增加推荐次数
   */
  private async incrementRecommendationCount(toolId: number): Promise<void> {
    try {
      await pool.query(
        `UPDATE mentor_tool_recommendations
   ...
   * 更新工具成功率
   */
  private async updateToolSuccessRate(toolId: number): Promise<void> {
    try {
