...
export class VerifyService {
  ...
  static async createSession(
    ...
    companyId: string,
    matchId: string
  ): Promise<{ sessionId: string; round1Question: Question }> {
    return withTransaction(async (client) => {
      // 检查是否已有进行中的会话
      const existingSession = await client.query(
        `SELECT id FROM verify_sessions
      ...
      const questionResult = await client.query(
        `SELECT id, question_text, judge_criteria
      ...
      const sessionResult = await client.query(
        `INSERT INTO verify_sessions
         ...
         RETURNING id`,
        [studentId, companyId, matchId, question.id]
      );
      ...
      // 更新匹配记录状态
      await client.query(
        `UPDATE student_company_matches
   ...
   * 提交第一轮回答
   */
  static async submitRound1Answer(
    sessionId: string,
    answer: string
    ...
    return withTransaction(async (client) => {
      // 获取会话信息
      const sessionResult = await client.query(
        `SELECT vs.*, vq.question_text, vq.judge_criteria,
   ...
   * 处理第一轮AI判断（异步）
   */
  private static async processRound1Judge(
    sessionId: string,
    session: any,
    answer: string
  ): Promise<void> {
    try {
      // 调用AI判断
      const judgeResult = await AIJudgeService.judgeRound1({
        sessionId,
      ...
      await withTransaction(async (client) => {
        if (judgeResult.result === 'pass') {
          // 通过，进入第二轮
          const round2Question = await client.query(
            `SELECT id, question_text, judge_criteria
             FROM verify_questions
             WHERE (company_id = $1 OR company_id IS NULL)
               AND round = 'round2'
          ...
          await client.query(
            `UPDATE verify_sessions
             SET status = 'round1_pass',
                 ...
                 updated_at = NOW()
             WHERE id = $1`,
            [sessionId, judgeResult.reason, round2Question.rows[0]?.id]
          );
            ...
            await client.query(
              `UPDATE verify_sessions
               SET status = 'round1_retry',
                   ...
                   updated_at = NOW()
               WHERE id = $1`,
              [sessionId, retryCount, judgeResult.reason, judgeResult.retry_prompt]
            );
          }
        } else {
          // 失败
   ...
   * 提交第二轮回答
   */
  static async submitRound2Answer(
    sessionId: string,
    answer: string
  ): Promise<{ status: string }> {
    return withTransaction(async (client) => {
   ...
   * 处理第二轮AI判断（异步）
   */
  private static async processRound2Judge(
    sessionId: string,
    session: any,
    answer: string
  ): Promise<void> {
    try {
      const judgeResult = await AIJudgeService.judgeRound2({
        sessionId,
        needDescription: session.need_description,
        questionText: session.question_text,
        criteria: session.judge_criteria,
   ...
   * 获取会话状态
   */
  static async getSessionStatus(sessionId: string): Promise<VerifySession> {
   ...
   * 获取第二轮题目
   */
  static async getRound2Question(sessionId: string): Promise<Question> {
    const result = await query<any>(
      `SELECT vq.id, vq.question_text, vq.judge_criteria
       FROM verify_sessions vs
       JOIN verify_questions vq ON vs.round2_question_id = vq.id
       WHERE vs.id = $1 AND vs.deleted_at IS NULL`,
      [sessionId]
