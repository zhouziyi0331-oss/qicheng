...
export class MatchService {
  ...
  static async recordTaskCompletion(
    ...
    companyId: string,
    earnings: number
  ): Promise<{ unlockEligible: boolean; matchId: string }> {
    return withTransaction(async (client) => {
      // 查询或创建匹配记录
      const existingMatch = await client.query(
        `SELECT * FROM student_company_matches
         WHERE student_id = $1 AND company_id = $2 AND deleted_at IS NULL`,
        [studentId, companyId]
      );
      ...
      if (existingMatch.rows.length === 0) {
        ...
        const result = await client.query(
          `INSERT INTO student_company_matches
           (student_id, company_id, completed_tasks, total_earnings, unlock_eligible)
           VALUES ($1, $2, 1, $3, false)
           RETURNING id, completed_tasks`,
          [studentId, companyId, earnings]
        );
        ...
        const result = await client.query(
          `UPDATE student_company_matches
           SET completed_tasks = completed_tasks + 1,
               ...
               updated_at = NOW()
           WHERE id = $1
           RETURNING id, completed_tasks, unlock_eligible`,
          [existingMatch.rows[0].id, studentId, earnings]
        );
        matchId = result.rows[0].id;
   ...
   * 获取学生的所有解锁资格
   */
  static async getEligibleMatches(studentId: string): Promise<MatchRecord[]> {
    const result = await query<any>(
      `SELECT m.*,
       ...
       ORDER BY m.unlock_triggered_at DESC`,
      [studentId]
    );
  ...
  static async calculateMatchScore(
    ...
    const matchScore = (approval_rate || 0) * 0.6 + speedScore * 0.4;
   ...
   * 生成匹配原因说明
   */
  static async generateMatchReason(
    studentId: string,
    companyId: string
  ): Promise<string> {
