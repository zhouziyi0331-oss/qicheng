...
export class UnlockService {
  private static readonly ENCRYPTION_KEY = process.env.CONTACT_ENCRYPTION_KEY || 'default-32-char-encryption-key!!';
  private static readonly UNLOCK_PRICE_FEN = 1900; // ¥19

  /**
   * 创建解锁支付订单
   */
  static async createUnlockPayment(
    studentId: string,
    sessionId: string
  ): Promise<{ paymentId: string; outTradeNo: string; amountFen: number }> {
    return withTransaction(async (client) => {
      ...
      const existingPayment = await client.query(
        `SELECT id FROM unlock_payments
         WHERE session_id = $1 AND status = 'paid' AND deleted_at IS NULL`,
        [sessionId]
      );
  ...
  ): Promise<{ unlockRecordId: string; contact: any }> {
    return withTransaction(async (client) => {
      // 查询支付记录
      const paymentResult = await client.query(
        `SELECT * FROM unlock_payments
         WHERE wx_out_trade_no = $1 AND deleted_at IS NULL`,
        [outTradeNo]
      );
   ...
   * 获取解锁记录
   */
  static async getUnlockRecord(studentId: string, companyId: string): Promise<any> {
    const result = await query<any>(
      `SELECT ur.*, cp.company_name
   ...
   * 记录学生查看联系方式
   */
  static async recordContactViewed(unlockRecordId: string): Promise<void> {
    await query(
      `UPDATE unlock_records
       SET contact_viewed_at = NOW()
       WHERE id = $1 AND contact_viewed_at IS NULL`,
      [unlockRecordId]
    );
  }
  ...
  static async collectFeedback(
    ...
    await query(
      `UPDATE unlock_records
       ...
       WHERE id = $1`,
      [unlockRecordId, studentContacted, merchantContacted]
    );
  }
}
