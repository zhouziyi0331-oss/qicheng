...
class QAService {
   ...
   * 获取学生在特定任务的对话历史
   */
  async getConversationByStudentAndTask(studentId: string, taskId: string): Promise<any> {
    try {
      const query = `
        SELECT * FROM conversations
        ...
        ORDER BY created_at ASC
      `;
      const result = await pool.query(query, [studentId, taskId]);
  ...
  shouldProvideDirectAnswer(question: string): boolean {
    ...
    return directAnswerKeywords.some(keyword =>
