...
- confidence_level: 置信度（0-1）`;
      ...
      if (taskId) {
        ...
        const versionResult = await pool.query(
          `SELECT COALESCE(MAX(assessment_version), 0) + 1 as next_version
           FROM risk_assessments WHERE task_id = $1`,
          [taskId]
        );
   ...
   * 获取任务的风险评估历史
   */
  async getTaskRiskHistory(taskId: string): Promise<RiskAssessment[]> {
    const result = await pool.query(
  ...
  async acknowledgeRisk(
    assessmentId: string,
    companyId: string,
    decision: 'proceed_anyway' | 'revise_task' | 'cancel',
    notes?: string
  ): Promise<void> {
    await pool.query(
   ...
   * 获取企业的风险统计
   */
  async getCompanyRiskStats(companyId: string): Promise<any> {
    const result = await pool.query(
      `SELECT
   ...
   * 获取常见风险类型
   */
  async getCommonRiskTypes(): Promise<any[]> {
    const result = await pool.query(
      `SELECT * FROM risk_types WHERE is_active = true ORDER BY category, risk_code`
    );
   ...
   * 计算总体风险分数
   */
  private calculateOverallRiskScore(dimensions: Record<string, RiskDimension>): number {
    let totalScore = 0;
    let count = 0;
   ...
   * 降级方案：基于规则的风险评估
   */
  private async generateFallbackAssessment(
    ...
    const fallbackDimensions: Record<string, RiskDimension> = {
   ...
   * 更新企业风险统计
   */
  private async updateRiskStats(companyId: string): Promise<void> {
    // 这里可以实现定期统计逻辑
    // 暂时留空，可以通过定时任务调用
  }
}
