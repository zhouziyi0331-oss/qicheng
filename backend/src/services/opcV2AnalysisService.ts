...
class OPCv2AnalysisService {
   ...
   * 开始新的测试
   */
  async startAssessment(userId: string): Promise<string> {
    const client = await pool.connect()
    try {
      const result = await client.query(
        `INSERT INTO opc_v2_assessments (student_id, status)
  ...
  async submitAnswer(
    ...
    answerData: { answerText?: string; selectedOption?: string }
  ): Promise<void> {
    const client = await pool.connect()
    try {
      await client.query(
        `INSERT INTO opc_v2_answers (assessment_id, question_id, answer_text, selected_option)
         ...
         DO UPDATE SET answer_text = $3, selected_option = $4, answered_at = NOW()`,
        [assessmentId, questionId, answerData.answerText, answerData.selectedOption]
      )
   ...
   * 完成测试并生成分析报告
   */
  async completeAssessment(assessmentId: string): Promise<OPCAnalysisResult> {
    const client = await pool.connect()
    try {
      // 获取所有答案
      const answersResult = await client.query(
        `SELECT question_id, question_type, answer_text, selected_option
         FROM opc_v2_answers
      ...
      // 分离前置题和选择题
      const preAnswers: PreQuestionAnswer[] = answers
        .filter(a => a.question_type === 'definition')
        .map(a => ({ questionId: a.question_id, answerText: a.answer_text }))

      const choiceAnswers: ChoiceQuestionAnswer[] = answers
  ...
  private async analyzeWithAI(
    preAnswers: PreQuestionAnswer[],
    choiceAnswers: ChoiceQuestionAnswer[]
  ): Promise<OPCAnalysisResult> {
    // 构建分析提示词
    ...
    const result: OPCAnalysisResult = JSON.parse(jsonMatch[0])
    return result
  }
...
请现在开始分析，只输出JSON，不要有其他内容。`
   ...
   * 计算六维度分数（基于选择题答案）
   */
  private calculateDimensionScores(choiceAnswers: ChoiceQuestionAnswer[]): {
    openness: number
    persistence: number
    creativity: number
    learning: number
  ...
  private async saveResult(
    ...
    try {
      ...
      // 更新用户的最新OPC结果引用
      const resultIdResult = await client.query(
        `SELECT id FROM opc_v2_results WHERE assessment_id = $1`,
        [assessmentId]
      )
      const resultId = resultIdResult.rows[0].id

      await client.query(
        `UPDATE users SET latest_opc_v2_result_id = $1 WHERE id = $2`,
        [resultId, userId]
      )
    } finally {
      client.release()
   ...
   * 获取测试结果
   */
  async getResult(assessmentId: string): Promise<OPCAnalysisResult | null> {
    const client = await pool.connect()
    try {
      const result = await client.query(
   ...
   * 获取用户最新的OPC结果
   */
  async getLatestResult(userId: string): Promise<OPCAnalysisResult | null> {
    const client = await pool.connect()
    try {
      const result = await client.query(
...
export default new OPCv2AnalysisService()
