...
class TieredDiscountService {
   ...
   * 获取所有折扣阶梯
   */
  async getAllTiers(): Promise<DiscountTier[]> {
    const result = await pool.query(
      `SELECT * FROM discount_tiers WHERE is_active = true ORDER BY tier_level ASC`
    );
   ...
   * 获取企业当前阶梯信息
   */
  async getCompanyTierInfo(companyId: string): Promise<CompanyTierInfo> {
    const now = new Date();
    ...
    // 调用数据库函数计算阶梯
    const result = await pool.query(
      `SELECT * FROM calculate_company_tier($1, $2, $3)`,
    ...
    const [currentTierResult, nextTierResult] = await Promise.all([
      pool.query(
        `SELECT * FROM discount_tiers WHERE tier_level = $1`,
        [tierInfo.tier_level]
      ),
  ...
  async getMonthlyStats(
    ...
    const result = await pool.query(
      `SELECT * FROM company_monthly_stats
  ...
  async applyDiscountToTask(
    ...
    const now = new Date();
    await pool.query(
      `UPDATE company_monthly_stats
       SET total_saved = total_saved + $1,
           updated_at = NOW()
       WHERE company_id = $2 AND year = $3 AND month = $4`,
      [discount.discount_amount, companyId, now.getFullYear(), now.getMonth() + 1]
    );
  ...
  async getDiscountHistory(
    ...
    const [applicationsResult, countResult] = await Promise.all([
      pool.query(
        `SELECT da.*, t.title as task_title
      ...
      pool.query(
        `SELECT COUNT(*) FROM discount_applications WHERE company_id = $1`,
        [companyId]
      ),
    ]);
   ...
   * 获取企业折扣进度（用于UI展示）
   */
  async getDiscountProgress(companyId: string): Promise<any> {
    const tierInfo = await this.getCompanyTierInfo(companyId);
    ...
    return {
      ...
      progress_percentage:
        tierInfo.next_tier_threshold
          ? Math.min(
              100,
              (tierInfo.tasks_count / tierInfo.next_tier_threshold) * 100
            )
          : 100,
    };
  }
  ...
  async refreshMonthlyStats(
    ...
    // 更新或插入统计
    await pool.query(
      `INSERT INTO company_monthly_stats
...
export default new TieredDiscountService();
