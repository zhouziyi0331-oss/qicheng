  ...
  "subtasks": [
    {
      ...
      try {
        ...
        logger.error('[DemandDecomposition] AI响应解析失败:', parseError);
      ...
      logger.info('[DemandDecomposition] AI拆解完成', {
        decompositionId,
    ...
    try {
      ...
      const matches = studentsResult.rows.map(student => {
        ...
        const skillMatchCount = requiredSkills.filter(skill =>
          studentTracks.some(track => track.includes(skill) || skill.includes(track))
        ).length;
      ...
      logger.info('[DemandDecomposition] 子任务推送完成', {
        subtaskId: params.subtaskId,
...
export default new DemandDecompositionService();
