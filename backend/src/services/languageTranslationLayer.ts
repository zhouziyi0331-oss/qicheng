...
class LanguageTranslationLayer {
   ...
   * 触发时机：学生接单后30秒
   */
  async translateTaskToFirstSteps(
    taskId: string,
    studentId: string
  ): Promise<string> {
    try {
      ...
      const student = await queryOne<{
        ...
        `SELECT profile_summary, skills FROM student_capabilities WHERE student_id = $1`,
        [studentId]
      );
      ...
      const translation = await queryOne<{
        ...
        `SELECT student_friendly_description, what_you_will_do
         FROM task_translations WHERE task_id = $1`,
        [taskId]
      );
...
直接返回3个步骤，不要其他解释：`;
   ...
   * 触发时机：学生主动求助
   */
  async reframeStudentDifficulty(
    orderId: string,
    ...
    try {
      ...
      const order = await queryOne<{
        ...
        `SELECT task_id, student_id FROM orders WHERE id = $1`,
        [orderId]
      );
...
直接返回你的引导话语（100字内），不要其他解释：`;
   ...
   * 触发时机：AI审核或企业打回交付物
   */
  async translateRejectionFeedback(
    orderId: string,
...
直接返回你的翻译（150字内），不要其他解释：`;
   ...
   * 触发时机：学生完成首单或升级
   */
  async translateStudentGrowthToValue(
    studentId: string
  ): Promise<string> {
    try {
      // 获取学生能力数据
      const capability = await queryOne<{
        ...
        `SELECT profile_summary, tasks_completed, avg_client_satisfaction, skills
      ...
      const recentTasks = await query<{
        ...
        `SELECT t.title as task_title, o.client_rating
         FROM orders o
...
直接返回能力描述，不要其他解释：`;
   ...
   * 触发时机：企业查看匹配推荐的学生列表
   */
  async translatePersonalityToBusinessValue(
    studentId: string
  ): Promise<string> {
    try {
      // 获取学生能力画像
      const capability = await queryOne<{
        ...
        `SELECT profile_summary, personality_style, skills
         FROM student_capabilities WHERE student_id = $1`,
        [studentId]
      );
...
export default new LanguageTranslationLayer();
