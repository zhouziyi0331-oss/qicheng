...
export class TalentTagInferenceService {
  
  /**
   * 从OPC测评结果推断天赋标签
   */
  static async inferFromOPC(studentId: string, opcScores: OPCScores): Promise<void> {
    const client = await pool.connect();
    
    try {
      logger.info(`[TalentInference] 开始为学生 ${studentId} 推断天赋标签`);
      
      await client.query('BEGIN');
      
      // 1. 信息处理维度 → 思维方式
      await this.inferFromDimension(
        client,
        studentId,
        'info_processing',
        opcScores.info_processing_tendency,
        opcScores.info_processing_score
      );
      ...
      await this.inferFromDimension(
        ...
        studentId,
        'creation_drive',
        opcScores.creation_drive_tendency,
        opcScores.creation_drive_score
      );
      ...
      await this.inferFromDimension(
        ...
        studentId,
        'tool_learning',
        opcScores.tool_learning_tendency,
        opcScores.tool_learning_score
      );
      ...
      await this.inferFromDimension(
        ...
        studentId,
        'task_execution',
        opcScores.task_execution_tendency,
        opcScores.task_execution_score
      );
      ...
      await this.inferFromDimension(
        ...
        studentId,
        'collaboration',
        opcScores.collaboration_tendency,
        opcScores.collaboration_score
      );
      ...
      await this.inferFromDimension(
        ...
        studentId,
        'risk_attitude',
        opcScores.risk_attitude_tendency,
        opcScores.risk_attitude_score
      );
      ...
      const result = await client.query(
        `SELECT tt.tag_name, stt.confidence, stt.strength
         FROM student_talent_tags stt
         JOIN talent_tags tt ON stt.tag_id = tt.id
         WHERE stt.student_id = $1 AND stt.source = 'opc_inferred'`,
        [studentId]
      );
      
      logger.info(`[TalentInference] 为学生 ${studentId} 推断了 ${result.rows.length} 个天赋标签`);
      result.rows.forEach(row => {
        logger.info(`  - ${row.tag_name} (置信度: ${row.confidence}, 强度: ${row.strength})`);
      });
      
    } catch (error) {
      await client.query('ROLLBACK');
      logger.error('[TalentInference] 推断失败:', error);
      throw error;
    } finally {
      client.release();
    }
   ...
   * 从单个维度推断标签
   */
  private static async inferFromDimension(
    client: any,
    studentId: string,
    dimension: string,
    tendency: string,
    ...
    const result = await client.query(
      `SELECT id, tag_name 
       ...
       AND (opc_score_range->>'max')::int >= $3`,
      [dimension, tendency, score]
    );
    ...
    for (const tag of result.rows) {
      await client.query(
        `INSERT INTO student_talent_tags 
      ...
      logger.info(`[TalentInference] 添加标签: ${tag.tag_name} (${dimension}, 置信度: ${confidence})`);
    }
  }
   ...
   * 从任务表现推断天赋标签
   */
  static async inferFromTaskPerformance(
    studentId: string,
    taskId: string,
    performanceData: {
    ...
    try {
      logger.info(`[TalentInference] 从任务 ${taskId} 推断学生 ${studentId} 的天赋`);
      
      // 记录任务表现
      await client.query(
        `INSERT INTO task_performance_records 
      ...
      // 基于表现推断标签
      const inferredTags: Array<{tagName: string, confidence: number, reason: string}> = [];
      ...
      for (const inferred of inferredTags) {
        const tagResult = await client.query(
          'SELECT id FROM talent_tags WHERE tag_name = $1',
          [inferred.tagName]
        );
        ...
        if (tagResult.rows.length > 0) {
          ...
          const existingResult = await client.query(
            'SELECT id, verified_count, confidence FROM student_talent_tags WHERE student_id = $1 AND tag_id = $2',
            [studentId, tagId]
          );
          ...
          if (existingResult.rows.length > 0) {
            ...
            if (newVerifiedCount >= 10) newStrength = 'core';
            else if (newVerifiedCount >= 5) newStrength = 'prominent';
            else if (newVerifiedCount >= 3) newStrength = 'clear';
            
            await client.query(
              `UPDATE student_talent_tags 
               SET verified_count = $1, 
                   ...
                   updated_at = NOW()
               WHERE id = $4`,
              [newVerifiedCount, newConfidence, newStrength, existing.id]
            );
            
            logger.info(`[TalentInference] 更新标签: ${inferred.tagName} (验证${newVerifiedCount}次, 置信度${newConfidence}, 强度${newStrength})`);
            ...
            logger.info(`[TalentInference] 新增标签: ${inferred.tagName} (${inferred.reason})`);
          }
        }
      }
      
      logger.info(`[TalentInference] 从任务推断完成，推断出 ${inferredTags.length} 个标签`);
      
    } catch (error) {
      logger.error('[TalentInference] 从任务推断失败:', error);
      throw error;
    } finally {
      client.release();
    }
