...
class AssetVisualizationService {
  ...
  async getDashboard(studentId: string): Promise<any> {
    ...
    try {
      ...
      const valuationResult = await client.query(
        `SELECT * FROM student_valuation_summary WHERE student_id = $1`,
        [studentId]
      )
   ...
   * 生成成长对比卡片
   */
  async generateGrowthComparisonCard(
    studentId: string,
    triggerType: string,
    ...
    try {
      ...
      const cardId = uuidv4()
      await client.query(
        `INSERT INTO growth_comparison_cards (
  ...
  async generateLevelUpCeremony(
    ...
    try {
      ...
      // 1. 获取学生成长数据
      const growthDataResult = await client.query(
        `SELECT
   ...
   * AI生成成长对比文案
   */
  private async generateComparisonMessage(data: any): Promise<string> {
...
现在请生成文案，只输出文案内容，不要其他内容。`
    ...
    try {
      ...
      console.error('AI生成对比文案失败:', error)
   ...
   * AI生成导师升级留言
   */
  private async generateMentorLevelUpMessage(data: any): Promise<string> {
...
现在请生成留言，只输出留言内容。`
    ...
    try {
      ...
      console.error('AI生成导师留言失败:', error)
   ...
   * 获取熟练度标签
   */
  private getMasteryLabel(mastery: string): string {
   ...
   * 获取等级解锁能力
   */
  private getUnlockedAbilities(level: number): string[] {
   ...
   * 获取下一级要求
   */
  private getNextLevelRequirements(currentLevel: number): any {
    return {
...
export default new AssetVisualizationService()
