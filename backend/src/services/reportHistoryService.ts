...
class ReportHistoryService {
  ...
  ): Promise<{ reports: ReportVersion[]; total: number }> {
    try {
      ...
      const countResult = await queryOne<{ count: string }>(
        `SELECT COUNT(*) as count FROM student_reports ${whereClause}`,
        params
      );
      ...
      logger.error('[报告历史] 查询失败', { error, studentId });
      throw error;
    }
  }
  ...
  async compareReports(
    ...
    try {
      ...
      // 获取两个报告
      const olderReport = await queryOne<ReportVersion>(
        `SELECT * FROM student_reports
         WHERE id = $1 AND student_id = $2`,
        [olderReportId, studentId]
      );

      const newerReport = await queryOne<ReportVersion>(
        `SELECT * FROM student_reports
         WHERE id = $1 AND student_id = $2`,
        [newerReportId, studentId]
      );
      ...
      const changes = this.analyzeChanges(
        olderReport.report_data,
        newerReport.report_data
      );

      // 生成总结
      const summary = this.generateComparisonSummary(changes);

      logger.info('[报告对比] 对比完成', {
        studentId,
        changes
      });
      ...
      logger.error('[报告对比] 对比失败', { error, studentId });
      throw error;
    }
  }
   ...
   * 分析报告变化
   */
  private analyzeChanges(olderData: any, newerData: any): any {
    const changes: any = {
      skillImprovements: [],
      newMilestones: 0,
      taskCountIncrease: 0,
    ...
    // 找出新增的技能
    const addedSkills = newStrengths.filter(
      (skill: string) => !oldStrengths.includes(skill)
    );
  ...
  async getGrowthCurve(
    studentId: string,
    timeRange: number = 90
  ): Promise<GrowthCurveData> {
    try {
      logger.info('[成长曲线] 生成成长曲线数据', { studentId, timeRange });
      ...
      // 构建数据点
      const dataPoints = reports.rows.map((report: any) => ({
      ...
      logger.error('[成长曲线] 生成失败', { error, studentId });
      throw error;
    }
  }
   ...
   * 获取技能雷达图数据
   */
  async getSkillRadarData(studentId: string): Promise<any> {
    try {
      logger.info('[技能雷达] 获取技能雷达数据', { studentId });

      // 获取最新报告
      const latestReport = await queryOne<ReportVersion>(
        `SELECT report_data FROM student_reports
         WHERE student_id = $1
         ORDER BY generated_at DESC
         LIMIT 1`,
        [studentId]
      ...
      logger.error('[技能雷达] 获取失败', { error, studentId });
      throw error;
    }
  }
  ...
  private calculateDimensionScore(
    ...
    const baseScore = 50;
    const strengthBonus = strengths.filter((s: string) =>
      s.toLowerCase().includes(dimension.split('_')[0])
    ).length * 10;
    const weaknessPenalty = weaknesses.filter((w: string) =>
      w.toLowerCase().includes(dimension.split('_')[0])
    ).length * 10;
   ...
   * 获取里程碑时间轴
   */
  async getMilestoneTimeline(studentId: string): Promise<any[]> {
    try {
      logger.info('[里程碑时间轴] 获取时间轴数据', { studentId });
      ...
      reports.rows.forEach((report: any) => {
      ...
      logger.error('[里程碑时间轴] 获取失败', { error, studentId });
      throw error;
    }
  }
   ...
   * 去重里程碑
   */
  private deduplicateMilestones(milestones: any[]): any[] {
    const seen = new Set();
