...
interface LevelUpData {
  ...
  oldLevel: number;
  newLevel: number;
  triggerReason: string; // 'task_milestone' | 'quality_breakthrough' | 'skill_mastery'
}
...
interface CeremonyContent {
  ...
  soundEffect: string; // 'level_up' | 'milestone' | 'breakthrough'
}

interface StudentGrowthContext {
  nickname: string;
  opcPersonality: string;
...
class LevelUpCeremonyService {
   ...
   * 触发升级通关仪式
   */
  async triggerLevelUpCeremony(data: LevelUpData): Promise<{
    success: boolean;
    ceremonyId: string;
    ceremonyContent: CeremonyContent;
  }> {
    try {
      logger.info('[LevelUpCeremony] 开始生成升级仪式', data);
      ...
      logger.info('[LevelUpCeremony] 升级仪式生成成功', {
        ceremonyId,
      ...
      logger.error('[LevelUpCeremony] 升级仪式生成失败', { error, data });
      throw error;
    }
  }
   ...
   * 获取学生成长上下文
   */
  private async getStudentGrowthContext(studentId: string): Promise<StudentGrowthContext> {
    // 获取学生基础信息
    const studentInfo = await queryOne<{
      nickname: string;
      current_opc_personality: string;
      created_at: Date;
    }>(
      `SELECT nickname, current_opc_personality, created_at
       FROM users
       WHERE id = $1`,
      [studentId]
    );
    ...
    // 获取最近突破
    const recentBreakthroughs = await query<{ breakthrough: string }>(
      `SELECT breakthrough
    ...
    const daysOnPlatform = Math.floor(
      (Date.now() - new Date(studentInfo.created_at).getTime()) / (1000 * 60 * 60 * 24)
    );

    return {
      nickname: studentInfo.nickname || '同学',
      opcPersonality: studentInfo.current_opc_personality || 'unknown',
      totalTasksCompleted: parseInt(taskStats?.total_tasks as any) || 0,
      averageRating: parseFloat(taskStats?.avg_rating as any) || 0,
   ...
   * 使用AI生成个性化庆祝文案
   */
  private async generateCeremonyContent(
    data: LevelUpData,
    context: StudentGrowthContext
...

{
  ...
  "celebrationEmoji": "🎉"
    ...
    try {
      ...
      logger.error('[LevelUpCeremony] AI生成文案失败，使用默认文案', { error });
  ...
  private generateDefaultCeremony(
    ...
    const nextLevelPreviews: { [key: number]: string } = {
   ...
   * 选择音效类型
   */
  private selectSoundEffect(level: number, triggerReason: string): string {
    if (level >= 5) return 'milestone';
    if (triggerReason === 'quality_breakthrough') return 'breakthrough';
    return 'level_up';
  }
   ...
   * 获取触发原因标签
   */
  private getTriggerReasonLabel(reason: string): string {
   ...
   * 保存仪式记录
   */
  private async saveCeremonyRecord(
    data: LevelUpData,
   ...
   */
  private async triggerCeremonyNotification(
    studentId: string,
    ceremonyId: string,
    content: CeremonyContent,
    newLevel: number
    ...
    try {
      ...
      const { frontendEventEmitter } = await import('../utils/frontendEventEmitter');
      ...
      logger.info('[LevelUpCeremony] 前端升级事件已触发', { studentId, ceremonyId, newLevel });
      ...
      logger.info('[LevelUpCeremony] 通知已加入队列', {
        studentId,
        ceremonyId
      });

    } catch (error) {
      logger.error('[LevelUpCeremony] 触发通知失败', { error, studentId, ceremonyId });
      // 不抛出错误，允许仪式继续
    }
  }
   ...
   * 获取单个仪式详情
   */
  async getCeremonyById(ceremonyId: string): Promise<any> {
    const ceremony = await queryOne(
      `SELECT
         id, student_id, old_level, new_level, trigger_reason,
         title, main_message, achievements, next_level_preview,
         celebration_emoji, sound_effect, created_at
...
export const levelUpCeremonyService = new LevelUpCeremonyService();
