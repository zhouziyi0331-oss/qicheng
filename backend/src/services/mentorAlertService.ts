...
interface AlertRule {
  id: string;
  rule_type: 'level_gap' | 'repeated_rejection' | 'deadline_pressure' | 'direction_mismatch';
  rule_name: string;
...
class MentorAlertService {
   ...
   */
  async scanAndTriggerAlerts(): Promise<void> {
    try {
      logger.info('[MentorAlert] 开始扫描风险条件');
      ...
      logger.info(`[MentorAlert] 找到 ${activeOrders.length} 个进行中的订单`);
   ...
   */
  private async checkLevelGapAlerts(order: any, rules: AlertRule[]): Promise<void> {
   ...
   */
  private async checkRepeatedRejectionAlerts(order: any, rules: AlertRule[]): Promise<void> {
    ...
    if (submissions.rows.length < 2) return;

    // 分析是否存在连续同类问题
    const issues = submissions.rows.map(s => {
      if (!s.ai_review_json || !s.ai_review_json.issues) return [];
      return s.ai_review_json.issues.map((issue: any) => issue.category || issue.type);
    });

    // 找出连续出现的问题类型
    const repeatedIssue = this.findRepeatedIssue(issues);
    if (!repeatedIssue) return;
   ...
   */
  private async checkDeadlinePressureAlerts(order: any, rules: AlertRule[]): Promise<void> {
    ...
    // 检查是否满足触发条件
    if (remainingPercent > condition.time_remaining_percent) return;
    if (remainingTime < 0) return; // 已过期不预警
   ...
   */
  private async checkDirectionMismatchAlerts(order: any, rules: AlertRule[]): Promise<void> {
    ...
    if (submission.rows.length === 0) return;

    const aiReview = submission.rows[0].ai_review_json;
    if (!aiReview || !aiReview.mismatch_score) return;

    const condition = rule.trigger_condition;

    // 检查是否满足触发条件
    if (aiReview.mismatch_score < condition.ai_review_mismatch_score) return;
  ...
  private async triggerAlert(
    ...
    try {
      ...
      const alertMessage = await this.generateAlertMessage(
        studentId,
        orderId,
        rule,
        triggerData
      ...
      logger.info(`[MentorAlert] 预警已触发: ${rule.rule_type} for student ${studentId}, order ${orderId}`);
  ...
  private async generateAlertMessage(
    ...
    Object.keys(triggerData).forEach(key => {
      const value = (triggerData as any)[key];
   ...
   * 找出连续出现的问题类型
   */
  private findRepeatedIssue(issuesArray: string[][]): string | null {
    if (issuesArray.length < 2) return null;
...
export default new MentorAlertService();
