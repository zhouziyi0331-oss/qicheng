...
class CompanyStudentBridgeService {
  ...
  async recordMilestone(params: {
    ...
    try {
      ...
      logger.info('[CompanyStudentBridge] 记录成长里程碑', {
        milestoneId,
   ...
   * 通知订阅的企业
   */
  private async notifySubscribedCompanies(
    milestoneId: string,
    studentId: string,
    ...
    try {
      ...
      if (milestoneResult.rows.length === 0) return;
      ...
      for (const sub of subscriptions.rows) {
        ...
        // 生成通知内容
        const { title, content } = this.generateNotificationContent(
          studentName,
          milestoneType,
          milestone
        );
      ...
      logger.info('[CompanyStudentBridge] 通知企业完成', {
        milestoneId,
        notifiedCount
      });
    } finally {
  ...
  async subscribeToStudent(params: {
    ...
    try {
      ...
      logger.info('[CompanyStudentBridge] 企业订阅学生', {
   ...
   * 获取学生的声誉标签（学生视角）
   */
  async getStudentReputationTags(studentId: string): Promise<ReputationTag[]> {
    const client = await pool.connect();
    try {
      const result = await client.query(`
        SELECT
  ...
  async getCompanyNotifications(params: {
    ...
    limit?: number;
    offset?: number;
  }): Promise<{ notifications: GrowthNotification[]; total: number }> {
    const client = await pool.connect();
    try {
      ...
      const countResult = await client.query(
        `SELECT COUNT(*) FROM student_growth_notifications WHERE ${whereClause}`,
        queryParams
      );
   ...
   * 标记通知为已读
   */
  async markNotificationAsRead(notificationId: number, companyId: string): Promise<boolean> {
    const client = await pool.connect();
    try {
      const result = await client.query(`
  ...
  async getStudentMilestones(params: {
    ...
    try {
      ...
      const queryParams: any[] = [params.studentId];
      let paramIndex = 2;
...
export default new CompanyStudentBridgeService();
