...
export class MentorPromptBuilder {
   ...
   * 获取模板
   */
  async getTemplate(stage: MentorStage, templateName?: string): Promise<PromptTemplate | null> {
    try {
      let sql: string;
      let params: any[];
  ...
  async buildPrompt(
    ...
    try {
      ...
      if (!template) {
        // 如果没有找到模板，使用默认模板
        logger.warn('未找到Prompt模板，使用默认模板', { stage, templateName });
  ...
  private buildDefaultPrompt(
    ...
    switch (stage) {
      case MentorStage.REQUIREMENT_UNDERSTANDING:
        return {
      ...
      case MentorStage.EXECUTION_GUIDANCE:
        return {
          systemPrompt: this.getDefaultSystemPrompt(stage),
      ...
      case MentorStage.QUALITY_REVIEW:
        return {
          systemPrompt: this.getDefaultSystemPrompt(stage),
      ...
      case MentorStage.COMMUNICATION_BRIDGE:
        return {
          systemPrompt: this.getDefaultSystemPrompt(stage),
   ...
   * 获取默认系统Prompt
   */
  private getDefaultSystemPrompt(stage: MentorStage): string {
...
- 鼓励：肯定学生的努力`;
   ...
   * 构建会话历史上下文
   */
  buildConversationContext(messages: Array<{ role: string; content: string }>, maxMessages: number = 10): string {
  ...
  async saveTemplate(template: Omit<PromptTemplate, 'id' | 'createdAt' | 'updatedAt'>): Promise<string> {
    try {
      ...
      const existing = await queryOne<{ id: string; version: number }>(
        `SELECT id, version FROM mentor_prompt_templates
      ...
      if (existing) {
        // 停用旧版本
        await query(
          'UPDATE mentor_prompt_templates SET is_active = false WHERE id = $1',
          [existing.id]
        );
...
export const mentorPromptBuilder = new MentorPromptBuilder();
