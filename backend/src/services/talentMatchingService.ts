...
interface StudentTalentProfile {
  studentId: string;
  talents: Array<{
    tagId: number;
    tagName: string;
...
export class TalentMatchingService {
   ...
   * 为任务匹配学生
   */
  static async matchStudentsForTask(
    taskId: string,
    topN: number = 20
  ): Promise<MatchResult[]> {
    ...
    try {
      ...
      logger.info(`[TalentMatching] 找到 ${students.length} 个候选学生`);
      ...
      logger.info(`[TalentMatching] 匹配完成，返回Top${topN}学生`);
      topMatches.slice(0, 5).forEach((match, index) => {
        logger.info(`  ${index + 1}. 学生${match.studentId}: ${match.overallScore.toFixed(2)}分 - ${match.recommendation}`);
      });
      
      return topMatches;
      
    } catch (error) {
      logger.error('[TalentMatching] 匹配失败:', error);
      throw error;
    } finally {
      client.release();
    }
   ...
   * 获取任务需求的特质
   */
  private static async getTaskRequirements(
    client: any,
    taskId: string
    ...
    return {
      taskId,
      requiredTraits: result.rows.map((row: any) => ({
   ...
   * 获取学生的天赋画像
   */
  private static async getStudentTalentProfile(
    client: any,
    studentId: string
  ): Promise<StudentTalentProfile> {
    // 获取学生的天赋标签
    ...
    return {
      studentId,
      talents: talentsResult.rows.map((row: any) => ({
        tagId: row.tag_id,
        tagName: row.tag_name,
  ...
  private static async calculateMatch(
    client: any,
    studentProfile: StudentTalentProfile,
  ...
  private static calculateTalentMatch(
    ...
    const matchedTraits: any[] = [];
    const missingRequiredTraits: string[] = [];
    ...
    for (const requirement of taskRequirements.requiredTraits) {
      ...
      // 查找学生是否有这个特质
      const studentTalent = studentProfile.talents.find(
        t => t.tagId === requirement.traitTagId
      );
      ...
      if (studentTalent) {
        ...
        reasoning.push(
          `✓ ${requirement.tagName}: ${studentTalent.strength}级别，置信度${(studentTalent.confidence * 100).toFixed(0)}%`
        );
  ...
  private static calculateOPCCompatibility(
    ...
    if (stdDev < 15) return 1.0;
    if (stdDev < 25) return 0.85;
    if (stdDev < 35) return 0.7;
    return 0.6;
  }
  ...
  private static calculateGrowthPotential(
    ...
    // 1. 学习能力相关的特质
    const learningTalents = studentProfile.talents.filter(
      t => ['快速学习', '举一反三', '知识整合', '实践学习型'].includes(t.tagName)
    );
    ...
    // 2. 主动性相关的特质
    const proactiveTalents = studentProfile.talents.filter(
      t => ['自驱力强', '主动优化', '挑战精神'].includes(t.tagName)
    );
