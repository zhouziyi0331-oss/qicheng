 ...
 * 橙子的决策树
 */
export class OrangeDecisionTree {
   ...
   * 规则引擎：情绪检测
   */
  private detectEmotionByRules(message: string): {
    emotion: string;
    severity: string;
   ...
   * 匹配常见问题
   */
  private matchCommonQuestion(message: string, context: StudentContext): any {
    const msg = message.toLowerCase();
   ...
   * 阶段特定引导
   */
  private getStageSpecificGuidance(message: string, context: StudentContext): any {
   ...
   * 检查进度里程碑
   */
  private checkProgressMilestone(context: StudentContext): any {
   ...
   * 获取阶段的第一步
   */
  private getFirstStepForStage(stage: string): string {
   ...
   * 获取阶段的具体步骤
   */
  private getStageFirstSteps(stage: string): string[] {
 ...
 * 启程老师的决策树
 */
export class QiChengDecisionTree {
   ...
   * 快速翻译常见术语
   */
  quickTranslateTerm(term: string): { translation: string; explanation: string } | null {
    const termDict: Record<string, { translation: string; explanation: string }> = {
      'ROI': {
        translation: '投资回报率',
...
// 导出实例
export const orangeDecisionTree = new OrangeDecisionTree();
export const qichengDecisionTree = new QiChengDecisionTree();
