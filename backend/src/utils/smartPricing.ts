 ...
 */
export async function calculateTaskBudget(
  ...
  const baseRate = baseRates[requiredLevel as keyof typeof baseRates] || 50;
...
`;
    ...
    logger.error('AI budget calculation failed, using rule-based', { error: (err as Error).message });
    return calculateBudgetRuleBased(baseBudget, taskType, requiredLevel, estimatedHours);
  }
}
 ...
 * 基于规则的金额计算（降级方案）
 */
function calculateBudgetRuleBased(
  baseBudget: number,
  taskType: string,
  requiredLevel: number,
  estimatedHours: number
): { suggestedBudget: number; reasoning: string } {
  let multiplier = 1.0;
  let reasons: string[] = [];
...
export async function analyzeTaskRequirements(
  taskDescription: string,
  taskType: string
): Promise<{ requirements: string[]; warnings: string[]; estimatedDifficulty: number }> {
  // 开发模式或未配置API Key：使用规则引擎
  if (process.env.NODE_ENV === 'development' || !process.env.ANTHROPIC_API_KEY) {
...
`;
    ...
    logger.error('AI requirements analysis failed', { error: (err as Error).message });
    return analyzeRequirementsRuleBased(taskDescription, taskType);
  }
}
 ...
 * 基于规则的要求分析（降级方案）
 */
function analyzeRequirementsRuleBased(
  taskDescription: string,
  taskType: string
): { requirements: string[]; warnings: string[]; estimatedDifficulty: number } {
  ...
  // 估算难度
  let difficulty = 2;
  if (taskDescription.length > 200) difficulty += 1;
  if (taskType === 'B') difficulty += 1;
