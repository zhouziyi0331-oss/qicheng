 ...
 */
export const getPersonalityStats = async (req: Request, res: Response) => {
  ...
  try {
    ...
    // 3. 查询最快完成首单的天数（用于鼓励）
    const fastestResult = await query<{ fastest_days: number }>(
      `SELECT MIN(EXTRACT(DAY FROM (o.completed_at - o.accepted_at))) as fastest_days
       FROM orders o
...
export const getTrackStats = async (req: Request, res: Response) => {
  ...
  try {
    // 1. 该赛道完成订单数
    const orderCountResult = await query<{ count: number }>(
      `SELECT COUNT(*) as count
       FROM orders o
       JOIN tasks t ON o.task_id = t.id
       WHERE t.track = $1 AND o.status = 'completed'`,
      [track]
    );

    // 2. 该赛道市场均价（中位数）
    const medianPriceResult = await query<{ median_price: number }>(
      `SELECT PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY budget_gross) as median_price
       FROM tasks
       WHERE track = $1 AND status = 'active'`,
      [track]
    );

    // 3. 该赛道平均评分
    const avgRatingResult = await query<{ avg_rating: number }>(
      `SELECT AVG(o.client_rating) as avg_rating
       FROM orders o
       JOIN tasks t ON o.task_id = t.id
       WHERE t.track = $1 AND o.status = 'completed' AND o.client_rating IS NOT NULL`,
      [track]
    );
 ...
 * 基于学生的真实订单历史计算市场估值
 */
export const getStudentValuation = async (req: Request, res: Response) => {
  ...
  try {
    ...
    logger.error('Failed to get student valuation:', error);
    res.status(500).json({
      error: '获取能力估值失败',
      message: error.message
    });
