...
export class PBLAgentController {
  ...
  // 任务拆解引导
  async guideTaskDecomposition(req: Request, res: Response) {
    try {
  ...
  // 评估任务拆解
  async evaluateDecomposition(req: Request, res: Response) {
    try {
      const { task_id, subtasks } = req.body;

      const evaluation = await pblAgentService.evaluateDecomposition(
        task_id,
        subtasks
      );
  ...
  // 建议MVP方案
  async suggestMVP(req: Request, res: Response) {
    try {
      const { task_id, user_context } = req.body;

      const mvp = await pblAgentService.suggestMVPSolution(task_id, user_context);
  ...
  async executeCode(req: Request, res: Response) {
    try {
      ...
      const result = await pblAgentService.executeCode(
        project_id,
        language,
        code,
        task_id
  ...
  // 引导反思
  async guideReflection(req: Request, res: Response) {
    try {
      const { project_id, reflection_type } = req.body;

      const questions = await pblAgentService.guideReflection(
        project_id,
        reflection_type
      );
  ...
  async saveReflection(req: Request, res: Response) {
    try {
      ...
      const log = await pblAgentService.saveReflectionLog(
        project_id,
        phase_id,
        reflection
      );
...
export const pblAgentController = new PBLAgentController();
