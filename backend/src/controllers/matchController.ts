 ...
 * 3. 生成个性化匹配理由
 */
export const matchTasksForStudent = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const tasksResult = await pool.query(
      ...
      ORDER BY
        ...
        t.created_at DESC
      LIMIT $4`,
      [studentLevel, userId, opcTag, limit]
    );
 ...
 */
function calculateMatchScore(student: any, task: any, opcScores: any): number {
  let score = 0;

  // 1. 技能匹配度（0-40分）
  const studentSkills = student.skills || [];
  const taskSkills = task.required_skills || [];
  const matchedSkills = studentSkills.filter((s: string) => taskSkills.includes(s));
 ...
 * 生成个性化匹配理由
 */
function generateMatchReason(student: any, task: any, opcScores: any): string {
  const reasons: string[] = [];

  // 1. 基于OPC人格标签的理由
  if (student.opc_personality_tag && task.required_personality_style) {
    if (student.opc_personality_tag === task.required_personality_style) {
      const styleDescriptions: any = {
 ...
 */
export const getTaskDetailWithMatch = async (req: Request, res: Response) => {
