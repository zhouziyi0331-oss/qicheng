...
export async function getClientContactInfo(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const collaborationResult = await pool.query(
      `SELECT COUNT(*) as count,
       ...
       WHERE ta.student_id = $1 AND t.client_id = $2 AND ta.status = 'completed'`,
      [studentId, clientId]
    );
    ...
    const clientResult = await pool.query(
      `SELECT u.name, c.contact_person, c.wechat, c.phone
       FROM users u
