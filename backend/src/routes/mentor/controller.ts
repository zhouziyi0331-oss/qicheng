...
export async function sendTaskStartGuidance(taskId: string, studentId: string): Promise<void> {
  try {
    // 获取任务信息
    ...
    }>('SELECT title, description, track_type, acceptance_criteria FROM tasks WHERE id = $1', [taskId]);
    ...
    const toolHistory = await query<{ tool: string }>(
      `SELECT DISTINCT jsonb_array_elements_text(milestone_data->'tools_used') as tool
       ...
       WHERE student_id = $1 AND milestone_type = 'new_tool'`,
      [studentId]
    );

    const usedTools = toolHistory.map((t: { tool: string }) => t.tool).join(', ') || '暂无';
...
如果卡住了，随时告诉我卡在哪里，我们一起想办法！`;
    ...
    // 保存对话记录
    await query(
      `INSERT INTO mentor_conversations (student_id, task_id, trigger_type, mentor_response, response_time_ms)
       VALUES ($1, $2, $3, $4, $5)`,
      [studentId, taskId, 'task_start', mentorResponse, responseTime]
    );
...
export async function handleStuckMessage(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    // 记录卡点
    await query(
      `INSERT INTO student_stuck_points (student_id, task_id, stuck_description)
       VALUES ($1, $2, $3)`,
    ...
    // 获取最近对话
    const recentConversations = await query<{ user_message: string; mentor_response: string }>(
      `SELECT user_message, mentor_response FROM mentor_conversations
    ...
    // 获取历史卡点
    const stuckHistory = await query<{ stuck_description: string; resolved: boolean }>(
      `SELECT stuck_description, resolved FROM student_stuck_points
       WHERE student_id = $1
       ORDER BY created_at DESC LIMIT 5`,
      [studentId]
    );
...
## 最近对话
${recentConversations.map((c: { user_message: string; mentor_response: string }) => `学生：${c.user_message || '（无）'}\n导师：${c.mentor_response}`).join('\n\n')}

## 学生历史卡点
${stuckHistory.map((s: { stuck_description: string; resolved: boolean }) => `- ${s.stuck_description} (${s.resolved ? '已解决' : '未解决'})`).join('\n')}
...
如果你能告诉我具体卡在哪里，我可以给你一些线索。`;
    ...
    // 保存对话记录
    await query(
      `INSERT INTO mentor_conversations (student_id, task_id, trigger_type, user_message, mentor_response, response_time_ms)
       VALUES ($1, $2, $3, $4, $5, $6)`,
      [studentId, taskId, 'stuck', message, mentorResponse, responseTime]
    );
...
export async function generateRejectionFeedback(
  ...
  try {
    ...
    const submission = await queryOne<{
      ...
      `SELECT student_id, task_id, submission_note, file_urls
       FROM task_submissions WHERE id = $1`,
      [submissionId]
    );
...
如果你不确定怎么改，告诉我你的想法，我们一起看看。`;
    ...
    await query(
      `INSERT INTO mentor_conversations (student_id, task_id, trigger_type, mentor_response, response_time_ms)
       VALUES ($1, $2, $3, $4, $5)`,
      [submission.student_id, submission.task_id, 'rejected', mentorResponse, responseTime]
    );
...
export async function checkIdleStudents(): Promise<void> {
  try {
    // 查找超过2小时没有操作的学生
    const idleStudents = await query<{
      student_id: string;
      task_id: string;
      title: string;
      deadline: string;
    ...
    for (const student of idleStudents) {
      // 获取轻推次数
      const nudgeLog = await queryOne<{ nudge_count: number }>(
        'SELECT nudge_count FROM mentor_nudge_log WHERE student_id = $1 AND task_id = $2',
        [student.student_id, student.task_id]
      );
      ...
      if (nudgeCount === 0) {
        ...
        const daysLeft = Math.ceil(
          (new Date(student.deadline).getTime() - Date.now()) / (1000 * 60 * 60 * 24)
        );
      ...
      await query(
        `INSERT INTO mentor_conversations (student_id, task_id, trigger_type, mentor_response)
         VALUES ($1, $2, $3, $4)`,
        [student.student_id, student.task_id, 'idle', message]
      );

      // 更新轻推记录
      await query(
        `INSERT INTO mentor_nudge_log (student_id, task_id, nudge_count)
         VALUES ($1, $2, 1)
         ON CONFLICT (student_id, task_id)
         DO UPDATE SET nudge_count = mentor_nudge_log.nudge_count + 1, last_nudge_at = NOW()`,
        [student.student_id, student.task_id]
      );
...
export async function celebrateMilestone(
  ...
  try {
    ...
    const milestoneHistory = await query<{ milestone_type: string; mentor_message: string }>(
      `SELECT milestone_type, mentor_message FROM student_milestones
       WHERE student_id = $1
       ORDER BY created_at DESC LIMIT 5`,
      [studentId]
    );
...
请生成见证消息（100字以内）：`;
    ...
    // 保存里程碑记录
    await query(
      `INSERT INTO student_milestones (student_id, milestone_type, milestone_data, mentor_message)
       VALUES ($1, $2, $3, $4)`,
      [studentId, milestoneType, JSON.stringify(milestoneData), mentorResponse]
    );

    // 保存对话记录
    await query(
      `INSERT INTO mentor_conversations (student_id, trigger_type, mentor_response, response_time_ms)
       VALUES ($1, $2, $3, $4)`,
      [studentId, 'milestone', mentorResponse, responseTime]
    );
...
export async function getConversations(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const conversations = await query<{
      ...
      `SELECT id, trigger_type, user_message, mentor_response, created_at
