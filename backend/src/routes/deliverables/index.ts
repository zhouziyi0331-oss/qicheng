...
router.post('/create', authenticateToken, async (req, res) => {
  try {
    ...
    const archive = await deliverableArchiveService.createArchive({
      task_id,
...
router.get('/list', authenticateToken, async (req, res) => {
  try {
    ...
    const result = await deliverableArchiveService.getArchives({
      companyId,
...
router.get('/:id', authenticateToken, async (req, res) => {
  try {
    ...
    const archive = await deliverableArchiveService.getArchiveById(id, companyId);
...
router.put('/:id', authenticateToken, async (req, res) => {
  try {
    ...
    const archive = await deliverableArchiveService.updateArchive(id, companyId, req.body);
...
router.delete('/:id', authenticateToken, async (req, res) => {
  try {
    ...
    await deliverableArchiveService.deleteArchive(id, companyId);
...
router.post('/:id/download', authenticateToken, async (req, res) => {
  try {
    ...
    await deliverableArchiveService.recordDownload(
      id,
      userId,
      files || [],
      method || 'all'
...
router.post('/:id/versions', authenticateToken, async (req, res) => {
  try {
    ...
    const version = await deliverableArchiveService.addVersion(
      id,
      files,
      change_notes || '',
      userId
...
router.post('/categories/create', authenticateToken, async (req, res) => {
  try {
    ...
    const category = await deliverableArchiveService.createCategory(
      companyId,
      name,
      description,
      color,
...
router.get('/categories/list', authenticateToken, async (req, res) => {
  try {
    ...
    const categories = await deliverableArchiveService.getCategories(companyId);
...
router.put('/categories/:id', authenticateToken, async (req, res) => {
  try {
    ...
    const category = await deliverableArchiveService.updateCategory(id, companyId, req.body);
...
router.delete('/categories/:id', authenticateToken, async (req, res) => {
  try {
    ...
    await deliverableArchiveService.deleteCategory(id, companyId);
...
router.post('/:id/share', authenticateToken, async (req, res) => {
  try {
    ...
    const share = await deliverableArchiveService.createShareLink(id, companyId, {
      password,
...
router.get('/share/:code', async (req, res) => {
  try {
    ...
    const share = await deliverableArchiveService.validateShareLink(
      code,
      password as string
    );
...
router.post('/share/:code/download', async (req, res) => {
  try {
    ...
    await deliverableArchiveService.validateShareLink(code, password);
    await deliverableArchiveService.recordShareDownload(code);
...
router.get('/stats/overview', authenticateToken, async (req, res) => {
  try {
    ...
    const stats = await deliverableArchiveService.getArchiveStats(companyId);
...
router.post('/batch-update', authenticateToken, async (req, res) => {
  try {
    ...
    await deliverableArchiveService.batchUpdateArchives(archive_ids, companyId, updates);
