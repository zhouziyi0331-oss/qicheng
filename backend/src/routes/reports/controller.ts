...
export async function listReports(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const purchasedReports = await query<{ report_type: string; status: string; generated_at: Date }>(
      `SELECT report_type, status, generated_at
       FROM opc_reports WHERE user_id = $1 AND deleted_at IS NULL`,
      [userId]
    );
...
export async function orderReport(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    const userId = req.user!.userId;
    ...
    const alreadyPurchased = await queryOne(
      `SELECT id FROM opc_reports WHERE user_id = $1 AND report_type = $2 AND deleted_at IS NULL`,
      [userId, reportType]
    );
    if (alreadyPurchased) throw new AppError(409, '你已购买过此报告', 'ALREADY_PURCHASED');
    ...
    await withTransaction(async (client) => {
      // 创建报告记录 (待支付)
      await client.query(
        `INSERT INTO opc_reports (id, user_id, report_type, status, paid_amount)
         VALUES ($1,$2,$3,'pending',$4)`,
        [reportId, userId, reportType, price]
      );
      ...
      await client.query(
        `INSERT INTO payments
         ...
         VALUES ($1,$2,'student',$3,0,$4,'pending')`,
        [paymentId, userId, price, price]
      );
    });
    ...
    if (paymentMethod === 'wechat') {
      paymentParams = generateWechatPayParams(paymentId, price, `购买OPC报告-${reportName}`);
    } else {
      paymentParams = generateAlipayParams(paymentId, price, `购买OPC报告-${reportName}`);
    }
...
export async function getReport(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const report = await queryOne<{
      report_type: string; status: string; content_json: unknown; generated_at: Date;
    }>(
      `SELECT report_type, status, content_json, generated_at
       FROM opc_reports WHERE id = $1 AND user_id = $2 AND deleted_at IS NULL`,
      [id, userId]
    );
    if (!report) throw new AppError(404, '报告不存在', 'REPORT_NOT_FOUND');
...
// ============================================================
export async function triggerReportGeneration(reportId: string, userId: string): Promise<void> {
  try {
    await query(
      `UPDATE opc_reports SET status = 'generating', generation_started_at = NOW() WHERE id = $1`,
      [reportId]
    );

    // 获取用户全量数据（包含具体任务历史）
    const userData = await query(
      `SELECT sp.*, tr.answers_json, tr.opc_label, tr.d1_score, tr.d2_score,
    ...
    const report = await queryOne<{ report_type: string }>(
      'SELECT report_type FROM opc_reports WHERE id = $1',
      [reportId]
    );
    if (!report) return;
