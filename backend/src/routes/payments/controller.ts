...
export async function getWithdrawalHistory(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    const userId = req.user!.userId;
    const withdrawals = await query(
      `SELECT id, amount, method, status, auto_processed, created_at, processed_at
       FROM withdrawals
