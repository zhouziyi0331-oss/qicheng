...
import { RitualService } from '../../services/trustAccelerator/ritualService';
...
router.get('/eligible-matches', async (req, res) => {
  try {
    ...
    const matches = await MatchService.getEligibleMatches(studentId);
...
router.get('/match-status/:companyId', async (req, res) => {
  try {
    ...
    const match = await MatchService.getMatchStatus(studentId, companyId);
    const matchReason = match
      ? await MatchService.generateMatchReason(studentId, companyId)
      : null;
...
router.post('/verify/start', async (req, res) => {
  try {
    ...
    const result = await VerifyService.createSession(studentId, companyId, matchId);
...
router.post('/verify/round1', async (req, res) => {
  try {
    ...
    const result = await VerifyService.submitRound1Answer(sessionId, answer);
...
router.post('/verify/round2', async (req, res) => {
  try {
    ...
    const result = await VerifyService.submitRound2Answer(sessionId, answer);
...
router.get('/verify/status/:sessionId', async (req, res) => {
  try {
    ...
    const session = await VerifyService.getSessionStatus(sessionId);

    // 如果是round1_pass，返回第二轮题目
    let round2Question = null;
    if (session.status === 'round1_pass') {
      round2Question = await VerifyService.getRound2Question(sessionId);
    }
...
router.post('/unlock/create-payment', async (req, res) => {
  try {
    ...
    const payment = await UnlockService.createUnlockPayment(studentId, sessionId);
...
router.post('/unlock/payment-callback', async (req, res) => {
  try {
    ...
    const result = await UnlockService.handlePaymentSuccess(out_trade_no, transaction_id);

    // 异步生成证书
    RitualService.generateCertificate(result.unlockRecordId).catch(err => {
...
router.get('/unlock/record/:companyId', async (req, res) => {
  try {
    ...
    const record = await UnlockService.getUnlockRecord(studentId, companyId);
...
router.post('/unlock/view-contact', async (req, res) => {
  try {
    ...
    await UnlockService.recordContactViewed(unlockRecordId);
...
router.post('/unlock/feedback', async (req, res) => {
  try {
    ...
    await UnlockService.collectFeedback(
      unlockRecordId,
      studentContacted,
      merchantContacted
    );
