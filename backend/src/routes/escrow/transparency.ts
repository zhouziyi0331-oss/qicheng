...
router.get('/:taskId/flow', authenticateToken, async (req, res) => {
  try {
    ...
    const overview = await escrowTransparencyService.getEscrowFlowOverview(taskId);
...
router.post('/:taskId/deposit', authenticateToken, async (req, res) => {
  try {
    ...
    const result = await escrowTransparencyService.depositFunds(
      taskId,
      companyId,
      amount,
    ...
    // 自动锁定资金
    await escrowTransparencyService.lockFunds(taskId, amount);
...
router.post('/:taskId/release', authenticateToken, async (req, res) => {
  try {
    ...
    await escrowTransparencyService.releaseFunds(taskId, studentId, amount, feeAmount);
...
router.post('/:taskId/refund', authenticateToken, async (req, res) => {
  try {
    ...
    await escrowTransparencyService.refundToCompany(taskId, companyId, amount, reason);
...
router.put('/nodes/:nodeId/status', authenticateToken, async (req, res) => {
  try {
    ...
    const node = await escrowTransparencyService.updateFlowNodeStatus(
      nodeId,
      status,
      userId,
      userRole,
...
router.post('/transactions/:transactionId/complete', authenticateToken, async (req, res) => {
  try {
    ...
    await escrowTransparencyService.completeTransaction(transactionId);
...
router.post('/transactions/:transactionId/fail', authenticateToken, async (req, res) => {
  try {
    ...
    await escrowTransparencyService.failTransaction(transactionId, reason);
