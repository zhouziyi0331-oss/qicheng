 ...
 * 获取导师详情
 */
export async function getMentorDetail(req: Request, res: Response) {
  try {
    const { id } = req.params;

    const mentor = await query<any>(
      `SELECT
        u.id,
        u.nickname,
        u.avatar_url,
 ...
 * 获取咨询会话列表
 */
export async function getMentorSessions(req: Request, res: Response) {
  try {
