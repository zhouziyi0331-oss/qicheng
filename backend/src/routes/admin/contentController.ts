 ...
 * 获取OPC故事墙列表
 */
export async function getOPCStories(req: Request, res: Response) {
  try {
 ...
 * 审核OPC故事
 */
export async function reviewOPCStory(req: Request, res: Response) {
  try {
    const { id } = req.params;
    ...
    await query(
      `UPDATE articles
       SET status = $1,
           updated_at = NOW()
       WHERE id = $2 AND category = 'opc_story'`,
      [status, id]
    );
 ...
 * 删除OPC故事
 */
export async function deleteOPCStory(req: Request, res: Response) {
  try {
    const { id } = req.params;
    ...
    await query(
      `UPDATE articles
       SET status = 'archived',
           updated_at = NOW()
       WHERE id = $1 AND category = 'opc_story'`,
      [id]
    );
...
export async function createAnnouncement(req: Request, res: Response) {
  try {
    ...
    const result = await query<any>(
      `INSERT INTO announcements (title, content, type, target_audience, status)
    ...
    await query(
      `UPDATE announcements
       SET title = $1,
           ...
           updated_at = NOW()
       WHERE id = $6`,
      [title, content, type, targetAudience, status, id]
    );
...
export async function createBanner(req: Request, res: Response) {
  try {
    ...
    const result = await query<any>(
      `INSERT INTO banners (title, image_url, link_value, order_index, status)
       VALUES ($1, $2, $3, $4, 'published')
       RETURNING id`,
      [title, imageUrl, linkUrl, sortOrder || 0]
    );
    ...
    await query(
      `UPDATE banners
       SET title = $1,
           ...
           updated_at = NOW()
       WHERE id = $6`,
      [title, imageUrl, linkUrl, sortOrder, status, id]
    );
