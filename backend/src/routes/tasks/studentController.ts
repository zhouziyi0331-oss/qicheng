...
// 获取匹配的任务（基于能力画像和邀请）
// ============================================================
export async function getMatchedTasks(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    const userId = req.user!.userId;

    // 获取学生收到的邀请任务
    const invitedTasks = await query<any>(
      `SELECT t.*, ti.custom_message, ti.created_at as invited_at
       FROM tasks t
    ...
    // 使用等级过滤服务获取匹配的任务
    const filteredResult = await levelFilterService.filterTasksByLevel(userId, {
      limit: 10,
      sortBy: 'match_score',
      sortOrder: 'DESC',
    });
...
export async function acceptTask(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    // 检查等级匹配 (硬性规则，不允许超纲接单)
    const canAccept = await levelFilterService.canAcceptTask(userId, taskId);
    if (!canAccept.canAccept) {
      throw new AppError(403, canAccept.reason || '无法接受此任务', 'CANNOT_ACCEPT_TASK');
    }
    ...
    // 获取学生等级用于AI任务拆解
    const student = await queryOne<{ current_level: number }>(
...
export async function updateProgress(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    if (typeof progress !== 'number' || progress < 0 || progress > 100) {
      throw new AppError(400, '进度值必须在0-100之间', 'INVALID_PROGRESS');
    }

    // 验证任务分配
    const assignment = await queryOne<{ id: string; status: string }>(
      'SELECT id, status FROM task_assignments WHERE task_id = $1 AND student_id = $2',
      [taskId, userId]
    );

    if (!assignment) {
      throw new AppError(404, '任务分配不存在', 'ASSIGNMENT_NOT_FOUND');
    }

    if (assignment.status === 'completed' || assignment.status === 'submitted') {
      throw new AppError(400, '已完成或已提交的任务不能更新进度', 'TASK_ALREADY_DONE');
    }

    // 更新进度
    await query(
      'UPDATE task_assignments SET progress = $1, updated_at = NOW() WHERE id = $2',
