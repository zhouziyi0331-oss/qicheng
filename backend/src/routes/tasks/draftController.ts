...
// 保存任务发布草稿（企业端）
export const saveTaskDraft = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const existingDraft = await query(
      'SELECT id FROM task_drafts WHERE user_id = $1 AND draft_type = $2',
      [userId, 'publish']
    );
    ...
    if (existingDraft.length > 0) {
      ...
      // 创建新草稿
      result = await query(
        `INSERT INTO task_drafts (user_id, draft_type, content, created_at, updated_at)
...
// 获取任务发布草稿（企业端）
export const getTaskDraft = async (req: AuthRequest, res: Response) => {
  try {
    const userId = req.user?.userId;
...
// 保存任务提交草稿（学生端）
export const saveSubmitDraft = async (req: AuthRequest, res: Response) => {
  try {
    const userId = req.user?.userId;
    ...
    // 检查是否已有该任务的草稿
    const existingDraft = await query(
      'SELECT id FROM task_drafts WHERE user_id = $1 AND draft_type = $2 AND task_id = $3',
      [userId, 'submit', taskId]
    );
    ...
    if (existingDraft.length > 0) {
      ...
      // 创建新草稿
      result = await query(
        `INSERT INTO task_drafts (user_id, draft_type, task_id, content, created_at, updated_at)
...
// 获取任务提交草稿（学生端）
export const getSubmitDraft = async (req: AuthRequest, res: Response) => {
  try {
    const userId = req.user?.userId;
...
export const deleteDraft = async (req: AuthRequest, res: Response) => {
  try {
    ...
    await query(
      'DELETE FROM task_drafts WHERE id = $1 AND user_id = $2',
      [draftId, userId]
    );
