...
describe('New Features API Tests', () => {
  ...
  afterAll(async () => {
    // 清理测试数据
    await query('DELETE FROM task_assignments WHERE student_id = $1', [testStudentId]);
    await query('DELETE FROM tasks WHERE id = $1', [testTaskId]);
    await query('DELETE FROM users WHERE phone IN ($1, $2)', ['13800000001', '13800000002']);
  });
