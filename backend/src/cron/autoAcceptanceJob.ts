...
export class AutoAcceptanceJob {
  ...
  async execute(): Promise<void> {
    ...
    try {
      ...
      logger.info(`找到${ordersToAccept.length}个需要自动确认的订单`);
      ...
      for (const order of ordersToAccept) {
        try {
          ...
          // 3. 记录自动确认
          await client.query(
            `INSERT INTO auto_acceptances (order_id, task_id, student_id, company_id, reason, executed_at)
          ...
          logger.info(`✅ 自动确认订单: ${order.order_id}, 任务: ${order.task_title}`);

        } catch (error: any) {
          await client.query('ROLLBACK');
          logger.error(`❌ 自动确认订单 ${order.order_id} 失败:`, error);
        }
      }

      logger.info(`48小时自动确认任务完成，处理了${ordersToAccept.length}个订单`);

    } catch (error: any) {
      logger.error('执行48小时自动确认任务失败:', error);
      throw error;
    } finally {
      client.release();
    }
