...
import mentorAutoTriggerService from '../src/services/mentorAutoTriggerService';
 ...
 * 测试T-02: Stuck响应 + 真实案例
 */
async function testT02StuckResponse(): Promise<void> {
  ...
  try {
    ...
    const response = await mentorCoreService.chat(
      student.id,
      testMessage
    );
 ...
 * 测试T-04: 轻推消息 + 真实对话引用
 */
async function testT04NudgeMessage(): Promise<void> {
  console.log('\n=== 测试 T-04: 轻推消息 + 真实对话引用 ===\n');

  try {
    // 1. 查找一个有对话历史的任务
    const taskWithHistory = await queryOne<{
      task_id: string;
      student_id: string;
      task_title: string;
    }>(
      `SELECT DISTINCT ms.task_id, ms.student_id, t.title as task_title
    ...
    // 2. 测试获取最后一条消息
    const lastMessage = await mentorContextEnhancer.getLastStudentMessage(
      taskWithHistory.task_id
    );
    ...
    if (lastMessage) {
      ...
      // 3. 触发T-04轻推消息
      const nudgeMessage = await mentorAutoTriggerService.triggerT04(
        taskWithHistory.task_id,
        taskWithHistory.student_id
      );
 ...
 * 测试T-05: 里程碑见证 + 真实成长对比
 */
async function testT05MilestoneWitness(): Promise<void> {
  console.log('\n=== 测试 T-05: 里程碑见证 + 真实成长对比 ===\n');

  try {
    // 1. 查找一个已完成的任务分配
    const completedAssignment = await queryOne<{
      id: string;
      student_id: string;
      task_id: string;
      student_name: string;
    ...
    const growthComparison = await mentorContextEnhancer.getGrowthComparison(
      completedAssignment.student_id,
      completedAssignment.id
    );

    console.log('📊 成长对比数据:');
    console.log(`初始能力缺口: ${growthComparison.initial_gaps.length}个`);
    growthComparison.initial_gaps.forEach((gap: string) => console.log(`  - ${gap}`));

    console.log(`\n本单展示能力: ${growthComparison.current_skills.length}个`);
    growthComparison.current_skills.forEach((skill: string) => console.log(`  - ${skill}`));

    console.log(`\n已闭合缺口: ${growthComparison.gaps_closed.length}个`);
    growthComparison.gaps_closed.forEach((gap: string) => console.log(`  - ${gap}`));
    ...
    // 3. 触发T-05里程碑见证
    console.log('\n生成里程碑见证消息...\n');
    const witnessMessage = await mentorAutoTriggerService.triggerT05(completedAssignment.id);
    ...
    const mentionsGaps = growthComparison.gaps_closed.length > 0 &&
      growthComparison.gaps_closed.some((gap: string) =>
        witnessMessage.content.includes(gap.substring(0, 10))
      );
 ...
 * 测试AI-07: 初心审核引擎
 */
async function testAI07PrincipleReview(): Promise<void> {
  console.log('\n=== 测试 AI-07: 初心审核引擎 ===\n');
