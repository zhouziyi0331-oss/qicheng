...
async function testSemanticMatching() {
  ...
  try {
    ...
    const tables = await pool.query(`
      ...
      ORDER BY tablename
    `);
    console.log('✅ 相关表存在:', tables.rows.map((r: any) => r.tablename).join(', '));
    ...
    console.log('\n4. 测试任务向量生成...');
    try {
      await vectorGenerationService.updateTaskEmbedding(testTask.id);
    ...
    try {
      await vectorGenerationService.updateStudentEmbedding(testStudent.id);
    ...
    console.log('\n6. 测试启程老师翻译服务...');
    try {
      await qichengTeacherService.analyzeAndTranslateTask(testTask.id);

      const translation = await pool.query(
        `SELECT student_friendly_title, difficulty_overall, learning_value
    ...
    console.log('\n8. 测试批量匹配（找出Top 5学生）...');
    try {
      const topMatches = await semanticMatchingEngine.findBestStudentsForTask(testTask.id, 5);
    ...
    // 9. 检查匹配记录
    console.log('\n9. 检查匹配记录表...');
    const matchRecords = await pool.query(`
