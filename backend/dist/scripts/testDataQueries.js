...
async function testDataAvailability() {
    ...
    try {
        // 查找学生
        const students = await (0, db_1.query)(`SELECT id, nickname FROM users WHERE user_type = 'student' LIMIT 5`);
        console.log(`找到 ${students.length} 个学生:`);
        students.forEach(s => console.log(`  - ${s.nickname} (${s.id})`));
        // 查找任务
        const tasks = await (0, db_1.query)(`SELECT id, title, track FROM tasks LIMIT 5`);
        console.log(`\n找到 ${tasks.length} 个任务:`);
        tasks.forEach(t => console.log(`  - ${t.title} [${t.track}]`));
        // 查找任务分配
        const assignments = await (0, db_1.query)(`SELECT id, status FROM task_assignments LIMIT 5`);
 ...
 */
async function testGetRealStuckCase() {
    console.log('\n=== 测试 T-02: getRealStuckCase ===\n');
    try {
        // 查找有卡点观察的任务
        const observation = await (0, db_1.queryOne)(`SELECT student_id, task_id, observation_content
       ...
       LIMIT 1`);
        if (observation) {
            ...
            // 测试查询
            const realCase = await mentorContextEnhancer_1.default.getRealStuckCase(observation.student_id, observation.task_id);
            if (realCase) {
                console.log('\n✅ 成功查询到真实案例:');
 ...
 */
async function testGetLastStudentMessage() {
    console.log('\n=== 测试 T-04: getLastStudentMessage ===\n');
    try {
        // 查找有消息的会话
        const session = await (0, db_1.queryOne)(`SELECT DISTINCT task_id
       ...
       LIMIT 1`);
        if (session) {
            ...
            // 测试查询
            const lastMessage = await mentorContextEnhancer_1.default.getLastStudentMessage(session.task_id);
            if (lastMessage) {
                console.log('\n✅ 成功查询到最后一条消息:');
 ...
 */
async function testGetGrowthComparison() {
    console.log('\n=== 测试 T-05: getGrowthComparison ===\n');
    try {
        // 查找已完成的任务分配
        const assignment = await (0, db_1.queryOne)(`SELECT id, student_id, task_id
       ...
       LIMIT 1`);
        if (assignment) {
            ...
            // 测试查询
            const growthComparison = await mentorContextEnhancer_1.default.getGrowthComparison(assignment.student_id, assignment.id);
            console.log('\n📊 成长对比数据:');
...
// 执行测试
main();
//# sourceMappingURL=testDataQueries.js.map