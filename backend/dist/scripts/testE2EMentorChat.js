...
async function testRealAIChat() {
    ...
    try {
        ...
        console.log(`\n消息1: ${message1}`);
        const response1 = await mentorCoreService_1.default.chat(student.id, message1);
        console.log(`\nAI回复1:\n${response1.response}\n`);
        console.log(`Token使用: ${response1.tokensUsed}`);
        console.log(`响应时间: ${response1.responseTime}ms`);
        console.log(`\n消息2: ${message2}`);
        const response2 = await mentorCoreService_1.default.chat(student.id, message2);
        console.log(`\nAI回复2:\n${response2.response}\n`);
...
async function testAI07Review() {
    ...
    try {
        ...
        const review1 = await principleReviewService_1.default.reviewMentorResponse(goodResponse, { studentLevel: 2, hasRealCaseData: true });
        ...
        const review2 = await principleReviewService_1.default.reviewMentorResponse(badResponse, { studentLevel: 2, hasRealCaseData: false });
...
async function testT02RealCase() {
    ...
    try {
        // 先创建一个测试卡点观察
        const student = await (0, db_1.queryOne)(`SELECT id FROM users WHERE user_type = 'student' LIMIT 1`);
        const task = await (0, db_1.queryOne)(`SELECT id, track FROM tasks LIMIT 1`);
        if (!student || !task) {
            results.push({
                test: 'T-02真实卡点案例',
                passed: false,
        ...
        // 查询是否有同赛道的卡点案例
        const realCase = await mentorContextEnhancer_1.default.getRealStuckCase(student.id, task.id);
        if (realCase) {
            console.log(`\n✅ 找到真实案例:`);
...
async function testDatabaseWrite() {
    ...
    try {
        ...
        // 查询会话创建前的消息数
        const beforeCount = await (0, db_1.queryOne)(`SELECT COUNT(*) as count FROM mentor_messages`);
        ...
        // 查询会话创建后的消息数
        const afterCount = await (0, db_1.queryOne)(`SELECT COUNT(*) as count FROM mentor_messages`);
        console.log(`发送后消息数: ${afterCount?.count || 0}`);
        // 验证该会话的消息
        const sessionMessages = await (0, db_1.query)(`SELECT role, content FROM mentor_messages WHERE session_id = $1 ORDER BY created_at`, [response.sessionId]);
        console.log(`\n该会话的消息记录:`);
...
// 执行测试
main();
//# sourceMappingURL=testE2EMentorChat.js.map