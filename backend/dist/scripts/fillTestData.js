...
async function generateOPCTestData() {
  ...
  `);
    ...
    for (const student of students.rows) {
        ...
        // 插入数据
        await db_1.pool.query(`INSERT INTO user_opc_results (
        ...
        // 更新用户表
        await db_1.pool.query(`UPDATE users SET opc_personality_tag = $1, opc_completed_at = NOW() WHERE id = $2`, [personalityTag, student.id]);
  ...
  `);
    ...
    for (const student of students.rows) {
        try {
            ...
            await vectorGenerationService_1.default.updateStudentEmbedding(student.id);
...
async function triggerTaskMatching() {
  ...
  `);
    if (tasks.rows.length === 0) {
        ...
        // 获取任意3个任务
        const anyTasks = await db_1.pool.query(`
...
async function matchTask(task) {
    try {
        console.log(`\n匹配任务: ${task.title}`);
        const matches = await semanticMatchingEngine_1.default.findBestStudentsForTask(task.id, 10);
        if (matches.length === 0) {
...
async function showStatistics() {
    ...
    // OPC测评数据
    const opcCount = await db_1.pool.query(`
  ...
  `);
    ...
    // 学生向量
    const vectorCount = await db_1.pool.query(`
    SELECT COUNT(*) as total,
  ...
  `);
    ...
    // 任务匹配
    const matchCount = await db_1.pool.query(`
    SELECT COUNT(*) as total,
  ...
  `);
    ...
    // 人格标签分布
    const tagDist = await db_1.pool.query(`
...
main();
//# sourceMappingURL=fillTestData.js.map