...
exports.printManualTestGuide = printManualTestGuide;
...
async function testStudentProfile(token) {
    log('📝', '测试2: 学生档案查询');
    try {
        const response = await axios_1.default.get(`${API_BASE}/student/profile`, {
...
// 测试3: 任务列表等级过滤
async function testTaskFiltering(token) {
    log('📝', '测试3: 任务列表等级过滤');
    try {
        const response = await axios_1.default.get(`${API_BASE}/tasks/matched`, {
            headers: { Authorization: `Bearer ${token}` },
        });
...
// 测试4: 跳级资格检查
async function testJumpEligibility(token) {
    log('📝', '测试4: 跳级资格检查');
    try {
        const response = await axios_1.default.get(`${API_BASE}/students/jump-eligibility`, {
            headers: { Authorization: `Bearer ${token}` },
        });
...
// 测试5: 组队创建权限
async function testTeamCreation(token, level) {
    log('📝', '测试5: 组队创建权限');
    try {
        const response = await axios_1.default.post(`${API_BASE}/teams`, {
            name: '测试队伍',
            track: 'content',
            description: '测试描述',
            maxMembers: 5,
