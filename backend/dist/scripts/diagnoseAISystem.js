...
async function checkDatabase() {
    console.log('\n🔍 检查数据库实际数据...\n');
    // 1. 检查OPC测试数据
    const opcResults = await db_1.pool.query(`
  ...
  `);
    ...
    // 2. 检查学生能力数据
    const studentCap = await db_1.pool.query(`
  ...
  `);
    ...
    // 3. 检查任务匹配数据
    const taskMatches = await db_1.pool.query(`
    SELECT COUNT(*) as count,
  ...
  `);
    ...
    // 4. 检查任务翻译数据
    const taskTranslations = await db_1.pool.query(`
    SELECT COUNT(*) as count
  ...
  `);
    ...
    // 5. 检查人格标签分布
    const personalityDist = await db_1.pool.query(`
...
main();
//# sourceMappingURL=diagnoseAISystem.js.map