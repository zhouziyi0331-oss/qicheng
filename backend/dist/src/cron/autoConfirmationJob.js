...
class AutoConfirmationJob {
    ...
    async confirmTask(client, task) {
       ...
       WHERE id = $1`, [task_id]);
        // 2. 记录自动确认
        await client.query(`INSERT INTO auto_confirmations (
        task_id,
        confirmed_at,
        reason,
        created_at
      ) VALUES ($1, NOW(), $2, NOW())`, [task_id, '7天自动确认']);
        // 3. 更新学生收入（85%的价格）
        const studentIncome = price * 0.85;
        await client.query(`UPDATE users
       SET balance = balance + $1,
           updated_at = NOW()
       WHERE id = $2`, [studentIncome, student_id]);
        // 4. 更新平台收入（15%的价格）
        const platformIncome = price * 0.15;
        await client.query(`INSERT INTO platform_income (
        task_id,
        amount,
        income_type,
        created_at
      ) VALUES ($1, $2, 'commission', NOW())`, [task_id, platformIncome]);
        // 5. 创建通知给企业
        ...
        // 7. 检查是否需要交换微信（连续合作2次）
        const collaborationResult = await client.query(`SELECT COUNT(*) as count
       FROM tasks
       WHERE company_id = $1
         AND student_id = $2
         AND status = 'confirmed'`, [company_id, student_id]);
        const collaborationCount = parseInt(collaborationResult.rows[0].count);
        if (collaborationCount >= 2) {
            // 检查是否已经交换过微信
            const exchangeResult = await client.query(`SELECT id FROM wechat_exchanges
         WHERE company_id = $1 AND student_id = $2`, [company_id, student_id]);
            if (exchangeResult.rows.length === 0) {
                // 记录微信交换
                await client.query(`INSERT INTO wechat_exchanges (
            company_id,
            student_id,
            exchanged_at,
            reason,
            created_at
          ) VALUES ($1, $2, NOW(), $3, NOW())`, [company_id, student_id, '连续合作2次自动交换']);
                // 通知双方
                await client.query(`INSERT INTO notifications (
            user_id,
            type,
            ...
            ($2, 'wechat_exchange', '可以交换微信了', '您与该企业已合作2次，现在可以交换微信直接沟通了！', NOW())`, [company_id, student_id]);
            }
        }
    }
    /**
...
exports.AutoConfirmationJob = AutoConfirmationJob;
//# sourceMappingURL=autoConfirmationJob.js.map