...
exports.mentorCompanionAgentEnhanced = exports.MentorCompanionAgentEnhanced = void 0;
const memoryService_1 = require("../services/memoryService");
const mentorService_1 = require("../services/mentorService");
const logger_1 = __importDefault(require("../utils/logger"));
const orchestrator_1 = require("../types/orchestrator");
class MentorCompanionAgentEnhanced {
    /**
     * 处理导师消息（增强版）
     * 流程：加载6层记忆 → 调用现有mentorService → 保存记忆更新
     */
    async handleMessage(userId, message, trigger, context) {
        ...
        try {
            ...
            if (process.env.NODE_ENV === 'development') {
                ...
                logger_1.default.info(`[测试模式] 跳过mentorService调用，触发场景: ${trigger}`);
            }
            else {
                response = await mentorService_1.mentorService.sendMessage(userId, sessionId, message);
            }
            // 4. 从response中提取记忆更新信号
            await this.updateMemoryFromResponse(userId, message, { content: response }, context?.taskId);
            // 5. 更新L6关系记忆（每次对话）
            await memoryService_1.memoryService.updateRelationshipMemory(userId, {
                addSummary: {
            ...
            logger_1.default.error('导师Agent增强层处理失败:', error);
            return {
                success: false,
                agentName: 'mentorCompanionAgentEnhanced',
     ...
     * 为导师加载完整记忆
     */
    async loadMemoryForMentor(userId, taskId) {
        try {
            ...
            logger_1.default.error('加载导师记忆失败:', error);
     ...
     * 从response中更新记忆
     */
    async updateMemoryFromResponse(userId, userMessage, mentorResponse, taskId) {
        try {
            // 检测情绪
            ...
            // 如果有任务上下文，更新L2
            if (taskId && emotion) {
                await memoryService_1.memoryService.updateTaskContext(userId, taskId, {
                    addEmotionEvent: {
                        emotion,
            ...
            logger_1.default.error('更新记忆失败:', error);
            // 记忆更新失败不影响核心功能
        }
    }
    /**
     ...
     */
    async handleTrigger(userId, trigger, context) {
        logger_1.default.info(`[handleTrigger] 收到触发场景: ${trigger}, userId: ${userId}`);
        // 调试：记录完整context
        console.log(`[handleTrigger] trigger=${trigger}, context=`, JSON.stringify(context));
        // 根据不同触发场景，构建不同的消息和处理策略
        switch (trigger) {
            case orchestrator_1.MentorTrigger.USER_INITIATED:
                return this.handleUserInitiated(userId, context);
            case orchestrator_1.MentorTrigger.TASK_ACCEPTED:
                return this.handleTaskAccepted(userId, context);
            case orchestrator_1.MentorTrigger.STUCK_HELP_REQUEST:
                return this.handleStuckHelpRequest(userId, context);
            case orchestrator_1.MentorTrigger.EMOTIONAL_DISTRESS_DETECTED:
                return this.handleEmotionalDistress(userId, context);
            case orchestrator_1.MentorTrigger.TASK_COMPLETED:
                return this.handleTaskCompleted(userId, context);
            case orchestrator_1.MentorTrigger.PROACTIVE_CHECKIN:
                return this.handleProactiveCheckin(userId, context);
            case orchestrator_1.MentorTrigger.TASK_REJECTED_COMFORT:
                return this.handleTaskRejectedComfort(userId, context);
            case orchestrator_1.MentorTrigger.MILESTONE_REACHED:
                logger_1.default.info(`[handleTrigger] 路由到handleMilestoneReached`);
                return this.handleMilestoneReached(userId, context);
            case orchestrator_1.MentorTrigger.LONG_SILENCE:
                return this.handleLongSilence(userId, context);
            case orchestrator_1.MentorTrigger.BREAKTHROUGH_MOMENT:
                return this.handleBreakthroughMoment(userId, context);
            case orchestrator_1.MentorTrigger.PATTERN_RECOGNITION:
                return this.handlePatternRecognition(userId, context);
            case orchestrator_1.MentorTrigger.RELATIONSHIP_DEEPENING:
                return this.handleRelationshipDeepening(userId, context);
            default:
                logger_1.default.warn(`[handleTrigger] 未知触发场景: ${trigger}`);
     ...
     * 策略：直接回应，保持苏格拉底式引导
     */
    async handleUserInitiated(userId, context) {
        const message = context.message || '';
        return this.handleMessage(userId, message, orchestrator_1.MentorTrigger.USER_INITIATED, context);
    }
    /**
     * 场景2: 任务接取
     * 策略：欢迎开场 + 任务要点提醒 + 建立L2任务记忆
     */
    async handleTaskAccepted(userId, context) {
        const { taskId, taskTitle, taskDifficulty } = context;
        // 初始化L2任务记忆
        if (taskId) {
            await memoryService_1.memoryService.createTaskContext(userId, taskId, {
                taskPhase: 'started',
                stuckPoints: [],
                hintsGiven: [],
                emotionTimeline: [],
        ...
        return this.handleMessage(userId, message, orchestrator_1.MentorTrigger.TASK_ACCEPTED, {
            ...context,
     ...
     */
    async handleStuckHelpRequest(userId, context) {
        ...
        return this.handleMessage(userId, message, orchestrator_1.MentorTrigger.STUCK_HELP_REQUEST, {
            ...context,
     ...
     */
    async handleEmotionalDistress(userId, context) {
        ...
        return this.handleMessage(userId, message, orchestrator_1.MentorTrigger.EMOTIONAL_DISTRESS_DETECTED, {
            ...context,
     ...
     */
    async handleTaskCompleted(userId, context) {
        ...
        if (taskId) {
            await memoryService_1.memoryService.updateGrowthArchive(userId, {
                addTaskReport: {
                    taskId,
            ...
            // 更新L3近期摘要（完成任务数+1）
            await memoryService_1.memoryService.updateRecentSummary(userId, {
                incrementTasksCompleted: true
            });
        }
        const message = `完成了任务：${taskTitle || '任务'}`;
        return this.handleMessage(userId, message, orchestrator_1.MentorTrigger.TASK_COMPLETED, {
            ...context,
     ...
     */
    async handleProactiveCheckin(userId, context) {
        ...
        return this.handleMessage(userId, checkinMessage, orchestrator_1.MentorTrigger.PROACTIVE_CHECKIN, {
            ...context,
     ...
     */
    async handleTaskRejectedComfort(userId, context) {
        ...
        return this.handleMessage(userId, message, orchestrator_1.MentorTrigger.TASK_REJECTED_COMFORT, {
            ...context,
     ...
     */
    async handleMilestoneReached(userId, context) {
        ...
        // 记录里程碑到L4
        try {
            logger_1.default.info(`[handleMilestoneReached] 开始记录里程碑: ${milestone}`);
            ...
            logger_1.default.info(`[handleMilestoneReached] 里程碑记录成功`);
        }
        catch (error) {
            console.error(`[DEBUG] updateGrowthArchive 失败:`, error);
            logger_1.default.error(`[handleMilestoneReached] 里程碑记录失败:`, error);
            // 继续执行，不因记忆更新失败而中断
        }
        // 检查是否应该升级关系阶段
        const memory = await this.loadMemoryForMentor(userId);
        ...
        return this.handleMessage(userId, message, orchestrator_1.MentorTrigger.MILESTONE_REACHED, {
            ...context,
     ...
     */
    async handleLongSilence(userId, context) {
        const memory = await this.loadMemoryForMentor(userId);
        ...
        return this.handleMessage(userId, silenceMessage, orchestrator_1.MentorTrigger.LONG_SILENCE, {
            ...context,
     ...
     */
    async handleBreakthroughMoment(userId, context) {
        ...
        return this.handleMessage(userId, message, orchestrator_1.MentorTrigger.BREAKTHROUGH_MOMENT, {
            ...context,
     ...
     * 策略：温和指出模式 + 提供选择而非指令
     */
    async handlePatternRecognition(userId, context) {
        ...
        return this.handleMessage(userId, message, orchestrator_1.MentorTrigger.PATTERN_RECOGNITION, {
            ...context,
     ...
     */
    async handleRelationshipDeepening(userId, context) {
        ...
        return this.handleMessage(userId, message, orchestrator_1.MentorTrigger.RELATIONSHIP_DEEPENING, {
            ...context,
            systemPrompt: '关系进入深层。可以分享一些导师（AI）的"脆弱性"或不确定性，建立更真实的连接。鼓励深度对话。'
        });
    }
}
exports.MentorCompanionAgentEnhanced = MentorCompanionAgentEnhanced;
exports.mentorCompanionAgentEnhanced = new MentorCompanionAgentEnhanced();
//# sourceMappingURL=mentorCompanionAgentEnhanced.js.map