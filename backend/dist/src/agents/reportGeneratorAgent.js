...
exports.reportGeneratorAgent = void 0;
...
class ReportGeneratorAgent {
    ...
    async generateReport(userId, options = {}) {
        ...
        try {
            ...
            logger_1.default.info(`[报告生成Agent] 报告生成完成: userId=${userId}, duration=${Date.now() - startTime}ms`);
            return report;
        }
        catch (error) {
     ...
     * 加载学生的完整记忆（L1-L6）
     */
    async loadStudentMemory(userId) {
        // 加载所有层级的记忆
     ...
     * 使用AI生成报告内容
     */
    async generateReportWithAI(userId, memory, options) {
     ...
     * 构建报告生成提示词
     */
    buildReportPrompt(memory, options) {
...
请返回完整的JSON对象。`;
     ...
     * 构建结构化报告
     */
    buildStructuredReport(userId, memory, aiReport) {
        return {
            studentId: userId,
            ...
            skillProfile: {
                strengths: aiReport.strengths?.map((s) => s.skill) || [],
                weaknesses: aiReport.areasForImprovement?.map((a) => a.area) || [],
                recommendations: aiReport.recommendations?.map((r) => r.action) || [],
            },
     ...
     * 计算完成率
     */
    calculateCompletionRate(memory) {
     ...
     * 计算平均质量
     */
    calculateAverageQuality(memory) {
        const reports = memory.growthArchive.taskMicroReports || [];
        if (reports.length === 0)
            return 0;
        const totalQuality = reports.reduce((sum, report) => {
            return sum + (report.quality || 0);
        }, 0);
        return totalQuality / reports.length;
    }
}
exports.reportGeneratorAgent = new ReportGeneratorAgent();
exports.default = exports.reportGeneratorAgent;
//# sourceMappingURL=reportGeneratorAgent.js.map