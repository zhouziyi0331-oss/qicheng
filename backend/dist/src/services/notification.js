...
(function (NotificationType) {
    NotificationType["TASK_MATCHED"] = "task_matched";
    NotificationType["TASK_ACCEPTED"] = "task_accepted";
    NotificationType["TASK_SUBMITTED"] = "task_submitted";
    NotificationType["TASK_APPROVED"] = "task_approved";
    NotificationType["TASK_REJECTED"] = "task_rejected";
    NotificationType["AMENDMENT_CREATED"] = "amendment_created";
    NotificationType["AMENDMENT_ACCEPTED"] = "amendment_accepted";
    ...
    NotificationType["MENTOR_MESSAGE"] = "mentor_message";
    NotificationType["SYSTEM_ANNOUNCEMENT"] = "system_announcement"; // 系统公告
...
(function (NotificationChannel) {
    ...
    NotificationChannel["WECHAT"] = "wechat"; // 微信模板消息
 ...
 * 保存站内信
 */
async function saveInAppNotification(userId, type, title, content, data, priority) {
    await (0, db_1.query)(`INSERT INTO notifications (user_id, type, title, content, data, priority, is_read, created_at)
     VALUES ($1, $2, $3, $4, $5, $6, false, NOW())`, [userId, type, title, content, JSON.stringify(data || {}), priority]);
}
/**
 * 获取用户通知偏好
 */
 ...
 * 判断是否应该发送到该渠道
 */
function shouldSendToChannel(channel, type, preferences) {
 ...
 * 发送短信
 */
async function sendSMS(userId, content) {
    // 获取用户手机号
    const userResult = await (0, db_1.query)(`SELECT phone FROM users WHERE id = $1`, [userId]);
    ...
    if (!ACCESS_KEY_ID || !ACCESS_KEY_SECRET) {
        logger_1.default.warn('SMS credentials not configured, skipping SMS');
        return;
    }
    // 这里简化处理，实际需要使用阿里云SDK
    logger_1.default.info(`[SMS] Sending to ${phone}: ${content}`);
    // 记录短信发送日志
...
async function sendEmail(userId, subject, content) {
    ...
    if (!EMAIL_API_KEY) {
        logger_1.default.warn('Email credentials not configured, skipping email');
        return;
    }
    logger_1.default.info(`[EMAIL] Sending to ${email}: ${subject}`);
    // 记录邮件发送日志
 ...
 * 发送微信模板消息
 */
async function sendWechatTemplateMessage(userId, type, data) {
    // 获取用户微信openid
    const userResult = await (0, db_1.query)(`SELECT wechat_openid FROM users WHERE id = $1`, [userId]);
    ...
    try {
        const response = await axios_1.default.post(WECHAT_API_URL, {
            touser: openid,
        ...
        logger_1.default.info(`[WECHAT] Template message sent to ${openid}`);
        // 记录发送日志
       ...
       VALUES ($1, 'wechat', $2, $3, 'sent', NOW())`, [userId, openid, JSON.stringify({ type, data })]);
        ...
        logger_1.default.error('Failed to send wechat template message:', error);
        throw error;
    }
}
/**
 ...
 * 获取微信模板ID
 */
function getWechatTemplateId(type) {
    const templates = {
 ...
 * 构造微信模板消息数据
 */
function buildWechatTemplateData(type, data) {
    switch (type) {
 ...
 * 批量发送通知
 */
async function sendBatchNotifications(userIds, type, title, content, channels, data) {
    const promises = userIds.map(userId => sendNotification({ userId, type, title, content, channels, data }));
...
async function getUnreadCount(userId) {
    const result = await (0, db_1.query)(`SELECT COUNT(*) as count FROM notifications WHERE user_id = $1 AND is_read = false`, [userId]);
...
async function markAsRead(notificationId, userId) {
    await (0, db_1.query)(`UPDATE notifications SET is_read = true, read_at = NOW() WHERE id = $1 AND user_id = $2`, [notificationId, userId]);
}
/**
 * 标记所有通知为已读
 */
