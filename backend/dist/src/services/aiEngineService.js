...
exports.AIQAEngine = exports.AITaskReviewEngine = exports.AITaskDecompositionEngine = exports.AIRequirementEngine = void 0;
...
class AIRequirementEngine {
     ...
     * 开始需求确认对话
     */
    static async startDialogue(companyId, taskDraftId) {
        const sessionId = `req_${Date.now()}_${companyId}`;
        // 创建初始对话
        await db_1.pool.query(`INSERT INTO ai_requirement_dialogues
     ...
     * 处理用户消息并生成AI回复
     */
    static async processMessage(sessionId, companyId, message) {
        // 保存用户消息
        await db_1.pool.query(`INSERT INTO ai_requirement_dialogues
       (company_id, session_id, message_type, message_content)
       VALUES ($1, $2, 'user', $3)`, [companyId, sessionId, message]);
        // 获取对话历史
        const historyResult = await db_1.pool.query(`SELECT message_type, message_content
       ...
       VALUES ($1, $2, 'assistant', $3, $4, $5)`, [companyId, sessionId, aiResponse.message, aiResponse.extractedInfo, aiResponse.confidence]);
        return aiResponse;
    }
    /**
     * 生成AI回复（模拟）
     */
    static async generateAIResponse(history, latestMessage) {
        // 实际应调用AI API，这里做简单模拟
        const extractedInfo = {};
        let confidence = 0.8;
        let message = '';
     ...
     * 获取对话历史
     */
    static async getDialogueHistory(sessionId) {
        const result = await db_1.pool.query(`SELECT * FROM ai_requirement_dialogues
       WHERE session_id = $1
       ORDER BY created_at ASC`, [sessionId]);
        return result.rows;
    }
}
exports.AIRequirementEngine = AIRequirementEngine;
/**
 * AI任务拆解引擎
 */
class AITaskDecompositionEngine {
    /**
     * 拆解任务
     */
    static async decomposeTask(taskId, taskDescription) {
        // 模拟AI拆解（实际应调用AI API）
        const decomposition = await this.generateDecomposition(taskDescription);
        // 保存拆解结果
        const result = await db_1.pool.query(`INSERT INTO ai_task_decompositions
     ...
     * 生成任务拆解（模拟）
     */
    static async generateDecomposition(description) {
        // 实际应调用AI API
        const subtasks = [
            {
                title: '需求分析',
     ...
     * 创建子任务
     */
    static async createSubtasks(decompositionId, parentTaskId) {
        const decompositionResult = await db_1.pool.query(`SELECT decomposition_result FROM ai_task_decompositions WHERE id = $1`, [decompositionId]);
        const subtasks = decompositionResult.rows[0].decomposition_result;
        for (let i = 0; i < subtasks.length; i++) {
            const subtask = subtasks[i];
            await db_1.pool.query(`INSERT INTO subtasks
    ...
    static async getSubtasks(parentTaskId) {
        const result = await db_1.pool.query(`SELECT * FROM subtasks
       WHERE parent_task_id = $1
       ORDER BY subtask_order ASC`, [parentTaskId]);
        return result.rows;
    }
}
exports.AITaskDecompositionEngine = AITaskDecompositionEngine;
/**
 * AI任务审核引擎
 */
class AITaskReviewEngine {
    /**
     * AI审核任务
     */
    static async reviewTask(taskId, taskData) {
        // 模拟AI审核（实际应调用AI API）
        const review = await this.generateReview(taskData);
        // 保存审核结果
        const result = await db_1.pool.query(`INSERT INTO ai_task_reviews
     ...
     * 生成审核结果（模拟）
     */
    static async generateReview(taskData) {
        const issues = [];
        let score = 100;
        // 检查标题
     ...
     * 人工审核
     */
    static async humanReview(reviewId, reviewerId, approved, feedback) {
        await db_1.pool.query(`UPDATE ai_task_reviews
       SET human_reviewer_id = $1,
           human_review_result = $2,
           human_feedback = $3,
           human_reviewed_at = CURRENT_TIMESTAMP
       WHERE id = $4`, [reviewerId, approved ? 'approved' : 'rejected', feedback, reviewId]);
    }
}
exports.AITaskReviewEngine = AITaskReviewEngine;
/**
 * AI问答引擎
 */
class AIQAEngine {
     ...
     * 回答问题
     */
    static async answerQuestion(userId, question, taskId) {
        const startTime = Date.now();
        // 从知识库匹配答案
        const answer = await this.matchAnswer(question);
        const responseTime = Date.now() - startTime;
        // 保存问答历史
        await db_1.pool.query(`INSERT INTO ai_qa_history
       (user_id, task_id, question, ai_answer, knowledge_base_id, confidence_score, response_time_ms)
       VALUES ($1, $2, $3, $4, $5, $6, $7)`, [userId, taskId, question, answer.answer, answer.knowledgeBaseId, answer.confidence, responseTime]);
        return answer;
    }
    /**
     * 从知识库匹配答案
     */
    static async matchAnswer(question) {
        const result = await db_1.pool.query(`SELECT * FROM ai_qa_knowledge_base
        ...
        for (const kb of result.rows) {
            ...
            for (const keyword of keywords) {
                if (question.includes(keyword)) {
                    // 更新使用次数
                    await db_1.pool.query(`UPDATE ai_qa_knowledge_base SET usage_count = usage_count + 1 WHERE id = $1`, [kb.id]);
                    return {
     ...
     * 标记答案是否有帮助
     */
    static async markHelpful(historyId, isHelpful) {
        const result = await db_1.pool.query(`UPDATE ai_qa_history
       SET is_helpful = $1
       WHERE id = $2
       RETURNING knowledge_base_id`, [isHelpful, historyId]);
        const knowledgeBaseId = result.rows[0]?.knowledge_base_id;
        if (knowledgeBaseId) {
            const field = isHelpful ? 'helpful_count' : 'not_helpful_count';
            await db_1.pool.query(`UPDATE ai_qa_knowledge_base SET ${field} = ${field} + 1 WHERE id = $1`, [knowledgeBaseId]);
        }
    }
}
exports.AIQAEngine = AIQAEngine;
