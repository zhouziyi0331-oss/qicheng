...
class ReasoningEngine {
    ...
    async think(context) {
        try {
            logger_1.default.info(`Starting deep thinking for student ${context.studentId}: ${context.question}`);
            // 第一步：回忆相关信息
            ...
            logger_1.default.info(`Completed deep thinking for student ${context.studentId}`);
            return thinkingProcess;
        }
        catch (error) {
            logger_1.default.error('Failed to complete thinking process:', error);
            throw error;
        }
    }
    /**
    ...
    async recall(context) {
        try {
            // 获取学生最近的行为
            const recentBehaviors = await teacherObservationService_1.default.getRecentBehaviors(context.studentId, 20);
            // 获取学生的关键时刻
            const keyMoments = await teacherObservationService_1.default.getKeyMoments(context.studentId, 5);
            // 获取长期记忆
            const longTermMemory = await (0, db_1.queryOne)(`SELECT * FROM teacher_long_term_memory WHERE student_id = $1`, [context.studentId]);
            // 获取最近的思考记录（类似案例）
            const recentThinking = await (0, db_1.query)(`SELECT * FROM teacher_thinking_records
         WHERE student_id = $1
         ORDER BY timestamp DESC
         LIMIT 3`, [context.studentId]);
            // 如果有taskId，获取该任务的行为
            let taskBehaviors = [];
            if (context.taskId) {
                taskBehaviors = await teacherObservationService_1.default.getTaskBehaviors(context.studentId, context.taskId);
            }
            return {
            ...
            logger_1.default.error('Failed to recall information:', error);
            return {
                studentHistory: [],
                similarCases: [],
                relevantPatterns: []
     ...
     * 提取行为模式
     */
    extractPatterns(behaviors, memory) {
        const patterns = [];
     ...
     * 第二步：形成多个假设
     */
    async generateHypotheses(context, recall) {
        try {
...
## 关键时刻
${recall.keyMoments?.map((m) => `- ${m.event_description}: ${m.teacher_insight}`).join('\n') || '暂无'}
...
只返回JSON，不要其他文字：`;
            ...
            if (!jsonMatch) {
                logger_1.default.warn('Failed to parse hypotheses JSON, using fallback');
            ...
            logger_1.default.error('Failed to generate hypotheses:', error);
            return this.getFallbackHypotheses(context);
        }
    }
    /**
     ...
     */
    async getFallbackHypotheses(context) {
        try {
            logger_1.default.info('Using enhanced fallback reasoning based on student data');
            // 1. 分析学生画像
            const profile = await this.analyzeStudentProfile(context.studentId);
            // 2. 理解"言外之意"
            const impliedMeaning = this.analyzeImpliedMeaning(context.currentSituation, profile);
            // 3. 生成智能假设
            const hypotheses = [];
            // 假设1：基于"言外之意"分析
            hypotheses.push({
         ...
         LIMIT 10`, [context.studentId]);
            ...
            logger_1.default.error('Enhanced fallback failed, using simple fallback:', error);
    ...
    async analyzeStudentProfile(studentId) {
       ...
       LIMIT 50`, [studentId]);
        ...
        // 获取长期记忆
        const memory = await (0, db_1.queryOne)(`SELECT core_strengths, growth_areas, deep_understanding
     ...
     */
    analyzeImpliedMeaning(studentMessage, profile) {
     ...
     * 第三步：推理验证
     */
    async reason(hypotheses, recall) {
        try {
            // 选择置信度最高的假设
            const mainHypothesis = hypotheses.reduce((prev, current) => current.confidence > prev.confidence ? current : prev);
...
## 其他假设
${hypotheses.filter(h => h !== mainHypothesis).map(h => `- ${h.hypothesis}（置信度${h.confidence}）`).join('\n')}
...
只返回JSON，不要其他文字：`;
            ...
            logger_1.default.error('Failed to reason:', error);
            ...
            try {
                ...
                logger_1.default.warn('Failed to get profile for reasoning fallback');
            }
            if (profile) {
                // 构建推理链
                let reasoning = '';
     ...
     * 第四步：形成洞察
     */
    async formInsight(reasoning, recall) {
        try {
...
只返回JSON，不要其他文字：`;
            ...
            logger_1.default.error('Failed to form insight:', error);
            ...
            try {
                ...
                logger_1.default.error('Enhanced insight fallback also failed:', fallbackError);
                // 最简单的fallback
                return {
     ...
     * 保存思考记录
     */
    async saveThinkingRecord(studentId, thinking) {
        try {
            await (0, db_1.query)(`INSERT INTO teacher_thinking_records (
          student_id, question, recall, hypotheses, reasoning, insight
            ...
            logger_1.default.error('Failed to save thinking record:', error);
        }
    }
}
exports.default = new ReasoningEngine();
