...
class AdaptiveGuidanceService {
     ...
     * 生成自适应引导回复
     */
    async generateAdaptiveGuidance(context) {
        try {
            // 1. 分析当前情绪
            const emotionResult = await emotionAnalysisService_1.emotionAnalysisService.analyzeEmotion(context.studentId, context.taskId, context.sessionId, null, context.currentMessage, this.buildContextSummary(context.conversationHistory));
            // 2. 检测成长里程碑
            const recentEmotions = await emotionAnalysisService_1.emotionAnalysisService.getRecentEmotions(context.studentId, 5);
            const milestone = await growthTrackingService_1.growthTrackingService.detectAndRecordMilestone(context.studentId, context.taskId, context.sessionId, {
            ...
            // 5. 获取情绪响应策略
            const emotionStrategy = await emotionAnalysisService_1.emotionAnalysisService.getResponseStrategy(emotionResult.emotion);
            // 6. 获取对话上下文
            const conversationContext = await emotionAnalysisService_1.emotionAnalysisService.getConversationContext(context.sessionId);
            // 7. 生成自适应回复
     ...
     * 使用AI生成自适应引导
     */
    async generateGuidanceWithAI(context, emotionResult, emotionStrategy, memoryRecall, profile, conversationContext, milestone) {
        // 构建系统提示
        const systemPrompt = this.buildSystemPrompt(emotionResult, emotionStrategy, memoryRecall, profile, conversationContext, milestone);
        // 构建对话历史
        const messages = [
        ...
        // 调用AI
        const response = await claudeService_1.claudeService.chat(messages, {
     ...
     * 构建系统提示
     */
    buildSystemPrompt(emotionResult, emotionStrategy, memoryRecall, profile, conversationContext, milestone) {
        let prompt = `你是启程小猫，一位温暖、有洞察力的AI导师。你的目标是通过启发式引导帮助学生成长。

## 当前学生状态

**情绪状态**: ${emotionResult.emotion}（强度: ${emotionResult.intensity.toFixed(2)}）
${emotionResult.signals.length > 0 ? `**情绪信号**: ${emotionResult.signals.map((s) => s.word).join('、')}` : ''}
...
**相关记忆**:
${memoryRecall.relevantMemories.slice(0, 3).map((m) => `- ${m.memory_title}: ${m.memory_content}`).join('\n')}

`;
            if (memoryRecall.insights.length > 0) {
                prompt += `**洞察**:
${memoryRecall.insights.map((i) => `- ${i}`).join('\n')}
...
`;
     ...
     * 分析学习模式并更新档案
     */
    async analyzeAndUpdateLearningPatterns(studentId, sessionId) {
        try {
            // 获取最近的情绪历史
            const emotions = await emotionAnalysisService_1.emotionAnalysisService.getRecentEmotions(studentId, 20);
            // 分析常见情绪
            const emotionCounts = {};
            emotions.forEach(e => {
            ...
            if (emotionCounts['anxious'] >= 3)
                stressTriggers.push('新挑战');
            if (emotionCounts['overwhelmed'] >= 2)
                stressTriggers.push('复杂任务');
            if (emotionCounts['frustrated'] >= 3)
                stressTriggers.push('技术难题');
            // 识别激励因素
            const motivationFactors = [];
            if (emotionCounts['excited'] >= 2)
                motivationFactors.push('突破时刻');
            if (emotionCounts['proud'] >= 2)
                motivationFactors.push('完成成就');
            if (emotionCounts['confident'] >= 3)
                motivationFactors.push('掌握技能');
            // 更新档案
            await database_1.pool.query(`UPDATE student_learning_profiles
         SET
           common_emotions = $1,
           stress_triggers = $2,
     ...
     * 获取自适应引导建议
     */
    async getGuidanceRecommendations(studentId, sessionId) {
        try {
            const profile = await this.getStudentProfile(studentId);
            const conversationContext = await emotionAnalysisService_1.emotionAnalysisService.getConversationContext(sessionId);
            const recentEmotions = await emotionAnalysisService_1.emotionAnalysisService.getRecentEmotions(studentId, 5);
            // 分析当前状态
...
exports.adaptiveGuidanceService = new AdaptiveGuidanceService();
//# sourceMappingURL=adaptiveGuidanceService.js.map