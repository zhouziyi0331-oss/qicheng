...
class GrowthTrackingService {
     ...
     * 检测并记录成长里程碑
     */
    async detectAndRecordMilestone(studentId, taskId, sessionId, context) {
        try {
            // 1. 分析是否达成里程碑
            ...
            // 2. 创建里程碑记录
            const milestone = await this.createMilestone(studentId, taskId, sessionId, analysis);
            // 3. 更新学生档案
     ...
     * 分析是否达成里程碑
     */
    async analyzePotentialMilestone(studentId, context) {
        // 1. 获取学生历史数据
     ...
     * 基于规则的里程碑检测
     */
    detectMilestoneByRules(context, profile, recentMilestones) {
        const { currentMessage, currentEmotion, previousEmotions, taskProgress } = context;
        // 检测：第一次主动提问
        if (profile?.total_mentor_interactions <= 1 &&
     ...
     * 使用AI进行深度里程碑分析
     */
    async detectMilestoneByAI(context, profile, recentMilestones) {
...
最近情绪变化：
${context.previousEmotions?.map((e) => `${e.emotion}(${e.intensity})`).join(' → ') || '无'}
...
{ "shouldCreateMilestone": false }`;
     ...
     * 创建里程碑记录
     */
    async createMilestone(studentId, taskId, sessionId, analysis) {
        const query = `
     ...
     * 生成庆祝消息
     */
    async generateCelebrationMessage(milestoneId, studentId) {
        try {
            // 获取里程碑信息
...
直接返回庆祝消息文本，不要JSON格式。`;
     ...
     * 更新学生档案
     */
    async updateStudentProfile(studentId, analysis) {
        // 更新技能和信心趋势
        const query = `
     ...
     * 获取最近的里程碑
     */
    async getRecentMilestones(studentId, limit = 5) {
        const query = `
      SELECT *
    ...
    async getMilestone(milestoneId) {
        ...
        const result = await database_1.pool.query(query, [milestoneId]);
        return result.rows[0] || null;
    }
    /**
     * 获取未庆祝的里程碑
     ...
     * 标记里程碑为已庆祝
     */
    async markAsCelebrated(milestoneId) {
        await database_1.pool.query(`UPDATE student_growth_milestones
     ...
     * 获取成长统计
     */
    async getGrowthStats(studentId) {
        try {
            // 总里程碑数
            const totalResult = await database_1.pool.query(`SELECT COUNT(*) as count FROM student_growth_milestones WHERE student_id = $1`, [studentId]);
            // 按类型统计
            const typeResult = await database_1.pool.query(`SELECT milestone_type, COUNT(*) as count
         FROM student_growth_milestones
         WHERE student_id = $1
         GROUP BY milestone_type`, [studentId]);
            // 信心趋势
            const profileResult = await database_1.pool.query(`SELECT confidence_trend FROM student_learning_profiles WHERE student_id = $1`, [studentId]);
            // 最近成长
            const recentMilestones = await this.getRecentMilestones(studentId, 5);
            return {
...
exports.growthTrackingService = new GrowthTrackingService();
//# sourceMappingURL=growthTrackingService.js.map