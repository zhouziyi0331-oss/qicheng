...
class AbilityDimensionUpdateService {
    ...
    async updateAbilityAfterOrder(orderId) {
        ...
        // 5. 保存新版本的能力画像
        const newVersion = await this.saveNewVersion(performance.studentId, newScores, `完成订单#${orderId}`);
        // 6. 记录历史变化
     ...
     * 获取任务表现数据
     */
    async getTaskPerformance(orderId) {
        ...
        try {
         ...
         WHERE o.id = $1`, [orderId]);
            ...
            // 获取AI审核数据（任务拆解完整度）
            const auditResult = await client.query(`SELECT audit_result
         FROM order_audits
         ...
         ORDER BY created_at DESC LIMIT 1`, [orderId]);
            ...
            // 获取导师观察数据（工具使用）
            const observationResult = await client.query(`SELECT skills_observed
         ...
         ORDER BY created_at DESC LIMIT 1`, [orderId]);
            ...
            // 获取打回次数
            const rejectionResult = await client.query(`SELECT COUNT(*) as rejection_count
         FROM order_revisions
         WHERE order_id = $1`, [orderId]);
            const rejectionCount = parseInt(rejectionResult.rows[0].rejection_count);
            // 判断是否按时交付
            const onTimeDelivery = order.time_spent_hours <= order.estimated_hours || rejectionCount === 0;
            // 判断是否独立完成（根据导师对话次数）
            const sessionResult = await client.query(`SELECT COUNT(*) as session_count
    ...
    calculatePerformanceScores(performance) {
        ...
        if (performance.onTimeDelivery)
            executionScore += 15;
        if (performance.rejectionCount === 0)
            executionScore += 15;
        else
            executionScore -= performance.rejectionCount * 5;
     ...
     * 获取学生当前的六维分数
     */
    async getCurrentScores(studentId) {
        const client = await database_1.pool.connect();
        try {
     ...
     */
    calculateNewScores(currentScores, performanceScores) {
        const newScores = {
     ...
     * 保存新版本的能力画像
     */
    async saveNewVersion(studentId, newScores, updateReason) {
        const client = await database_1.pool.connect();
        try {
            await client.query('BEGIN');
            // 1. 将当前版本标记为非当前
            await client.query(`UPDATE user_ability_profiles
         SET is_current = false
         WHERE user_id = $1 AND is_current = true`, [studentId]);
            // 2. 获取下一个版本号
         ...
         WHERE user_id = $1`, [studentId]);
            ...
            // 3. 插入新版本
            await client.query(`INSERT INTO user_ability_profiles
     ...
     * 记录历史变化
     */
    async recordHistory(studentId, version, orderId, changeDetails) {
        const client = await database_1.pool.connect();
        try {
            await client.query(`INSERT INTO ability_dimension_history
     ...
     * 调用AI生成文字解读（严格按照技术规格）
     */
    async generateAIInterpretation(studentId, oldScores, newScores, orderId) {
        // 获取学生初始画像和导师观察数据
...
【输出格式】
{
  ...
  "total_word_count": 总字数
        ...
        const userPrompt = this.buildInterpretationPrompt(oldScores, newScores, data);
        ...
        result.dimension_updates.forEach((dim) => {
     ...
     * 获取AI解读所需的数据
     */
    async getInterpretationData(studentId, orderId) {
        const client = await database_1.pool.connect();
        try {
            // 获取初始画像
            const initialProfile = await client.query(`SELECT * FROM user_ability_profiles
         WHERE user_id = $1 AND version = 1 LIMIT 1`, [studentId]);
            // 获取导师观察
            const observations = await client.query(`SELECT * FROM mentor_growth_observations
         WHERE order_id = $1`, [orderId]);
            return {
     ...
     * 构建AI解读的提示词
     */
    buildInterpretationPrompt(oldScores, newScores, data) {
     ...
     * 更新维度描述
     */
    async updateDimensionDescriptions(studentId, version, interpretation) {
        const client = await database_1.pool.connect();
        try {
            await client.query(`UPDATE user_ability_profiles
         SET dimension_descriptions = $1
         WHERE user_id = $2 AND version = $3`, [JSON.stringify(interpretation.dimension_updates), studentId, version]);
        }
        finally {
            client.release();
        }
     ...
     * 获取学生的能力变化历史
     */
    async getAbilityHistory(studentId) {
        const client = await database_1.pool.connect();
        try {
            const result = await client.query(`SELECT * FROM ability_dimension_history
         WHERE user_id = $1
         ORDER BY created_at ASC`, [studentId]);
            return result.rows;
        }
        finally {
            client.release();
     ...
     * 获取学生的所有画像版本
     */
    async getProfileVersions(studentId) {
        const client = await database_1.pool.connect();
        try {
            const result = await client.query(`SELECT * FROM user_ability_profiles
         WHERE user_id = $1
         ORDER BY version ASC`, [studentId]);
            return result.rows;
        }
        finally {
            client.release();
...
exports.default = new AbilityDimensionUpdateService();
//# sourceMappingURL=abilityDimensionUpdateService.js.map