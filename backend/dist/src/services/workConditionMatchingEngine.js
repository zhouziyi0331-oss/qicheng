...
class WorkConditionMatchingEngine {
    ...
    async findBestStudentsForTask(taskId, limit = 10) {
        ...
        if (projectProfile.requirement_vector) {
            logger_1.default.info('Using vector similarity search');
            studentProfiles = await this.getStudentsByVectorSimilarity(projectProfile.requirement_vector, limit * 3 // 获取3倍数量，再用规则匹配精排
            );
        }
        else {
            logger_1.default.info('Vector not available, using all student profiles');
     ...
     * 使用向量相似度检索学生
     */
    async getStudentsByVectorSimilarity(requirementVector, limit) {
        try {
            // 将向量转换为数组格式
     ...
     * 结合规则匹配和向量相似度
     */
    async analyzeMatch(projectProfile, studentProfile) {
        // 计算向量相似度（如果有）
        let vectorSimilarity;
        if (studentProfile.similarity !== undefined) {
            ...
            try {
                ...
                vectorSimilarity = vectorEmbeddingService_1.default.calculateCosineSimilarity(projectVec, studentVec);
            }
            catch (error) {
        ...
        const dimensionMatches = {
            informationReception: this.matchInformationReception(studentProfile.information_reception, projectProfile.information_reception_need),
            creationDrive: this.matchCreationDrive(studentProfile.creation_drive, projectProfile.creation_drive_need),
            learningApproach: this.matchLearningApproach(studentProfile.learning_approach, projectProfile.learning_approach_need),
            executionRhythm: this.matchExecutionRhythm(studentProfile.execution_rhythm, projectProfile.execution_rhythm_need),
            autonomy: this.matchAutonomy(studentProfile.autonomy_need, projectProfile.autonomy_need),
            riskTolerance: this.matchRiskTolerance(studentProfile.risk_tolerance, projectProfile.risk_level)
        };
        // 计算整体适配度（规则匹配分数）
     ...
     * 匹配维度1：信息接收方式
     */
    matchInformationReception(studentPref, projectNeed) {
     ...
     * 匹配维度2：创作驱动
     */
    matchCreationDrive(studentDrive, projectNeed) {
     ...
     * 匹配维度3：学习切入方式
     */
    matchLearningApproach(studentApproach, projectNeed) {
     ...
     * 匹配维度4：执行节奏
     */
    matchExecutionRhythm(studentRhythm, projectNeed) {
     ...
     * 匹配维度5：自主度
     */
    matchAutonomy(studentNeed, projectNeed) {
     ...
     * 匹配维度6：风险承受度
     */
    matchRiskTolerance(studentTolerance, projectRisk) {
     ...
     * 计算整体适配度
     */
    calculateOverallFit(dimensionMatches) {
        // 六个维度的加权平均
        const weights = {
        ...
        let totalScore = 0;
        Object.entries(dimensionMatches).forEach(([dimension, match]) => {
            totalScore += match.score * weights[dimension];
        });
     ...
     * 生成调整建议
     */
    generateAdjustmentSuggestions(frictionPoints, dimensionMatches) {
        const suggestions = [];
     ...
     * 生成面向学生的推荐理由
     */
    generateStudentRecommendation(matchPoints, projectProfile) {
     ...
     * 生成面向企业的推荐理由
     */
    generateCompanyRecommendation(matchPoints, studentProfile) {
        if (matchPoints.length === 0) {
     ...
     * 获取项目画像
     */
    async getProjectProfile(taskId) {
        return await (0, db_1.queryOne)(`SELECT * FROM project_requirement_profiles WHERE task_id = $1`, [taskId]);
    }
    /**
     * 获取所有学生画像
     */
    async getAllStudentProfiles() {
        return await (0, db_1.query)(`SELECT * FROM student_work_condition_profiles`);
    }
    /**
     * 保存匹配记录
     */
    async saveMatchRecord(match) {
        try {
            await (0, db_1.queryOne)(`INSERT INTO work_condition_matches (
        ...
        DO UPDATE SET
          ...
          vector_similarity = $11`, [
            ...
            logger_1.default.error('Failed to save match record:', error);
        }
    }
}
exports.default = new WorkConditionMatchingEngine();
//# sourceMappingURL=workConditionMatchingEngine.js.map