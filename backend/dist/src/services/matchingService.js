...
class MatchingService {
     ...
     * 为任务匹配最合适的学生（Top N）
     */
    async matchStudentsForTask(taskId, topN = 10) {
        try {
            // 1. 获取任务需求
            const taskResult = await db_1.pool.query(`SELECT id, track, level, required_openness, required_persistence,
         ...
         FROM tasks WHERE id = $1`, [taskId]);
            ...
            // 2. 获取所有学生能力画像
            const studentsResult = await db_1.pool.query(`SELECT user_id, openness, persistence, creativity, primary_track,
         ...
         WHERE current_level <= $1 + 1`, // 只匹配当前等级±1的学生
            ...
            logger_1.default.error('Error matching students for task', { taskId, error });
            throw error;
        }
    }
    /**
     * 为学生推荐最合适的任务（Top N）
     */
    async matchTasksForStudent(userId, topN = 20) {
        try {
            // 1. 获取学生能力画像
            const studentResult = await db_1.pool.query(`SELECT user_id, openness, persistence, creativity, primary_track,
                current_level, total_completed_tasks, skills
         FROM student_abilities WHERE user_id = $1`, [userId]);
            ...
            // 2. 获取可用任务（当前等级±1）
            const tasksResult = await db_1.pool.query(`SELECT id, track, level, required_openness, required_persistence,
                required_creativity, is_stretch_project
         FROM tasks
         WHERE status = 'published'
           AND level >= $1 - 1
           AND level <= $1 + 1
           AND accepted_student_id IS NULL`, [student.currentLevel]);
            // 3. 计算每个任务的匹配度
            const matches = tasksResult.rows.map((task) => {
            ...
            logger_1.default.error('Error matching tasks for student', { userId, error });
            throw error;
        }
    }
    /**
     ...
     * 获取任务的匹配学生列表
     */
    async getMatchedStudentsForTask(taskId, limit = 10) {
     ...
     * 获取学生的推荐任务列表
     */
    async getMatchedTasksForStudent(userId, limit = 20) {
        const result = await db_1.pool.query(`SELECT
         am.task_id,
         am.match_score,
         am.difficulty_level,
