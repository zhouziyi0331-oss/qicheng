...
class ProactiveFollowUpService {
     ...
     * 生成跟进消息
     */
    async generateFollowUpMessage(sessionId, studentId, reason) {
        try {
            // 获取会话信息
            const session = await mentorStageService_1.mentorStageService.getSession(sessionId.toString());
            if (!session) {
                return {
                    content: '',
                    tone: 'neutral',
            ...
            // 获取对话历史
            const messages = await mentorStageService_1.mentorStageService.getMessages(sessionId.toString(), 10);
            ...
            switch (reason) {
                case 'inactive':
                    message = await this.generateInactiveFollowUp(studentId, conversationHistory, humanizedContext);
                    break;
                case 'struggling':
                    message = await this.generateStrugglingFollowUp(studentId, conversationHistory, humanizedContext);
                    tone = 'supportive';
                    break;
                case 'tool_feedback':
                    message = await this.generateToolFollowUp(studentId, conversationHistory, humanizedContext);
                    tone = 'curious';
                    break;
                case 'celebrate':
                    message = await this.generateCelebrationFollowUp(studentId, conversationHistory, humanizedContext);
                    tone = 'excited';
                    break;
                default:
    ...
    async sendFollowUp(sessionId, message) {
        try {
            await mentorStageService_1.mentorStageService.saveMessage(sessionId.toString(), 'mentor', message, {
                extra: {
            ...
            // 更新人性化上下文
            await database_1.pool.query(`UPDATE mentor_humanized_context
     ...
     * 查找未庆祝的里程碑
     */
    async findUncelebratedMilestones() {
        try {
            const query = `
        SELECT DISTINCT
          s.id as session_id,
     ...
     * 生成不活跃跟进消息
     */
    async generateInactiveFollowUp(studentId, conversationHistory, humanizedContext) {
     ...
     * 生成困难跟进消息
     */
    async generateStrugglingFollowUp(studentId, conversationHistory, humanizedContext) {
        // 获取具体困难
        const struggle = await database_1.pool.query(`SELECT struggle_description, student_original_words
...
或者你已经找到方法了？跟我说说呗，我也想知道你是怎么解决的。`;
     ...
     * 生成工具跟进消息
     */
    async generateToolFollowUp(studentId, conversationHistory, humanizedContext) {
        // 获取推荐的工具
        const tool = await database_1.pool.query(`SELECT t.tool_name
...
你的感受最重要～`;
     ...
     * 生成庆祝跟进消息
     */
    async generateCelebrationFollowUp(studentId, conversationHistory, humanizedContext) {
        // 获取里程碑
        const milestone = await database_1.pool.query(`SELECT milestone_title, milestone_description, celebration_message
...
你自己有没有感觉到这个变化？`;
     ...
     * 获取人性化上下文
     */
    async getHumanizedContext(sessionId) {
        try {
    ...
    async executeFollowUps() {
        try {
            ...
            for (const task of tasks) {
                const message = await this.generateFollowUpMessage(task.sessionId, task.studentId, task.reason);
...
exports.proactiveFollowUpService = new ProactiveFollowUpService();
//# sourceMappingURL=proactiveFollowUpService.js.map