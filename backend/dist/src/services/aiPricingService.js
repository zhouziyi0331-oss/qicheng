...
class AIPricingService {
     ...
     * 获取智能定价建议
     */
    async getPricingSuggestion(input) {
        const client = await db_1.pool.connect();
        try {
            // 1. 计算任务复杂度
            const complexityScore = await this.calculateComplexity(client, input);
            // 2. 获取市场基准价格
            const marketBenchmark = await this.getMarketBenchmark(client, input.category || 'general', input.difficulty_level || 'intermediate');
            // 3. 计算各个定价因子
            ...
            // 5. 计算最终建议价格
            const finalSuggestion = this.calculateFinalPrice(aiAnalysis, marketBenchmark, factors, complexityScore);
            logger_1.default.info('Pricing suggestion generated', {
            ...
            logger_1.default.error('Failed to get pricing suggestion', { error, input });
            throw error;
        }
        finally {
            client.release();
     ...
     * 计算任务复杂度
     */
    async calculateComplexity(client, input) {
        const result = await client.query(`SELECT calculate_task_complexity($1, $2, $3, $4) as score`, [
            input.description,
            input.requirements || '',
            input.deliverables || '',
     ...
     * 获取市场基准价格
     */
    async getMarketBenchmark(client, category, difficulty) {
        const result = await client.query(`SELECT * FROM get_market_benchmark($1, $2)`, [category, difficulty]);
     ...
     * 计算各个定价因子
     */
    async calculatePricingFactors(client, input, complexityScore) {
        // 获取定价因子权重
        const factorsResult = await client.query(`SELECT * FROM pricing_factors WHERE is_active = true ORDER BY weight DESC`);
        const factors = [];
     ...
     * 获取市场需求分数
     */
    async getMarketDemandScore(client, category) {
        if (!category)
            return 50;
        // 计算该分类的供需比
       ...
       FROM tasks
       WHERE category = $1
         AND created_at >= CURRENT_DATE - INTERVAL '30 days'`, [category]);
     ...
     * 获取企业信誉分数
     */
    async getCompanyReputationScore(client, companyId) {
        const result = await client.query(`SELECT
     ...
     * 使用AI进行综合分析
     */
    async getAIAnalysis(input, complexityScore, marketBenchmark, factors) {
...
confidence_score: 0-100，表示对这个定价的信心程度`;
     ...
     * 计算最终建议价格
     */
    calculateFinalPrice(aiAnalysis, marketBenchmark, factors, complexityScore) {
        // 基于因子计算的价格
     ...
     * 保存定价历史
     */
    async savePricingHistory(taskId, companyId, suggestion, actualMin, actualMax) {
        const client = await db_1.pool.connect();
        try {
            // 获取任务信息
            const taskResult = await client.query(`SELECT category, difficulty_level, description, requirements, deliverables, estimated_hours, required_abilities
         FROM tasks WHERE id = $1`, [taskId]);
            if (taskResult.rows.length === 0) {
                throw new Error('Task not found');
            }
            const task = taskResult.rows[0];
            const result = await client.query(`INSERT INTO pricing_history (
        ...
        RETURNING id`, [
         ...
         WHERE id = $3`, [suggestion.confidence_score, historyId, taskId]);
            logger_1.default.info('Pricing history saved', { taskId, historyId });
            return historyId;
        }
        catch (error) {
            logger_1.default.error('Failed to save pricing history', { error, taskId });
            throw error;
        }
        finally {
            client.release();
     ...
     * 记录定价调整
     */
    async recordPricingAdjustment(taskId, companyId, originalMin, originalMax, adjustedMin, adjustedMax, reason, note) {
        const client = await db_1.pool.connect();
        try {
            await client.query(`INSERT INTO pricing_adjustments (
          task_id, company_id, original_min, original_max,
          adjusted_min, adjusted_max, adjustment_reason, adjustment_note
        ) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`, [taskId, companyId, originalMin, originalMax, adjustedMin, adjustedMax, reason, note]);
            logger_1.default.info('Pricing adjustment recorded', { taskId, reason });
        }
        catch (error) {
            logger_1.default.error('Failed to record pricing adjustment', { error, taskId });
            throw error;
        }
        finally {
            client.release();
    ...
    async updateMarketBenchmarks() {
        ...
        try {
            ...
            logger_1.default.info('Market benchmarks updated successfully');
        }
        catch (error) {
            logger_1.default.error('Failed to update market benchmarks', { error });
            throw error;
        }
        finally {
            client.release();
     ...
     * 获取定价准确度分析
     */
    async getPricingAccuracy(category, difficulty) {
        const client = await db_1.pool.connect();
        try {
...
exports.aiPricingService = new AIPricingService();
