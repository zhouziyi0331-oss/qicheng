...
class PDFGeneratorService {
     ...
     * 生成创业报告PDF
     */
    static async generateStartupReportPDF(reportContent, userName) {
        return new Promise((resolve, reject) => {
            try {
                ...
                logger_1.default.error('PDF生成失败', { error: error.message });
                reject(error);
            }
        });
    }
     ...
     * 添加封面
     */
    static addCoverPage(doc, userName) {
     ...
     * 添加定制化分析
     */
    static addCustomizedAnalysis(doc, analysis) {
        // 标题
     ...
     * 添加通用创业指南
     */
    static addStartupGuides(doc, guides) {
        doc.addPage();
     ...
     * 添加章节
     */
    static addSection(doc, title, content) {
    ...
    static addPageNumbers(doc) {
        ...
        for (let i = 0; i < pages.count; i++) {
            ...
            doc.text(`第 ${i + 1} 页 / 共 ${pages.count} 页`, 50, doc.page.height - 30, { align: 'center' });
        }
    }
}
