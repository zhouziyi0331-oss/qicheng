...
class DeliverableTemplateService {
    ...
    async createTemplate(template, userId) {
        ...
        try {
            const result = await client.query(`INSERT INTO deliverable_templates (
        ...
        RETURNING id`, [
            ...
            logger_1.default.info('Created deliverable template', { id: result.rows[0].id });
     ...
     * 获取模板列表
     */
    async getTemplates(filter = {}, limit = 50) {
        const client = await db_1.pool.connect();
        try {
    ...
    async getTemplate(templateId) {
        const client = await db_1.pool.connect();
        try {
            const result = await client.query('SELECT * FROM deliverable_templates WHERE id = $1', [templateId]);
    ...
    async updateTemplate(templateId, updates) {
        ...
        try {
            ...
            logger_1.default.info('Updated template', { templateId });
        }
        catch (error) {
    ...
    async deleteTemplate(templateId) {
        const client = await db_1.pool.connect();
        try {
            await client.query('DELETE FROM deliverable_templates WHERE id = $1', [templateId]);
            logger_1.default.info('Deleted template', { templateId });
        }
        catch (error) {
     ...
     * 为任务应用模板
     */
    async applyTemplateToTask(taskId, templateId, userId, customizations) {
        const client = await db_1.pool.connect();
        try {
            await client.query('BEGIN');
            // 检查模板是否存在
            const templateResult = await client.query('SELECT * FROM deliverable_templates WHERE id = $1', [templateId]);
            if (templateResult.rows.length === 0) {
                throw new Error('Template not found');
            }
            // 插入或更新关联
            await client.query(`INSERT INTO task_deliverable_templates (
        ...
        ON CONFLICT (task_id, template_id) DO UPDATE SET
          ...
          applied_at = NOW()`, [
            ...
            // 增加模板使用次数
            await client.query('UPDATE deliverable_templates SET usage_count = usage_count + 1 WHERE id = $1', [templateId]);
            await client.query('COMMIT');
            logger_1.default.info('Applied template to task', { taskId, templateId });
        }
        catch (error) {
     ...
     * 获取任务的交付标准
     */
    async getTaskDeliverableStandards(taskId) {
        const client = await db_1.pool.connect();
        try {
    ...
    async getCategories() {
        const client = await db_1.pool.connect();
        try {
            const result = await client.query(`SELECT category, COUNT(*) as count
     ...
     * 根据任务类型推荐模板
     */
    async recommendTemplates(taskDescription, limit = 5) {
        const client = await db_1.pool.connect();
        try {
            // 简单的关键词匹配推荐
            const keywords = {
            ...
            const result = await client.query(`SELECT * FROM deliverable_templates
     ...
     * 更新模板成功率
     */
    async updateTemplateSuccessRate(templateId, wasSuccessful) {
        const client = await db_1.pool.connect();
        try {
            // 获取当前成功率和使用次数
            const result = await client.query('SELECT success_rate, usage_count FROM deliverable_templates WHERE id = $1', [templateId]);
            if (result.rows.length === 0) {
                return;
            }
            ...
            await client.query('UPDATE deliverable_templates SET success_rate = $1 WHERE id = $2', [newRate, templateId]);
            logger_1.default.info('Updated template success rate', { templateId, newRate });
        }
        catch (error) {
            logger_1.default.error('Error updating template success rate:', error);
            throw error;
        }
        finally {
            client.release();
...
exports.default = new DeliverableTemplateService();
//# sourceMappingURL=deliverableTemplateService.js.map