...
exports.AITaskType = exports.aiTaskQueue = void 0;
 ...
 */
// 创建队列
exports.aiTaskQueue = new bull_1.default('ai-tasks', config_1.config.redis.url, {
    defaultJobOptions: {
        attempts: 3,
        backoff: {
            type: 'exponential',
...
var AITaskType;
(function (AITaskType) {
    AITaskType["PROFILE_ANALYSIS"] = "profile-analysis";
    AITaskType["PROJECT_CONDITION_ANALYSIS"] = "project-condition-analysis";
    AITaskType["MATCH_ANALYSIS"] = "match-analysis";
    AITaskType["SUBMISSION_REVIEW"] = "submission-review";
    AITaskType["GROWTH_REPORT"] = "growth-report";
    AITaskType["MENTOR_GUIDANCE"] = "mentor-guidance"; // AI-06: 导师引导
 ...
 */
// 画像分析处理器（并发3个）
exports.aiTaskQueue.process(AITaskType.PROFILE_ANALYSIS, 3, async (job) => {
    const data = job.data;
    logger_1.default.info(`Processing AI task: ${AITaskType.PROFILE_ANALYSIS}`, { jobId: job.id });
    return await processProfileAnalysis(data);
});
// 项目条件分析处理器（并发5个）
exports.aiTaskQueue.process(AITaskType.PROJECT_CONDITION_ANALYSIS, 5, async (job) => {
    const data = job.data;
    logger_1.default.info(`Processing AI task: ${AITaskType.PROJECT_CONDITION_ANALYSIS}`, { jobId: job.id });
    return await processProjectConditionAnalysis(data);
});
// 匹配分析处理器（并发5个）
exports.aiTaskQueue.process(AITaskType.MATCH_ANALYSIS, 5, async (job) => {
    const data = job.data;
    logger_1.default.info(`Processing AI task: ${AITaskType.MATCH_ANALYSIS}`, { jobId: job.id });
    return await processMatchAnalysis(data);
});
// 导师引导处理器（并发2个）
exports.aiTaskQueue.process(AITaskType.MENTOR_GUIDANCE, 2, async (job) => {
    const data = job.data;
    logger_1.default.info(`Processing AI task: ${AITaskType.MENTOR_GUIDANCE}`, { jobId: job.id });
    return await processMentorGuidance(data);
});
/**
 * 处理学生画像生成
 */
async function processProfileAnalysis(data) {
    logger_1.default.info(`Generating work condition profile for student ${data.studentId}`);
    // 1. 生成工作条件画像
    ...
    logger_1.default.info(`Work condition profile generated for student ${data.studentId}`);
    return {
        success: true,
        studentId: data.studentId,
 ...
 * 处理项目需求条件分析
 */
async function processProjectConditionAnalysis(data) {
    logger_1.default.info(`Generating requirement profile for task ${data.taskId}`);
    // 判断沟通风格
    ...
    logger_1.default.info(`Requirement profile generated for task ${data.taskId}`);
    return {
        success: true,
        taskId: data.taskId,
 ...
 * 处理匹配分析
 */
async function processMatchAnalysis(data) {
    logger_1.default.info(`Running match analysis for task ${data.taskId}`);
    const workConditionMatchingEngine = require('./workConditionMatchingEngine').default;
    // 执行匹配
    const matches = await workConditionMatchingEngine.findBestStudentsForTask(data.taskId, data.studentIds ? data.studentIds.length : 20);
    logger_1.default.info(`Match analysis completed for task ${data.taskId}, found ${matches.length} matches`);
    return {
        success: true,
        taskId: data.taskId,
...
async function processMentorGuidance(data) {
    logger_1.default.info(`Generating mentor guidance for order ${data.orderId}, scenario: ${data.scenario}`);
    ...
    try {
        ...
        await query(`INSERT INTO mentor_sessions (order_id, student_id, scenario, message, created_at)
       VALUES ($1, $2, $3, $4, NOW())`, [orderId, studentId, scenario, mentorMessage]);
        logger_1.default.info(`Mentor guidance generated for order ${orderId}, scenario: ${scenario}`);
        return {
            success: true,
            orderId,
            scenario,
        ...
        logger_1.default.error(`Failed to generate mentor guidance for scenario ${scenario}:`, error);
        throw error;
    }
}
/**
 ...
 */
async function generateT01TaskBreakdown(context) {
...
直接输出引导内容，不要前缀：`;
    ...
    try {
        ...
        // 记录AI调用日志
        await aiLogService_1.default.logAICall({
            ...
            costYuan: aiLogService_1.default.calculateClaudeCost('claude-haiku-4-5', response.usage.input_tokens, response.usage.output_tokens),
            durationMs: duration,
            status: 'success',
        });
        ...
        logger_1.default.error('Failed to generate T01 guidance:', error);
        // 降级：返回基础模板
        return `🎯 欢迎接单！
 ...
 */
async function generateT02SocraticGuidance(context) {
...
直接输出回复内容，不要前缀：`;
    ...
    try {
        ...
        logger_1.default.error('Failed to generate T02 guidance:', error);
        // 降级：返回基础引导
 ...
 */
async function generateT03FeedbackTranslation(context) {
...
直接输出翻译内容，不要前缀：`;
    try {
        ...
        logger_1.default.error('Failed to generate T03 guidance:', error);
        // 降级：返回基础翻译
 ...
 */
async function generateT04Nudge(context) {
...
直接输出轻推内容，不要前缀：`;
    try {
        ...
        logger_1.default.error('Failed to generate T04 guidance:', error);
        // 降级：返回基础轻推
        return `👋 嗨，好久不见！
 ...
 */
async function generateT05Milestone(context) {
...
直接输出见证内容，不要前缀：`;
    ...
    try {
        ...
        await aiLogService_1.default.logAICall({
            ...
            completionTokens: response.usage.output_tokens,
            totalTokens: response.usage.input_tokens + response.usage.output_tokens,
            costYuan: aiLogService_1.default.calculateClaudeCost('claude-3-5-sonnet-20241022', response.usage.input_tokens, response.usage.output_tokens),
            durationMs: duration,
            status: 'success',
        });
        if (content.type === 'text') {
        ...
        logger_1.default.error('Failed to generate T05 guidance:', error);
        // 降级：返回基础见证
 ...
 * 队列事件监听
 */
exports.aiTaskQueue.on('completed', (job, result) => {
    logger_1.default.info(`AI task completed: ${job.data.type}`, {
        jobId: job.id,
        result
    });
    // 通过WebSocket推送结果给前端
    try {
        ...
        switch (type) {
            case AITaskType.PROFILE_ANALYSIS:
                // 学生画像生成完成
                websocketService_1.default.notifyProfileAnalysisComplete(job.data.studentId, result);
                break;
            case AITaskType.PROJECT_CONDITION_ANALYSIS:
                ...
                const getCompanyId = async () => {
                    ...
                    const task = await queryOne('SELECT company_id FROM tasks WHERE id = $1', [job.data.taskId]);
                    return task?.company_id;
                };
                getCompanyId().then(companyId => {
                    if (companyId) {
                        websocketService_1.default.notifyRequirementAnalysisComplete(companyId, job.data.taskId, result);
                    }
                });
                break;
            case AITaskType.MATCH_ANALYSIS:
                ...
                getCompanyIdForMatch().then(companyId => {
                    if (companyId) {
                        websocketService_1.default.notifyMatchComplete(companyId, job.data.taskId, result.matchCount);
                    }
                });
                break;
            case AITaskType.MENTOR_GUIDANCE:
                // 导师消息推送
                websocketService_1.default.notifyMentorMessage(job.data.studentId, result);
                break;
            case AITaskType.SUBMISSION_REVIEW:
                // 交付物审核完成
                websocketService_1.default.notifySubmissionReviewed(job.data.studentId, job.data.orderId, result);
                break;
            case AITaskType.GROWTH_REPORT:
                // 成长报告生成完成
                websocketService_1.default.notifyGrowthReportReady(job.data.studentId, result.reportId);
                break;
        }
    }
    catch (error) {
        logger_1.default.error('Failed to push WebSocket notification:', error);
    }
});
exports.aiTaskQueue.on('failed', (job, err) => {
    logger_1.default.error(`AI task failed: ${job.data.type}`, {
        jobId: job.id,
        error: err.message
    });
});
exports.aiTaskQueue.on('stalled', (job) => {
    logger_1.default.warn(`AI task stalled: ${job.data.type}`, { jobId: job.id });
});
/**
 * 辅助函数：添加任务到队列
 */
async function enqueueAITask(taskData, options) {
    const job = await exports.aiTaskQueue.add(taskData, options);
    logger_1.default.info(`AI task enqueued: ${taskData.type}`, { jobId: job.id });
    return job;
}
/**
 * 获取队列状态
...
async function getQueueStats() {
    const [waiting, active, completed, failed, delayed] = await Promise.all([
        exports.aiTaskQueue.getWaitingCount(),
        exports.aiTaskQueue.getActiveCount(),
        exports.aiTaskQueue.getCompletedCount(),
        exports.aiTaskQueue.getFailedCount(),
        exports.aiTaskQueue.getDelayedCount()
    ]);
    return {
        waiting,
        active,
...
exports.default = exports.aiTaskQueue;
