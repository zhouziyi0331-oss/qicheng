...
class TaskExperienceService {
    ...
    async saveDraft(data) {
        const { company_id, title, description, category, required_skills, budget, deadline, requirements, deliverables, is_template, template_name, } = data;
        const result = await database_1.pool.query(`INSERT INTO task_drafts
     ...
     * 更新草稿
     */
    async updateDraft(draftId, companyId, updates) {
        const allowedFields = [
            'title',
            'description',
            'category',
    ...
    async getDrafts(companyId) {
        const result = await database_1.pool.query(`SELECT * FROM task_drafts WHERE company_id = $1 ORDER BY last_edited_at DESC`, [companyId]);
        return result.rows;
    }
    /**
     * 删除草稿
    ...
    async deleteDraft(draftId, companyId) {
        const result = await database_1.pool.query(`DELETE FROM task_drafts WHERE id = $1 AND company_id = $2`, [draftId, companyId]);
     ...
     * 从草稿创建任务
     */
    async publishFromDraft(draftId, companyId) {
        const draft = await database_1.pool.query(`SELECT * FROM task_drafts WHERE id = $1 AND company_id = $2`, [draftId, companyId]);
    ...
    async getTemplateById(templateId) {
        const result = await database_1.pool.query(`SELECT * FROM task_templates WHERE id = $1 AND is_active = true`, [templateId]);
        if (result.rows.length === 0) {
     ...
     * 使用模板创建草稿
     */
    async createDraftFromTemplate(templateId, companyId, customData) {
        const template = await this.getTemplateById(templateId);
        // 记录模板使用
        await database_1.pool.query(`INSERT INTO template_usage (id, template_id, company_id) VALUES ($1, $2, $3)`, [(0, uuid_1.v4)(), templateId, companyId]);
        // 创建草稿
     ...
     * E-01b: 智能预算建议
     */
    async suggestBudget(params, companyId) {
        const { task_category, task_description, required_skills, quality_expectation } = params;
        // 1. 查询历史数据
        const similarTasks = await database_1.pool.query(`SELECT budget, client_rating, status
       FROM tasks
       WHERE category = $1
         AND status = 'completed'
       ORDER BY completed_at DESC
       LIMIT 100`, [task_category]);
        ...
        // 4. 使用AI生成reasoning
        const reasoning = await this.generateBudgetReasoning(task_category, similarTasks.rows.length, suggestedOptimal, quality_expectation || 'standard');
        // 5. 保存建议记录
        const result = await database_1.pool.query(`INSERT INTO budget_suggestions
     ...
     * 使用AI生成预算建议理由
     */
    async generateBudgetReasoning(category, sampleSize, optimalBudget, quality) {
...
回复要简洁、专业，直接给建议，不要过多解释。`;
     ...
     */
    async getCategories() {
        const result = await database_1.pool.query(`SELECT DISTINCT category, COUNT(*) as template_count
       FROM task_templates
    ...
    async searchTemplates(keyword) {
        const result = await database_1.pool.query(`SELECT * FROM task_templates
       WHERE is_active = true
         AND (
...
exports.default = new TaskExperienceService();
//# sourceMappingURL=taskExperienceService.js.map