...
class GraduationReportService {
    ...
    async generateGraduationReport(userId) {
        logger_1.default.info(`[毕业报告] 开始为学生 ${userId} 生成毕业报告`);
        // 1. 检查学生是否达到Lv.6
        ...
        if (existingReport) {
            logger_1.default.info(`[毕业报告] 学生已有报告，报告ID: ${existingReport.id}`);
        ...
        logger_1.default.info(`[毕业报告] 报告生成完成，报告ID: ${reportId}`);
        return reportId;
    }
    /**
     * 检查用户等级
    ...
    async generateFullReport(studentData) {
        logger_1.default.info(`[毕业报告] 开始生成六章报告`);
        const chapters = [];
        // 生成六章
        for (let i = 1; i <= 6; i++) {
            logger_1.default.info(`[毕业报告] 生成第 ${i} 章...`);
        ...
        logger_1.default.info(`[毕业报告] 六章报告生成完成，总字数: ${totalWordCount}`);
        return {
            chapters,
    ...
    async generateChapter(chapterNumber, studentData, retryCount = 0) {
        ...
        logger_1.default.info(`[毕业报告] 生成第${chapterNumber}章，目标字数: ${chapterConfig.targetWords}字`);
        ...
        logger_1.default.info(`[毕业报告] 第${chapterNumber}章实际字数: ${wordCount}字`);
        // 【关键】字数验证
        if (wordCount < chapterConfig.minWords && retryCount === 0) {
            logger_1.default.warn(`[毕业报告] 第${chapterNumber}章字数不足(${wordCount}/${chapterConfig.minWords})，重试生成...`);
            // 重试时增加maxTokens
            return this.generateChapter(chapterNumber, studentData, 1);
        }
        // 重试后仍不足，记录错误
        if (wordCount < chapterConfig.minWords) {
            logger_1.default.error(`[毕业报告] 第${chapterNumber}章重试后字数仍不足: ${wordCount}/${chapterConfig.minWords}字`);
        }
        return {
...
直接输出章节正文，不要包含章节标题。如果学生有组队经历，必须引用；如果没有，推荐具体的互补技能类型。`,
    ...
    async processPayment(reportId, userId, paymentMethod, transactionId, pointsUsed = 0) {
        ...
        try {
         ...
         WHERE id = $2`, [amount, reportId]);
            ...
            logger_1.default.info(`[毕业报告] 付费成功，报告ID: ${reportId}`);
        }
        catch (error) {
    ...
    async updateReport(reportId, userId) {
        logger_1.default.info(`[毕业报告] 开始更新报告 ${reportId}`);
        // 1. 收集最新数据
        const studentData = await this.collectStudentData(userId);
        // 2. 重新生成报告
        const report = await this.generateFullReport(studentData);
        ...
        try {
         ...
         WHERE id = $3`, [JSON.stringify(report), report.preview_content, reportId]);
            logger_1.default.info(`[毕业报告] 报告更新完成`);
        }
        finally {
            client.release();
        }
