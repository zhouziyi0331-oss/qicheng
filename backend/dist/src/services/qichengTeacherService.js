...
class QichengTeacherService {
     ...
     */
    async translateRequirement(taskId) {
        try {
            const task = await (0, db_1.queryOne)(`SELECT title, description, company_id FROM tasks WHERE id = $1`, [taskId]);
            if (!task) {
                throw new Error(`Task ${taskId} not found`);
            }
            // 获取企业背景信息
            const company = await (0, db_1.queryOne)(`SELECT username, company_name, industry FROM users WHERE id = $1`, [task.company_id]);
...
直接返回翻译后的需求描述，不要其他解释：`;
            ...
            logger_1.default.info(`Translated requirement for task ${taskId}`);
            return translatedRequirement;
        }
        catch (error) {
            logger_1.default.error(`Failed to translate requirement for task ${taskId}:`, error);
            throw error;
        }
    }
    /**
     * 分析并翻译任务（完整版）
     */
    async analyzeAndTranslateTask(taskId) {
        try {
            // 获取任务信息
...
请以JSON格式返回分析结果，不要包含其他解释文字：
{
  ...
  "careerImpact": 0.7
            ...
            logger_1.default.info(`Generated translation for task ${taskId}`);
            return translation;
        }
        catch (error) {
            logger_1.default.error(`Failed to analyze and translate task ${taskId}:`, error);
            throw error;
        }
    }
    /**
...
[
  {
    ...
    "estimatedHours": 8
            ...
            logger_1.default.error('Failed to breakdown functional modules:', error);
            return [];
        }
    }
    /**
     * 生成学生友好描述
     */
    async generateStudentFriendlyDescription(taskTitle, taskDescription) {
        try {
...
直接返回翻译后的描述，不要其他内容：`;
            ...
            logger_1.default.error('Failed to generate student friendly description:', error);
...
返回JSON格式：
{
  ...
  "overall": 6
            ...
            logger_1.default.error('Failed to assess task difficulty:', error);
            return {
                technical: 5,
                cognitive: 5,
                execution: 5,
...
[
  {
    ...
    "why": "需要构建用户界面"
            ...
            logger_1.default.error('Failed to extract skill requirements:', error);
            return [];
        }
    }
    /**
    ...
    async saveTranslation(translation) {
        try {
            await (0, db_1.query)(`INSERT INTO task_translations (
        ...
        ON CONFLICT (task_id) DO UPDATE SET
          ...
          updated_at = NOW()`, [
            ...
            logger_1.default.info(`Saved translation for task ${translation.taskId}`);
        }
        catch (error) {
            logger_1.default.error(`Failed to save translation for task ${translation.taskId}:`, error);
            throw error;
        }
    }
    /**
     * 获取任务翻译
     */
    async getTaskTranslation(taskId) {
        try {
            const translation = await (0, db_1.queryOne)(`SELECT * FROM task_translations WHERE task_id = $1`, [taskId]);
            if (!translation) {
                return null;
            }
            return {
            ...
            logger_1.default.error(`Failed to get task translation for ${taskId}:`, error);
            return null;
        }
    }
    /**
     ...
     */
    async generateProjectRequirementSummary(taskId) {
        try {
         ...
         FROM tasks WHERE id = $1`, [taskId]);
            ...
            // 获取企业信息
            const company = await (0, db_1.queryOne)(`SELECT u.company_name, u.industry
         FROM tasks t
...
请严格按照上述结构生成摘要（约200字），每个维度用一句话描述：`;
            ...
            logger_1.default.info(`Generated requirement summary for task ${taskId}`);
            return summary;
        }
        catch (error) {
            logger_1.default.error(`Failed to generate requirement summary for task ${taskId}:`, error);
            throw error;
        }
    }
}
exports.default = new QichengTeacherService();
//# sourceMappingURL=qichengTeacherService.js.map