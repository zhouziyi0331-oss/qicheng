...
以JSON数组格式返回：[{...}]`;
            ...
            logger_1.default.error('Error breaking down functional modules:', error);
            return [];
        }
    }
    /**
...
以JSON格式返回：{"technical": X, "cognitive": X, "execution": X, "communication": X}`;
            ...
            logger_1.default.error('Error assessing task difficulty:', error);
            return {
                technical: 5,
                cognitive: 5,
                execution: 5,
...
以JSON数组格式返回：[{...}]`;
            ...
            logger_1.default.error('Error extracting skill requirements:', error);
            return [];
        }
    }
    /**
    ...
    async saveTranslation(translation) {
        ...
        try {
            await client.query(`INSERT INTO task_translations (
    ...
    async getTranslation(taskId) {
        const client = await db_1.pool.connect();
        try {
            const result = await client.query('SELECT * FROM task_translations WHERE task_id = $1', [taskId]);
    ...
    async translateTask(taskId) {
        ...
        try {
            // 获取任务信息
            const taskResult = await client.query('SELECT id, title, description, budget, duration FROM tasks WHERE id = $1', [taskId]);
