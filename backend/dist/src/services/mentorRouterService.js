...
// 智能导师路由服务
class MentorRouterService {
    // 分析消息类型
    async analyzeMessageType(message, context) {
...
请以JSON格式返回：
{
  ...
  "project_indicators": ["关键词1", "关键词2"]
    ...
    async getUserContext(userId) {
        ...
        try {
            // 获取最近对话
            const recentMessages = await client.query(`SELECT role, content, mentor_type, created_at
         FROM unified_mentor_conversations
         WHERE user_id = $1
         ORDER BY created_at DESC
         LIMIT 5`, [userId]);
            // 获取导师模式
            const modeResult = await client.query('SELECT * FROM mentor_modes WHERE user_id = $1', [userId]);
            // 获取活跃项目
            const projectResult = await client.query(`SELECT id, title, status, progress_percentage
         FROM pbl_projects
    ...
    // 记录路由决策
    async logRouting(userId, analysis, sessionId) {
        const client = await database_1.pool.connect();
        try {
            await client.query(`INSERT INTO mentor_collaboration_logs (
    ...
    // 生成过渡语
    async generateTransition(emotionalResponse, projectResponse) {
        const transitions = [
...
exports.MentorRouterService = MentorRouterService;
exports.mentorRouterService = new MentorRouterService();
//# sourceMappingURL=mentorRouterService.js.map