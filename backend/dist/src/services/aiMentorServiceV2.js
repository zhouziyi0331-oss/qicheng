...
class AIMentorService {
     ...
     * 发送导师消息（支持5大触发场景）
     */
    async sendMentorMessage(studentId, userMessage, context) {
        ...
        try {
            ...
            // 保存学生消息
            if (userMessage) {
                await client.query(`INSERT INTO mentor_conversations_v2 (student_id, task_id, role, message, trigger_type, trigger_context)
           VALUES ($1, $2, 'student', $3, $4, $5)`, [studentId, context.taskId, userMessage, context.triggerType, JSON.stringify(context)]);
            }
            // 保存导师回复
            const mentorMessageId = (0, uuid_1.v4)();
            await client.query(`INSERT INTO mentor_conversations_v2 (
     ...
     * T-01: 接单后第一步引导
     */
    async triggerT01Onboarding(studentId, taskId, taskData) {
        const context = {
        ...
        const message = await this.sendMentorMessage(studentId, '', // 系统触发，无用户消息
        context);
        return message;
    }
    /**
     * T-02: 学生说"我卡住了"
     */
    async triggerT02Stuck(studentId, taskId, userMessage, stuckDescription) {
        const client = await database_1.pool.connect();
        try {
            // 查询同类卡点
            const similarStuckPoints = await this.findSimilarStuckPoints(client, stuckDescription, studentId);
            // 记录卡点
            await client.query(`INSERT INTO student_stuck_points (
          student_id, task_id, stuck_type, stuck_description
        ) VALUES ($1, $2, 'other', $3)`, [studentId, taskId, stuckDescription]);
            const context = {
     ...
     * T-03: 交付物被打回
     */
    async triggerT03Rejected(studentId, taskId, rejectionReason, feedbackDetails) {
        const context = {
     ...
     * T-05: 里程碑见证
     */
    async triggerT05Milestone(studentId, milestoneType, milestoneData) {
        const context = {
        ...
        try {
            ...
            await client.query(`INSERT INTO student_milestones (
          id, student_id, milestone_type, milestone_data, comparison_data
            ...
            // 更新里程碑记录
            await client.query(`UPDATE student_milestones
         SET mentor_witnessed = true, witness_message = $1
         WHERE id = $2`, [message.message, milestoneId]);
            return message;
        }
        finally {
            client.release();
     ...
     * 获取对话历史（最近10轮）
     */
    async getConversationHistory(client, studentId, taskId) {
        const result = await client.query(`SELECT role, message, created_at
       FROM mentor_conversations_v2
       WHERE student_id = $1
         AND ($2::uuid IS NULL OR task_id = $2)
       ORDER BY created_at DESC
       LIMIT 10`, [studentId, taskId]);
        return result.rows.reverse(); // 按时间正序
    }
    /**
     * 查询同类卡点（用于T-02）
     */
    async findSimilarStuckPoints(client, description, excludeStudentId) {
       ...
       LIMIT 3`, [excludeStudentId]);
        return result.rows;
    }
    /**
     * 获取导师人设记忆（偶尔引用）
     */
    async getPersonaMemory(client, applicableTypes) {
        // 20%概率引用记忆
        if (Math.random() > 0.2) {
            return null;
        }
        const result = await client.query(`SELECT memory_text
       FROM mentor_persona_memories
       WHERE is_active = true
         AND (applicable_stuck_types && $1 OR applicable_stuck_types IS NULL)
       ORDER BY RANDOM()
       LIMIT 1`, [applicableTypes]);
        if (result.rows.length > 0) {
            // 更新使用记录
            await client.query(`UPDATE mentor_persona_memories
     ...
     * 构建AI Prompt
     */
    async buildPrompt(client, studentId, userMessage, context, conversationHistory) {
        // 获取学生信息
        const studentResult = await client.query(`SELECT
         nickname,
...
`;
     ...
     * 构建T-02卡住响应Prompt
     */
    async buildT02Prompt(client, context) {
...
${similarCases.length > 0
            ? similarCases.map((c, i) => `${i + 1}. Lv.${c.student_level}同学遇到类似问题，${c.resolved ? `用时${c.time_to_resolve_minutes}分钟解决` : '仍在处理中'}`).join('\n')
            : '暂无同类案例'}
...
`;
     ...
     * 调用AI
     */
    async callAI(prompt, triggerType) {
        try {
...
exports.default = new AIMentorService();
//# sourceMappingURL=aiMentorServiceV2.js.map