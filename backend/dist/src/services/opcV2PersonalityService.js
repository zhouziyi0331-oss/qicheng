...
class OPCAnalysisService {
     ...
     * 提交测试答案并进行AI分析
     */
    async submitAndAnalyze(userId, answers) {
        ...
        try {
            ...
            for (const answer of answers) {
                await client.query(`INSERT INTO opc_v2_user_answers (user_id, question_id, answer_value, test_session_id)
           VALUES ($1, $2, $3, $4)`, [userId, answer.questionId, JSON.stringify(answer.answerValue), sessionId]);
            }
            // 调用AI进行分析
            const analysisResult = await this.analyzeWithAI(answers);
            // 保存分析结果
            await client.query(`INSERT INTO opc_v2_user_profiles (
         ...
         WHERE id = $3`, [analysisResult.personalityType, analysisResult.initialLevel, userId]);
            await client.query('COMMIT');
            return {
                sessionId,
                analysisResult: {
                    ...analysisResult,
                    personalityTypeLabel: PERSONALITY_LABELS[analysisResult.personalityType],
                    trackRecommendationLabel: TRACK_LABELS[analysisResult.trackRecommendation]
                }
            };
        }
        catch (error) {
...
现在请分析并输出JSON。`;
        try {
            ...
            logger_1.default.error('AI分析失败:', error);
    ...
    async getLatestProfile(userId) {
        ...
        try {
         ...
         LIMIT 1`, [userId]);
            ...
            return {
                ...profile,
                personalityTypeLabel: PERSONALITY_LABELS[profile.personality_type],
                trackRecommendationLabel: TRACK_LABELS[profile.track_recommendation],
    ...
    async getQuestions() {
        const client = await database_1.pool.connect();
        try {
            const result = await client.query(`SELECT id, question_number, dimension, question_text, question_type, options
...
//# sourceMappingURL=opcV2PersonalityService.js.map