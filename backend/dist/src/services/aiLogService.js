...
class AILogService {
    ...
    async logAICall(log) {
        try {
            await (0, db_1.query)(`INSERT INTO ai_call_logs (
          ...
          request_data, response_data, created_at
            ...
            logger_1.default.info(`AI call logged: ${log.engineName} - ${log.status}`, {
            ...
            // 日志记录失败不应该影响主流程
            logger_1.default.error('Failed to log AI call:', error);
        }
    }
    /**
     * 计算Claude API的成本
     ...
     */
    calculateClaudeCost(modelName, promptTokens, completionTokens) {
        // Claude 3.5 Sonnet 定价（2024年价格）
        // Input: $3 per million tokens
        // Output: $15 per million tokens
        const inputCostPerMillion = 3;
        const outputCostPerMillion = 15;
        const inputCostUSD = (promptTokens / 1000000) * inputCostPerMillion;
        const outputCostUSD = (completionTokens / 1000000) * outputCostPerMillion;
     ...
     * 获取AI调用统计
     */
    async getAICallStats(startDate, endDate) {
        try {
...
exports.default = new AILogService();
