...
class ProjectAnalysisService {
     ...
     * 从项目信息生成需求条件画像
     */
    async generateRequirementProfile(projectInfo) {
        logger_1.default.info(`Generating requirement profile for task ${projectInfo.taskId}`);
        // 1. 分析信息接收需求
        ...
        // 8. 生成综合需求文本
        const requirementText = this.generateRequirementText(projectType, informationReceptionNeed, creationDriveNeed, learningApproachNeed, executionRhythmNeed, autonomyNeed, riskLevel);
        return {
     ...
     * 分析信息接收需求
     */
    analyzeInformationReceptionNeed(projectInfo) {
     ...
     * 分析创作驱动需求
     */
    analyzeCreationDriveNeed(projectInfo) {
     ...
     * 分析学习切入需求
     */
    analyzeLearningApproachNeed(projectInfo) {
     ...
     * 分析执行节奏需求
     */
    analyzeExecutionRhythmNeed(projectInfo) {
     ...
     * 分析自主度需求
     */
    analyzeAutonomyNeed(projectInfo) {
     ...
     * 生成综合需求文本
     */
    generateRequirementText(projectType, informationReceptionNeed, creationDriveNeed, learningApproachNeed, executionRhythmNeed, autonomyNeed, riskLevel) {
     ...
     * 保存项目需求画像到数据库
     */
    async saveRequirementProfile(profile) {
        try {
            // 生成向量
            const requirementVector = await vectorEmbeddingService_1.default.generateProjectRequirementVector(profile.requirementText);
            await (0, db_1.queryOne)(`INSERT INTO project_requirement_profiles (
          task_id,
        ...
        DO UPDATE SET
          ...
          updated_at = NOW()`, [
            ...
            logger_1.default.info(`Saved requirement profile for task ${profile.taskId}${requirementVector ? ' with vector' : ''}`);
        }
        catch (error) {
            logger_1.default.error('Failed to save requirement profile:', error);
            throw error;
        }
    }
}
exports.default = new ProjectAnalysisService();
//# sourceMappingURL=projectAnalysisService.js.map