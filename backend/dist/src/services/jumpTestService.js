...
const aiLogService_1 = __importDefault(require("./aiLogService"));
...
class JumpTestService {
     ...
     * 检查学生是否满足跳级条件
     */
    async checkJumpEligibility(studentId) {
        try {
            // 获取学生信息
            const student = await (0, db_1.queryOne)(`SELECT current_level, track, jump_cooling_orders_needed FROM users WHERE id = $1`, [studentId]);
            if (!student) {
            ...
            // 获取当前等级配置
            const levelConfig = await (0, db_1.queryOne)(`SELECT required_orders, min_rating FROM level_configs WHERE level = $1 AND track = $2`, [currentLevel, student.track]);
            if (!levelConfig) {
         ...
         WHERE student_id = $1 AND status = 'completed' AND order_type = 'normal'`, [studentId]);
            ...
            logger_1.default.error('Failed to check jump eligibility:', error);
            throw error;
        }
    }
    /**
     * 申请跳级
     */
    async applyForJumpTest(studentId) {
        try {
            // 检查资格
            ...
            // 获取学生信息
            const student = await (0, db_1.queryOne)(`SELECT track FROM users WHERE id = $1`, [studentId]);
            if (!student) {
                throw new Error('Student not found');
            }
            // 获取跳级测试模板
            ...
            // 创建跳级记录
            const jumpRecord = await (0, db_1.queryOne)(`INSERT INTO jump_test_records (
          student_id, from_level, target_level, track, status
        ) VALUES ($1, $2, $3, $4, 'pending')
        RETURNING id`, [studentId, eligibility.currentLevel, eligibility.targetLevel, student.track]);
            if (!jumpRecord) {
                throw new Error('Failed to create jump record');
            }
            logger_1.default.info(`Jump test applied: student ${studentId}, from Lv.${eligibility.currentLevel} to Lv.${eligibility.targetLevel}`);
            return {
                success: true,
            ...
            logger_1.default.error('Failed to apply for jump test:', error);
            throw error;
        }
    }
    /**
     * 获取跳级测试模板
     */
    async getJumpTestTemplate(track, targetLevel) {
        try {
            // 从模板表获取
            const template = await (0, db_1.queryOne)(`SELECT title, description, requirements, acceptance_criteria, difficulty, estimated_hours
         FROM jump_test_templates
         WHERE track = $1 AND target_level = $2 AND is_active = true`, [track, targetLevel]);
            if (template) {
                return {
            ...
            // 如果没有模板，使用AI生成
            logger_1.default.info(`No template found for ${track} Lv.${targetLevel}, generating with AI`);
            return await this.generateJumpTestTaskWithAI(track, targetLevel);
        }
        catch (error) {
            logger_1.default.error('Failed to get jump test template:', error);
            throw error;
        }
    }
    /**
     * 使用AI生成跳级测试任务
     */
    async generateJumpTestTaskWithAI(track, targetLevel) {
...
请直接输出JSON，不要其他内容。`;
        ...
        try {
            ...
            logger_1.default.error('Failed to generate jump test task with AI:', error);
            // 降级：返回通用模板
            return {
     ...
     * 推送跳级测试任务
     */
    async pushJumpTestTask(studentId, jumpRecordId, testTask) {
        try {
            // 创建测试订单
            const order = await (0, db_1.queryOne)(`INSERT INTO orders (
        ...
        RETURNING id`, [studentId]);
            ...
            // 更新跳级记录
            await (0, db_1.query)(`UPDATE jump_test_records
         SET test_order_id = $1, status = 'testing'
         WHERE id = $2`, [order.id, jumpRecordId]);
            // 发送导师T-01消息
...
准备好了就开始吧。`;
           ...
           VALUES ($1, $2, 'T01-JumpTest', $3, NOW())`, [order.id, studentId, mentorMessage]);
            }
            logger_1.default.info(`Jump test task pushed: order ${order.id}, student ${studentId}`);
            return order.id;
        }
        catch (error) {
            logger_1.default.error('Failed to push jump test task:', error);
            throw error;
        }
    }
    /**
     * 审核跳级测试（AI-03跳级模式）
     */
    async reviewJumpTest(orderId, submissionContent, fileUrls) {
        try {
            // 获取订单和跳级记录
            const order = await (0, db_1.queryOne)(`SELECT student_id FROM orders WHERE id = $1 AND order_type = 'jump_test'`, [orderId]);
            if (!order) {
                throw new Error('Jump test order not found');
            }
            const jumpRecord = await (0, db_1.queryOne)(`SELECT id, from_level, target_level, track
         FROM jump_test_records
         WHERE test_order_id = $1 AND status = 'testing'`, [orderId]);
            if (!jumpRecord) {
            ...
            // 调用AI审核（跳级模式）
            const aiReview = await this.aiReviewJumpTest(template, submissionContent, fileUrls, jumpRecord.target_level);
            const passed = aiReview.score >= 85;
            // 更新跳级记录
            await (0, db_1.query)(`UPDATE jump_test_records
         SET status = $1, ai_score = $2, ai_feedback = $3, submitted_at = NOW(), reviewed_at = NOW()
         WHERE id = $4`, [passed ? 'passed' : 'failed', aiReview.score, aiReview.feedback, jumpRecord.id]);
            if (passed) {
                // 通过：升级
...
继续保持这个势头！`;
                await (0, db_1.query)(`INSERT INTO mentor_sessions (order_id, student_id, scenario, message, created_at)
           VALUES ($1, $2, 'T05-JumpSuccess', $3, NOW())`, [orderId, order.student_id, mentorMessage]);
            }
            else {
                // 失败：设置冷却期
                await (0, db_1.query)(`UPDATE users SET jump_cooling_orders_needed = 2 WHERE id = $1`, [order.student_id]);
                // 发送导师消息
...
不是能力不够，可能是准备还差一点。完成当前等级的2个新任务后，你可以再次申请。到时候我会帮你回顾这次的经验。`;
                await (0, db_1.query)(`INSERT INTO mentor_sessions (order_id, student_id, scenario, message, created_at)
           VALUES ($1, $2, 'T05-JumpFailed', $3, NOW())`, [orderId, order.student_id, mentorMessage]);
            }
            logger_1.default.info(`Jump test reviewed: order ${orderId}, passed: ${passed}, score: ${aiReview.score}`);
            return {
                passed,
            ...
            logger_1.default.error('Failed to review jump test:', error);
            throw error;
        }
    }
    /**
     * AI审核跳级测试（更严格的标准）
     */
    async aiReviewJumpTest(template, submissionContent, fileUrls, targetLevel) {
...
请直接输出JSON。`;
        ...
        try {
            ...
            logger_1.default.error('Failed to AI review jump test:', error);
            // 降级：返回保守评分
            return {
                score: 70,
     ...
     * 升级学生等级
     */
    async upgradeStudent(studentId, newLevel) {
        await (0, db_1.query)(`UPDATE users
       SET current_level = $1,
           jump_success_count = jump_success_count + 1,
           jump_cooling_orders_needed = 0,
           updated_at = NOW()
       WHERE id = $2`, [newLevel, studentId]);
        logger_1.default.info(`Student ${studentId} upgraded to Lv.${newLevel}`);
    }
    /**
     * 订单完成后减少冷却期计数
     */
    async decreaseCoolingOrders(studentId) {
        await (0, db_1.query)(`UPDATE users
       SET jump_cooling_orders_needed = GREATEST(0, jump_cooling_orders_needed - 1)
       WHERE id = $1 AND jump_cooling_orders_needed > 0`, [studentId]);
    }
}
exports.default = new JumpTestService();
//# sourceMappingURL=jumpTestService.js.map