...
class OPCIdentityCardService {
     ...
     * 生成身份卡片
     */
    async generateCard(studentId, options = {}) {
        ...
        try {
         ...
         WHERE id = $1`, [studentId]);
            ...
            if (options.includeStats !== false) {
                ...
                stats.daysOnPlatform = Math.floor((now.getTime() - joinDate.getTime()) / (1000 * 60 * 60 * 24));
                // 获取任务统计
                const taskStatsResult = await client.query(`SELECT
           ...
           WHERE student_id = $1 AND status = 'approved'`, [studentId]);
            ...
            // 6. 保存卡片记录
            await client.query(`INSERT INTO opc_identity_cards (
          ...
          visual_theme, share_url, view_count
            ...
            logger_1.default.info('[OPCIdentityCard] 身份卡片生成成功', {
                cardId,
                studentId,
            ...
            logger_1.default.error('[OPCIdentityCard] 生成卡片失败:', error);
            throw error;
        }
        finally {
            client.release();
     ...
     * 获取卡片详情（用于分享链接访问）
     */
    async getCardById(cardId, incrementView = false) {
        const client = await database_1.pool.connect();
        try {
            // 增加浏览次数
            if (incrementView) {
                await client.query(`UPDATE opc_identity_cards
         ...
         WHERE id = $1`, [cardId]);
            ...
            logger_1.default.error('[OPCIdentityCard] 获取卡片失败:', error);
            throw error;
        }
        finally {
            client.release();
     ...
     * 获取学生的所有身份卡片
     */
    async getStudentCards(studentId, limit = 10) {
        const client = await database_1.pool.connect();
        try {
            const result = await client.query(`SELECT
          id as card_id,
         ...
         LIMIT $2`, [studentId, limit]);
            ...
            logger_1.default.error('[OPCIdentityCard] 获取学生卡片列表失败:', error);
            throw error;
        }
        finally {
            client.release();
     ...
     * 删除卡片
     */
    async deleteCard(cardId, studentId) {
        const client = await database_1.pool.connect();
        try {
            const result = await client.query(`DELETE FROM opc_identity_cards
         WHERE id = $1 AND student_id = $2`, [cardId, studentId]);
            return result.rowCount > 0;
        }
        catch (error) {
            logger_1.default.error('[OPCIdentityCard] 删除卡片失败:', error);
            throw error;
        }
        finally {
            client.release();
     ...
     * 根据人格类型选择视觉主题
     */
    selectThemeByPersonality(personalityType) {
        const themeMap = {
     ...
     * 获取人格类型中文标签
     */
    getPersonalityLabel(personalityType) {
        const labels = {
...
exports.default = new OPCIdentityCardService();
//# sourceMappingURL=opcIdentityCardService.js.map