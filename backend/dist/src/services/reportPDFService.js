...
class ReportPDFService {
     ...
     * 导出报告为PDF
     */
    async exportReportToPDF(options) {
        const { studentId, reportId, includeCharts = true, format = 'A4' } = options;
        try {
            logger_1.default.info('[PDF导出] 开始生成PDF', { studentId, reportId });
            // 1. 获取报告数据
            const report = await (0, db_1.queryOne)(`SELECT sr.*, u.nickname, u.avatar_url
         FROM student_reports sr
         JOIN users u ON sr.student_id = u.id
         WHERE sr.id = $1 AND sr.student_id = $2`, [reportId, studentId]);
            if (!report) {
            ...
            await browser.close();
            logger_1.default.info('[PDF导出] PDF生成完成', {
                studentId,
                reportId,
            ...
            logger_1.default.error('[PDF导出] 生成失败', { error, studentId, reportId });
            throw error;
        }
    }
    /**
     * 生成报告HTML模板
     */
    generateReportHTML(report, includeCharts) {
  ...
  <div class="container">
    ...
    <div class="section">
      ...
      <ul class="strengths-list">
        ${reportData.skillProfile.strengths.map((s) => `<li>${s}</li>`).join('')}
      </ul>
    </div>
    ` : ''}
    ...
    <div class="section">
      ...
      <ul class="recommendations-list">
        ${reportData.skillProfile.recommendations.map((r) => `<li>${r}</li>`).join('')}
      </ul>
    </div>
    ` : ''}
    ...
    <div class="section">
      ...
      <div class="milestones">
        ${reportData.milestones.slice(0, 10).map((m) => `
...
exports.reportPDFService = new ReportPDFService();
exports.default = exports.reportPDFService;
//# sourceMappingURL=reportPDFService.js.map