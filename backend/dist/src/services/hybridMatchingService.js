...
exports.hybridMatchingService = exports.HybridMatchingService = void 0;
...
class HybridMatchingService {
     ...
     */
    async matchStudentsForTask(taskId, topN = 10, useAI = true) {
        try {
            // 1. 获取任务信息
            const taskResult = await db_1.pool.query(`SELECT id, title, description, track, level,
         ...
         FROM tasks WHERE id = $1`, [taskId]);
            ...
            // 2. 获取候选学生
            const studentsResult = await db_1.pool.query(`SELECT u.id, u.nickname as username, u.avatar_url as avatar,
         ...
         WHERE sa.current_level <= $1 + 1
           AND u.role = 'student'`, [task.level]);
            // 3. 计算每个学生的匹配分数
            const matches = await Promise.all(studentsResult.rows.map(async (student) => {
                // 规则匹配分数（0-100）
                ...
                if (useAI && task.combined_embedding && student.profile_embedding) {
                    try {
                        ...
                        const similarity = embeddingService_1.embeddingService.calculateCosineSimilarity(taskEmbedding, studentEmbedding);
                        aiScore = similarity * 100; // 转换为0-100分
                    }
                    catch (error) {
                        logger_1.default.warn('Failed to calculate vector similarity', { error });
                    }
                }
                // 混合分数：规则60% + AI 40%
            ...
            logger_1.default.error('Error in hybrid matching for task', { taskId, error });
            throw error;
        }
    }
    /**
     * 混合匹配：为学生推荐任务
     */
    async matchTasksForStudent(userId, topN = 20, useAI = true) {
        try {
            // 1. 获取学生信息
            const studentResult = await db_1.pool.query(`SELECT u.id, u.profile_embedding,
                sa.openness, sa.persistence, sa.creativity,
                sa.primary_track, sa.current_level, sa.total_completed_tasks
         FROM users u
         JOIN student_abilities sa ON u.id = sa.user_id
         WHERE u.id = $1`, [userId]);
            ...
            // 2. 获取可用任务
            const tasksResult = await db_1.pool.query(`SELECT id, title, description, track, level,
                required_openness, required_persistence, required_creativity,
         ...
         WHERE status = 'active'
           AND level >= $1 - 1
           AND level <= $1 + 1
           AND deleted_at IS NULL`, [student.current_level]);
            // 3. 计算每个任务的匹配分数
            const matches = await Promise.all(tasksResult.rows.map(async (task) => {
                // 规则匹配分数
                const ruleScore = this.calculateRuleBasedScore(student, task);
                // AI向量相似度分数
                let aiScore = 0;
                if (useAI && student.profile_embedding && task.combined_embedding) {
                    try {
                        ...
                        const taskEmbedding = typeof task.combined_embedding === 'string'
                            ? JSON.parse(task.combined_embedding)
                            : task.combined_embedding;
                        const similarity = embeddingService_1.embeddingService.calculateCosineSimilarity(studentEmbedding, taskEmbedding);
            ...
            logger_1.default.error('Error in hybrid matching for student', { userId, error });
            throw error;
        }
    }
    /**
     * 规则匹配算法（保留原有逻辑）
     */
    calculateRuleBasedScore(student, task) {
        let totalScore = 0;
        // 1. 赛道匹配（30%）
        let trackScore = 0;
     ...
     * 保存匹配日志
     */
    async saveMatchLogs(taskId, matches) {
        ...
        try {
            ...
            for (const match of matches) {
                ...
                await client.query(`INSERT INTO ai_matches
           ...
           VALUES ($1, $2, $3, $4, $5)`, [
                ...
                await client.query(`INSERT INTO ai_match_logs
           ...
           VALUES ($1, $2, $3, $4, $5, $6)`, [
            ...
            logger_1.default.info('Match logs saved successfully', { taskId, matchCount: matches.length });
        }
        catch (error) {
            await client.query('ROLLBACK');
            logger_1.default.error('Error saving match logs', { error });
            throw error;
        }
        finally {
            client.release();
     ...
     * 为任务生成并保存 embedding
     */
    async generateTaskEmbedding(taskId) {
        try {
            const taskResult = await db_1.pool.query('SELECT id, title, description FROM tasks WHERE id = $1', [taskId]);
            if (taskResult.rows.length === 0) {
                throw new Error('Task not found');
            }
            const task = taskResult.rows[0];
            const embeddings = await embeddingService_1.embeddingService.generateTaskEmbedding(task.title, task.description);
            await db_1.pool.query(`UPDATE tasks
         ...
         WHERE id = $4`, [
            ...
            logger_1.default.info('Task embedding generated', { taskId });
        }
        catch (error) {
            logger_1.default.error('Error generating task embedding', { taskId, error });
            throw error;
        }
    }
    /**
     * 为学生生成并保存 embedding
     */
    async generateStudentEmbedding(userId) {
        try {
            const studentResult = await db_1.pool.query(`SELECT
           u.id,
           u.nickname,
           u.university,
           u.major,
...
${student.creativity >= 7 ? '富有创造力，善于提出创新想法' : student.creativity >= 4 ? '有一定创新思维，能够改进现有方案' : '偏好按照既定方法执行'}
      `.trim();
            const embeddings = await embeddingService_1.embeddingService.generateStudentEmbedding({
                skills: [skillsText],
         ...
         WHERE id = $4`, [
            ...
            logger_1.default.info('Student embedding generated', { userId });
        }
        catch (error) {
            logger_1.default.error('Error generating student embedding', { userId, error });
            throw error;
        }
    }
}
exports.HybridMatchingService = HybridMatchingService;
exports.hybridMatchingService = new HybridMatchingService();
//# sourceMappingURL=hybridMatchingService.js.map