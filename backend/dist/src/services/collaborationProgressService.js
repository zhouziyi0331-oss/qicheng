...
class CollaborationProgressService {
     ...
     * 获取进度提示文案
     */
    getProgressHint(progress, currentUserType) {
        const { completedCount, canUnlockContact, contactUnlocked, studentAgreed, companyAgreed } = progress;
        // 已解锁
        if (contactUnlocked) {
     ...
     * 获取所有合作进度（用于学生查看所有企业）
     */
    async getStudentCollaborations(studentId) {
        const result = await db_1.default.query(`SELECT * FROM collaboration_progress
       WHERE student_id = $1
     ...
     * 获取所有合作进度（用于企业查看所有学生）
     */
    async getCompanyCollaborations(companyId) {
        const result = await db_1.default.query(`SELECT * FROM collaboration_progress
       WHERE company_id = $1
     ...
     * 获取进度百分比（用于UI展示）
     */
    getProgressPercentage(completedCount) {
        const target = 2; // 2单解锁
...
exports.default = new CollaborationProgressService();
//# sourceMappingURL=collaborationProgressService.js.map