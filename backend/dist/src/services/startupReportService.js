...
exports.StartupReportService = void 0;
...
class StartupReportService {
     ...
     * 生成完整的创业综合报告
     */
    static async generateStartupReport(userId, reportId) {
        try {
            logger_1.default.info('开始生成创业报告', { userId, reportId });
            // 1. 收集用户数据
            ...
            logger_1.default.info('创业报告生成成功', { userId, reportId });
            return report;
        }
        catch (error) {
            logger_1.default.error('创业报告生成失败', { userId, reportId, error: error.message });
            throw error;
        }
    }
    /**
    ...
    static async collectUserData(userId) {
       ...
       WHERE user_id = $1`, [userId]);
        ...
        // 获取任务历史（最近10个已完成任务）
        const tasks = await (0, db_1.query)(`SELECT t.title, t.description, ts.approved_at, ts.company_score, ts.ai_feedback
       ...
       LIMIT 10`, [userId]);
        // 获取成长亮点
        const highlights = await (0, db_1.query)(`SELECT event_title, event_desc
     ...
     */
    static async generateCustomizedAnalysis(userData) {
        ...
        try {
            ...
            logger_1.default.error('Claude API 调用失败', { error: error.message });
            // 降级方案：使用规则引擎生成基础分析
...
8. **返回纯 JSON 格式**，不要额外的解释文字`;
     ...
     * 降级方案：基于规则的分析
     */
    static generateFallbackAnalysis(userData) {
        return {
     ...
     * 获取通用创业指南
     */
    static async getStartupGuides() {
        const guides = await (0, db_1.query)(`SELECT title, content
       FROM startup_guides
...
exports.StartupReportService = StartupReportService;
//# sourceMappingURL=startupReportService.js.map