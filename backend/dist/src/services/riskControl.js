...
async function checkTransactionRisk(studentId, enterpriseId, taskId) {
    ...
    const studentOrders = await (0, db_1.query)(`SELECT COUNT(*) as count
     FROM orders
     WHERE student_id = $1 AND created_at > NOW() - INTERVAL '1 hour'`, [studentId]);
    ...
    const enterpriseOrders = await (0, db_1.query)(`SELECT COUNT(*) as count
     FROM orders
     WHERE client_id = $1 AND created_at > NOW() - INTERVAL '1 hour'`, [enterpriseId]);
...
async function recordRiskEvent(studentId, enterpriseId, taskId, riskCheck) {
    ...
    logger_1.default.info('风险事件已记录:', { studentId, enterpriseId, taskId, action: riskCheck.action });
}
/**
 * 添加到黑名单 - 真实保存到Redis
 */
async function addToBlacklist(userId, reason, durationDays = 30) {
    ...
    await redis_1.default.expire(`risk:blacklist:${userId}`, durationDays * 24 * 60 * 60);
    // 同时记录到数据库
    await (0, db_1.query)(`INSERT INTO risk_blacklist (user_id, reason, duration_days, created_at, expires_at)
     VALUES ($1, $2, $3, NOW(), NOW() + INTERVAL '${durationDays} days')`, [userId, reason, durationDays]);
    logger_1.default.warn('用户已加入黑名单:', { userId, reason, durationDays });
}
/**
 * 从黑名单移除 - 真实删除
 */
async function removeFromBlacklist(userId) {
    await redis_1.default.srem('risk:blacklist', userId);
    await (0, db_1.query)(`UPDATE risk_blacklist SET removed_at = NOW() WHERE user_id = $1 AND removed_at IS NULL`, [userId]);
    logger_1.default.info('用户已从黑名单移除:', { userId });
}
