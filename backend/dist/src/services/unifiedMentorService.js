...
exports.unifiedMentorService = exports.UnifiedMentorService = void 0;
const database_1 = require("../config/database");
const mentorRouterService_1 = require("./mentorRouterService");
const pblAgentService_1 = require("./pblAgentService");
const uuid_1 = require("uuid");
// 统一导师服务
class UnifiedMentorService {
    // 统一对话接口
    async chat(userId, message, options) {
        const sessionId = options?.session_id || (0, uuid_1.v4)();
        // 1. 保存用户消息
        await this.saveMessage(userId, sessionId, 'user', message);
        // 2. 获取用户上下文
        const context = await mentorRouterService_1.mentorRouterService.getUserContext(userId);
        // 3. 检查用户的导师模式偏好
        const mentorMode = context.mentor_mode;
        // 4. 如果用户设置了固定模式且不自动切换
        if (mentorMode?.preferred_mode && !mentorMode.auto_switch) {
            return await this.directRoute(userId, sessionId, message, mentorMode.preferred_mode, context);
        }
        // 5. 智能路由分析
        const analysis = await mentorRouterService_1.mentorRouterService.analyzeMessageType(message, context);
        // 6. 记录路由决策
        await mentorRouterService_1.mentorRouterService.logRouting(userId, analysis, sessionId);
        // 7. 根据分析结果路由
    ...
    // 直接路由（用户指定模式）
    async directRoute(userId, sessionId, message, mode, context) {
        if (mode === 'emotional') {
            return await this.emotionalMentorResponse(userId, sessionId, message, context);
        }
    ...
    // 情感导师响应
    async emotionalMentorResponse(userId, sessionId, message, context) {
        // 调用现有的情感导师服务（启程小猫）
        // 这里假设已有的导师服务，需要适配
        ...
        // 检查是否需要建议切换到项目导师
        const switchSuggestion = await this.checkSwitchSuggestion(userId, message, response, 'emotional', context);
        return {
    ...
    // 项目导师响应
    async projectMentorResponse(userId, sessionId, message, context) {
        // 获取或创建项目
        let projectId = context.active_project?.id;
        if (!projectId) {
            // 如果没有活跃项目，检查是否需要创建
            if (this.isProjectInitiationMessage(message)) {
                const project = await pblAgentService_1.pblAgentService.initializeProject(userId, message);
        ...
        // 进行苏格拉底式对话
        const response = await pblAgentService_1.pblAgentService.conductSocraticDialogue(projectId, message, context);
        // 保存响应
    ...
    // 协同响应
    async coordinatedResponse(userId, sessionId, message, context, analysis) {
        // 1. 情感导师建立连接（简短模式）
        const emotionalResponse = await this.callEmotionalMentor(userId, message, { ...context, mode: 'brief' });
        // 2. 生成过渡语
        const transition = await mentorRouterService_1.mentorRouterService.generateTransition(emotionalResponse, null);
        // 3. 项目导师提供实践方向
        let projectResponse;
        if (context.active_project) {
            projectResponse = await pblAgentService_1.pblAgentService.conductSocraticDialogue(context.active_project.id, message, { ...context, emotional_context: emotionalResponse });
        }
        else {
            // 引导创建项目
            projectResponse = {
    ...
    // 调用情感导师（适配现有系统）
    async callEmotionalMentor(userId, message, context) {
        // 这里需要调用现有的导师服务
        // 暂时返回模拟响应
        // TODO: 集成现有的导师服务
    ...
    // 检查是否需要建议切换导师
    async checkSwitchSuggestion(userId, userMessage, mentorResponse, currentMentor, context) {
        // 如果情感导师检测到可以转化为项目
        if (currentMentor === 'emotional') {
            ...
            const hasProjectIntent = projectIndicators.some(indicator => userMessage.includes(indicator));
            if (hasProjectIntent) {
                return {
                    suggested: true,
                    to_mentor: 'project',
        ...
        if (currentMentor === 'project') {
            ...
            const hasEmotionalNeed = emotionalIndicators.some(indicator => userMessage.includes(indicator));
            if (hasEmotionalNeed) {
                return {
                    suggested: true,
                    to_mentor: 'emotional',
    ...
    // 判断是否是项目初始化消息
    isProjectInitiationMessage(message) {
    ...
    async saveMessage(userId, sessionId, role, content, metadata) {
        ...
        try {
            await client.query(`INSERT INTO unified_mentor_conversations (
    ...
    // 切换导师模式
    async switchMode(userId, mode) {
        const client = await database_1.pool.connect();
        try {
            await client.query(`INSERT INTO mentor_modes (user_id, current_mode, preferred_mode, auto_switch)
         VALUES ($1, $2, $3, $4)
    ...
    async getConversationHistory(userId, sessionId, limit = 20) {
        const client = await database_1.pool.connect();
        try {
            const result = await client.query(`SELECT * FROM unified_mentor_conversations
    ...
    // 创建情感-项目关联
    async linkEmotionToProject(userId, emotionalData, projectId, linkType, transformationStory) {
        const client = await database_1.pool.connect();
        try {
            const result = await client.query(`INSERT INTO emotional_project_links (
    ...
    async getGrowthJourney(userId) {
        ...
        try {
         ...
         WHERE epl.user_id = $1
         ORDER BY epl.created_at DESC`, [userId]);
            return links.rows;
        }
        finally {
            client.release();
...
exports.UnifiedMentorService = UnifiedMentorService;
exports.unifiedMentorService = new UnifiedMentorService();
//# sourceMappingURL=unifiedMentorService.js.map