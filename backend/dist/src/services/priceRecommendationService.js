...
class PriceRecommendationService {
    ...
    async calculatePriceRecommendation(input) {
        ...
        if (historicalData.avgPrice && historicalData.count >= 5) {
            ...
            console.log(`[价格推荐] 基于历史数据：均价=${historicalData.avgPrice}, 样本数=${historicalData.count}`);
        }
        else {
            // 历史数据不足，基于基准价计算
            priceMin = Math.round(basePrice * 0.85);
            priceMax = Math.round(basePrice * 1.3);
            console.log(`[价格推荐] 基于基准价：basePrice=${basePrice}`);
        }
        ...
        console.log('[价格推荐] 计算完成', recommendation);
    ...
    async getHistoricalData(input) {
        ...
        try {
         ...
         WHERE t.track = $1
           ...
           AND t.student_price > 0`, [input.track, input.difficulty]);
     ...
     * 保存价格计算历史（用于后续优化算法）
     */
    async savePriceCalculationHistory(taskId, input, recommendation) {
        const client = await database_1.pool.connect();
        try {
            await client.query(`INSERT INTO price_calculation_history (
          ...
          historical_avg_price, similar_tasks_count
            ...
            console.log(`[价格推荐] 已保存计算历史: taskId=${taskId}`);
        }
        finally {
            client.release();
        }
     ...
     * 验证企业出价是否合理
     */
    validateEnterprisePrice(enterprisePrice, recommendation) {
...
exports.default = new PriceRecommendationService();
