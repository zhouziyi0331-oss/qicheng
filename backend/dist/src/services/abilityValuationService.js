...
class AbilityValuationService {
     ...
     * 生成学生的资产仪表盘
     */
    async generateDashboard(studentId) {
        const client = await database_1.pool.connect();
        try {
            // 1. 获取学生的所有能力数据
            const abilitiesResult = await client.query(`SELECT
          ability_name,
          current_level,
          experience_points,
          last_updated
         ...
         ORDER BY current_level DESC, experience_points DESC`, [studentId]);
            // 2. 计算每项能力的市场价值
            const assets = [];
            let totalValue = 0;
            for (const ability of abilitiesResult.rows) {
                const valuation = this.calculateAbilityValue(ability.ability_name, ability.current_level, ability.experience_points);
                const monthlyGrowth = await this.calculateMonthlyGrowth(studentId, ability.ability_name);
                assets.push({
            ...
            logger_1.default.info('[AbilityValuation] 资产仪表盘生成成功', {
                studentId,
                totalValue,
            ...
            logger_1.default.error('[AbilityValuation] 生成资产仪表盘失败:', error);
            throw error;
        }
        finally {
            client.release();
     ...
     * 计算单项能力的市场价值
     */
    calculateAbilityValue(abilityName, level, experiencePoints) {
        // 基础价值
     ...
     * 计算月均收入
     */
    async calculateMonthlyIncome(studentId) {
        const client = await database_1.pool.connect();
        try {
            const result = await client.query(`SELECT COALESCE(SUM(amount), 0) as total
         FROM earnings
         WHERE student_id = $1
           AND created_at >= NOW() - INTERVAL '30 days'`, [studentId]);
            ...
            logger_1.default.error('[AbilityValuation] 计算月均收入失败:', error);
            return 0;
        }
        finally {
            client.release();
     ...
     * 计算能力月度成长值
     */
    async calculateMonthlyGrowth(studentId, abilityName) {
        const client = await database_1.pool.connect();
        try {
            // 获取30天前的能力数据
            const historyResult = await client.query(`SELECT current_level, experience_points
         FROM ability_growth_history
         WHERE student_id = $1
           AND ability_name = $2
           AND recorded_at >= NOW() - INTERVAL '30 days'
         ORDER BY recorded_at ASC
         LIMIT 1`, [studentId, abilityName]);
            ...
            // 获取当前能力数据
            const currentResult = await client.query(`SELECT current_level, experience_points
         FROM student_abilities
         WHERE student_id = $1 AND ability_name = $2`, [studentId, abilityName]);
            ...
            logger_1.default.error('[AbilityValuation] 计算月度成长失败:', error);
            return 0;
        }
        finally {
            client.release();
     ...
     * 获取价值变化趋势（最近7天）
     */
    async getValueTrends(studentId) {
        const client = await database_1.pool.connect();
        try {
            // 获取最近7天的能力快照
         ...
         ORDER BY recorded_at DESC`, [studentId]);
            // 按日期分组计算总价值
            const trendMap = new Map();
            for (const row of result.rows) {
                const date = row.date.toISOString().split('T')[0];
                const value = this.calculateAbilityValue(row.ability_name, row.current_level, row.experience_points).currentValue;
            ...
            logger_1.default.error('[AbilityValuation] 获取价值趋势失败:', error);
            return [];
        }
        finally {
            client.release();
     ...
     * 获取市场对比数据
     */
    async getMarketComparison(studentId, totalValue) {
        const client = await database_1.pool.connect();
        try {
            // 计算所有学生的能力价值分布
            const result = await client.query(`SELECT
         ...
         FROM (
           ...
           GROUP BY student_id
            ...
            logger_1.default.error('[AbilityValuation] 获取市场对比数据失败:', error);
            return {
                percentile: 50,
                averageValue: 3000,
     ...
     * 计算下一个里程碑
     */
    calculateNextMilestone(currentValue, assets) {
...
exports.default = new AbilityValuationService();
//# sourceMappingURL=abilityValuationService.js.map