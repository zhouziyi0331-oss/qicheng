...
exports.CommunicationService = exports.ContactFilterService = void 0;
...
class ContactFilterService {
    ...
    static async loadRules() {
        const result = await db_1.pool.query('SELECT rule_type, pattern, replacement, priority FROM contact_filter_rules WHERE is_active = TRUE ORDER BY priority DESC');
...
ContactFilterService.filterRules = [];
/**
 * 任务沟通服务
 */
class CommunicationService {
     ...
     * 企业添加任务补充说明
     */
    static async addClarification(taskId, companyId, content, attachments = []) {
        const result = await db_1.pool.query(`INSERT INTO task_clarifications (task_id, company_id, content, attachments)
       VALUES ($1, $2, $3, $4)
       RETURNING *`, [taskId, companyId, content, JSON.stringify(attachments)]);
        return result.rows[0];
    }
    /**
     * 获取任务的所有补充说明
     */
    static async getClarifications(taskId) {
        const result = await db_1.pool.query(`SELECT c.*, u.username as company_name
       FROM task_clarifications c
       JOIN users u ON c.company_id = u.id
       WHERE c.task_id = $1
       ORDER BY c.created_at DESC`, [taskId]);
        return result.rows;
    }
    /**
     * 学生提问（先问AI）
     */
    static async askQuestion(taskId, studentId, question) {
        // 1. 先尝试AI回答
        const aiAnswer = await this.getAIAnswer(question);
        // 2. 保存问题和AI回答
        const result = await db_1.pool.query(`INSERT INTO task_questions (task_id, student_id, question, question_type, ai_answer, ai_confidence, status)
     ...
     * AI回答问题（基于知识库匹配）
     */
    static async getAIAnswer(question) {
        // 简单的关键词匹配算法
        const result = await db_1.pool.query(`SELECT answer, confidence_threshold, question_keywords
     ...
     * 转发问题给企业
     */
    static async forwardToCompany(questionId, studentId) {
        // 1. 获取问题信息
        const questionResult = await db_1.pool.query(`SELECT tq.*, t.company_id
       FROM task_questions tq
       JOIN tasks t ON tq.task_id = t.id
       WHERE tq.id = $1 AND tq.student_id = $2`, [questionId, studentId]);
        if (questionResult.rows.length === 0) {
            throw new Error('问题不存在或无权限');
        }
        const question = questionResult.rows[0];
        // 2. 更新问题状态
        await db_1.pool.query(`UPDATE task_questions
       SET status = 'forwarded', question_type = 'company'
       WHERE id = $1`, [questionId]);
        // 3. 记录转发
        await db_1.pool.query(`INSERT INTO question_forwards (question_id, forwarded_by, forwarded_to, reason)
       VALUES ($1, $2, $3, $4)`, [questionId, studentId, question.company_id, 'AI无法准确回答']);
        return { success: true, message: '问题已转发给企业，预计24小时内回复' };
    }
    /**
     * 企业回答学生问题
     */
    static async answerQuestion(questionId, companyId, answer) {
        const result = await db_1.pool.query(`UPDATE task_questions tq
       ...
       RETURNING tq.*`, [answer, companyId, questionId, companyId]);
     ...
     * 获取任务的所有问答
     */
    static async getQuestions(taskId, userId, userRole) {
        let query = `
      SELECT tq.*,
     ...
     * 发送中转消息（自动过滤联系方式）
     */
    static async sendRelayMessage(taskId, senderId, receiverId, content, attachments = []) {
        // 1. 过滤联系方式
        const { filtered, keywords } = ContactFilterService.filterContent(content);
        // 2. 保存消息
        const result = await db_1.pool.query(`INSERT INTO relay_messages (task_id, sender_id, receiver_id, content, original_content, filtered_keywords, attachments)
       VALUES ($1, $2, $3, $4, $5, $6, $7)
       RETURNING *`, [taskId, senderId, receiverId, filtered, content, JSON.stringify(keywords), JSON.stringify(attachments)]);
        return {
            ...result.rows[0],
     ...
     * 获取中转消息列表
     */
    static async getRelayMessages(taskId, userId) {
        const result = await db_1.pool.query(`SELECT rm.*,
       ...
       ORDER BY rm.created_at ASC`, [taskId, userId]);
        // 标记已读
        await db_1.pool.query(`UPDATE relay_messages
       SET is_read = TRUE, read_at = CURRENT_TIMESTAMP
       WHERE task_id = $1 AND receiver_id = $2 AND is_read = FALSE`, [taskId, userId]);
        return result.rows;
    }
    /**
     * 获取未读消息数
     ...
     * 标记AI回答是否有帮助
     */
    static async markAIAnswerHelpful(questionId, isHelpful) {
        if (isHelpful) {
            // 增加知识库的有帮助计数
            await db_1.pool.query(`UPDATE ai_qa_knowledge
