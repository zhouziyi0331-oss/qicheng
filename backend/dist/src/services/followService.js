...
class FollowService {
     ...
     * 关注学生
     */
    async followStudent(data) {
        const { company_id, student_id, follow_source, follow_reason, tags, notes } = data;
        // 检查是否已关注
        const existing = await database_1.pool.query(`SELECT id FROM student_follows WHERE company_id = $1 AND student_id = $2`, [company_id, student_id]);
        if (existing.rows.length > 0) {
            throw new Error('已经关注过该学生');
        }
        const result = await database_1.pool.query(`INSERT INTO student_follows
       (id, company_id, student_id, follow_source, follow_reason, tags, notes)
       VALUES ($1, $2, $3, $4, $5, $6, $7)
       RETURNING *`, [(0, uuid_1.v4)(), company_id, student_id, follow_source, follow_reason, tags || [], notes]);
        return result.rows[0];
    }
    /**
     * 取消关注
     */
    async unfollowStudent(companyId, studentId) {
        const result = await database_1.pool.query(`DELETE FROM student_follows WHERE company_id = $1 AND student_id = $2`, [companyId, studentId]);
     ...
     * 检查是否关注
     */
    async isFollowing(companyId, studentId) {
        const result = await database_1.pool.query(`SELECT id FROM student_follows WHERE company_id = $1 AND student_id = $2`, [companyId, studentId]);
        return result.rows.length > 0;
    }
    /**
     * 获取企业关注的学生列表
     */
    async getFollowingStudents(companyId, limit = 50, offset = 0) {
     ...
     * 获取关注该学生的企业列表
     */
    async getFollowers(studentId, limit = 50, offset = 0) {
        const result = await database_1.pool.query(`SELECT
         sf.*,
         u.id as company_id,
         u.company_name,
     ...
     * 更新关注设置
     */
    async updateFollowSettings(companyId, studentId, settings) {
        const updates = [];
        const values = [];
        let paramIndex = 1;
     ...
     * 获取学生动态
     */
    async getStudentActivities(studentId, limit = 20, offset = 0) {
        const result = await database_1.pool.query(`SELECT * FROM student_activities
     ...
     * 获取关注学生的动态流
     */
    async getFollowingActivitiesFeed(companyId, limit = 30) {
        const result = await database_1.pool.query(`SELECT
         sa.*,
     ...
     * 获取关注通知
     */
    async getFollowNotifications(companyId, limit = 50, offset = 0) {
        const result = await database_1.pool.query(`SELECT
         fn.*,
         u.username as student_name,
         u.avatar as student_avatar
    ...
    async markNotificationAsRead(notificationId, companyId) {
        await database_1.pool.query(`UPDATE follow_notifications
       SET is_read = true, read_at = NOW()
       WHERE id = $1 AND company_id = $2`, [notificationId, companyId]);
    }
    /**
     * 标记所有通知已读
     */
     ...
     * 获取未读通知数
     */
    async getUnreadNotificationCount(companyId) {
        const result = await database_1.pool.query(`SELECT COUNT(*) as count FROM follow_notifications
       WHERE company_id = $1 AND is_read = false`, [companyId]);
    ...
    async createCollection(data) {
        ...
        const result = await database_1.pool.query(`INSERT INTO student_collections (id, company_id, name, description, color)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING *`, [(0, uuid_1.v4)(), company_id, name, description, color || '#1890ff']);
        return result.rows[0];
    }
    /**
     * 获取企业的收藏夹列表
     */
    async getCollections(companyId) {
        const result = await database_1.pool.query(`SELECT * FROM student_collections
       WHERE company_id = $1
     ...
     * 删除收藏夹
     */
    async deleteCollection(collectionId, companyId) {
        const result = await database_1.pool.query(`DELETE FROM student_collections WHERE id = $1 AND company_id = $2`, [collectionId, companyId]);
        if (result.rowCount === 0) {
            throw new Error('收藏夹不存在');
        }
    }
     ...
     * 将学生添加到收藏夹
     */
    async addStudentToCollection(collectionId, studentId) {
        try {
            await database_1.pool.query(`INSERT INTO collection_students (id, collection_id, student_id)
         VALUES ($1, $2, $3)`, [(0, uuid_1.v4)(), collectionId, studentId]);
        }
        catch (error) {
            if (error.code === '23505') {
     ...
     * 从收藏夹移除学生
     */
    async removeStudentFromCollection(collectionId, studentId) {
        const result = await database_1.pool.query(`DELETE FROM collection_students WHERE collection_id = $1 AND student_id = $2`, [collectionId, studentId]);
        if (result.rowCount === 0) {
     ...
     * 获取收藏夹中的学生
     */
    async getCollectionStudents(collectionId) {
        const result = await database_1.pool.query(`SELECT
         u.id,
         u.username,
         u.avatar,
       ...
       ORDER BY cs.added_at DESC`, [collectionId]);
        return result.rows;
    }
    /**
     * 获取关注统计
     */
    async getFollowStats(companyId) {
        const result = await database_1.pool.query(`SELECT
     ...
     * 获取推荐关注的学生
     */
    async getRecommendedStudents(companyId, limit = 10) {
        // 基于企业历史任务，推荐相似技能的学生
        const result = await database_1.pool.query(`SELECT DISTINCT
         u.id,
         u.username,
         u.avatar,
         u.student_level,
