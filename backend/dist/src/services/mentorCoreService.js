...
exports.MentorCoreService = void 0;
...
**重要提示：** 这是真实案例，你可以简单提及"之前有同学也卡在这里"，但不要直接告诉学生答案。引导学生思考这个案例给他的启发。`;
        ...
        if (task && mentorExampleService_1.default.shouldShowExample(conversationHistory)) {
            ...
            if (similarCase) {
                const formattedCase = mentorExampleService_1.default.formatCaseForDisplay(similarCase);
                basePrompt += `
...
**重要：** 在你的回复中，先简单提及这个案例（1-2句话），然后引导学生思考可以借鉴哪一步。不要直接复述案例内容。`;
                // 记录范例展示
                await mentorExampleService_1.default.recordExampleShown(student.id, task.id, similarCase.order_id, similarCase.similarity_score);
                logger_1.default.info(`[MentorCore] 已注入范例: ${similarCase.order_id}, 相似度=${similarCase.similarity_score}`);
            }
        }
        basePrompt += `
...
请开始你的回复：`;
    ...
    async callClaudeAPI(prompt) {
        try {
            logger_1.default.info('开始调用Claude API', {
            ...
            logger_1.default.info('Claude API调用成功', {
            ...
            if (response.length < 300) {
                logger_1.default.warn('AI回复过短，重新生成', { length: response.length });
                // 可以选择重试或使用备用策略
            }
            return response;
        }
        catch (error) {
            logger_1.default.error('Claude API调用失败:', {
                error: error.message,
                stack: error.stack,
                name: error.name,
                status: error.status,
     ...
     * 检测信号
     */
    detectSignals(studentMessage, mentorResponse) {
     ...
     * 构建上下文
     */
    async buildContext(studentId, taskId, sessionId) {
        // 1. 加载学生信息
    ...
    async loadStudentContext(studentId) {
      ...
      WHERE u.id = $1`, [studentId]);
        ...
        // 加载使用过的工具
        const tools = await (0, db_1.query)(`SELECT DISTINCT jsonb_array_elements_text(milestone_data->'tools_used') as tool
       FROM student_milestones
       WHERE student_id = $1::uuid AND milestone_type = 'new_tool'
       LIMIT 10`, [studentId]);
        // 加载最近的卡点
        const stuckPoints = await (0, db_1.query)(`SELECT content
    ...
    async loadTaskContext(taskId) {
        const task = await (0, db_1.queryOne)(`SELECT id, title, description, track, acceptance_criteria
       FROM tasks
       WHERE id = $1`, [taskId]);
        if (!task)
            return undefined;
    ...
    async loadConversationHistory(sessionId) {
        const messages = await (0, db_1.query)(`SELECT role, content, created_at
       FROM mentor_messages
       WHERE session_id = $1
     ...
     * 统计检测到的信号
     */
    async countDetectedSignals(sessionId) {
        if (!sessionId) {
     ...
     * - >20条：压缩前面的对话，保留最近10条
     */
    async buildContextHistory(conversationHistory) {
...
直接输出摘要，不要前缀：`;
            ...
            logger_1.default.info('对话摘要生成成功', {
            ...
            logger_1.default.error('生成对话摘要失败:', error);
            // 降级：返回简单的统计信息
     ...
     * 获取或创建会话
     */
    async getOrCreateSession(studentId, taskId, sessionId) {
        if (sessionId) {
            const session = await (0, db_1.queryOne)('SELECT id FROM mentor_sessions WHERE id = $1', [sessionId]);
            if (session)
                return session;
        }
        // 创建新会话
        const newSession = await (0, db_1.queryOne)(`INSERT INTO mentor_sessions (student_id, task_id, tenant_id, status)
       VALUES ($1::uuid, $2, $3::uuid, 'active')
       RETURNING id`, [studentId, taskId, '00000000-0000-0000-0000-000000000000'] // 使用默认租户
        );
        return newSession;
    }
    /**
    ...
    async saveMessage(sessionId, role, content, metadata) {
        await (0, db_1.query)(`INSERT INTO mentor_messages (session_id, role, content, tokens_used, detected_signals)
       VALUES ($1, $2, $3, $4, $5)`, [
            sessionId,
            role,
            content,
    ...
    async updateSessionStats(sessionId) {
        await (0, db_1.query)(`UPDATE mentor_sessions
    ...
    async getSessionMessages(sessionId, limit = 50, offset = 0) {
        const messages = await (0, db_1.query)(`SELECT role, content, created_at, detected_signals
       FROM mentor_messages
       WHERE session_id = $1
       ORDER BY created_at ASC
    ...
    async getStudentStats(studentId) {
       ...
       GROUP BY ms.student_id`, [studentId]);
        return {
     ...
     */
    async chatStream(studentId, message, onChunk, taskId, sessionId) {
        const startTime = Date.now();
        try {
            // 1. 加载或创建会话
            const session = await this.getOrCreateSession(studentId, taskId, sessionId);
            ...
            logger_1.default.info('Claude API流式调用成功', {
                model: this.defaultModel,
                inputTokens,
                outputTokens,
            ...
            logger_1.default.error('AI导师流式对话失败:', error);
            throw error;
        }
    }
    /**
     ...
     * 创建学生消息记录（用于API路由）
     */
    async createStudentMessage(studentId, orderId, message) {
        try {
            const { v4: uuidv4 } = require('uuid');
            const sessionId = uuidv4();
            await (0, db_1.query)(`INSERT INTO mentor_sessions (
          id, user_id, order_id, trigger_type, sender_type, message, created_at
        ) VALUES ($1, $2, $3, $4, $5, $6, NOW())`, [sessionId, studentId, orderId, 'user_message', 'student', message]);
            return sessionId;
        }
        catch (error) {
            logger_1.default.error('创建学生消息失败:', error);
            throw error;
        }
    }
    /**
     ...
     */
    async handleStudentMessage(studentId, orderId, message, sessionId) {
        try {
            // 调用chat方法生成回复
            await this.chat(studentId, message, orderId, sessionId);
        }
        catch (error) {
            logger_1.default.error('处理学生消息失败:', error);
            throw error;
        }
    }
    /**
     * 生成提交前自查清单（T-07场景）
     */
    async generatePreSubmitChecklist(studentId, orderId, submissionPreview) {
        try {
            // 获取订单信息
            const order = await (0, db_1.queryOne)(`SELECT o.*, p.title, p.description, p.deliverable_type
        FROM orders o
        ...
        WHERE o.id = $1`, [orderId]);
            ...
            // 获取上次打回原因（如有）
            const lastRevision = await (0, db_1.queryOne)(`SELECT revision_feedback
...
请生成自查清单：`;
            ...
            await (0, db_1.query)(`INSERT INTO mentor_sessions (
          id, user_id, order_id, trigger_type, sender_type, message, created_at
        ) VALUES ($1, $2, $3, $4, $5, $6, NOW())`, [uuidv4(), studentId, orderId, 'pre_submit', 'ai', checklist]);
            return checklist;
        }
        catch (error) {
            logger_1.default.error('生成自查清单失败:', error);
            throw error;
        }
    }
}
exports.MentorCoreService = MentorCoreService;
exports.default = new MentorCoreService();
//# sourceMappingURL=mentorCoreService.js.map