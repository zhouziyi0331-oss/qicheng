...
class MentorService {
     ...
     * ✅ P2安全: 过滤Prompt注入
     */
    sanitizeUserMessage(message, userId) {
        let sanitized = message;
        // 检测注入模式
        for (const pattern of INJECTION_PATTERNS) {
            if (pattern.test(sanitized)) {
                logger_1.default.warn('检测到可能的Prompt注入尝试', {
                    userId,
        ...
        if (sanitized.length > 2000) {
            ...
            logger_1.default.warn('消息长度超限被截断', { userId, originalLength: message.length });
        }
        return sanitized;
    }
    /**
...
现在，请根据学生的问题提供帮助。`;
    ...
    async sendMessage(userId, sessionId, message) {
        ...
        if (this.isAnomalousResponse(aiResponse)) {
            logger_1.default.error('AI返回异常响应，可能存在注入', {
                userId,
                sessionId,
     ...
     */
    async callAI(systemPrompt, userMessage, sessionId) {
