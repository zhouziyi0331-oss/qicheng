...
class MentorAutoTriggerService {
    ...
    async triggerT01(orderId) {
        ...
        try {
         ...
         WHERE ta.id = $1`, [orderId]);
            if (result.rows.length === 0) {
                throw new Error('订单不存在');
            }
            const data = result.rows[0];
            // 获取任务翻译（启程老师的拆解）
            const translationResult = await client.query(`SELECT functional_modules, student_friendly_description, what_you_will_do, what_you_will_learn
         FROM task_translations
         WHERE task_id = $1`, [data.task_id]);
            ...
            logger_1.default.info(`T-01触发成功: 订单${orderId}`);
     ...
     * 引用学生的最后一条消息，给予温和提醒
     */
    async triggerT04(taskId, studentId) {
        const client = await database_1.pool.connect();
        try {
            // 获取学生最后一条消息（真实数据）
            const lastMessage = await mentorContextEnhancer_1.default.getLastStudentMessage(taskId);
            if (!lastMessage) {
                logger_1.default.warn(`T-04: No student message found for task ${taskId}`);
         ...
         JOIN users u ON ta.student_id = u.id
         LEFT JOIN opc_v2_results r ON u.latest_opc_v2_result_id = r.id
         WHERE ta.task_id = $1 AND ta.student_id = $2`, [taskId, studentId]);
            if (result.rows.length === 0) {
                throw new Error('订单不存在');
            }
            const data = result.rows[0];
            ...
            // 保存到导师对话历史
            await this.saveMentorMessage(data.task_id, data.student_id, mentorMessage);
            logger_1.default.info(`T-04触发成功: 任务${taskId}, 学生${studentId}, 间隔${hoursSince}小时`);
            return mentorMessage;
        }
        finally {
            client.release();
     ...
     * 理解企业反馈，翻译成学生能懂的具体指导
     */
    async triggerT03(orderId, rejectionReason) {
        const client = await database_1.pool.connect();
        try {
            // 获取订单、任务、学生信息
            const result = await client.query(`SELECT
         ...
         LIMIT 1`, [orderId]);
            ...
            const data = result.rows[0];
            // 获取历史对话（查找之前的卡点）
            const historyResult = await client.query(`SELECT content, context
         FROM mentor_messages
         WHERE task_id = $1 AND student_id = $2
         ORDER BY created_at DESC
         LIMIT 10`, [data.task_id, data.student_id]);
            ...
            // 保存到导师对话历史
            await this.saveMentorMessage(data.task_id, data.student_id, mentorMessage);
            logger_1.default.info(`T-03触发成功: 订单${orderId}`);
            return mentorMessage;
        }
        finally {
            client.release();
    ...
    async triggerT05(orderId) {
        ...
        try {
         ...
         WHERE ta.id = $1`, [orderId]);
            ...
            const data = result.rows[0];
            // 【T-05核心】获取真实成长对比数据
            const growthComparison = await mentorContextEnhancer_1.default.getGrowthComparison(data.student_id, orderId);
            logger_1.default.info('T-05: Growth comparison loaded', {
                orderId,
            ...
            // 获取完整的对话历史（查找卡点和突破）
            const historyResult = await client.query(`SELECT content, context, created_at
         FROM mentor_messages
         WHERE task_id = $1 AND student_id = $2
         ORDER BY created_at ASC`, [data.task_id, data.student_id]);
            const history = historyResult.rows;
            // 分析历史对话，提取关键时刻
            ...
            // 保存到导师对话历史
            await this.saveMentorMessage(data.task_id, data.student_id, mentorMessage);
            logger_1.default.info(`T-05触发成功: 订单${orderId}`);
            return mentorMessage;
        }
        finally {
            client.release();
     ...
     */
    buildT04Prompt(data, lastMessage, hoursSince) {
        const personalityTags = data.personality_tags?.map((t) => t.name).join('、') || '未知';
...
现在生成轻推消息，只输出消息内容，不要有其他说明。`;
     ...
     * 构建T-01提示词
     */
    buildT01Prompt(data, translation) {
        const personalityTags = data.personality_tags?.map((t) => t.name).join('、') || '未知';
...
现在生成引导消息，只输出消息内容，不要有其他说明。`;
     ...
     * 构建T-03提示词
     */
    buildT03Prompt(data, rejectionReason, history) {
        const personalityTags = data.personality_tags?.map((t) => t.name).join('、') || '未知';
...
现在生成引导消息，只输出消息内容，不要有其他说明。`;
     ...
     */
    buildT05Prompt(data, history, keyMoments, growthComparison) {
        const personalityTags = data.personality_tags?.map((t) => t.name).join('、') || '未知';
...
现在生成见证消息，只输出消息内容，不要有其他说明。`;
    ...
    extractKeyMoments(history) {
        const moments = [];
        // 查找"卡住了"的时刻
        const stuckMessages = history.filter(h => h.context === 'stuck' || h.content.includes('卡住') || h.content.includes('困难'));
        ...
        // 查找突破的时刻
        const breakthroughMessages = history.filter(h => h.content.includes('明白了') || h.content.includes('懂了') || h.content.includes('解决了'));
     ...
     * 保存导师消息到数据库
     */
    async saveMentorMessage(taskId, studentId, message) {
        const client = await database_1.pool.connect();
        try {
            await client.query(`INSERT INTO mentor_messages (task_id, student_id, role, content, context, triggered_by)
         VALUES ($1, $2, $3, $4, $5, $6)`, [taskId, studentId, message.role, message.content, message.context, message.triggeredBy]);
        }
        finally {
            client.release();
        }
...
exports.default = new MentorAutoTriggerService();
//# sourceMappingURL=mentorAutoTriggerService.js.map