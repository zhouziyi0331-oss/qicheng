...
class OrderStatusService {
    ...
    async updateOrderStatus(orderId, newStatus, metadata) {
        try {
            // 1. 获取订单信息
            const order = await (0, db_1.queryOne)(`SELECT id, student_id, task_id, company_id, status
         FROM orders
         WHERE id = $1`, [orderId]);
            if (!order) {
         ...
         WHERE id = $2`, [newStatus, orderId]);
            logger_1.default.info(`Order status updated: ${orderId} ${oldStatus} -> ${newStatus}`);
            // 3. 触发状态变更事件
    ...
    async handleStatusChange(event) {
        ...
        try {
            ...
            websocketService.notifyOrderStatusChange(studentId, orderId, newStatus, this.getStatusMessage(newStatus));
        }
        catch (error) {
            logger_1.default.error('Failed to handle status change:', error);
        }
    }
    /**
     * T-01: 接单后30秒，任务拆解引导
     */
    async scheduleT01Guidance(orderId, studentId, taskId) {
        logger_1.default.info(`Scheduling T-01 guidance for order ${orderId}`);
        // 获取学生画像和项目需求
        const [studentProfile, projectProfile] = await Promise.all([
            (0, db_1.queryOne)(`SELECT profile_text, core_strengths
         FROM student_work_condition_profiles
         WHERE student_id = $1`, [studentId]),
            (0, db_1.queryOne)(`SELECT requirement_text, project_type
         FROM project_requirement_profiles
         WHERE task_id = $1`, [taskId])
        ]);
        // 获取任务详情
        const task = await (0, db_1.queryOne)(`SELECT title, description, deliverable_type
       FROM tasks
       WHERE id = $1`, [taskId]);
        // 30秒后触发
        setTimeout(async () => {
            await (0, aiTaskQueue_1.enqueueAITask)({
                type: aiTaskQueue_1.AITaskType.MENTOR_GUIDANCE,
                orderId,
                studentId,
                scenario: 'T01',
                context: {
     ...
     */
    async scheduleT04Monitoring(orderId, studentId) {
        logger_1.default.info(`Starting T-04 monitoring for order ${orderId}`);
        // TODO: 实现定时检查逻辑
        // 可以使用cron job或定时任务
     ...
     * AI-03: 交付物预审核
     */
    async triggerSubmissionReview(orderId, studentId, metadata) {
        logger_1.default.info(`Triggering submission review for order ${orderId}`);
        // 获取提交物信息
        const submission = await (0, db_1.queryOne)(`SELECT id, submission_content, submission_files
       FROM order_submissions
       WHERE order_id = $1
       ORDER BY created_at DESC
       LIMIT 1`, [orderId]);
        // 获取任务要求
        const task = await (0, db_1.queryOne)(`SELECT title, description, deliverable_type, acceptance_criteria
       FROM tasks t
       JOIN orders o ON t.id = o.task_id
       WHERE o.id = $1`, [orderId]);
        await (0, aiTaskQueue_1.enqueueAITask)({
            type: aiTaskQueue_1.AITaskType.SUBMISSION_REVIEW,
            orderId,
            studentId,
            context: {
     ...
     * T-03: 企业打回，翻译反馈
     */
    async triggerT03Guidance(orderId, studentId, metadata) {
        logger_1.default.info(`Triggering T-03 guidance for order ${orderId}`);
        // 获取企业反馈
        const feedback = metadata?.companyFeedback || '';
        // 获取提交物
        const submission = await (0, db_1.queryOne)(`SELECT submission_content, submission_files
       FROM order_submissions
       WHERE order_id = $1
       ORDER BY created_at DESC
       LIMIT 1`, [orderId]);
        // 获取任务要求
        const task = await (0, db_1.queryOne)(`SELECT title, description, acceptance_criteria
       FROM tasks t
       JOIN orders o ON t.id = o.task_id
       WHERE o.id = $1`, [orderId]);
        await (0, aiTaskQueue_1.enqueueAITask)({
     ...
     */
    async triggerCompletionTasks(orderId, studentId) {
        logger_1.default.info(`Triggering completion tasks for order ${orderId}`);
        // 获取任务完成数据
        const orderData = await (0, db_1.queryOne)(`SELECT o.*, t.title, t.description,
       ...
       WHERE o.id = $1`, [orderId]);
        // 触发AI-04成长报告
        await (0, aiTaskQueue_1.enqueueAITask)({
            type: aiTaskQueue_1.AITaskType.GROWTH_REPORT,
            orderId,
            studentId,
            context: {
                orderData,
     ...
     * 清理订单相关任务
     */
    async cleanupOrderTasks(orderId) {
        logger_1.default.info(`Cleaning up tasks for cancelled order ${orderId}`);
        // TODO: 取消队列中的相关任务
    }
    /**
     * 获取状态变更消息
     ...
     * 记录学生活动
     */
    async recordStudentActivity(orderId) {
        await (0, db_1.query)(`UPDATE orders
       SET last_activity_at = NOW()
       WHERE id = $1`, [orderId]);
    }
}
exports.default = new OrderStatusService();
