...
class TaskBreakdownService {
     ...
     * 拆解任务需求
     */
    async breakdownTask(rawDescription, options = {}) {
        ...
        try {
            ...
            logger_1.default.info('Breaking down task:', { rawDescription: rawDescription.substring(0, 100) });
            ...
            logger_1.default.info('Task breakdown completed', {
            ...
            logger_1.default.error('Error breaking down task:', error);
            // 返回降级结果
     ...
     * 构建AI拆解提示词
     */
    buildBreakdownPrompt(rawDescription, additionalContext) {
...
请返回完整的JSON对象。`;
     ...
     * 创建降级拆解结果（当AI失败时）
     */
    createFallbackBreakdown(rawDescription) {
        return {
            subtasks: [{
                    id: 'st_1',
                    title: '主要任务',
     ...
     * 保存拆解结果到数据库
     */
    async saveBreakdownResult(taskId, rawDescription, result, options = {}) {
        ...
        WHERE id = $8`, [
            ...
            // 插入历史记录
            const historyResult = await client.query(`INSERT INTO task_breakdown_history (
        ...
        RETURNING id`, [
            ...
            logger_1.default.info('Saved breakdown result', { taskId, historyId: historyResult.rows[0].id });
            return historyResult.rows[0].id;
        }
        catch (error) {
            await client.query('ROLLBACK');
            logger_1.default.error('Error saving breakdown result:', error);
            throw error;
        }
        finally {
            client.release();
     ...
     * 获取任务的拆解结果
     */
    async getBreakdownResult(taskId) {
        const client = await db_1.pool.connect();
        try {
            const result = await client.query('SELECT breakdown_result FROM tasks WHERE id = $1', [taskId]);
            if (result.rows.length === 0 || !result.rows[0].breakdown_result) {
                return null;
            }
            return result.rows[0].breakdown_result;
        }
        finally {
            client.release();
        }
     ...
     * 获取拆解历史
     */
    async getBreakdownHistory(taskId, limit = 10) {
        const client = await db_1.pool.connect();
        try {
     ...
     * 用户接受拆解结果
     */
    async acceptBreakdown(historyId, feedback) {
        const client = await db_1.pool.connect();
        try {
            await client.query(`UPDATE task_breakdown_history SET
          user_accepted = true,
          user_feedback = $1
        WHERE id = $2`, [feedback, historyId]);
            logger_1.default.info('User accepted breakdown', { historyId });
        }
        finally {
            client.release();
        }
     ...
     * 用户修改拆解结果
     */
    async modifyBreakdown(historyId, modifiedResult) {
        const client = await db_1.pool.connect();
        try {
            // 获取原始结果
            const originalResult = await client.query('SELECT breakdown_result FROM task_breakdown_history WHERE id = $1', [historyId]);
        ...
        WHERE id = $2`, [JSON.stringify(modified), historyId]);
            logger_1.default.info('User modified breakdown', { historyId });
        }
        finally {
            client.release();
        }
     ...
     * 获取拆解统计
     */
    async getBreakdownStats(days = 30) {
        const client = await db_1.pool.connect();
        try {
            const result = await client.query(`SELECT
...
exports.default = new TaskBreakdownService();
