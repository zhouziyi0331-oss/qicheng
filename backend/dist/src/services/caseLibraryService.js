...
class CaseLibraryService {
     ...
     */
    async extractCasesFromObservations() {
      ...
      `);
        ...
        `, [
            ...
            logger_1.default.info('[CaseLibrary] 提取案例完成', { extractedCount });
    ...
    async searchCases(filter) {
        ...
        try {
          ...
          solution ILIKE $${paramIndex}
            ...
            // 获取总数
            const countResult = await client.query(`SELECT COUNT(*) FROM case_library WHERE ${whereClause}`, params);
     ...
     * 标记案例为有帮助
     */
    async markCaseHelpful(caseId, studentId) {
        const client = await database_1.pool.connect();
        try {
            // 检查是否已经标记过
    ...
    extractSituation(context, stuckDuration) {
        const parts = [];
    ...
    calculateDifficulty(stuckDuration, context) {
        let difficulty = 3; // 默认中等
        if (stuckDuration > 180)
            difficulty = 5;
        else if (stuckDuration > 120)
            difficulty = 4;
    ...
    extractTags(context) {
        const tags = [];
        if (context.task_type)
            tags.push(context.task_type);
        if (context.ability_type)
            tags.push(context.ability_type);
        if (context.skill_name)
            tags.push(context.skill_name);
        if (context.technology)
            tags.push(context.technology);
        if (context.error_type)
            tags.push(context.error_type);
        // 添加通用标签
        if (context.required_debugging)
            tags.push('调试');
        if (context.required_research)
            tags.push('研究');
        if (context.required_collaboration)
            tags.push('协作');
        return [...new Set(tags)]; // 去重
    }
}
exports.default = new CaseLibraryService();
//# sourceMappingURL=caseLibraryService.js.map