...
class MentorScheduler {
     ...
     */
    start() {
        logger_1.logger.info('启动AI导师定时任务调度器');
        // 1. 主动跟进任务（每小时执行一次）
     ...
     */
    stop() {
        logger_1.logger.info('停止AI导师定时任务调度器');
        this.tasks.forEach((task, name) => {
            task.stop();
            logger_1.logger.info('定时任务已停止', { taskName: name });
        });
        this.tasks.clear();
    }
    /**
    ...
    scheduleFollowUps() {
        const task = node_cron_1.default.schedule('0 * * * *', async () => {
            try {
                ...
                logger_1.logger.error('主动跟进任务失败', { error });
            }
        });
        this.tasks.set('followUps', task);
        logger_1.logger.info('主动跟进任务已调度', { schedule: '每小时' });
    }
    /**
     * 清理过期记忆
     * 每天凌晨3点执行
    ...
    scheduleMemoryCleanup() {
        ...
        logger_1.logger.info('记忆清理任务已调度', { schedule: '每天凌晨3点' });
    }
    /**
     * 学习模式分析
     * 每6小时执行一次
    ...
    scheduleLearningPatternAnalysis() {
        const task = node_cron_1.default.schedule('0 */6 * * *', async () => {
            try {
                logger_1.logger.info('开始学习模式分析');
                // 这里可以添加批量分析学习模式的逻辑
                // 例如：分析所有活跃学生的学习模式，更新档案
                logger_1.logger.info('学习模式分析完成');
            }
            catch (error) {
                logger_1.logger.error('学习模式分析失败', { error });
            }
        });
        this.tasks.set('learningPatternAnalysis', task);
        logger_1.logger.info('学习模式分析任务已调度', { schedule: '每6小时' });
    }
    /**
     * 手动触发主动跟进（用于测试）
     */
    async triggerFollowUps() {
        try {
            logger_1.logger.info('手动触发主动跟进任务');
            const result = await proactiveFollowUpService_1.proactiveFollowUpService.executeFollowUps();
            logger_1.logger.info('手动触发完成', result);
            return result;
        }
        catch (error) {
            logger_1.logger.error('手动触发失败', { error });
            throw error;
        }
    }
    /**
