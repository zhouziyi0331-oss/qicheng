...
exports.contactExchangeService = void 0;
...
class ContactExchangeService {
     ...
     * 在任务完成时自动调用
     */
    async checkAndPromptExchange(studentId, companyId, taskId) {
        try {
            // 1. 查询合作次数
            const count = await this.getCollaborationCount(studentId, companyId);
            logger_1.default.info(`Collaboration count: ${count} for student ${studentId} and company ${companyId}`);
            // 2. 检查是否已经交换过
     ...
     * 检查是否已经交换过
     */
    async hasExchanged(studentId, companyId) {
        const sql = `
      SELECT exchanged
     ...
     * 获取现有的交换请求
     */
    async getExistingRequest(studentId, companyId) {
        const sql = `
      SELECT *
      FROM contact_exchange_requests
      WHERE student_id = $1 AND company_id = $2
     ...
     * 创建交换请求
     */
    async createExchangeRequest(studentId, companyId, taskId, collaborationCount) {
        const sql = `
      ...
      RETURNING id
    `;
        const result = await (0, db_1.queryOne)(sql, [studentId, companyId, taskId, collaborationCount]);
        return result?.id;
    }
    /**
     * 询问双方是否愿意交换联系方式
     */
    async promptBothSides(studentId, companyId, taskId) {
        // 获取合作历史统计
        ...
        logger_1.default.info(`Sent exchange prompts to student ${studentId} and company ${companyId}`);
    }
    /**
     * 获取合作统计
     */
    async getCollaborationStats(studentId, companyId, perspective) {
    ...
    `;
        const results = await (0, db_1.query)(sql, [studentId, companyId]);
     ...
     * 生成询问消息
     */
    generatePromptMessage(role, otherPartyName, stats) {
...
【同意】 【暂不需要】`;
     ...
     * 发送询问（集成通知系统）
     */
    async sendPrompt(userId, message, taskId) {
        // TODO: 集成通知系统（WebSocket、推送、站内信等）
        logger_1.default.info(`Sending exchange prompt to user ${userId}: ${message}`);
        // 这里可以保存到系统消息表
     ...
     * 用户同意交换联系方式
     */
    async agreeToExchange(userId, studentId, companyId) {
        try {
            // 1. 获取用户角色
     ...
     * 执行联系方式交换
     */
    async executeExchange(studentId, companyId) {
        try {
            // 1. 获取双方联系方式
        ...
        WHERE student_id = $1 AND company_id = $2
      `;
            await (0, db_1.query)(sql, [studentId, companyId]);
            logger_1.default.info(`Successfully exchanged contacts between ${studentId} and ${companyId}`);
        }
        catch (error) {
            logger_1.default.error('Failed to execute exchange:', error);
            throw error;
        }
    }
    /**
    ...
    async getContactInfo(userId) {
      ...
      WHERE id = $1
    `;
        const user = await (0, db_1.queryOne)(sql, [userId]);
        if (!user) {
     ...
     * 推送联系方式
     */
    async pushContactInfo(toUserId, contactInfo, contactType) {
        const message = this.generateContactMessage(contactInfo, contactType);
        // TODO: 集成通知系统
        logger_1.default.info(`Pushing contact info to user ${toUserId}: ${message}`);
        // 这里可以保存到系统消息表
     ...
     * 生成联系方式推送消息
     */
    generateContactMessage(contactInfo, contactType) {
...
- 这样有问题时平台可以帮忙协调`;
     ...
     * 获取交换请求状态
     */
    async getExchangeStatus(studentId, companyId) {
        const sql = `
      SELECT *
      FROM contact_exchange_requests
      WHERE student_id = $1 AND company_id = $2
...
// 导出单例
// =====================================================
exports.contactExchangeService = new ContactExchangeService();
//# sourceMappingURL=contactExchangeService.js.map