...
class CommunityService {
    ...
    async createPost(params) {
        try {
            ...
            if (params.type === 'recruit') {
                const author = await (0, db_1.queryOne)(`SELECT current_level FROM users WHERE id = $1`, [params.authorId]);
        ...
        RETURNING id`, [
            ...
            logger_1.default.info(`Community post created: ${post.id} by ${params.authorId}, type: ${params.type}`);
            return post.id;
        }
        catch (error) {
    ...
    async getPosts(options = {}) {
        try {
            const { type, track, status = 'open', limit = 20, offset = 0, } = options;
            ...
            // 获取帖子列表
            const posts = await (0, db_1.query)(`SELECT
          cp.*,
         ...
         LIMIT $${paramIndex} OFFSET $${paramIndex + 1}`, [...params, limit, offset]);
            ...
            logger_1.default.error('Failed to get community posts:', error);
            throw error;
        }
    }
    /**
        ...
        try {
         ...
         WHERE cp.id = $1`, [postId]);
           ...
           ORDER BY
            ...
            logger_1.default.error('Failed to get post detail:', error);
            return null;
        }
    }
    /**
     * 申请加入（招募帖）
     */
    async applyToPost(postId, applicantId, message, skillsOffered) {
        try {
            // 获取帖子信息
            const post = await (0, db_1.queryOne)(`SELECT type, team_id, status, vacancy_count, total_applicants
         ...
         WHERE id = $1`, [postId]);
            ...
            // 检查是否已申请
            const existing = await (0, db_1.queryOne)(`SELECT id FROM community_post_applications WHERE post_id = $1 AND applicant_id = $2`, [postId, applicantId]);
            if (existing) {
                throw new Error('你已经申请过此招募');
            }
            // 创建申请记录
            await (0, db_1.query)(`INSERT INTO community_post_applications (
          post_id, applicant_id, team_id, message, skills_offered, status
        ) VALUES ($1, $2, $3, $4, $5, 'pending')`, [postId, applicantId, post.team_id, message || null, JSON.stringify(skillsOffered || [])]);
            // 更新帖子申请人数
            await (0, db_1.query)(`UPDATE community_posts SET total_applicants = total_applicants + 1 WHERE id = $1`, [postId]);
            logger_1.default.info(`User ${applicantId} applied to post ${postId}`);
        }
        catch (error) {
            logger_1.default.error('Failed to apply to post:', error);
            throw error;
        }
    }
    /**
     * 审核申请
     */
    async reviewApplication(postId, authorId, applicantId, approved) {
        try {
            // 验证帖子作者
            const post = await (0, db_1.queryOne)(`SELECT author_id, team_id FROM community_posts WHERE id = $1`, [postId]);
            if (!post) {
                throw new Error('帖子不存在');
            }
            ...
            if (approved) {
                // 通过申请
                await (0, db_1.query)(`UPDATE community_post_applications
           SET status = 'approved', reviewed_at = NOW()
           WHERE post_id = $1 AND applicant_id = $2`, [postId, applicantId]);
                // 如果关联了队伍，自动添加到队伍
                if (post.team_id) {
                ...
                logger_1.default.info(`Application approved: ${applicantId} for post ${postId}`);
            }
            else {
                // 拒绝申请
                await (0, db_1.query)(`UPDATE community_post_applications
           ...
           WHERE post_id = $1 AND applicant_id = $2`, [postId, applicantId]);
                logger_1.default.info(`Application rejected: ${applicantId} for post ${postId}`);
            }
        }
        catch (error) {
            logger_1.default.error('Failed to review application:', error);
            throw error;
        }
    }
    /**
     * 获取帖子的申请列表
     */
    async getPostApplications(postId, authorId) {
        try {
            // 验证帖子作者
            ...
            // 获取申请列表
            const applications = await (0, db_1.query)(`SELECT
          cpa.*,
     ...
     * 关闭帖子
     */
    async closePost(postId, authorId) {
        try {
            // 验证帖子作者
            const post = await (0, db_1.queryOne)(`SELECT author_id FROM community_posts WHERE id = $1`, [postId]);
            if (!post) {
            ...
            // 关闭帖子
            await (0, db_1.query)(`UPDATE community_posts SET status = 'closed', updated_at = NOW() WHERE id = $1`, [postId]);
            logger_1.default.info(`Post ${postId} closed by ${authorId}`);
        }
        catch (error) {
            logger_1.default.error('Failed to close post:', error);
            throw error;
        }
    }
    /**
    ...
    async deletePost(postId, authorId) {
        try {
            ...
            logger_1.default.info(`Post ${postId} deleted by ${authorId}`);
        }
        catch (error) {
    ...
    async getUserPosts(userId, limit = 20, offset = 0) {
        try {
         ...
         LIMIT $2 OFFSET $3`, [userId, limit, offset]);
            ...
            logger_1.default.error('Failed to get user posts:', error);
            throw error;
        }
    }
    /**
