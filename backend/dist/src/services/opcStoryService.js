...
class OpcStoryService {
    ...
    async createStory(params) {
      ...
      `, [
          ...
          `, [storyId, tag]);
            ...
            logger_1.default.info('[OpcStory] 创建故事', { storyId, studentId: params.studentId });
            return {
                success: true,
                storyId,
                message: '故事已提交，等待审核'
    ...
    async searchStories(filter) {
        ...
        try {
          ...
          to_tsvector('chinese', title || ' ' || story_content) @@ plainto_tsquery('chinese', $${paramIndex})
            ...
            // 获取总数
            const countResult = await client.query(`SELECT COUNT(*) FROM opc_stories WHERE ${whereClause}`, params);
     ...
     * 获取故事详情
     */
    async getStoryById(storyId, viewerId) {
        const client = await database_1.pool.connect();
        try {
            // 增加浏览次数
            await client.query(`
     ...
     * 推荐相似故事（基于OPC类型和情绪标签）
     */
    async recommendSimilarStories(storyId, limit = 5) {
        const client = await database_1.pool.connect();
        try {
            // 获取当前故事信息
...
exports.default = new OpcStoryService();
//# sourceMappingURL=opcStoryService.js.map