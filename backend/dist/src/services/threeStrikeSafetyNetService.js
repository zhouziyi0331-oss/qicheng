...
class ThreeStrikeSafetyNetService {
     ...
     * 检查是否触发三次审核兜底
     */
    async checkThreeStrikeTrigger(taskId, studentId) {
        const submissions = await (0, db_1.query)(`SELECT version, ai_review_score
       FROM task_submissions
       WHERE task_id = $1
       ORDER BY version DESC`, [taskId]);
        // 检查是否已经提交了3次且都未通过
        if (submissions.length >= 3) {
           ...
           WHERE task_id = $1 AND version = 3`, [taskId]);
                logger_1.default.warn('Three strike safety net triggered', {
                    taskId,
                    studentId,
    ...
    async getTransferCandidates(taskId, currentStudentId) {
       ...
       LIMIT 10`, [currentStudentId, task.track, task.level_required]);
        return candidates;
    }
    /**
     * 执行转单
    ...
    async transferTask(options) {
        ...
        await (0, db_1.withTransaction)(async (client) => {
            ...
            await client.query(`UPDATE task_assignments
         SET status = 'transferred',
             transfer_to = $2,
             transfer_reason = $3,
             transferred_at = NOW()
         WHERE task_id = $1 AND student_id = $4`, [taskId, toStudentId, reason, fromStudentId]);
            // 3. 创建新的任务分配给接包学生
            await client.query(`INSERT INTO task_assignments (
           task_id, student_id, status, match_score, match_reason, assigned_at
         ) VALUES ($1, $2, 'accepted', 0, '转单接包', NOW())`, [taskId, toStudentId]);
            // 4. 记录转单收入（原学生）
            await client.query(`INSERT INTO income_records (
           user_id, task_id, amount, type, status, description
         ) VALUES ($1, $2, $3, 'transfer_fee', 'pending', '转单费用（20%）')`, [fromStudentId, taskId, transferFee]);
            // 5. 更新任务状态
            await client.query(`UPDATE tasks
         SET status = 'in_progress',
             accepted_student_id = $2,
             student_price = $3
         WHERE id = $1`, [taskId, toStudentId, receiverIncome]);
            logger_1.default.info('Task transferred successfully', {
                taskId,
                fromStudentId,
                toStudentId,
                transferFee,
    ...
    async summonMaster(options) {
        ...
        await (0, db_1.withTransaction)(async (client) => {
         ...
         WHERE task_id = $1 AND student_id = $4`, [taskId, masterId, masterFee, studentId]);
            // 3. 创建导师会话（大师模式）
            await client.query(`INSERT INTO mentor_stage_sessions (
           student_id, task_id, current_stage, created_at
         ) VALUES ($1, $2, 'T-02', NOW())`, [studentId, taskId]);
            // 4. 发送通知给大师
           ...
           user_id, type, title, body, link_url
            ...
            await client.query(`INSERT INTO income_records (
           user_id, task_id, amount, type, status, description
         ) VALUES ($1, $2, $3, 'master_fee', 'frozen', '大师指导费用（冻结）')`, [studentId, taskId, -masterFee]);
            logger_1.default.info('Master summoned successfully', {
                taskId,
                studentId,
                masterId,
                masterFee,
     ...
     * 获取三次审核兜底状态
     */
    async getThreeStrikeStatus(taskId, studentId) {
        const submissions = await (0, db_1.query)(`SELECT version, ai_review_score, is_final_fail, created_at
       FROM task_submissions
       WHERE task_id = $1
       ORDER BY version DESC`, [taskId]);
        const assignment = await (0, db_1.queryOne)(`SELECT status, transfer_to, master_id
...
exports.default = new ThreeStrikeSafetyNetService();
//# sourceMappingURL=threeStrikeSafetyNetService.js.map