...
class StudentProfileEnhancer {
     ...
     * 生成增强的学生档案
     */
    async generateEnhancedProfile(studentId) {
        try {
            logger_1.default.info(`Generating enhanced profile for student: ${studentId}`);
            // 1. 获取学生基础信息
            ...
            logger_1.default.error('Error generating enhanced profile:', error);
            throw error;
        }
    }
    /**
    ...
    async getStudentBasicInfo(studentId) {
        ...
        try {
            const result = await client.query('SELECT id, name, avatar, level, created_at FROM users WHERE id = $1', [studentId]);
     ...
     * 获取任务历史
     */
    async getTaskHistory(studentId) {
        const client = await db_1.pool.connect();
        try {
            const result = await client.query(`SELECT t.title, t.budget, ts.quality_score, ts.completed_at
     ...
     * 使用AI生成成长故事和亮点
     */
    async generateGrowthStory(student, capability, taskHistory) {
        try {
...
以JSON格式返回：
{
  ...
  "investment_highlights": ["亮点1", "亮点2", "亮点3"]
            ...
            logger_1.default.error('Error generating growth story:', error);
            // 返回降级内容
            return {
     ...
     * 提取关键里程碑
     */
    async extractMilestones(studentId, taskHistory) {
        const milestones = [];
        // TODO: 从数据库获取学生的重要事件
        // 这里使用模拟数据
     ...
     * 生成技能雷达图数据
     */
    generateSkillRadar(capability) {
        const skills = capability.skills || {};
        const skillArray = Object.entries(skills).map(([skill, data]) => ({
            skill,
     ...
     * 生成特色标签
     */
    generateTags(student, capability) {
        const tags = [];
        // 等级标签
        if (student.level >= 5) {
            tags.push('高级学生');
     ...
     * 批量生成增强档案
     */
    async batchGenerateProfiles(studentIds) {
        ...
        for (const studentId of studentIds) {
            try {
                ...
                logger_1.default.error(`Failed to generate profile for ${studentId}:`, error);
            }
        }
        return profiles;
    }
}
exports.default = new StudentProfileEnhancer();
//# sourceMappingURL=studentProfileEnhancer.js.map