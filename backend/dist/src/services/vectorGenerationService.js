...
exports.VectorGenerationService = void 0;
...
class VectorGenerationService {
     ...
     */
    async generateTaskVector(taskDescription, requirements) {
        logger_1.default.info('[Vector] Generating task vector from features...');
        // 1. 提取技能向量 (64维)
        ...
        logger_1.default.info(`[Vector] Task vector generated: ${vector.length} dimensions`);
        return vector;
    }
    /**
     * 生成学生向量 (完全基于特征工程)
     ...
     */
    async generateStudentVector(studentId) {
        logger_1.default.info(`[Vector] Generating student vector for ${studentId}...`);
      ...
      `, [studentId]);
            ...
            logger_1.default.info(`[Vector] Student vector generated: ${vector.length} dimensions`);
            return vector;
        }
        finally {
            client.release();
     ...
     * 提取技能向量 (64维)
     */
    extractSkillVector(description, requirements) {
     ...
     * 计算难度分数
     */
    calculateDifficultyScore(description, requirements) {
        const text = (description + JSON.stringify(requirements)).toLowerCase();
        let score = 0.5;
     ...
     * 计算复杂度分数
     */
    calculateComplexityScore(description, requirements) {
        let score = 0;
     ...
     * 估算时间
     */
    estimateTime(description, requirements) {
        let hours = 4;
        const text = description + JSON.stringify(requirements);
        if (text.includes('快速') || text.includes('1小时'))
            hours = 2;
        if (text.includes('一天') || text.includes('周末'))
            hours = 8;
        if (text.includes('一周') || text.includes('长期'))
            hours = 20;
        const stepCount = this.countSteps(requirements);
        hours += stepCount * 0.5;
     ...
     * 提取交付物类型向量 (10维)
     */
    extractDeliverableType(description, requirements) {
        const types = [
            '文案', '视频', '图像', '音频', '代码',
            '报告', '方案', '模型', '数据', '其他'
        ];
     ...
     * 计算技能熟练度向量 (64维)
     */
    calculateSkillProficiencyVector(enrollments) {
        const vector = new Array(64).fill(0);
        let index = 0;
        for (const [category, skills] of Object.entries(SKILL_CATEGORIES)) {
            for (const skill of skills) {
     ...
     * 计算领域经验向量 (7维)
     */
    calculateDomainExperienceVector(enrollments) {
        const vector = new Array(7).fill(0);
        const domains = Object.keys(DOMAIN_KEYWORDS);
        for (let i = 0; i < domains.length; i++) {
            const keywords = DOMAIN_KEYWORDS[domains[i]];
     ...
     * 计算学习速度
     */
    calculateLearningSpeed(enrollments) {
        ...
        for (const enrollment of completed) {
            if (enrollment.completed_at && enrollment.enrolled_at) {
                const days = Math.ceil((new Date(enrollment.completed_at).getTime() - new Date(enrollment.enrolled_at).getTime())
                    / (1000 * 60 * 60 * 24));
    ...
    countTools(description, requirements) {
        const text = description + JSON.stringify(requirements);
        let count = 0;
        for (const skills of Object.values(SKILL_CATEGORIES)) {
            for (const skill of skills) {
    ...
    async updateTaskEmbedding(taskId) {
        const client = await db_1.pool.connect();
        try {
            const result = await client.query('SELECT id, scenario, stages FROM atomic_courses WHERE id = $1', [taskId]);
            if (result.rows.length === 0) {
                throw new Error(`Task ${taskId} not found`);
            }
            const task = result.rows[0];
            await this.generateTaskVector(task.scenario, task.stages);
            logger_1.default.info(`[Vector] Task embedding updated: ${taskId}`);
        }
        finally {
            client.release();
        }
    ...
    async updateStudentEmbedding(studentId) {
        ...
        logger_1.default.info(`[Vector] Student embedding updated: ${studentId}`);
    }
}
exports.VectorGenerationService = VectorGenerationService;
