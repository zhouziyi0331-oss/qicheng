...
exports.checkImageSecurity = checkImageSecurity;
exports.checkTextSecurity = checkTextSecurity;
exports.clearWechatTokenCache = clearWechatTokenCache;
...
const tokenCache = new node_cache_1.default({ stdTTL: 7000 });
...
async function getWechatAccessToken() {
    // 1. 先从缓存读取
    const cachedToken = tokenCache.get('wechat_access_token');
    if (cachedToken) {
...
async function checkImageSecurity(imageBuffer) {
    try {
        ...
        const response = await axios_1.default.post(`https://api.weixin.qq.com/wxa/img_sec_check?access_token=${accessToken}`, formData, {
 ...
 * @returns 检查结果
 */
async function checkTextSecurity(content, openid, scene = 2) {
    try {
        // 空内容直接通过
        ...
        const accessToken = await getWechatAccessToken();
        const response = await axios_1.default.post(`https://api.weixin.qq.com/wxa/msg_sec_check?access_token=${accessToken}`, {
            version: 2,
            openid: openid,
            scene: scene,
 ...
 */
function clearWechatTokenCache() {
