...
exports.RequirementBreakdownService = void 0;
const db_1 = require("../utils/db");
const logger_1 = __importDefault(require("../utils/logger"));
class RequirementBreakdownService {
    /**
     * 为任务创建需求拆解
     */
    static async createBreakdown(taskId, breakdown) {
        const client = await db_1.pool.connect();
        try {
            await client.query('BEGIN');
            logger_1.default.info(`[RequirementBreakdown] 为任务 ${taskId} 创建需求拆解`);
            // 递归插入节点
            for (const node of breakdown) {
                await this.insertNodeRecursive(client, taskId, node, null);
            }
            await client.query('COMMIT');
            logger_1.default.info(`[RequirementBreakdown] 拆解创建成功`);
        }
        catch (error) {
            await client.query('ROLLBACK');
            logger_1.default.error('[RequirementBreakdown] 创建失败:', error);
            throw error;
        }
        finally {
            client.release();
     ...
     * 递归插入节点
     */
    static async insertNodeRecursive(client, taskId, node, parentId) {
        // 插入当前节点
        const result = await client.query(`INSERT INTO task_requirement_breakdown 
     ...
     * 获取任务的完整拆解树
     */
    static async getBreakdownTree(taskId) {
        const client = await db_1.pool.connect();
        try {
            // 获取所有节点
            const result = await client.query(`SELECT id, level, parent_id, requirement_name, requirement_description,
                sequence_order, dependencies, estimated_hours, difficulty_level,
                required_capabilities, is_mandatory, can_be_learned
         FROM task_requirement_breakdown
         WHERE task_id = $1
         ORDER BY level, sequence_order`, [taskId]);
     ...
     * 为单个子需求匹配学生
     */
    static async matchStudentsForRequirement(taskId, requirementId, topN = 10) {
        const client = await db_1.pool.connect();
        try {
            // 获取需求详情
            const reqResult = await client.query(`SELECT requirement_name, required_capabilities, can_be_learned
         FROM task_requirement_breakdown
         WHERE id = $1`, [requirementId]);
            ...
            // 获取所有学生
            const studentsResult = await client.query(`SELECT DISTINCT u.id as student_id
         FROM users u
         WHERE u.role = 'student'`);
            const matches = [];
            for (const student of studentsResult.rows) {
                const matchResult = await this.calculateRequirementMatch(client, student.student_id, requiredCaps, requirement.can_be_learned);
                matches.push({
     ...
     * 计算学生与单个需求的匹配度
     */
    static async calculateRequirementMatch(client, studentId, requiredCapabilities, canBeLearned) {
        const matched = {
            talents: [],
            tools: [],
            domainKnowledge: [],
        ...
        if (requiredCapabilities.talents && requiredCapabilities.talents.length > 0) {
            const talentsResult = await client.query(`SELECT tt.tag_name, stt.strength, stt.confidence
         FROM student_talent_tags stt
         JOIN talent_tags tt ON stt.tag_id = tt.id
         WHERE stt.student_id = $1 AND tt.tag_name = ANY($2)`, [studentId, requiredCapabilities.talents]);
            const studentTalents = talentsResult.rows.map((r) => r.tag_name);
        ...
        if (requiredCapabilities.tools && requiredCapabilities.tools.length > 0) {
            const toolsResult = await client.query(`SELECT tool_name, proficiency_level
         FROM student_tool_usage
         WHERE student_id = $1 AND tool_name = ANY($2)`, [studentId, requiredCapabilities.tools]);
            const studentTools = toolsResult.rows.map((r) => r.tool_name);
        ...
        if (requiredCapabilities.domainKnowledge && requiredCapabilities.domainKnowledge.length > 0) {
            const domainResult = await client.query(`SELECT domain_aspect, understanding_level
         FROM student_domain_understanding
         WHERE student_id = $1 AND domain_aspect = ANY($2)`, [studentId, requiredCapabilities.domainKnowledge]);
            const studentDomains = domainResult.rows.map((r) => r.domain_aspect);
        ...
        if (requiredCapabilities.caseExperience && requiredCapabilities.caseExperience.length > 0) {
            const caseResult = await client.query(`SELECT case_type, experience_count
         FROM student_case_experience
         WHERE student_id = $1 AND case_type = ANY($2)`, [studentId, requiredCapabilities.caseExperience]);
            const studentCases = caseResult.rows.map((r) => r.case_type);
     ...
     */
    static createEcommerceCustomerServiceAgentBreakdown() {
        return [
            {
                level: 1,
...
exports.RequirementBreakdownService = RequirementBreakdownService;
//# sourceMappingURL=requirementBreakdownService.js.map