...
class MessageRelayService {
    ...
    async sendMessage(params) {
        ...
        try {
            ...
            logger_1.default.info(`Collaboration count between ${fromUserId} and ${toUserId}: ${collabCount}`);
            // 2. 检测和屏蔽联系方式
     ...
     * 获取合作次数
     */
    async getCollaborationCount(userId1, userId2) {
        // 判断谁是学生，谁是企业
     ...
     * 检测和屏蔽联系方式
     */
    filterContactInfo(message, collabCount) {
        // 如果已经合作3次及以上，不屏蔽
        if (collabCount >= 3) {
            return {
     ...
     * 优化语气（调用AI）
     */
    async optimizeTone(message, fromUserId, toUserId) {
        try {
...
只返回优化后的消息，不要解释。`;
            ...
            logger_1.default.error('Failed to optimize tone:', error);
            // 如果AI调用失败，返回原消息
            return {
    ...
    async saveMessage(params) {
      ...
      RETURNING id
    `;
        const result = await (0, db_1.queryOne)(sql, [
            params.taskId,
            params.fromUserId,
            params.toUserId,
     ...
     * 保存屏蔽日志
     */
    async saveFilterLogs(messageId, userId, detectedContacts, collabCount) {
        const sql = `
     ...
     * 保存语气优化日志
     */
    async saveToneOptimizationLog(params) {
        const sql = `
     ...
     */
    async notifyRecipient(userId, messageId) {
        // TODO: 集成WebSocket或推送服务
        logger_1.default.info(`Notifying user ${userId} about new message ${messageId}`);
    }
    /**
     * 提醒发送者关于屏蔽
     */
    async notifySenderAboutFilter(userId, collabCount) {
...
我可以帮你们交换联系方式 :)`;
        // TODO: 发送系统消息给用户
        logger_1.default.info(`Notifying user ${userId} about contact filter`);
        return message;
    }
    /**
     * 提醒发送者关于语气优化
     */
    async notifySenderAboutOptimization(userId) {
...
- 学生会更愿意配合`;
        // TODO: 发送系统消息给用户
        logger_1.default.info(`Notifying user ${userId} about tone optimization`);
    }
    /**
     * 获取用户信息
     */
     ...
     */
    async relayMessage(fromUserId, toUserId, taskId, content) {
     ...
     * 获取消息历史（带权限验证）
     */
    async getMessages(taskId, userId, limit = 50, offset = 0) {
        // 验证用户是否有权限查看该任务的消息
        const taskSql = `
      ...
      WHERE id = $1
    `;
        const task = await (0, db_1.queryOne)(taskSql, [taskId]);
        if (!task) {
      ...
      LIMIT $2 OFFSET $3
    `;
        const messages = await (0, db_1.query)(sql, [taskId, limit, offset]);
        return messages;
    }
    /**
     * 获取统计数据（平台端）
    ...
    async getStatistics(params) {
      ...
      ${whereClause}
    `;
        const stats = await (0, db_1.queryOne)(sql, queryParams);
        // 获取屏蔽统计
    ...
    `;
        const filterStats = await (0, db_1.query)(filterStatsSql, queryParams);
        return {
            ...stats,
     ...
     * 获取违规记录（平台端）
     */
    async getViolations(userId, limit = 20, offset = 0) {
        let whereClauses = [];
        let queryParams = [];
        let paramIndex = 1;
        if (userId) {
    ...
    `;
        const violations = await (0, db_1.query)(sql, queryParams);
        return violations;
    }
    /**
     * 标记消息为已读
    ...
    `;
        await (0, db_1.query)(sql, [messageId, userId]);
    }
}
// =====================================================
// 导出单例
// =====================================================
exports.messageRelayService = new MessageRelayService();
//# sourceMappingURL=messageRelayService.js.map