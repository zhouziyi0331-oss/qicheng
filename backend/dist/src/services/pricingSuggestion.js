...
const getPricingSuggestion = async (req, res) => {
    try {
        ...
        // 1. 检查缓存（相似任务的定价建议）
        const cachedSuggestion = await (0, db_1.query)(`SELECT * FROM pricing_suggestions
       WHERE task_description = $1 AND task_level = $2 AND task_track = $3
       AND expires_at > NOW()
       ORDER BY created_at DESC
       LIMIT 1`, [description, level, track]);
        ...
        // 2. 查询历史相似任务的平均价格
        const historicalData = await (0, db_1.query)(`SELECT
         AVG(budget) as avg_budget,
         MIN(budget) as min_budget,
         MAX(budget) as max_budget,
         COUNT(*) as task_count
       FROM tasks
       WHERE level = $1 AND track = $2 AND status = 'completed'`, [level, track]);
...
请以JSON格式返回：
{
  ...
  "reasoning": "定价理由说明"
        ...
        // 4. 保存到缓存
        const savedSuggestion = await (0, db_1.query)(`INSERT INTO pricing_suggestions
 ...
 * 规则引擎定价（降级方案）
 */
function getRuleBasedPricing(level, track, marketData) {
    // 基础价格矩阵
    const basePrices = {
...
//# sourceMappingURL=pricingSuggestion.js.map