...
exports.triggerTaskMatching = triggerTaskMatching;
 ...
 * 只返回学生选择的赛道的任务
 */
async function getRecommendedTasksForStudent(studentId, limit = 20) {
    try {
        // 1. 获取学生的赛道选择
        const studentResult = await db_1.pool.query('SELECT selected_track FROM users WHERE id = $1', [studentId]);
        ...
        if (!selectedTrack) {
            ...
            logger_1.default.warn('Student has not selected track yet', { studentId });
            return [];
        }
        // 2. 查询该赛道的任务，并进行匹配
        const tasksResult = await db_1.pool.query(`SELECT
        t.id,
        t.title,
        t.description,
        t.track,
       ...
       ORDER BY
         ...
         t.created_at DESC
       LIMIT $3`, [studentId, selectedTrack, limit]);
        return tasksResult.rows;
    }
    catch (error) {
        logger_1.default.error('Get recommended tasks failed', { error, studentId });
        throw error;
    }
}
/**
 ...
 * 只匹配符合任务赛道的学生
 */
async function triggerTaskMatching(taskId) {
    try {
        // 1. 获取任务信息，包括赛道
        const taskResult = await db_1.pool.query('SELECT id, track, level_required FROM tasks WHERE id = $1', [taskId]);
        if (taskResult.rows.length === 0) {
            throw new Error('Task not found');
        }
        const task = taskResult.rows[0];
        // 2. 查询选择了该赛道的学生
        const studentsResult = await db_1.pool.query(`SELECT
        u.id,
        u.selected_track,
        sc.skills,
        sc.tasks_completed,
       ...
       LIMIT 100`, [task.track]);
        if (studentsResult.rows.length === 0) {
            logger_1.default.warn('No students found for track', { taskId, track: task.track });
            return 0;
        }
        // 3. 为每个学生计算匹配分数
        let matchedCount = 0;
        ...
        logger_1.default.info('Task matching completed', { taskId, matchedCount });
        return matchedCount;
    }
    catch (error) {
        logger_1.default.error('Trigger task matching failed', { error, taskId });
        throw error;
    }
}
/**
 * 计算匹配分数（简化版）
 */
function calculateMatchScore(task, student) {
    // 这里是简化的匹配算法
    // 实际应该调用完整的6维度匹配引擎
    ...
    const overallScore = (skillMatchScore * 0.3 +
...
//# sourceMappingURL=trackAwareMatchingEngine.js.map