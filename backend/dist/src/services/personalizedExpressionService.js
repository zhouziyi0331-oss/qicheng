...
const reasoningEngine_1 = __importDefault(require("./reasoningEngine"));
...
class PersonalizedExpressionService {
    ...
    async generateResponse(context) {
        try {
            logger_1.default.info(`Generating personalized response for student ${context.studentId}`);
            // 第一步：深度思考
            const thinking = await reasoningEngine_1.default.think({
            ...
            // 第二步：获取长期记忆
            const memory = await (0, db_1.queryOne)(`SELECT * FROM teacher_long_term_memory WHERE student_id = $1`, [context.studentId]);
            // 第三步：基于思考结果生成个性化表达
            ...
            logger_1.default.info(`Generated personalized response for student ${context.studentId}`);
            return {
                thinking,
                response,
            ...
            logger_1.default.error('Failed to generate personalized response:', error);
            throw error;
        }
    }
    /**
     * 基于思考结果生成表达
     */
    async express(thinking, memory, context) {
        try {
...
直接输出你的回复（100-150字），不要其他解释：`;
            ...
            logger_1.default.error('Failed to express:', error);
            // 返回基于洞察的简单回复
     ...
     * 保存到短期记忆
     */
    async saveToShortTermMemory(studentId, context, response) {
        try {
            await (0, db_1.queryOne)(`INSERT INTO teacher_short_term_memory (
          student_id, context, student_state, teacher_response
            ...
            logger_1.default.error('Failed to save to short term memory:', error);
        }
    }
    /**
     * 快速回复（不需要深度思考的场景）
     */
    async quickResponse(studentId, situation, responseType) {
        try {
...
给学生一句简短的回应（30字以内）：`;
            ...
            logger_1.default.error('Failed to generate quick response:', error);
            return '收到，继续加油！';
        }
    }
}
exports.default = new PersonalizedExpressionService();
//# sourceMappingURL=personalizedExpressionService.js.map