...
class BoleService {
     ...
     * 创建伯乐推荐
     */
    async createDiscovery(data) {
        const { discoverer_id, student_id, discovery_reason, recommended_skills, potential_rating } = data;
        // 检查是否已经推荐过
        const existing = await database_1.pool.query(`SELECT id FROM talent_discoveries WHERE discoverer_id = $1 AND student_id = $2`, [discoverer_id, student_id]);
        if (existing.rows.length > 0) {
            throw new Error('已经推荐过该学生');
        }
        // 获取学生当前状态
        const student = await database_1.pool.query(`SELECT student_level,
              (SELECT COUNT(*) FROM tasks WHERE student_id = $1 AND status = 'completed') as tasks_completed
       FROM users WHERE id = $1`, [student_id]);
        ...
        const result = await database_1.pool.query(`INSERT INTO talent_discoveries
     ...
     * 获取企业的推荐列表
     */
    async getCompanyDiscoveries(companyId) {
        const result = await database_1.pool.query(`SELECT td.*,
       ...
       ORDER BY td.created_at DESC`, [companyId]);
        return result.rows;
    }
    /**
     * 获取学生被推荐记录
     */
    async getStudentDiscoveries(studentId) {
        const result = await database_1.pool.query(`SELECT td.*,
       ...
       ORDER BY td.created_at DESC`, [studentId]);
        return result.rows;
    }
    /**
     * 获取推荐详情
     */
    async getDiscoveryById(discoveryId) {
        const result = await database_1.pool.query(`SELECT td.*,
     ...
     * 获取伯乐标签
     */
    async getBoleBadges(companyId) {
        const result = await database_1.pool.query(`SELECT * FROM bole_badges
     ...
     * 检查并授予伯乐标签
     */
    async checkAndAwardBadge(companyId) {
        // 获取企业推荐统计
       ...
       WHERE discoverer_id = $1`, [companyId]);
        ...
        for (const badge of badgeRequirements) {
            if (parseInt(total_discoveries, 10) >= badge.discoveries &&
                parseInt(validated_discoveries, 10) >= badge.validated) {
                // 检查是否已有该标签
                const existingBadge = await database_1.pool.query(`SELECT id FROM bole_badges WHERE company_id = $1 AND badge_level = $2`, [companyId, badge.level]);
                if (existingBadge.rows.length === 0) {
                    // 授予新标签
                    const result = await database_1.pool.query(`INSERT INTO bole_badges
             ...
             RETURNING *`, [
                    ...
                    // 更新用户表
                    await database_1.pool.query(`UPDATE users SET bole_badge_level = $1 WHERE id = $2`, [badge.level, companyId]);
                    return result.rows[0];
                }
                return null; // 已有该标签
            }
    ...
    async getLeaderboard(month) {
        ...
        const result = await database_1.pool.query(`SELECT bl.*,
              u.company_name,
              u.avatar,
              u.bole_badge_level
       ...
       LIMIT 50`, [targetMonth]);
        return result.rows;
    }
    /**
     * 获取企业伯乐统计
     */
    async getCompanyBoleStats(companyId) {
       ...
       GROUP BY u.id`, [companyId]);
        ...
        const rankResult = await database_1.pool.query(`SELECT rank FROM bole_leaderboard WHERE company_id = $1 AND month = $2`, [companyId, currentMonth]);
        return {
            ...stats,
    ...
    async getRewardConfig() {
        const result = await database_1.pool.query(`SELECT * FROM bole_reward_config WHERE is_active = true ORDER BY created_at DESC LIMIT 1`);
        if (result.rows.length === 0) {
            // 返回默认配置
            return {
     ...
     * 手动验证推荐（管理员）
     */
    async validateDiscovery(discoveryId, adminId) {
        const result = await database_1.pool.query(`UPDATE talent_discoveries
       SET is_validated = true,
           validated_at = NOW(),
           status = 'validated',
           reward_status = 'approved'
       WHERE id = $1
       RETURNING *`, [discoveryId]);
        if (result.rows.length === 0) {
            throw new Error('推荐记录不存在');
        }
        // 检查并授予标签
    ...
    async createGrowthSnapshot() {
        await database_1.pool.query(`SELECT create_student_growth_snapshot()`);
    }
    /**
     * 更新伯乐排行榜
     */
    async updateLeaderboard() {
        await database_1.pool.query(`SELECT update_bole_leaderboard()`);
    }
    /**
     * 获取学生成长轨迹
     */
    async getStudentGrowthTrack(studentId, months = 12) {
        const result = await database_1.pool.query(`SELECT * FROM student_growth_tracking
       WHERE student_id = $1
       ORDER BY snapshot_month DESC
       LIMIT $2`, [studentId, months]);
        return result.rows;
    }
    /**
     * 推荐候选学生（AI推荐）
...
exports.default = new BoleService();
//# sourceMappingURL=boleService.js.map