...
class HumanizedConversationService {
     ...
     */
    async generateHumanizedResponse(studentId, taskId, sessionId, studentMessage, conversationHistory, currentEmotion) {
        try {
            // 1. 检测深层模式
            const patternDetection = await deepGuidanceService_1.deepGuidanceService.detectDeepPattern(studentId, studentMessage, conversationHistory, currentEmotion);
            // 2. 如果检测到深层模式，使用深层引导
            if (patternDetection.detected && patternDetection.pattern) {
                const deepGuidance = await deepGuidanceService_1.deepGuidanceService.generateDeepGuidance(studentId, studentMessage, conversationHistory, patternDetection, currentEmotion);
            ...
            const toolRecommendations = await this.analyzeAndRecommendTools(taskId, struggle, studentId);
            const systemPrompt = this.buildHumanizedSystemPrompt(humanizedContext, struggle, phrases, memories, toolRecommendations, currentEmotion);
     ...
     * 构建人性化的系统提示
     */
    buildHumanizedSystemPrompt(humanizedContext, struggle, phrases, memories, toolRecommendations, currentEmotion) {
...
**具体记忆**:
${memories.relevantMemories.map((m) => `- ${m.memory_title}: ${m.memory_content}`).join('\n')}
...
`;
            if (humanizedContext.important_moments && humanizedContext.important_moments.length > 0) {
                prompt += `**重要时刻**：
${humanizedContext.important_moments.map((m) => `- ${m.moment}: ${m.detail} (${m.date})`).join('\n')}

`;
            }
            if (humanizedContext.shared_experiences && humanizedContext.shared_experiences.length > 0) {
                prompt += `**我们一起经历的**：
${humanizedContext.shared_experiences.map((e) => `- ${e}`).join('\n')}

`;
            }
            if (humanizedContext.memorable_quotes && humanizedContext.memorable_quotes.length > 0) {
                prompt += `**学生说过的话**（可以引用）：
${humanizedContext.memorable_quotes.map((q) => `- "${q.quote}" (${q.context})`).join('\n')}
...
`;
     ...
     * 分析学生的具体困难
     */
    async analyzeSpecificStruggle(studentMessage, conversationHistory, currentEmotion) {
        // 检测困难信号词
...
返回JSON：
{
  ...
  "situation": "当前情境"
     ...
     * 分析任务并推荐工具
     */
    async analyzeAndRecommendTools(taskId, struggle, studentId) {
        try {
            // 获取任务信息
            const taskResult = await database_1.pool.query('SELECT title, description, requirements FROM tasks WHERE id = $1', [taskId]);
            ...
            // 获取推荐的工具
            const toolsResult = await database_1.pool.query(`SELECT * FROM mentor_tool_recommendations
         WHERE suitable_for @> $1::jsonb
         ORDER BY times_recommended DESC, success_rate DESC NULLS LAST
         LIMIT 3`, [JSON.stringify({ task_type: [taskType] })]);
     ...
     * 获取合适的对话片段
     */
    async getAppropriatePhrase(emotion, situation) {
        try {
            const query = `
     ...
     * 获取人性化上下文
     */
    async getHumanizedContext(sessionId, studentId) {
        try {
            const result = await database_1.pool.query('SELECT * FROM mentor_humanized_context WHERE session_id = $1', [sessionId]);
            if (result.rows.length === 0) {
                // 创建新的上下文
                await database_1.pool.query(`INSERT INTO mentor_humanized_context (session_id, student_id)
           VALUES ($1, $2)`, [sessionId, studentId]);
                return {
     ...
     * 更新人性化上下文
     */
    async updateHumanizedContext(sessionId, studentId, studentMessage, mentorResponse, struggle) {
        try {
            // 检测是否有值得记住的话
     ...
     * 检测重要时刻
     */
    detectImportantMoment(studentMessage, mentorResponse, struggle) {
     ...
     * 检测共同经历
     */
    detectSharedExperience(studentMessage, mentorResponse) {
     ...
     * 记录具体困难
     */
    async recordSpecificStruggle(studentId, taskId, sessionId, struggle, toolRecommendations) {
        try {
            const toolId = toolRecommendations.length > 0
                ? await this.getToolId(toolRecommendations[0].toolName)
                : null;
            await database_1.pool.query(`INSERT INTO mentor_student_specific_struggles
     ...
     * 获取工具ID
     */
    async getToolId(toolName) {
        try {
            const result = await database_1.pool.query('SELECT id FROM mentor_tool_recommendations WHERE tool_name = $1', [toolName]);
     ...
     * 生成跟进话题
     */
    async generateFollowUpTopics(studentMessage, struggle) {
        const topics = [];
        if (struggle) {
     ...
     */
    detectTone(content) {
        if (content.includes('哇') || content.includes('太棒了'))
            return 'excited';
        if (content.includes('嗯嗯') || content.includes('我懂'))
            return 'understanding';
        if (content.includes('别担心') || content.includes('没关系'))
            return 'comforting';
        return 'supportive';
    }
    /**
...
exports.humanizedConversationService = new HumanizedConversationService();
//# sourceMappingURL=humanizedConversationService.js.map