...
class OPCV2AssessmentService {
     ...
     * 开始新测试
     */
    static async startAssessment(studentId) {
        // 检查是否有进行中的测试
        const existingResult = await db_1.pool.query(`SELECT id FROM opc_v2_assessments
       WHERE student_id = $1 AND status = 'in_progress'`, [studentId]);
        if (existingResult.rows.length > 0) {
            // 返回现有测试
            return await this.getAssessmentProgress(existingResult.rows[0].id);
        }
        // 创建新测试会话
        const assessmentResult = await db_1.pool.query(`INSERT INTO opc_v2_assessments (student_id, status, current_question, total_questions)
       VALUES ($1, 'in_progress', 1, 38)
       RETURNING *`, [studentId]);
        // 获取所有题目
        const questionsResult = await db_1.pool.query(`SELECT id, question_number, question_type, question_text, prompt_text,
    ...
    static async submitAnswer(assessmentId, questionId, answer) {
        // 获取题目信息
        const questionResult = await db_1.pool.query(`SELECT question_number, question_type, dimension, options
       FROM opc_v2_questions WHERE id = $1`, [questionId]);
        ...
        if (question.question_type === 'definition') {
           ...
           WHERE id = $2`, [answer.selfDefinedIdentity || [], assessmentId]);
            }
            else if (question.question_number === 2) {
                // 题目2: 自我定义的"厉害"
                await db_1.pool.query(`UPDATE opc_v2_assessments
           SET self_defined_awesome = $1
           WHERE id = $2`, [answer.selfDefinedAwesome || '', assessmentId]);
            }
            // 保存答案记录
            await db_1.pool.query(`INSERT INTO opc_v2_answers (assessment_id, question_id, answer_text)
         VALUES ($1, $2, $3)
         ON CONFLICT (assessment_id, question_id) DO UPDATE
         SET answer_text = EXCLUDED.answer_text`, [assessmentId, questionId, answer.answerText || '']);
        }
        else {
            // 处理选择题
            ...
            // 查找选项的计分信息
            const optionData = options.find((opt) => opt.label === selectedOption);
            if (!optionData) {
                throw new Error('无效的选项');
            }
            const scoring = optionData.scoring;
            // 保存答案记录
            await db_1.pool.query(`INSERT INTO opc_v2_answers
    ...
    static async completeAssessment(assessmentId) {
        // 获取所有答案
        const answersResult = await db_1.pool.query(`SELECT dimension, score_value, score_direction
       FROM opc_v2_answers
       WHERE assessment_id = $1 AND dimension IS NOT NULL`, [assessmentId]);
        // 计算六个维度的原始分
        ...
        // 获取学生ID
        const assessmentInfo = await db_1.pool.query(`SELECT student_id FROM opc_v2_assessments WHERE id = $1`, [assessmentId]);
        // 保存结果
        const savedResult = await db_1.pool.query(`INSERT INTO opc_v2_results (
     ...
     * 判定人格标签
     */
    static async determinePersonalityLabel(scores) {
        // 获取所有标签定义
        const labelsResult = await db_1.pool.query(`SELECT * FROM opc_v2_personality_labels
       WHERE is_active = TRUE
       ORDER BY priority ASC`);
        ...
        // 默认返回混合型
        const mixedLabel = labelsResult.rows.find((l) => l.label_name === '混合型');
        return mixedLabel || labelsResult.rows[labelsResult.rows.length - 1];
    }
    /**
     * 生成维度描述
     */
    static generateDimensionDescriptions(scores) {
        const descriptions = {};
        // 信息处理
     ...
     * 获取测试进度
     */
    static async getAssessmentProgress(assessmentId) {
        const assessmentResult = await db_1.pool.query(`SELECT * FROM opc_v2_assessments WHERE id = $1`, [assessmentId]);
        if (assessmentResult.rows.length === 0) {
            throw new Error('测试不存在');
        }
        const assessment = assessmentResult.rows[0];
        // 获取已答题目
        const answersResult = await db_1.pool.query(`SELECT question_id FROM opc_v2_answers WHERE assessment_id = $1`, [assessmentId]);
        const answeredQuestionIds = answersResult.rows.map((r) => r.question_id);
        return {
            assessment,
            answeredQuestionIds
        };
    ...
    static async getAssessmentResult(assessmentId) {
        const result = await db_1.pool.query(`SELECT * FROM opc_v2_results WHERE assessment_id = $1`, [assessmentId]);
     ...
     * 获取学生的最新测试结果
     */
    static async getLatestResult(studentId) {
        const result = await db_1.pool.query(`SELECT r.* FROM opc_v2_results r
...
exports.OPCV2AssessmentService = OPCV2AssessmentService;
//# sourceMappingURL=opcV2AssessmentService.js.map