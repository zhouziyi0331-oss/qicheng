...
exports.updateSixDimScores = updateSixDimScores;
...
async function updateSixDimScores(userId, taskId, companyScore, taskType, taskDifficulty) {
    try {
        ...
        // 获取任务详情
        const taskInfo = await (0, db_1.query)(`SELECT title, description, track_type, level_required FROM tasks WHERE id = $1`, [taskId]);
...
`;
        ...
        if (process.env.NODE_ENV === 'development' || !process.env.ANTHROPIC_API_KEY) {
            changes = calculateScoreChangesRuleBased(companyScore, task.track_type, taskDifficulty);
        }
        else {
            try {
                ...
                logger_1.default.error('AI score update failed, using rule-based', { error: err.message });
                changes = calculateScoreChangesRuleBased(companyScore, task.track_type, taskDifficulty);
            }
        }
        // 应用变化（限制在0-100范围内）
        ...
        logger_1.default.error('Update six dim scores error', { error: err.message });
    }
}
/**
 * 基于规则的分数变化计算（降级方案）
 */
function calculateScoreChangesRuleBased(companyScore, trackType, difficulty) {
...
//# sourceMappingURL=sixDimUpdater.js.map