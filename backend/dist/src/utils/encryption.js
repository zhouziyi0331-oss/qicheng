...
exports.encryptOpenid = encryptOpenid;
exports.decryptOpenid = decryptOpenid;
 ...
 */
// 从环境变量读取加密密钥（32字节hex）
const ENCRYPTION_KEY = Buffer.from(config_1.config.encryption?.key || process.env.ENCRYPTION_KEY_DEFAULT || '', 'hex');
...
function encryptPhone(phone) {
    ...
    // 生成SHA256哈希用于索引查找
    const hash = crypto_1.default.createHash('sha256').update(phone).digest('hex');
...
function encryptOpenid(openid) {
    ...
    encrypted += cipher.final('hex');
    const authTag = cipher.getAuthTag();
    const hash = crypto_1.default.createHash('sha256').update(openid).digest('hex');
    return {
        encrypted: `${iv.toString('hex')}:${encrypted}:${authTag.toString('hex')}`,
        hash
    };
...
function decryptOpenid(encrypted) {
    ...
    try {
        ...
        throw new Error(`Failed to decrypt openid: ${error.message}`);
    }
}
/**
 * 生成字符串的SHA256哈希（用于索引）
