 ...
 */
async function reviewTaskSubmission(taskTitle, taskDescription, acceptanceCriteria, submissionNote, fileUrls) {
    // 开发模式或未配置API Key：使用规则引擎
    if (process.env.NODE_ENV === 'development' || !process.env.ANTHROPIC_API_KEY) {
        return reviewSubmissionRuleBased(taskDescription, acceptanceCriteria, submissionNote, fileUrls);
    }
    try {
        const prompt = `
...
`;
        ...
        logger_1.default.error('AI review failed, using rule-based', { error: err.message });
        return reviewSubmissionRuleBased(taskDescription, acceptanceCriteria, submissionNote, fileUrls);
    }
}
/**
 * 基于规则的验收检测（降级方案）
 */
function reviewSubmissionRuleBased(taskDescription, acceptanceCriteria, submissionNote, fileUrls) {
    const issues = [];
    const highlights = [];
    let score = 80;
    // 检查提交说明长度
    ...
    const matchedKeywords = keywords.filter(kw => submissionNote.toLowerCase().includes(kw.toLowerCase()));
...
//# sourceMappingURL=smartReview.js.map