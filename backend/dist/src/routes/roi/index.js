...
router.get('/dashboard', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const dashboard = await roiAnalyticsService_1.default.getROIDashboard(companyId, period || 'monthly');
        res.json({
            success: true,
            data: dashboard,
        });
...
router.get('/financial-stats', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const stats = await roiAnalyticsService_1.default.getFinancialStats(companyId, targetYear, targetMonth);
        res.json({
            success: true,
            data: stats,
        });
...
router.get('/historical-stats', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const stats = await roiAnalyticsService_1.default.getHistoricalStats(companyId, months ? parseInt(months, 10) : 6);
        res.json({
            success: true,
            data: {
                stats,
...
router.post('/cost-comparison', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const analysis = await roiAnalyticsService_1.default.createCostComparisonAnalysis(companyId, period, new Date(startDate), new Date(endDate));
        res.json({
            success: true,
            data: analysis,
            message: '成本对比分析创建成功',
...
router.post('/refresh-stats', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        await roiAnalyticsService_1.default.refreshFinancialStats(companyId, year, month);
        res.json({
            success: true,
            message: '财务统计已刷新',
        });
