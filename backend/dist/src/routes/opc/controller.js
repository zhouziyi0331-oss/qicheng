...
exports.getOpcReport = getOpcReport;
...
async function getTestResult(req, res, next) {
    try {
        ...
        // 查询最新的测试结果
        const result = await (0, db_1.queryOne)(`SELECT id, test_version,
       ...
       LIMIT 1`, [userId]);
        if (!result) {
            throw new errorHandler_1.AppError(404, '未找到测试结果，请先完成OPC测试', 'TEST_NOT_COMPLETED');
        }
        // 构建六维得分对象
        const sixDimensions = {
...
// 生成或获取用户的OPC详细报告
// ============================================================
async function getOpcReport(req, res, next) {
    try {
        const { userId } = req.params;
        const requesterId = req.user.userId;
        // 权限检查：只能查看自己的报告，或管理员可以查看所有报告
        if (requesterId !== userId && req.user.role !== 'admin') {
            throw new errorHandler_1.AppError(403, '无权查看该用户的报告', 'FORBIDDEN');
        }
        // 查询测试结果
        const testResult = await (0, db_1.queryOne)(`SELECT id, test_version, personality_tag, personality_description,
       ...
       LIMIT 1`, [userId]);
        if (!testResult) {
            throw new errorHandler_1.AppError(404, '该用户尚未完成OPC测试', 'TEST_NOT_FOUND');
        }
        // 查询用户基本信息
...
async function getTestQuestions(req, res, next) {
    try {
        const questions = await (0, db_1.query)(`SELECT question_number, dimension, question_text,
              option_a, option_b, option_c, option_d
       FROM opc_test_questions
       ORDER BY question_number ASC`);
        if (!questions || questions.length === 0) {
            throw new errorHandler_1.AppError(404, '测试题目未初始化', 'QUESTIONS_NOT_FOUND');
        }
        res.json({
            success: true,
            data: {
...
async function submitTest(req, res, next) {
    try {
        ...
        if (!answers || !Array.isArray(answers) || answers.length !== 36) {
            throw new errorHandler_1.AppError(400, '答案格式错误，需要36道题的答案', 'INVALID_ANSWERS');
        }
        // 获取所有题目及其评分规则
        const questions = await (0, db_1.query)('SELECT question_number, dimension, score_a, score_b, score_c, score_d FROM opc_test_questions ORDER BY question_number');
        // 计算六维原始得分
        ...
        const answersWithScores = answers.map((answer) => {
            ...
            if (!question) {
                throw new errorHandler_1.AppError(400, `题目 ${answer.questionNumber} 不存在`, 'INVALID_QUESTION');
            }
            const answerUpper = answer.answer.toUpperCase();
            if (!['A', 'B', 'C', 'D'].includes(answerUpper)) {
                throw new errorHandler_1.AppError(400, `题目 ${answer.questionNumber} 答案无效`, 'INVALID_ANSWER');
            }
            const scoreKey = `score_${answerUpper.toLowerCase()}`;
        ...
        // 保存测试结果
        const result = await (0, db_1.queryOne)(`INSERT INTO user_opc_results (
       ...
       WHERE id = $2`, [personalityTag, userId]);
        res.json({
            success: true,
            data: {
                testId: result?.id,
...
function generatePersonalityTag(scores) {
    const tags = [
        { tag: '视觉叙事者', condition: (s) => s.creation_drive > 70 && s.information_processing > 60 },
        { tag: '系统构建者', condition: (s) => s.task_execution > 70 && s.tool_learning > 60 },
        { tag: '快速学习者', condition: (s) => s.tool_learning > 75 },
        { tag: '协作推动者', condition: (s) => s.collaboration > 70 },
        { tag: '冒险探索者', condition: (s) => s.risk_attitude > 70 },
        { tag: '稳健执行者', condition: (s) => s.task_execution > 70 && s.risk_attitude < 50 },
    ];
...
// 辅助函数：生成人格描述
// ============================================================
function generatePersonalityDescription(tag, scores) {
    const descriptions = {
...
// 辅助函数：生成维度解读
// ============================================================
function generateDimensionInterpretations(scores) {
    return {
        information_processing: {
...
// 辅助函数：生成推荐信息
// ============================================================
function generateRecommendations(scores, tag) {
    const trackMap = {
        视觉叙事者: '设计与创意',
        系统构建者: '技术开发',
        快速学习者: '全栈开发',
