...
async function verifyCapability(req, res, next) {
    try {
        ...
        const taskResult = await database_1.default.query(`SELECT t.id, t.title, t.required_skills, t.difficulty_level, t.estimated_hours
       FROM tasks t
       WHERE t.id = $1`, [taskId]);
        ...
        const studentSkillsResult = await database_1.default.query(`SELECT skill_name, skill_level, case_count
       FROM student_skills
       WHERE student_id = $1`, [studentId]);
        const studentSkills = new Map(studentSkillsResult.rows.map(row => [row.skill_name, { level: row.skill_level, cases: row.case_count }]));
        const matchedSkills = [];
        const weakSkills = [];
        ...
        const studentHistoryResult = await database_1.default.query(`SELECT COUNT(*) as total_completed,
...
//# sourceMappingURL=capability-verify.js.map