...
exports.getPeerStats = getPeerStats;
const database_1 = __importDefault(require("../../config/database"));
async function getPeerStats(req, res, next) {
    try {
        const studentId = req.user?.id;
        const profileResult = await database_1.default.query('SELECT personality_label FROM student_profiles WHERE student_id = $1', [studentId]);
        if (profileResult.rows.length === 0) {
            res.status(404).json({ success: false, message: 'Profile not found' });
            return;
        }
        const personalityLabel = profileResult.rows[0].personality_label || '创作者';
        const peerStatsResult = await database_1.default.query(`SELECT
       ...
       WHERE sp.personality_label = $1`, [personalityLabel]);
        ...
        const myStatsResult = await database_1.default.query(`SELECT
       ...
       WHERE student_id = $1 AND status = 'completed'`, [studentId]);
        ...
        const qualityPercentileResult = await database_1.default.query(`WITH peer_quality AS (
      ...
      FROM peer_quality`, [personalityLabel, myQuality]);
        const speedPercentileResult = await database_1.default.query(`WITH peer_speed AS (
      ...
      FROM peer_speed`, [personalityLabel, mySpeed]);
