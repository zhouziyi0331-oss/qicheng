...
exports.isFirstOrder = isFirstOrder;
exports.getPaymentStatus = getPaymentStatus;
exports.getAvailableSkipTests = getAvailableSkipTests;
exports.applySkipTest = applySkipTest;
exports.submitSkipTest = submitSkipTest;
exports.getSkipTestResult = getSkipTestResult;
exports.getGrowthComparison = getGrowthComparison;
exports.getAssetDashboard = getAssetDashboard;
exports.generateIdentityCard = generateIdentityCard;
const database_1 = __importDefault(require("../../config/database"));
async function getProfile(req, res, next) {
    try {
        const studentId = req.user?.id;
        const result = await database_1.default.query('SELECT * FROM student_profiles WHERE student_id = $1', [studentId]);
...
async function updateProfile(req, res, next) {
    try {
        ...
        const result = await database_1.default.query(`UPDATE student_profiles
       SET bio = COALESCE($1, bio),
           avatar_url = COALESCE($2, avatar_url),
           updated_at = NOW()
       WHERE student_id = $3
       RETURNING *`, [updates.bio, updates.avatarUrl, studentId]);
        res.json({ success: true, data: result.rows[0] });
    }
    catch (err) {
        next(err);
...
async function submitTest(req, res, next) {
    try {
        ...
        const score = 85;
        await database_1.default.query('UPDATE student_profiles SET test_completed = TRUE, test_score = $1 WHERE student_id = $2', [score, studentId]);
...
async function getTestResult(req, res, next) {
    try {
        const studentId = req.user?.id;
        const result = await database_1.default.query('SELECT test_score FROM student_profiles WHERE student_id = $1', [studentId]);
...
async function getOnboardingStatus(req, res, next) {
    try {
        const studentId = req.user?.id;
        const result = await database_1.default.query('SELECT onboarding_step FROM student_profiles WHERE student_id = $1', [studentId]);
        res.json({
            success: true,
            data: {
...
async function completeOnboardingStep(req, res, next) {
    try {
        ...
        await database_1.default.query('UPDATE student_profiles SET onboarding_step = $1 WHERE student_id = $2', [parseInt(step), studentId]);
...
async function getEmotionSignals(req, res, next) {
    try {
        const studentId = req.user?.id;
        const result = await database_1.default.query(`SELECT emotion_type, intensity, detected_at, context
...
async function getLevel(req, res, next) {
    try {
        const studentId = req.user?.id;
        const result = await database_1.default.query('SELECT level, experience_points FROM student_profiles WHERE student_id = $1', [studentId]);
...
async function checkLevelUpgrade(req, res, next) {
    try {
        const studentId = req.user?.id;
        const result = await database_1.default.query('SELECT level, experience_points FROM student_profiles WHERE student_id = $1', [studentId]);
        const level = result.rows[0]?.level || 1;
        ...
        if (canUpgrade) {
            await database_1.default.query('UPDATE student_profiles SET level = level + 1, experience_points = $1 WHERE student_id = $2', [experiencePoints - nextLevelXp, studentId]);
        }
...
async function getNextLevel(req, res, next) {
    try {
        const studentId = req.user?.id;
        const result = await database_1.default.query('SELECT level FROM student_profiles WHERE student_id = $1', [studentId]);
...
async function isFirstOrder(req, res, next) {
    try {
        const studentId = req.user?.id;
        const result = await database_1.default.query(`SELECT COUNT(*) as count
       FROM task_assignments
       WHERE student_id = $1 AND status IN ('in_progress', 'completed')`, [studentId]);
...
async function getPaymentStatus(req, res, next) {
    try {
        ...
        const orderResult = await database_1.default.query(`SELECT total_amount, status
       FROM task_assignments
       WHERE id = $1 AND student_id = $2`, [orderId, studentId]);
        if (orderResult.rows.length === 0) {
            res.status(404).json({ success: false, message: 'Order not found' });
            return;
        }
        const totalAmount = parseFloat(orderResult.rows[0].total_amount);
        const milestonesResult = await database_1.default.query(`SELECT stage, percentage, status, completed_at
       FROM task_milestones
       WHERE task_assignment_id = $1
       ORDER BY stage`, [orderId]);
        let milestones;
...
async function getAvailableSkipTests(req, res, next) {
    try {
        const studentId = req.user?.id;
        const profileResult = await database_1.default.query('SELECT level FROM student_profiles WHERE student_id = $1', [studentId]);
        const currentLevel = profileResult.rows[0]?.level || 1;
        const attemptsResult = await database_1.default.query(`SELECT target_level, attempt_count, tasks_completed_since_failure, can_retry_after
...
async function applySkipTest(req, res, next) {
    try {
        const studentId = req.user?.id;
        const { targetLevel } = req.body;
        const questions = generateSkipTestQuestions(targetLevel);
        const result = await database_1.default.query(`INSERT INTO level_skip_tests (student_id, target_level, test_type, questions, taken_at)
       VALUES ($1, $2, $3, $4, NOW())
       RETURNING id`, [studentId, targetLevel, 'skip', JSON.stringify(questions)]);
...
function generateSkipTestQuestions(targetLevel) {
    if (targetLevel === 2) {
        return [
...
async function submitSkipTest(req, res, next) {
    try {
        const studentId = req.user?.id;
        const { testId, answers } = req.body;
        const testResult = await database_1.default.query('SELECT target_level, questions FROM level_skip_tests WHERE id = $1 AND student_id = $2', [testId, studentId]);
        ...
        await database_1.default.query(`UPDATE level_skip_tests
       SET answers = $1, score = $2, passed = $3, result_at = NOW()
       WHERE id = $4`, [JSON.stringify(answers), score, passed, testId]);
        if (passed) {
            await database_1.default.query('UPDATE student_profiles SET level = $1 WHERE student_id = $2', [targetLevel, studentId]);
        }
        else {
            await database_1.default.query(`INSERT INTO level_skip_attempts (student_id, target_level, attempt_count, last_failed_at, tasks_completed_since_failure)
         VALUES ($1, $2, 1, NOW(), 0)
         ON CONFLICT (student_id, target_level)
         DO UPDATE SET
           attempt_count = level_skip_attempts.attempt_count + 1,
           last_failed_at = NOW(),
           tasks_completed_since_failure = 0`, [studentId, targetLevel]);
        }
        res.json({
            success: true,
            data: {
...
async function getSkipTestResult(req, res, next) {
    try {
        const studentId = req.user?.id;
        const { testId } = req.params;
        const result = await database_1.default.query(`SELECT target_level, score, passed, result_at
       FROM level_skip_tests
       WHERE id = $1 AND student_id = $2`, [testId, studentId]);
        if (result.rows.length === 0) {
...
async function getGrowthComparison(req, res, next) {
    try {
       ...
       WHERE student_id = $1 AND status = 'completed'`, [studentId]);
        ...
        const firstOrderResult = await database_1.default.query(`SELECT
       ...
       LIMIT 1`, [studentId]);
        const currentOrderResult = await database_1.default.query(`SELECT
        EXTRACT(DAY FROM (completed_at - started_at)) as days,
        stuck_count,
        ai_help_count,
        client_rating
...
async function getAssetDashboard(req, res, next) {
    try {
        const studentId = req.user?.id;
        const skillsResult = await database_1.default.query(`SELECT skill_name, skill_level, hourly_rate
       FROM student_skills
       WHERE student_id = $1
       ORDER BY skill_level DESC`, [studentId]);
        ...
        const experiencesResult = await database_1.default.query(`SELECT domain, case_count, avg_rating
       FROM student_experiences
       WHERE student_id = $1
       ORDER BY case_count DESC`, [studentId]);
        const experiences = experiencesResult.rows.map(row => ({
            domain: row.domain,
            caseCount: row.case_count,
            rating: parseFloat(row.avg_rating)
        }));
        const incomeResult = await database_1.default.query(`SELECT
       ...
       WHERE student_id = $1 AND status = 'completed'`, [studentId]);
        ...
        const percentileResult = await database_1.default.query(`WITH student_value AS (
      ...
      FROM student_value`, [marketValue]);
...
async function generateIdentityCard(req, res, next) {
    try {
        const studentId = req.user?.id;
        const profileResult = await database_1.default.query(`SELECT u.name, sp.level, sp.personality_label
       FROM users u
       ...
       WHERE ta.student_id = $1 AND ta.status = 'completed'`, [studentId]);
