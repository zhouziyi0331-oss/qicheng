...
exports.getChallengeHistory = getChallengeHistory;
...
async function startChallenge(req, res, next) {
    try {
        ...
        if (targetLevel > 5) {
            throw new errorHandler_1.AppError(400, '目标等级不能超过5级', 'INVALID_TARGET_LEVEL');
        }
        // 3. 检查是否有未完成的挑战
       ...
       ORDER BY created_at DESC LIMIT 1`, [userId]);
        ...
        // 6. 创建挑战记录
        const challengeId = await (0, db_1.queryOne)(`INSERT INTO level_challenge_tests
       (user_id, current_level, target_level, track, questions_json, answers_json)
       VALUES ($1, $2, $3, $4, $5, '{}')
       RETURNING id`, [userId, currentLevel, targetLevel, profile.track, JSON.stringify(questions)]);
        logger_1.default.info('Challenge test started', { userId, currentLevel, targetLevel });
        res.json({
            success: true,
            data: {
...
async function submitChallenge(req, res, next) {
    try {
        ...
        // 1. 获取挑战记录
        const challenge = await (0, db_1.queryOne)(`SELECT * FROM level_challenge_tests
       WHERE id = $1 AND user_id = $2 AND deleted_at IS NULL`, [challengeId, userId]);
        if (!challenge) {
            throw new errorHandler_1.AppError(404, '挑战记录不存在', 'CHALLENGE_NOT_FOUND');
        }
        if (challenge.is_passed !== null) {
            throw new errorHandler_1.AppError(400, '该挑战已经完成', 'CHALLENGE_COMPLETED');
        }
        // 2. 调用AI评分
        const aiResult = await evaluateChallengeAnswers(challenge.questions_json, answers, challenge.current_level, challenge.target_level);
        const isPassed = aiResult.score >= 80; // 默认80分通过
        // 3. 更新挑战记录
        await (0, db_1.withTransaction)(async (client) => {
            await client.query(`UPDATE level_challenge_tests
         ...
         WHERE id = $8`, [
           ...
           VALUES ($1, 'level_up', '跳级成功', $2, $3, $4)`, [
                ...
                await client.query(`INSERT INTO six_dim_history
...
// 辅助函数：生成挑战题目（调用AI）
// ============================================================
async function generateChallengeQuestions(currentLevel, targetLevel, track) {
    // TODO: 调用AI服务生成10道题目
    // 这里先返回模拟数据
    const questions = [];
    for (let i = 1; i <= 10; i++) {
...
// 辅助函数：AI评分
// ============================================================
async function evaluateChallengeAnswers(questions, answers, currentLevel, targetLevel) {
    // TODO: 调用AI服务进行评分
    // 这里先返回模拟结果
