...
async function getQuestions(req, res, next) {
    try {
        ...
        const questions = await (0, db_1.query)(sql, params);
...
async function submitAnswer(req, res, next) {
    try {
        ...
        // 验证问题是否存在
        const question = await (0, db_1.queryOne)(`SELECT id FROM life_questions WHERE id = $1 AND deleted_at IS NULL`, [questionId]);
        if (!question) {
            throw new errorHandler_1.AppError(404, '问题不存在', 'QUESTION_NOT_FOUND');
        }
        // 保存答案
        const result = await (0, db_1.query)(`INSERT INTO life_question_answers
       (user_id, question_id, answer_text, emotional_state, insights, answered_at, created_at)
       VALUES ($1, $2, $3, $4, $5, NOW(), NOW())
       RETURNING id`, [userId, questionId, answerText, emotionalState, insights || []]);
...
async function getReflections(req, res, next) {
    ...
    `;
        ...
        const reflections = await (0, db_1.query)(sql, params);
...
async function getInsights(req, res, next) {
    try {
       ...
       WHERE a.user_id = $1 AND a.deleted_at IS NULL`, [userId]);
        // 情绪状态分布
        const emotionalDistribution = await (0, db_1.query)(`SELECT emotional_state, COUNT(*) as count
       ...
       ORDER BY count DESC`, [userId]);
        // 最常探索的类别
        const topCategories = await (0, db_1.query)(`SELECT q.category, COUNT(*) as reflection_count
       FROM life_question_answers a
       JOIN life_questions q ON a.question_id = q.id
       WHERE a.user_id = $1 AND a.deleted_at IS NULL
       GROUP BY q.category
       ...
       LIMIT 5`, [userId]);
        // 最近的深度洞察
        const recentInsights = await (0, db_1.query)(`SELECT a.insights, a.answered_at, q.question_text
       FROM life_question_answers a
       JOIN life_questions q ON a.question_id = q.id
       WHERE a.user_id = $1 AND a.deleted_at IS NULL
