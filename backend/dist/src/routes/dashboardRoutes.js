...
router.get('/dashboard/data', async (req, res) => {
    try {
        ...
        // 游戏化数据
        const gamificationQuery = await database_1.pool.query(`SELECT thinking_points, streak, ability_fragments
       FROM user_gamification WHERE user_id = $1`, [userId]);
        ...
        // 学习统计
        const statsQuery = await database_1.pool.query(`SELECT
       ...
       FROM user_learning_stats WHERE user_id::text = $1`, [userId]);
        ...
        // 进行中的学习会话
        const activeSessionsQuery = await database_1.pool.query(`SELECT
        s.id,
       ...
       LIMIT 3`, [userId]);
        ...
        // 推荐课程 - 简化查询
        const recommendedCoursesQuery = await database_1.pool.query(`SELECT
        t.id,
        t.title as name,
