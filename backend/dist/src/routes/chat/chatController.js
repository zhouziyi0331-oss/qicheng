...
const getOrCreateSession = async (req, res) => {
    try {
        ...
        // 验证任务是否存在且学生已接单
        const taskCheck = await (0, db_1.query)(`SELECT t.id, t.company_id, tm.student_id, tm.status
       FROM tasks t
       ...
       WHERE t.id = $2 AND t.company_id = $3`, [studentId, taskId, companyId]);
        ...
        // 查找或创建会话
        let session = await (0, db_1.query)(`SELECT * FROM chat_sessions
       WHERE task_id = $1 AND student_id = $2 AND company_id = $3`, [taskId, studentId, companyId]);
        if (session.length === 0) {
            // 创建新会话
            session = await (0, db_1.query)(`INSERT INTO chat_sessions (task_id, student_id, company_id)
         VALUES ($1, $2, $3)
         RETURNING *`, [taskId, studentId, companyId]);
            // 发送系统消息
            await (0, db_1.query)(`INSERT INTO chat_messages (session_id, sender_id, sender_type, message_type, content)
         VALUES ($1, $2, 'system', 'system', '聊天已开启，请保持友好沟通')`, [session[0].id, userId]);
        }
        res.json({
            success: true,
            data: session[0]
...
const getChatMessages = async (req, res) => {
    try {
        ...
        // 验证权限：只有参与者才能查看消息
        const sessionCheck = await (0, db_1.query)(`SELECT * FROM chat_sessions
       ...
       LIMIT $2 OFFSET $3`, [sessionId, limit, offset]);
        // 获取总数
        const countResult = await (0, db_1.query)(`SELECT COUNT(*) as total FROM chat_messages WHERE session_id = $1`, [sessionId]);
        res.json({
            success: true,
            data: {
...
const sendMessage = async (req, res) => {
    try {
       ...
       WHERE id = $1 AND (student_id = $2 OR company_id = $2) AND status = 'active'`, [sessionId, userId]);
        ...
        // 插入消息
        const message = await (0, db_1.query)(`INSERT INTO chat_messages
       ...
       RETURNING *`, [sessionId, userId, senderType, messageType, content, fileUrl, fileName, fileSize]);
        // 获取发送者信息
        const sender = await (0, db_1.query)(`SELECT nickname, avatar FROM users WHERE id = $1`, [userId]);
        const result = {
            ...message[0],
...
const markMessagesAsRead = async (req, res) => {
    try {
       ...
       WHERE id = $1 AND (student_id = $2 OR company_id = $2)`, [sessionId, userId]);
        ...
        // 获取所有未读消息
        const unreadMessages = await (0, db_1.query)(`SELECT id FROM chat_messages
       WHERE session_id = $1
         AND sender_id != $2
        ...
        if (unreadMessages.length > 0) {
         ...
         WHERE id = ANY($1)`, [messageIds]);
            ...
            for (const msg of unreadMessages) {
                await (0, db_1.query)(`INSERT INTO message_read_status (message_id, user_id)
           VALUES ($1, $2)
           ON CONFLICT (message_id, user_id) DO NOTHING`, [msg.id, userId]);
            }
            // 重置未读计数
            if (isStudent) {
                await (0, db_1.query)(`UPDATE chat_sessions SET student_unread_count = 0 WHERE id = $1`, [sessionId]);
            }
            else {
                await (0, db_1.query)(`UPDATE chat_sessions SET company_unread_count = 0 WHERE id = $1`, [sessionId]);
            }
        }
        res.json({
            success: true,
...
const archiveSession = async (req, res) => {
    try {
       ...
       WHERE id = $1 AND (student_id = $2 OR company_id = $2)`, [sessionId, userId]);
        ...
        await (0, db_1.query)(`UPDATE chat_sessions SET status = 'archived' WHERE id = $1`, [sessionId]);
        res.json({
            success: true,
            message: '会话已归档'
        });
