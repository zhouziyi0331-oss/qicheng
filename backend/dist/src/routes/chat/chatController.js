...
const getOrCreateSession = async (req, res) => {
    try {
        ...
        // 验证任务是否存在且学生已接单
        const taskCheck = await db_1.default.query(`SELECT t.id, t.company_id, tm.student_id, tm.status
       FROM tasks t
       LEFT JOIN task_matches tm ON t.id = tm.task_id AND tm.student_id = $1
       WHERE t.id = $2 AND t.company_id = $3`, [studentId, taskId, companyId]);
        ...
        // 查找或创建会话
        let session = await db_1.default.query(`SELECT * FROM chat_sessions
       WHERE task_id = $1 AND student_id = $2 AND company_id = $3`, [taskId, studentId, companyId]);
        if (session.rows.length === 0) {
            // 创建新会话
            session = await db_1.default.query(`INSERT INTO chat_sessions (task_id, student_id, company_id)
         VALUES ($1, $2, $3)
         RETURNING *`, [taskId, studentId, companyId]);
            // 发送系统消息
            await db_1.default.query(`INSERT INTO chat_messages (session_id, sender_id, sender_type, message_type, content)
         VALUES ($1, $2, 'system', 'system', '聊天已开启，请保持友好沟通')`, [session.rows[0].id, userId]);
        }
        res.json({
            success: true,
            data: session.rows[0]
...
const getChatSessions = async (req, res) => {
    try {
        ...
        const sessions = await db_1.default.query(`SELECT
         cs.*,
       ...
       ORDER BY cs.last_message_at DESC`, [userId]);
        res.json({
            success: true,
            data: sessions.rows
        });
...
const getChatMessages = async (req, res) => {
    try {
        ...
        // 验证权限：只有参与者才能查看消息
        const sessionCheck = await db_1.default.query(`SELECT * FROM chat_sessions
       WHERE id = $1 AND (student_id = $2 OR company_id = $2)`, [sessionId, userId]);
        ...
        // 获取消息列表
        const messages = await db_1.default.query(`SELECT
         cm.*,
       ...
       LIMIT $2 OFFSET $3`, [sessionId, limit, offset]);
        // 获取总数
        const countResult = await db_1.default.query(`SELECT COUNT(*) as total FROM chat_messages WHERE session_id = $1`, [sessionId]);
        res.json({
            success: true,
            data: {
...
const sendMessage = async (req, res) => {
    try {
        ...
        // 验证权限并获取会话信息
        const sessionCheck = await db_1.default.query(`SELECT * FROM chat_sessions
       WHERE id = $1 AND (student_id = $2 OR company_id = $2) AND status = 'active'`, [sessionId, userId]);
        if (sessionCheck.rows.length === 0) {
            return res.status(403).json({
                success: false,
        ...
        // 插入消息
        const message = await db_1.default.query(`INSERT INTO chat_messages
       (session_id, sender_id, sender_type, message_type, content, file_url, file_name, file_size)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
       RETURNING *`, [sessionId, userId, senderType, messageType, content, fileUrl, fileName, fileSize]);
        // 获取发送者信息
        const sender = await db_1.default.query(`SELECT nickname, avatar FROM users WHERE id = $1`, [userId]);
        const result = {
            ...message.rows[0],
...
const markMessagesAsRead = async (req, res) => {
    try {
       ...
       WHERE id = $1 AND (student_id = $2 OR company_id = $2)`, [sessionId, userId]);
        ...
        // 获取所有未读消息
        const unreadMessages = await db_1.default.query(`SELECT id FROM chat_messages
       WHERE session_id = $1
         AND sender_id != $2
         AND is_read = FALSE`, [sessionId, userId]);
        // 批量标记为已读
        if (unreadMessages.rows.length > 0) {
            const messageIds = unreadMessages.rows.map(m => m.id);
            await db_1.default.query(`UPDATE chat_messages
         SET is_read = TRUE, read_at = CURRENT_TIMESTAMP
         WHERE id = ANY($1)`, [messageIds]);
            // 插入已读记录
            for (const msg of unreadMessages.rows) {
                await db_1.default.query(`INSERT INTO message_read_status (message_id, user_id)
           VALUES ($1, $2)
           ON CONFLICT (message_id, user_id) DO NOTHING`, [msg.id, userId]);
            }
            // 重置未读计数
            if (isStudent) {
                await db_1.default.query(`UPDATE chat_sessions SET student_unread_count = 0 WHERE id = $1`, [sessionId]);
            }
            else {
                await db_1.default.query(`UPDATE chat_sessions SET company_unread_count = 0 WHERE id = $1`, [sessionId]);
            }
        }
        res.json({
            success: true,
...
const getUnreadCount = async (req, res) => {
    try {
       ...
       FROM chat_sessions
       WHERE (student_id = $1 OR company_id = $1) AND status = 'active'`, [userId]);
        res.json({
            success: true,
            data: {
...
const archiveSession = async (req, res) => {
    try {
       ...
       WHERE id = $1 AND (student_id = $2 OR company_id = $2)`, [sessionId, userId]);
        ...
        await db_1.default.query(`UPDATE chat_sessions SET status = 'archived' WHERE id = $1`, [sessionId]);
        res.json({
            success: true,
            message: '会话已归档'
        });
