...
router.post('/assess', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const assessment = await riskDetectionService_1.default.assessTaskRisk({
            title,
            description,
            budget,
...
router.post('/tasks/:taskId/assess', auth_1.authenticate, async (req, res) => {
    try {
        ...
        // 获取任务信息
        const taskResult = await req.app.locals.pool.query(`SELECT * FROM tasks WHERE id = $1 AND company_id = $2`, [taskId, userId]);
...
router.get('/tasks/:taskId/history', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const userRole = req.user.role;
        // 权限检查
        const taskResult = await req.app.locals.pool.query(`SELECT company_id FROM tasks WHERE id = $1`, [taskId]);
        if (taskResult.rows.length === 0) {
            return res.status(404).json({
                success: false,
                message: '任务不存在',
        ...
        const hasAccess = userRole === 'admin' || taskResult.rows[0].company_id === userId;
        if (!hasAccess) {
            return res.status(403).json({
                success: false,
                message: '无权查看风险评估历史',
        ...
        const history = await riskDetectionService_1.default.getTaskRiskHistory(taskId);
        res.json({
            success: true,
            data: {
                history,
...
router.post('/assessments/:assessmentId/acknowledge', auth_1.authenticate, async (req, res) => {
    try {
        ...
        // 验证评估归属
        const assessmentResult = await req.app.locals.pool.query(`SELECT ra.*, t.company_id
       FROM risk_assessments ra
       JOIN tasks t ON ra.task_id = t.id
       WHERE ra.id = $1`, [assessmentId]);
        ...
        await riskDetectionService_1.default.acknowledgeRisk(assessmentId, companyId, decision, notes);
        res.json({
            success: true,
            message: '风险确认成功',
        });
...
router.get('/my-stats', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const stats = await riskDetectionService_1.default.getCompanyRiskStats(companyId);
        res.json({
            success: true,
            data: stats,
        });
