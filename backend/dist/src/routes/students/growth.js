...
router.get('/:id/growth-timeline', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const timeline = await studentGrowthService_1.default.getGrowthTimeline(studentId, options);
        res.json({
            success: true,
            data: timeline,
        });
...
router.get('/:id/milestones', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const timeline = await studentGrowthService_1.default.getGrowthTimeline(studentId, {
            limit: 0, // 只获取里程碑
        });
        res.json({
            success: true,
...
router.post('/:id/growth-events', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const event = await studentGrowthService_1.default.recordGrowthEvent({
            studentId,
            eventType,
            title,
            description,
