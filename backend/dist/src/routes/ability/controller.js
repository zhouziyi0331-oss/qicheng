...
async function getDetailedRadar(req, res, next) {
    try {
        const userId = req.user.userId;
        // 检查是否已付费
       ...
       WHERE user_id = $1 AND report_type = 'R1' AND status = 'done'`, [userId]);
        ...
        // 已付费，返回详细分析
        const report = await (0, db_1.queryOne)(`SELECT content_json FROM opc_reports WHERE user_id = $1 AND report_type = 'R1' AND status = 'done'`, [userId]);
...
// ============================================================
function buildDimensionDescriptions(scores) {
    const desc = {
        d1: {
            name: '专业技能',
            score: scores.d1 || 0,
