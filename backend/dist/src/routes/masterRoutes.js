 ...
 * 权限：Lv.5+可申请
 */
router.post('/apply', auth_1.authenticate, async (req, res, next) => {
    try {
        ...
        await (0, db_1.query)(`UPDATE users
         ...
         WHERE id = $1`, [userId, JSON.stringify(specialties), fee, minTaskPrice, bio, acceptDesignated, allowNegotiation]);
        // 发送通知给管理员
        await (0, db_1.query)(`INSERT INTO notifications (user_id, type, title, body, link_url)
         SELECT id, 'master_application', '新的大师申请', '有新的大师认证申请待审核', '/admin/masters'
         FROM users WHERE role = 'admin'`);
        logger_1.default.info('Master application submitted via API', { userId });
        res.json({
            success: true,
...
router.get('/dashboard', auth_1.authenticate, async (req, res, next) => {
    try {
        ...
        const stats = await (0, db_1.queryOne)(`SELECT
         ...
         WHERE ta.master_id = $1`, [userId]);
        // 获取待处理的指导请求
        const pendingRequests = await (0, db_1.query)(`SELECT
           ta.id,
           ta.task_id,
           ta.student_id,
...
router.put('/settings', auth_1.authenticate, async (req, res, next) => {
    try {
        ...
        await (0, db_1.query)(`UPDATE users SET ${updates.join(', ')} WHERE id = $1`, values);
        logger_1.default.info('Master settings updated via API', { userId });
        res.json({
            success: true,
            message: '设置已更新',
        });
...
router.get('/', auth_1.authenticate, async (req, res, next) => {
    try {
        ...
        params.push(limit ? parseInt(limit) : 20, offset ? parseInt(offset) : 0);
 ...
 * 权限：仅企业可邀请
 */
router.post('/:id/invite-master', auth_1.authenticate, async (req, res, next) => {
    try {
        ...
        await (0, db_1.query)(`INSERT INTO project_invitations (
         ...
         ) VALUES ($1, $2, $3, $4, 'pending')`, [taskId, masterId, offer, message]);
        // 通知大师
        await (0, db_1.query)(`INSERT INTO notifications (
           user_id, type, title, body, link_url
         ) VALUES ($1, 'master_invitation', '企业邀请', $2, $3)`, [masterId, '企业邀请你指导一个项目', `/tasks/${taskId}`]);
        logger_1.default.info('Master invited via API', { taskId, masterId, offer });
        res.json({
            success: true,
            message: '邀请已发送',
        });
 ...
 * 权限：仅被邀请的大师可响应
 */
router.post('/:id/respond', auth_1.authenticate, async (req, res, next) => {
    try {
        ...
        await (0, db_1.withTransaction)(async (client) => {
            if (action === 'accept') {
             ...
             WHERE id = $1`, [invitation.task_id, masterId]);
            }
             ...
             SET status = 'negotiating',
                 ...
                 responded_at = NOW()
             WHERE id = $1`, [invitationId, counterOffer]);
            }
            // 通知企业
        ...
        logger_1.default.info('Master responded to invitation via API', {
            invitationId,
            masterId,
            action,
        });
 ...
 * 权限：仅指派的大师可发送
 */
router.post('/guidance/:taskId', auth_1.authenticate, async (req, res, next) => {
    try {
        ...
        // 检查是否为该任务的指派大师
        const assignment = await (0, db_1.queryOne)(`SELECT ta.student_id, mss.id as session_id
         FROM task_assignments ta
        ...
        await (0, db_1.query)(`INSERT INTO mentor_stage_messages (
         ...
         ) VALUES ($1, 'human_master', $2, $3, NOW())`, [assignment.session_id, masterId, message]);
        // 通知学生
        await (0, db_1.query)(`INSERT INTO notifications (
           user_id, type, title, body, link_url
         ) VALUES ($1, 'master_message', '大师指导', '大师发来了新的指导消息', $2)`, [assignment.student_id, `/tasks/${taskId}/mentor`]);
        logger_1.default.info('Master guidance message sent via API', { taskId, masterId });
        res.json({
            success: true,
            message: '指导消息已发送',
        });
