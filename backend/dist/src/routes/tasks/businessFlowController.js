...
exports.getAIPriceSuggestion = getAIPriceSuggestion;
exports.publishTaskWithDeposit = publishTaskWithDeposit;
exports.triggerAIMatching = triggerAIMatching;
exports.getMatchedStudents = getMatchedStudents;
exports.selectStudentsForInvitation = selectStudentsForInvitation;
...
// 1. 企业发布任务 - 获取AI价格建议
// ============================================
async function getAIPriceSuggestion(req, res, next) {
    try {
        const { title, description, taskType, estimatedMinutes } = req.body;
        if (!title || !description) {
            throw new errorHandler_1.AppError(400, '任务标题和描述为必填项', 'MISSING_FIELDS');
        }
        // TODO: 调用AI服务分析任务复杂度
        // 这里先用简单规则模拟
        let minPrice = 500;
        ...
        logger_1.default.info('AI price suggestion generated', { title, minPrice, maxPrice });
        res.json({
            success: true,
            data: {
                aiPriceMin: minPrice,
...
// 2. 企业发布任务 - 支付定金
// ============================================
async function publishTaskWithDeposit(req, res, next) {
    try {
        const companyId = req.user.userId;
        const { title, description, taskType, track, levelRequired, acceptanceCriteria, deadline, estimatedMinutes, aiPriceMin, aiPriceMax, companyPrice, // 企业在AI建议区间内定价
        paymentMethod, transactionId } = req.body;
        // 验证必填字段
        if (!title || !description || !companyPrice || !acceptanceCriteria) {
            throw new errorHandler_1.AppError(400, '标题、描述、定价和验收标准为必填项', 'MISSING_FIELDS');
        }
        // 验证企业定价是否在AI建议区间内
        const price = parseFloat(companyPrice);
        if (aiPriceMin && aiPriceMax) {
            if (price < aiPriceMin || price > aiPriceMax) {
                throw new errorHandler_1.AppError(400, `定价需在AI建议区间 ¥${aiPriceMin} - ¥${aiPriceMax} 内`, 'PRICE_OUT_OF_RANGE');
            }
        }
        // 计算价格分配
        ...
        RETURNING id`, [
          ...
          amount, payment_type, payment_method, transaction_id, status, paid_at
            ...
            // 3. 更新企业统计
            await client.query('UPDATE company_profiles SET total_tasks_posted = total_tasks_posted + 1 WHERE user_id = $1', [companyId]);
            // 4. 创建通知
         ...
         VALUES ($1, 'company', 'task_published', '任务发布成功', $2, $3)`, [
            ...
            logger_1.default.info('Task published with deposit', {
                taskId,
                companyId,
                companyPrice: price,
                studentPrice,
...
// 3. AI匹配10个学生（后台任务触发）
// ============================================
async function triggerAIMatching(taskId) {
    try {
        // 获取任务信息
       ...
       VALUES ($1, 'company', 'matching_complete', 'AI匹配完成', $2, $3)`, [
        ...
        logger_1.default.info('AI matching completed', { taskId, matchedCount: students.length });
    }
    catch (err) {
        logger_1.default.error('AI matching failed', { taskId, error: err });
        throw err;
    }
}
// ============================================
// 4. 企业查看匹配的10个学生
// ============================================
async function getMatchedStudents(req, res, next) {
    try {
        const companyId = req.user.userId;
...
// 5. 企业从10个中选择5个学生
// ============================================
async function selectStudentsForInvitation(req, res, next) {
    try {
        const companyId = req.user.userId;
        const { taskId } = req.params;
        const { selectedMatchIds } = req.body; // 选中的匹配记录ID数组
        if (!selectedMatchIds || selectedMatchIds.length !== 5) {
            throw new errorHandler_1.AppError(400, '请选择5位学生', 'INVALID_SELECTION');
        }
        // 验证任务归属
        const task = await (0, db_1.queryOne)(`SELECT * FROM tasks WHERE id = $1 AND company_id = $2`, [taskId, companyId]);
        if (!task) {
        ...
        await (0, db_1.withTransaction)(async (client) => {
         ...
         WHERE id = ANY($1) AND task_id = $2`, [selectedMatchIds, taskId]);
            // 2. 获取选中的学生ID
            const selectedStudents = await client.query(`SELECT student_id FROM ai_matches WHERE id = ANY($1)`, [selectedMatchIds]);
            // 3. 给每个学生发送邀请通知
           ...
           VALUES ($1, 'student', 'task_invitation', '收到任务邀请', $2, $3)`, [
            ...
            // 4. 更新任务状态
            await client.query(`UPDATE tasks SET status = 'pending_accept' WHERE id = $1`, [taskId]);
            logger_1.default.info('Students selected for invitation', {
                taskId,
                companyId,
...
//# sourceMappingURL=businessFlowController.js.map