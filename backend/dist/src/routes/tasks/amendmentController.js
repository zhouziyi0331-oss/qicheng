...
const createAmendment = async (req, res) => {
    try {
        ...
        // 验证任务存在且属于该企业
        const taskCheck = await (0, db_1.query)(`SELECT t.*, ts.student_id, t.deadline, t.budget
       FROM tasks t
       LEFT JOIN task_students ts ON t.id = ts.task_id
       WHERE t.id = $1 AND t.company_id = $2`, [taskId, companyId]);
        ...
        // 创建追加需求记录
        const result = await (0, db_1.query)(`INSERT INTO task_amendments
...
// 学生响应追加需求
const respondToAmendment = async (req, res) => {
    try {
        const studentId = req.user?.userId;
        const { amendmentId } = req.params;
        const { action, studentResponse } = req.body; // action: 'accept' 或 'reject'
        // 获取追加需求详情
        const amendmentCheck = await (0, db_1.query)(`SELECT a.*, t.title as task_title, ts.student_id
       ...
       WHERE a.id = $1`, [amendmentId]);
        ...
        await (0, db_1.query)(`UPDATE task_amendments
       SET status = $1, student_response = $2, responded_at = NOW()
       WHERE id = $3`, [newStatus, studentResponse, amendmentId]);
        // 如果学生接受，更新任务信息
        if (action === 'accept') {
            if (amendment.amendment_type === 'extend_deadline' && amendment.new_deadline) {
                await (0, db_1.query)('UPDATE tasks SET deadline = $1 WHERE id = $2', [amendment.new_deadline, amendment.task_id]);
            }
            if (amendment.amendment_type === 'increase_budget' && amendment.new_budget) {
                await (0, db_1.query)('UPDATE tasks SET budget = $1 WHERE id = $2', [amendment.new_budget, amendment.task_id]);
            }
        }
        // 发送通知给企业
        await (0, db_1.query)(`INSERT INTO notifications (user_id, type, title, content, related_id, created_at)
...
exports.respondToAmendment = respondToAmendment;
// 获取任务的追加需求列表
const getTaskAmendments = async (req, res) => {
    try {
        const userId = req.user?.userId;
        const { taskId } = req.params;
        // 验证用户是否有权限查看
        const taskCheck = await (0, db_1.query)(`SELECT t.company_id, ts.student_id
       FROM tasks t
       LEFT JOIN task_students ts ON t.id = ts.task_id
       WHERE t.id = $1`, [taskId]);
        if (taskCheck.length === 0) {
        ...
        // 获取追加需求列表
        const result = await (0, db_1.query)(`SELECT * FROM task_amendments
       WHERE task_id = $1
...
// 获取我的待处理追加需求（学生端）
const getMyPendingAmendments = async (req, res) => {
    try {
        const studentId = req.user?.userId;
        const result = await (0, db_1.query)(`SELECT a.*, t.title as task_title, c.nickname as company_name
       FROM task_amendments a
       JOIN tasks t ON a.task_id = t.id
...
exports.getMyPendingAmendments = getMyPendingAmendments;
//# sourceMappingURL=amendmentController.js.map