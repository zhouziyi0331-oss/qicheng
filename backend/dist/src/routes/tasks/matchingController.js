...
exports.rematchTask = rematchTask;
...
async function triggerMatching(req, res) {
    try {
        ...
        // 验证任务存在且属于当前企业
        const task = await (0, db_1.queryOne)(`SELECT id, company_id, status FROM tasks WHERE id = $1`, [taskId]);
        if (!task) {
        ...
        // 2. 生成任务翻译
        await qichengTeacherService_1.default.analyzeAndTranslateTask(taskId);
        // 3. 找出最匹配的100个学生
       ...
       WHERE id = $3`, [matches.length, topScore, taskId]);
        logger_1.default.info(`Matching completed for task ${taskId}, found ${matches.length} students`);
        res.json({
            success: true,
 ...
 */
async function getMatchedStudents(req, res) {
    try {
        const { taskId } = req.params;
        const userId = req.user?.id;
       ...
       LIMIT $2`, [taskId, limit]);
        ...
        logger_1.default.error('Failed to get matched students:', error);
 ...
 */
async function pushToStudents(req, res) {
    try {
        const { taskId } = req.params;
        ...
        for (const studentId of studentIds) {
            await (0, db_1.query)(`UPDATE task_student_matches SET
          is_pushed = true,
          pushed_at = NOW()
         WHERE task_id = $1 AND student_id = $2`, [taskId, studentId]);
        }
        logger_1.default.info(`Pushed task ${taskId} to ${studentIds.length} students`);
        res.json({
            success: true,
        ...
        logger_1.default.error('Failed to push to students:', error);
...
async function getRecommendedTasks(req, res) {
    try {
       ...
       WHERE tsm.student_id = $1
         ...
         AND t.status = 'open'
       ORDER BY tsm.pushed_at DESC`, [userId]);
        // 标记为已查看
        await (0, db_1.query)(`UPDATE task_student_matches SET
        student_viewed = true,
        viewed_at = NOW()
       WHERE student_id = $1 AND is_pushed = true AND student_viewed = false`, [userId]);
        res.json({
            success: true,
            tasks: tasks.map(t => ({
                taskId: t.task_id,
        ...
        logger_1.default.error('Failed to get recommended tasks:', error);
 ...
 */
async function getTaskTranslation(req, res) {
    try {
        const { taskId } = req.params;
        // 获取任务翻译
        ...
        logger_1.default.error('Failed to get task translation:', error);
...
async function acceptRecommendation(req, res) {
    try {
        ...
        const userId = req.user?.id;
        // 验证是否推送给该学生
        const match = await (0, db_1.queryOne)(`SELECT task_id, is_pushed FROM task_student_matches
       WHERE task_id = $1 AND student_id = $2`, [taskId, userId]);
        ...
        accepted_at = NOW()
       WHERE task_id = $1 AND student_id = $2`, [taskId, userId]);
        logger_1.default.info(`Student ${userId} accepted task ${taskId}`);
        res.json({
            success: true,
            message: '已接受任务推荐'
        });
        ...
        logger_1.default.error('Failed to accept recommendation:', error);
       ...
       WHERE task_id = $1`, [taskId]);
        res.json({
            ...
            stats: {
                ...
                pushedCount: parseInt(stats?.pushed_count) || 0,
                viewedCount: parseInt(stats?.viewed_count) || 0,
        ...
        logger_1.default.error('Failed to get matching stats:', error);
 ...
 */
async function rematchTask(req, res) {
    try {
        const { taskId } = req.params;
        const userId = req.user?.id;
        // 验证任务存在且属于当前企业
        ...
        logger_1.default.error('Failed to rematch task:', error);
