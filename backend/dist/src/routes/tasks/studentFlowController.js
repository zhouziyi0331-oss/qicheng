...
exports.getTaskInvitations = getTaskInvitations;
exports.acceptTaskInvitation = acceptTaskInvitation;
exports.rejectTaskInvitation = rejectTaskInvitation;
exports.updateTaskProgress = updateTaskProgress;
exports.submitDeliverables = submitDeliverables;
...
// 1. 学生查看收到的任务邀请
// ============================================
async function getTaskInvitations(req, res, next) {
    try {
        const studentId = req.user.userId;
        // 获取所有待接受的邀请
        const invitations = await (0, db_1.query)(`SELECT
        am.id as match_id,
        am.match_score,
        am.match_reason,
       ...
       ORDER BY am.created_at DESC`, [studentId]);
        res.json({
            success: true,
            data: {
...
// 2. 学生接受任务邀请
// ============================================
async function acceptTaskInvitation(req, res, next) {
    try {
        const studentId = req.user.userId;
        ...
        await (0, db_1.withTransaction)(async (client) => {
            ...
            if (taskData.status !== 'pending_accept') {
                throw new errorHandler_1.AppError(400, '该任务已被其他学生接单', 'TASK_ALREADY_ACCEPTED');
            }
            // 2. 检查学生是否收到邀请
            const match = await client.query(`SELECT * FROM ai_matches
         WHERE task_id = $1 AND student_id = $2 AND is_invited = true`, [taskId, studentId]);
            if (match.rows.length === 0) {
                throw new errorHandler_1.AppError(403, '您未收到该任务的邀请', 'NOT_INVITED');
            }
            const matchData = match.rows[0];
            if (matchData.invitation_status !== 'pending') {
                throw new errorHandler_1.AppError(400, '邀请已失效', 'INVITATION_EXPIRED');
            }
            // 3. 更新任务状态 - 学生接单
         ...
         WHERE task_id = $1 AND student_id != $2 AND invitation_status = 'pending'`, [taskId, studentId]);
            // 6. 通知企业
         ...
         VALUES ($1, 'company', 'task_accepted', '学生已接单', $2, $3)`, [
            ...
            // 7. 通知其他学生邀请已过期
            const otherStudents = await client.query(`SELECT student_id FROM ai_matches
         WHERE task_id = $1 AND student_id != $2 AND is_invited = true`, [taskId, studentId]);
...
// 3. 学生拒绝任务邀请
// ============================================
async function rejectTaskInvitation(req, res, next) {
    try {
        const studentId = req.user.userId;
        const { taskId } = req.params;
        ...
        await (0, db_1.withTransaction)(async (client) => {
         ...
         SET invitation_status = 'rejected',
             responded_at = NOW()
         WHERE task_id = $1 AND student_id = $2 AND invitation_status = 'pending'`, [taskId, studentId]);
            logger_1.default.info('Student rejected task invitation', { taskId, studentId, reason });
            res.json({
                success: true,
                data: {
                    message: '已拒绝该任务邀请'
...
async function updateTaskProgress(req, res, next) {
    try {
        ...
        // 记录进度
        await (0, db_1.query)(`INSERT INTO task_progress (task_id, student_id, progress_percentage, progress_description, milestone)
       VALUES ($1, $2, $3, $4, $5)`, [taskId, studentId, progressPercentage, progressDescription, milestone]);
        // 通知企业
        await (0, db_1.query)(`INSERT INTO notifications (user_id, user_type, type, title, content, related_task_id)
       VALUES ($1, 'company', 'progress_update', '任务进度更新', $2, $3)`, [
            task.company_id,
            `任务《${task.title}》进度更新：${progressPercentage}% - ${progressDescription}`,
            taskId
        ]);
        logger_1.default.info('Task progress updated', { taskId, studentId, progressPercentage });
        res.json({
            success: true,
            data: {
                taskId,
...
// 5. 学生提交交付物
// ============================================
async function submitDeliverables(req, res, next) {
    try {
        const studentId = req.user.userId;
        const { taskId } = req.params;
        ...
        await (0, db_1.withTransaction)(async (client) => {
         ...
         VALUES ($1, 'company', 'deliverables_submitted', '学生已提交交付物', $2, $3)`, [
            ...
            logger_1.default.info('Deliverables submitted', { taskId, studentId, count: deliverables.length });
            res.json({
                success: true,
                data: {
                    taskId,
...
// 6. AI审核交付物（后台任务）
// ============================================
async function triggerAIReview(taskId) {
    try {
        // 获取交付物
        const deliverables = await (0, db_1.query)(`SELECT * FROM task_deliverables WHERE task_id = $1`, [taskId]);
        // TODO: 调用AI服务审核
        // 这里先模拟审核通过
        ...
        await (0, db_1.withTransaction)(async (client) => {
            // 更新交付物审核状态
            for (const item of deliverables) {
                await client.query(`UPDATE task_deliverables
           SET ai_review_status = $1,
               ai_review_result = $2,
               ai_reviewed_at = NOW()
           WHERE id = $3`, [aiReviewPassed ? 'passed' : 'failed', aiReviewResult, item.id]);
            }
            if (aiReviewPassed) {
                // AI审核通过，更新任务状态为待企业验收
                await client.query(`UPDATE tasks SET status = 'pending_verification' WHERE id = $1`, [taskId]);
                // 获取任务信息
           ...
           VALUES ($1, 'company', 'ready_for_verification', '请验收任务', $2, $3)`, [
            ...
            logger_1.default.info('AI review completed', { taskId, passed: aiReviewPassed });
        });
    }
    catch (err) {
        logger_1.default.error('AI review error', { taskId, error: err });
        throw err;
    }
}
// 导出所有函数
...
//# sourceMappingURL=studentFlowController.js.map