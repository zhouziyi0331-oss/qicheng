 ...
 */
router.get('/summaries/:orderId', auth_1.authenticate, async (req, res) => {
    try {
        const { orderId } = req.params;
        // 生成或获取缓存的总结
        const summary = await instantGrowthSummaryService_1.default.generateInstantSummary(orderId);
        res.json({
            success: true,
            data: summary,
        });
 ...
 */
router.post('/summaries/:summaryId/view', auth_1.authenticate, async (req, res) => {
    try {
        const { summaryId } = req.params;
        await instantGrowthSummaryService_1.default.markAsViewed(summaryId);
        res.json({
            success: true,
            message: '已标记为已查看',
        });
 ...
 */
router.post('/summaries/:summaryId/feedback', auth_1.authenticate, async (req, res) => {
    try {
        const { summaryId } = req.params;
        ...
        await instantGrowthSummaryService_1.default.submitFeedback(summaryId, feedback);
        res.json({
            success: true,
            message: '反馈提交成功',
        });
 ...
 */
router.post('/ability-update/:orderId', auth_1.authenticate, async (req, res) => {
    try {
        const { orderId } = req.params;
        const result = await abilityDimensionUpdateService_1.default.updateAbilityAfterOrder(orderId);
        res.json({
            success: true,
            data: result,
            message: '能力更新成功',
 ...
 */
router.get('/graduation-report/:reportId', auth_1.authenticate, async (req, res) => {
    try {
        const { reportId } = req.params;
        const report = await graduationReportService_1.default.getFullReport(reportId);
        if (!report) {
            return res.status(404).json({
                success: false,
                message: '报告不存在',
...
router.post('/graduation-report/:reportId/pay', auth_1.authenticate, async (req, res) => {
    try {
        ...
        await graduationReportService_1.default.processPayment(reportId, userId, paymentMethod, transactionId, pointsUsed || 0);
        res.json({
            success: true,
            message: '付费成功，报告已解锁',
        });
...
router.get('/overview', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const [summaries, profileVersions, reportPreview] = await Promise.all([
            ...
            abilityDimensionUpdateService_1.default.getProfileVersions(userId),
            graduationReportService_1.default.getReportPreview(userId),
        ]);
        // 获取当前画像
