...
exports.getFavoriteStudents = getFavoriteStudents;
exports.addFavoriteStudent = addFavoriteStudent;
exports.removeFavoriteStudent = removeFavoriteStudent;
exports.getRequirementSupplements = getRequirementSupplements;
...
async function getReport(req, res, next) {
    try {
        ...
        // 概览数据
        const overview = await (0, db_1.queryOne)(`SELECT
       ...
       FROM tasks t
       WHERE t.company_id = $1 ${dateFilter}`, [userId]);
        // 月度统计
       ...
       LIMIT 12`, [userId]);
        // 任务类型分布
        const categoryStats = await (0, db_1.query)(`SELECT
        t.track as category,
        COUNT(*) as count,
       ...
       ORDER BY count DESC`, [userId]);
        // 合作学生排行
        const topStudents = await (0, db_1.query)(`SELECT
        u.id,
        u.nickname,
...
async function getStudentProfile(req, res, next) {
    try {
       ...
       WHERE u.id = $1`, [studentId]);
        ...
        // OPC测评结果
        const opcResult = await (0, db_1.queryOne)(`SELECT
       ...
       LIMIT 1`, [studentId]);
        // 兴趣和技能标签
        const tags = await (0, db_1.query)(`SELECT tag_name, tag_type
       FROM user_tags
       WHERE user_id = $1`, [studentId]);
        ...
        // 历史作品
        const portfolio = await (0, db_1.query)(`SELECT
...
async function getTaskProgress(req, res, next) {
    try {
        ...
        const userId = req.user.userId;
        // 任务基本信息
        const task = await (0, db_1.queryOne)(`SELECT t.id, t.title, t.status, u.nickname as student_name
       FROM tasks t
       LEFT JOIN users u ON u.id = t.student_id
       WHERE t.id = $1 AND t.company_id = $2`, [taskId, userId]);
        if (!task)
...
async function getFavoriteStudents(req, res, next) {
    try {
        const userId = req.user.userId;
        const favorites = await (0, db_1.query)(`SELECT
...
async function addFavoriteStudent(req, res, next) {
    try {
        const userId = req.user.userId;
        const { studentId } = req.body;
        if (!studentId) {
            throw new errorHandler_1.AppError(400, '学生ID不能为空', 'MISSING_STUDENT_ID');
        }
        // 检查是否已收藏
        const existing = await (0, db_1.queryOne)(`SELECT id FROM favorite_students WHERE company_id = $1 AND student_id = $2`, [userId, studentId]);
        if (existing) {
            throw new errorHandler_1.AppError(400, '已经收藏过该学生', 'ALREADY_FAVORITED');
        }
        await (0, db_1.query)(`INSERT INTO favorite_students (company_id, student_id) VALUES ($1, $2)`, [userId, studentId]);
        res.json({
            success: true,
            message: '收藏成功'
        });
...
async function removeFavoriteStudent(req, res, next) {
    try {
        const userId = req.user.userId;
        const { studentId } = req.params;
        const result = await (0, db_1.query)(`DELETE FROM favorite_students WHERE company_id = $1 AND student_id = $2 RETURNING id`, [userId, studentId]);
        if (result.length === 0) {
            throw new errorHandler_1.AppError(404, '未找到收藏记录', 'NOT_FOUND');
        }
        res.json({
            success: true,
            message: '取消收藏成功'
...
async function getRequirementSupplements(req, res, next) {
    try {
        const userId = req.user.userId;
        const { taskId } = req.params;
        // 验证任务归属
        const task = await (0, db_1.queryOne)(`SELECT id FROM tasks WHERE id = $1 AND company_id = $2`, [taskId, userId]);
        if (!task) {
            throw new errorHandler_1.AppError(404, '任务不存在', 'NOT_FOUND');
        }
        const supplements = await (0, db_1.query)(`SELECT
        id,
        content,
        estimated_days,
        old_deadline,
