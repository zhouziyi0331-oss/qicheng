...
exports.getMySubcontracts = getMySubcontracts;
exports.completeSubcontract = completeSubcontract;
...
// ============================================================
async function createSubcontract(req, res, next) {
    try {
        ...
        // 1. 验证任务是否属于该学生
        const assignment = await (0, db_1.queryOne)(`SELECT ta.*, t.budget_net, t.title, t.description
       FROM task_assignments ta
       JOIN tasks t ON ta.task_id = t.id
       WHERE ta.task_id = $1 AND ta.student_id = $2 AND ta.status = 'accepted'`, [taskId, userId]);
        if (!assignment) {
            throw new errorHandler_1.AppError(404, '任务不存在或你没有权限转包', 'TASK_NOT_FOUND');
        }
        const task = await (0, db_1.queryOne)('SELECT budget_net, title, description FROM tasks WHERE id = $1', [taskId]);
        if (!task) {
        ...
        if (priceDiff < minPriceDiff) {
            throw new errorHandler_1.AppError(400, `转包差价不能低于 ${minPriceDiff} 元`, 'PRICE_DIFF_TOO_LOW');
        }
        if (subcontractBudget <= 0) {
            throw new errorHandler_1.AppError(400, '转包预算必须大于0', 'INVALID_BUDGET');
        }
        // 3. 调用AI审核转包理由
        const aiApproval = await evaluateSubcontractReason(reason, task.title, task.description);
        if (!aiApproval.approved) {
            throw new errorHandler_1.AppError(400, `转包申请被拒绝：${aiApproval.feedback}`, 'SUBCONTRACT_REJECTED');
        }
        // 4. 创建转包任务和记录
        const result = await (0, db_1.withTransaction)(async (client) => {
            // 4a. 创建新任务（转包任务）
            const newTask = await client.query(`INSERT INTO tasks
         ...
         RETURNING id`, [subcontractBudget * 1.2, subcontractBudget, taskId, userId] // 假设平台抽成20%
            );
            const newTaskId = newTask.rows[0].id;
            // 4b. 创建转包记录
            const subcontract = await client.query(`INSERT INTO task_subcontracts
        ...
        logger_1.default.info('Subcontract created', { userId, taskId, newTaskId: result.newTaskId });
        res.json({
            success: true,
...
// ============================================================
async function getMySubcontracts(req, res, next) {
    try {
        const userId = req.user.userId;
        const subcontracts = await (0, db_1.query)(`SELECT
         ts.*,
...
// ============================================================
async function completeSubcontract(req, res, next) {
    try {
        const userId = req.user.userId;
        const { id } = req.params;
        // 1. 获取转包记录
        const subcontract = await (0, db_1.queryOne)(`SELECT * FROM task_subcontracts WHERE id = $1 AND deleted_at IS NULL`, [id]);
        if (!subcontract) {
            throw new errorHandler_1.AppError(404, '转包记录不存在', 'SUBCONTRACT_NOT_FOUND');
        }
        if (subcontract.original_student_id !== userId) {
            throw new errorHandler_1.AppError(403, '无权操作此转包记录', 'FORBIDDEN');
        }
        if (subcontract.status === 'completed') {
            throw new errorHandler_1.AppError(400, '该转包已完成', 'ALREADY_COMPLETED');
        }
        // 2. 检查新任务是否已完成
        const newTask = await (0, db_1.queryOne)('SELECT status FROM tasks WHERE id = $1', [subcontract.new_task_id]);
        if (newTask?.status !== 'completed') {
            throw new errorHandler_1.AppError(400, '转包任务尚未完成', 'TASK_NOT_COMPLETED');
        }
        // 3. 结算差价给原学生
        await (0, db_1.withTransaction)(async (client) => {
            // 3a. 更新转包状态
            await client.query(`UPDATE task_subcontracts SET status = 'completed', completed_at = NOW()
         WHERE id = $1`, [id]);
            // 3b. 增加原学生余额
            await client.query(`UPDATE student_balances
         SET balance = balance + $1, total_earned = total_earned + $1, updated_at = NOW()
         WHERE user_id = $2`, [subcontract.price_difference, userId]);
            // 3c. 创建支付记录
            await client.query(`INSERT INTO payments
         (task_id, student_id, payer, gross_amount, platform_fee, net_amount, status, settled_at)
         VALUES ($1, $2, 'platform', $3, 0, $3, 'settled', NOW())`, [subcontract.new_task_id, userId, subcontract.price_difference]);
        });
        logger_1.default.info('Subcontract completed', { userId, subcontractId: id, priceDiff: subcontract.price_difference });
        res.json({
            success: true,
            message: '转包完成，差价已到账',
            data: {
