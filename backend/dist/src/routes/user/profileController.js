...
async function getProfile(req, res, next) {
    try {
        ...
        const user = await (0, db_1.queryOne)(`SELECT
      ...
      WHERE u.id = $1 AND u.deleted_at IS NULL`, [userId]);
        ...
        if (user.user_type === 'student') {
            const studentProfile = await (0, db_1.queryOne)(`SELECT
        ...
        WHERE u.id = $1`, [userId]);
            ...
            const companyProfile = await (0, db_1.queryOne)(`SELECT
...
async function updateNickname(req, res, next) {
    try {
        ...
        if (!nickname || nickname.trim().length === 0) {
            throw new errorHandler_1.AppError(400, '昵称不能为空', 'INVALID_NICKNAME');
        }
        if (nickname.length > 20) {
            throw new errorHandler_1.AppError(400, '昵称不能超过20个字符', 'NICKNAME_TOO_LONG');
        }
        // 检查昵称是否包含敏感词（简单示例）
        const sensitiveWords = ['管理员', 'admin', '客服', '官方'];
        if (sensitiveWords.some(word => nickname.toLowerCase().includes(word))) {
            throw new errorHandler_1.AppError(400, '昵称包含敏感词', 'SENSITIVE_NICKNAME');
        }
        await (0, db_1.query)('UPDATE users SET nickname = $1 WHERE id = $2', [nickname.trim(), userId]);
        logger_1.default.info('User nickname updated', { userId, nickname });
        res.json({
            success: true,
            message: '昵称更新成功',
...
async function updateAvatar(req, res, next) {
    try {
        ...
        if (!avatar || !avatar.startsWith('http')) {
            throw new errorHandler_1.AppError(400, '请提供有效的头像URL', 'INVALID_AVATAR');
        }
        await (0, db_1.query)('UPDATE users SET avatar = $1 WHERE id = $2', [avatar, userId]);
        logger_1.default.info('User avatar updated', { userId });
        res.json({
            success: true,
            message: '头像更新成功',
            data: { avatar },
...
async function updateProfile(req, res, next) {
    try {
        ...
        const user = await (0, db_1.queryOne)('SELECT user_type FROM users WHERE id = $1', [userId]);
        if (!user) {
            throw new errorHandler_1.AppError(404, '用户不存在', 'USER_NOT_FOUND');
        }
        // 更新基本信息
        if (nickname || avatar) {
            ...
            if (updates.length > 0) {
                values.push(userId);
                await (0, db_1.query)(`UPDATE users SET ${updates.join(', ')} WHERE id = $${paramIndex}`, values);
            }
        }
        // 更新学生资料
        if (user.user_type === 'student' && studentProfile) {
            const { real_name, gender, birth_date, university, major, grade, student_id, id_card, bio, } = studentProfile;
            const updates = [];
            const values = [];
            let paramIndex = 1;
        ...
        if (user.user_type === 'company' && companyProfile) {
            const { company_name, contact_name, contact_phone, industry, company_size, business_license, credit_code, } = companyProfile;
            const updates = [];
            const values = [];
            let paramIndex = 1;
...
async function bindPhone(req, res, next) {
    try {
        ...
        if (!phone || !code) {
            throw new errorHandler_1.AppError(400, '手机号和验证码不能为空', 'INVALID_INPUT');
        }
        // 验证手机号格式
        const phoneRegex = /^1[3-9]\d{9}$/;
        if (!phoneRegex.test(phone)) {
            throw new errorHandler_1.AppError(400, '手机号格式不正确', 'INVALID_PHONE');
        }
        // TODO: 验证验证码（从Redis中获取并验证）
        // 这里简化处理，实际应该验证验证码是否正确
        logger_1.default.info('Binding phone', { userId, phone, code });
        // 检查手机号是否已被其他用户绑定
        const existingUser = await (0, db_1.queryOne)('SELECT id FROM users WHERE phone = $1 AND id != $2 AND deleted_at IS NULL', [phone, userId]);
        if (existingUser) {
            throw new errorHandler_1.AppError(400, '该手机号已被其他用户绑定', 'PHONE_ALREADY_BOUND');
        }
        // 更新用户手机号
        await (0, db_1.query)('UPDATE users SET phone = $1 WHERE id = $2', [phone, userId]);
        logger_1.default.info('Phone bound successfully', { userId, phone });
        res.json({
            success: true,
            message: '手机号绑定成功',
            data: { phone },
