...
const createDispute = async (req, res) => {
    try {
        ...
        // 验证任务存在且用户有权限申诉
        const taskCheck = await (0, db_1.query)(`SELECT t.*, ts.student_id, t.company_id
       FROM tasks t
       ...
       WHERE t.id = $1`, [taskId]);
        ...
        // 检查是否已有未处理的申诉
        const existingDispute = await (0, db_1.query)(`SELECT id FROM disputes
       WHERE task_id = $1 AND initiator_id = $2 AND status IN ('pending', 'investigating')`, [taskId, userId]);
        if (existingDispute.length > 0) {
            return res.status(400).json({ success: false, message: '该任务已有待处理的申诉，请勿重复提交' });
        }
        // 创建申诉记录
        const result = await (0, db_1.query)(`INSERT INTO disputes
...
exports.getDisputeDetail = getDisputeDetail;
// 管理员处理申诉
const handleDispute = async (req, res) => {
    try {
        ...
        const result = await (0, db_1.query)(`UPDATE disputes
       SET status = $1,
           admin_response = $2,
           resolution = $3,
           resolved_at = $4,
       ...
       VALUES ($1, 'dispute_handled', '申诉处理结果', $2, $3, NOW())`, [
        ...
        // 记录管理员操作日志
        await (0, db_1.query)(`INSERT INTO admin_logs (admin_id, action, target_type, target_id, details, created_at)
       VALUES ($1, 'handle_dispute', 'dispute', $2, $3, NOW())`, [adminId, disputeId, JSON.stringify({ action, adminResponse, resolution })]);
        res.json({
            success: true,
            message: '申诉处理成功',
            data: result[0]
...
const getAllDisputes = async (req, res) => {
    ...
    `;
        ...
        const result = await (0, db_1.query)(queryText, params);
        // 获取总数
        const countResult = await (0, db_1.query)(`SELECT COUNT(*) FROM disputes WHERE 1=1
