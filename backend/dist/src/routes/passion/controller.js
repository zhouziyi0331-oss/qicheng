...
exports.getSparks = getSparks;
exports.recordSpark = recordSpark;
exports.getAnalysis = getAnalysis;
exports.getRecommendations = getRecommendations;
const db_1 = require("../../utils/db");
const errorHandler_1 = require("../../middleware/errorHandler");
// GET /passion/sparks/:userId — 获取用户的热情火花记录
async function getSparks(req, res, next) {
    try {
        const { userId } = req.params;
        const sparks = await (0, db_1.query)(`SELECT id, activity_name, description, intensity_level, context,
              triggered_at, tags, created_at
       FROM passion_sparks
       WHERE user_id = $1 AND deleted_at IS NULL
       ORDER BY triggered_at DESC`, [userId]);
...
async function recordSpark(req, res, next) {
    try {
        ...
        if (intensityLevel < 1 || intensityLevel > 10) {
            throw new errorHandler_1.AppError(400, '强度等级必须在1-10之间', 'INVALID_INTENSITY');
        }
        const result = await (0, db_1.query)(`INSERT INTO passion_sparks
       (user_id, activity_name, description, intensity_level, context,
        triggered_at, tags, created_at)
       VALUES ($1, $2, $3, $4, $5, NOW(), $6, NOW())
       RETURNING id`, [userId, activityName, description, intensityLevel, context, tags || []]);
...
async function getAnalysis(req, res, next) {
    try {
       ...
       WHERE user_id = $1 AND deleted_at IS NULL`, [userId]);
        // 获取最近的高强度火花
        const recentHighIntensity = await (0, db_1.query)(`SELECT activity_name, intensity_level, triggered_at
       FROM passion_sparks
       WHERE user_id = $1 AND intensity_level >= 7 AND deleted_at IS NULL
       ORDER BY triggered_at DESC
       LIMIT 10`, [userId]);
        // 生成热情趋势（按月统计）
        const trends = await (0, db_1.query)(`SELECT
...
async function getRecommendations(req, res, next) {
    try {
        const { userId } = req.params;
        // 获取用户的热情火花数据
        const sparks = await (0, db_1.query)(`SELECT activity_name, intensity_level, tags
       FROM passion_sparks
       WHERE user_id = $1 AND deleted_at IS NULL
