...
const getUserRatingStats = async (req, res) => {
    try {
        ...
        const result = await (0, db_1.query)('SELECT * FROM user_rating_stats WHERE user_id = $1', [userId]);
        if (result.length === 0) {
            return res.json({
                stats: {
                    total_ratings: 0,
...
const getUserReceivedRatings = async (req, res) => {
    try {
      ...
      LIMIT $2 OFFSET $3`, [userId, limit, offset]);
        const countResult = await (0, db_1.query)('SELECT COUNT(*) FROM task_ratings WHERE ratee_id = $1 AND is_public = true', [userId]);
        // 处理匿名评价
        const ratings = result.map((rating) => {
...
const getUserGivenRatings = async (req, res) => {
    try {
      ...
      ORDER BY tr.created_at DESC
      LIMIT $2 OFFSET $3`, [userId, limit, offset]);
        const countResult = await (0, db_1.query)('SELECT COUNT(*) FROM task_ratings WHERE rater_id = $1', [userId]);
        res.json({
            ratings: result,
            pagination: {
                page,
...
const replyToRating = async (req, res) => {
    try {
        ...
        // 验证评价是否存在且是评价该企业的
        const ratingResult = await (0, db_1.query)('SELECT * FROM task_ratings WHERE id = $1 AND ratee_id = $2 AND ratee_type = $3', [ratingId, userId, 'company']);
        if (ratingResult.length === 0) {
            return res.status(404).json({ error: '评价不存在或无权限回复' });
        }
        // 更新回复
        const result = await (0, db_1.query)(`UPDATE task_ratings
...
const getPendingRatingTasks = async (req, res) => {
    ...
    `;
      ...
      `;
        ...
        const result = await (0, db_1.query)(queryStr, [userId]);
