...
exports.initiateRequirementConfirmation = initiateRequirementConfirmation;
exports.analyzeStudentUnderstanding = analyzeStudentUnderstanding;
exports.provideInspirationalGuidance = provideInspirationalGuidance;
exports.celebrateProgressAndGuideNext = celebrateProgressAndGuideNext;
exports.reviewSubmission = reviewSubmission;
exports.translateCompanyFeedback = translateCompanyFeedback;
exports.translateStudentQuestion = translateStudentQuestion;
 ...
 */
async function initiateRequirementConfirmation(req, res, next) {
    try {
...
请生成第一条消息（100字以内）：`;
        ...
        // 保存对话记录
        await (0, db_1.query)(`INSERT INTO mentor_conversations 
       (student_id, task_id, stage, trigger_type, mentor_response, created_at)
       VALUES ($1, $2, $3, $4, $5, NOW())`, [studentId, taskId, 'requirement_confirmation', 'task_matched', mentorResponse]);
        // 创建需求确认会话
        await (0, db_1.query)(`INSERT INTO requirement_confirmation_sessions 
       (task_id, student_id, status, started_at)
       VALUES ($1, $2, $3, NOW())`, [taskId, studentId, 'in_progress']);
        res.json({
            success: true,
            data: {
 ...
 * 分析学生对需求的理解是否准确
 */
async function analyzeStudentUnderstanding(req, res, next) {
    try {
        const { taskId } = req.params;
        ...
        // 获取任务详情和企业需求
        const task = await (0, db_1.queryOne)('SELECT * FROM tasks WHERE id = $1', [taskId]);
        const companyRequirement = await (0, db_1.queryOne)('SELECT * FROM task_requirements WHERE task_id = $1', [taskId]);
        // 获取对话历史
        const conversationHistory = await (0, db_1.query)(`SELECT mentor_response, student_message FROM mentor_conversations 
...
## 对话历史
${conversationHistory.map((c) => `导师：${c.mentor_response}\n学生：${c.student_message || ''}`).join('\n')}
...
请返回JSON：`;
        ...
        // 保存学生回复和分析结果
        await (0, db_1.query)(`UPDATE mentor_conversations 
       SET student_message = $1, analysis_result = $2, accuracy_score = $3
       WHERE task_id = $4 AND student_id = $5 AND student_message IS NULL
       ORDER BY created_at DESC LIMIT 1`, [studentResponse, analysis, analysis.accuracy, taskId, studentId]);
        // 如果理解准确，更新会话状态并生成PRD
        if (analysis.isAccurate) {
            await (0, db_1.query)(`UPDATE requirement_confirmation_sessions 
         SET status = $1, accuracy_score = $2, product_framework = $3, completed_at = NOW()
         WHERE task_id = $4 AND student_id = $5`, ['confirmed', analysis.accuracy, analysis.productFramework, taskId, studentId]);
        }
        // 保存下一条消息
        await (0, db_1.query)(`INSERT INTO mentor_conversations 
       (student_id, task_id, stage, trigger_type, mentor_response, created_at)
       VALUES ($1, $2, $3, $4, $5, NOW())`, [studentId, taskId, 'requirement_confirmation', 'student_response', analysis.nextMessage]);
        res.json({
            success: true,
            data: {
 ...
 * 学生求助时的启发式引导
 */
async function provideInspirationalGuidance(req, res, next) {
    try {
        const { taskId } = req.params;
        const { question, currentStep, context } = req.body;
        const studentId = req.user.userId;
        // 获取学生信息和历史
        const student = await (0, db_1.queryOne)('SELECT * FROM student_capabilities WHERE student_id = $1', [studentId]);
        const taskProgress = await (0, db_1.queryOne)('SELECT * FROM task_progress WHERE task_id = $1 AND student_id = $2', [taskId, studentId]);
        // 获取最近的对话历史
        const recentConversations = await (0, db_1.query)(`SELECT * FROM mentor_conversations 
       WHERE task_id = $1 AND student_id = $2 
       ORDER BY created_at DESC LIMIT 5`, [taskId, studentId]);
        // 构建启发式引导Prompt
...
## 最近对话
${recentConversations.map((c) => `学生：${c.student_message}\n导师：${c.mentor_response}`).join('\n')}
...
请生成引导消息（150字以内）：`;
       ...
       VALUES ($1, $2, $3, $4, $5, $6, NOW())`, [studentId, taskId, 'execution_guidance', 'student_question', question, guidanceResponse]);
        // 记录学生求助次数（用于后续分析）
        await (0, db_1.query)(`UPDATE task_progress 
 ...
 * 学生完成一个步骤后的鼓励和下一步引导
 */
async function celebrateProgressAndGuideNext(req, res, next) {
    try {
        const { taskId } = req.params;
        ...
        // 获取学生历史数据
        const student = await (0, db_1.queryOne)('SELECT * FROM student_capabilities WHERE student_id = $1', [studentId]);
        const previousTasks = await (0, db_1.query)(`SELECT * FROM task_completions 
       WHERE student_id = $1 
       ORDER BY completed_at DESC LIMIT 3`, [studentId]);
        // 构建鼓励Prompt
...
请生成鼓励消息（100字以内）：`;
        ...
        await (0, db_1.query)(`INSERT INTO mentor_conversations 
       (student_id, task_id, stage, trigger_type, mentor_response, created_at)
       VALUES ($1, $2, $3, $4, $5, NOW())`, [studentId, taskId, 'execution_guidance', 'step_completed', encouragement]);
        res.json({
            success: true,
            data: {
...
async function reviewSubmission(req, res, next) {
    try {
        ...
        // 获取任务需求
        const task = await (0, db_1.queryOne)('SELECT * FROM tasks WHERE id = $1', [taskId]);
        const requirement = await (0, db_1.queryOne)('SELECT * FROM task_requirements WHERE task_id = $1', [taskId]);
        // 构建审核Prompt
...
请返回JSON：`;
        ...
        // 保存审核结果
        await (0, db_1.query)(`INSERT INTO ai_submission_reviews 
       (task_id, student_id, review_result, pass_review, score, issues, created_at)
       VALUES ($1, $2, $3, $4, $5, $6, NOW())`, [taskId, studentId, review, review.passReview, review.score, JSON.stringify(review.issues)]);
        // 如果通过审核，推送给企业
        if (review.passReview) {
            await (0, db_1.query)(`UPDATE task_submissions 
         SET status = $1, ai_review_passed = true, ai_review_score = $2
         WHERE task_id = $3 AND student_id = $4`, ['pending_company_review', review.score, taskId, studentId]);
            // 通知企业
         ...
         WHERE task_id = $3 AND student_id = $4`, ['needs_revision', review.score, taskId, studentId]);
        }
        // 保存反馈消息
        await (0, db_1.query)(`INSERT INTO mentor_conversations 
       (student_id, task_id, stage, trigger_type, mentor_response, created_at)
       VALUES ($1, $2, $3, $4, $5, NOW())`, [studentId, taskId, 'quality_review', 'submission_review', review.overallFeedback]);
        res.json({
            success: true,
            data: {
 ...
 * 企业提出修改意见，AI翻译给学生
 */
async function translateCompanyFeedback(req, res, next) {
    try {
        const { taskId } = req.params;
...
请生成翻译后的消息（200字以内）：`;
       ...
       VALUES ($1, $2, $3, $4, $5, $6, NOW())`, [studentId, taskId, 'communication_bridge', 'company_feedback', translatedFeedback, companyFeedback]);
        res.json({
            success: true,
            data: {
                translatedFeedback,
 ...
 * 学生有疑问，AI翻译给企业
 */
async function translateStudentQuestion(req, res, next) {
    try {
        const { taskId } = req.params;
...
请生成翻译后的消息（150字以内）：`;
        ...
        await (0, db_1.query)(`INSERT INTO mentor_conversations 
       (student_id, task_id, stage, trigger_type, student_message, mentor_response, created_at)
       VALUES ($1, $2, $3, $4, $5, $6, NOW())`, [studentId, taskId, 'communication_bridge', 'student_question_to_company', studentQuestion, translatedQuestion]);
        // 发送给企业
        await (0, db_1.query)(`INSERT INTO task_communications 
       (task_id, from_user_id, to_user_id, message, translated_message, created_at)
       VALUES ($1, $2, $3, $4, $5, NOW())`, [taskId, studentId, task.company_id, studentQuestion, translatedQuestion]);
        res.json({
            success: true,
            data: {
                translatedQuestion,
...
//# sourceMappingURL=enhanced-controller.js.map