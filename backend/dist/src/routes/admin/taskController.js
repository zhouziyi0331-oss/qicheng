...
exports.getPendingReviewTasks = getPendingReviewTasks;
...
async function getTaskList(req, res) {
    try {
        const { page = 1, pageSize = 20, keyword, status, track, sortBy = 'created_at', sortOrder = 'DESC' } = req.query;
...
async function getTaskDetail(req, res) {
    try {
        const { id } = req.params;
        const taskInfo = await (0, db_1.query)(`SELECT
        t.*,
        c.company_name as company_name,
 ...
 * 获取待审核项目列表
 */
async function getPendingReviewTasks(req, res) {
    try {
       ...
       LIMIT $1 OFFSET $2`, [Number(pageSize), offset]);
        ...
        logger_1.default.error('获取待审核项目列表失败:', error);
...
async function reviewTask(req, res) {
    try {
        ...
        // 获取任务信息
        const taskInfo = await (0, db_1.query)(`SELECT company_id, title FROM tasks WHERE id = $1`, [id]);
        if (taskInfo.length === 0) {
            return res.status(404).json({ error: '任务不存在' });
        }
        const { company_id, title } = taskInfo[0];
        // 获取企业用户ID
        const companyUser = await (0, db_1.query)(`SELECT user_id FROM company_profiles WHERE id = $1`, [company_id]);
        if (approved) {
            // 审核通过，发布项目
         ...
         WHERE id = $1`, [id]);
            ...
            if (companyUser.length > 0) {
                await (0, db_1.query)(`INSERT INTO system_notifications (user_id, type, title, content, related_type, related_id)
           VALUES ($1, 'task_approved', '任务审核通过', $2, 'task', $3)`, [companyUser[0].user_id, `您的任务"${title}"已通过审核，现已发布。`, id]);
            }
        }
        else {
            // 审核拒绝
         ...
         WHERE id = $2`, [reason, id]);
            ...
            if (companyUser.length > 0) {
                await (0, db_1.query)(`INSERT INTO system_notifications (user_id, type, title, content, related_type, related_id)
           VALUES ($1, 'task_rejected', '任务审核未通过', $2, 'task', $3)`, [companyUser[0].user_id, `您的任务"${title}"未通过审核。原因：${reason || '不符合发布要求'}`, id]);
            }
        }
        // 记录操作日志
        const adminId = req.user.userId;
        await (0, db_1.query)(`INSERT INTO admin_operation_logs (admin_id, operation_type, target_type, target_id, details)
       VALUES ($1, 'review_task', 'task', $2, $3)`, [adminId, id, JSON.stringify({ approved, reason, taskTitle: title })]);
        res.json({ message: approved ? '项目已发布' : '项目已拒绝' });
    }
    catch (error) {
        logger_1.default.error('审核项目失败:', error);
...
async function toggleTaskStatus(req, res) {
    try {
       ...
       WHERE id = $2`, [newStatus, id]);
        ...
        const adminId = req.user.userId;
        await (0, db_1.query)(`INSERT INTO admin_operation_logs (admin_id, operation_type, target_type, target_id, details)
       VALUES ($1, 'toggle_task_status', 'task', $2, $3)`, [adminId, id, JSON.stringify({ action, reason })]);
        res.json({ message: action === 'publish' ? '项目已上架' : '项目已下架' });
    }
    catch (error) {
        logger_1.default.error('更新项目状态失败:', error);
...
async function getTaskCategories(req, res) {
    try {
       ...
       ORDER BY count DESC`);
        // 标签统计
        const tagStats = await (0, db_1.query)(`SELECT
        unnest(tags) as tag,
        COUNT(*) as count
       FROM tasks
       ...
       LIMIT 50`);
        ...
        logger_1.default.error('获取项目分类统计失败:', error);
...
async function updateTask(req, res) {
    try {
       ...
       WHERE id = $${paramIndex}`, params);
        ...
        const adminId = req.user.userId;
        await (0, db_1.query)(`INSERT INTO admin_operation_logs (admin_id, operation_type, target_type, target_id, details)
       VALUES ($1, 'update_task', 'task', $2, $3)`, [adminId, id, JSON.stringify(updates)]);
