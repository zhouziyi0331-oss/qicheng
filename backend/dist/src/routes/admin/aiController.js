 ...
 * 获取AI调用日志列表
 */
async function getAICallLogs(req, res) {
    try {
        const { page = 1, pageSize = 20, userId, model, startDate, endDate, sortBy = 'created_at', sortOrder = 'DESC' } = req.query;
 ...
 * 获取AI调用统计
 */
async function getAICallStats(req, res) {
    try {
        ...
        const whereClause = conditions.length > 0 ? `WHERE ${conditions.join(' AND ')}` : '';
        // 总体统计
        const totalStats = await (0, db_1.query)(`SELECT
       ...
       ${whereClause}`, params);
        // 按模型统计
        const modelStats = await (0, db_1.query)(`SELECT
        model,
        COUNT(*) as call_count,
        SUM(total_tokens) as total_tokens,
        SUM(cost) as total_cost,
       ...
       ${whereClause}
       GROUP BY model
       ORDER BY call_count DESC`, params);
        // 按日期统计（最近30天）
        const dailyStats = await (0, db_1.query)(`SELECT
...
async function getPromptTemplates(req, res) {
    try {
       ...
       ${whereClause}`, params);
        const total = parseInt(countResult[0].total);
        params.push(Number(pageSize), offset);
        const templates = await (0, db_1.query)(`SELECT
        id,
        name,
        category,
        description,
...
async function createPromptTemplate(req, res) {
    try {
        ...
        const result = await (0, db_1.query)(`INSERT INTO ai_prompt_templates (name, category, description, template, variables, status)
       VALUES ($1, $2, $3, $4, $5, 'active')
       RETURNING id`, [name, category, description, template, JSON.stringify(variables)]);
...
async function updatePromptTemplate(req, res) {
    try {
        ...
        await (0, db_1.query)(`UPDATE ai_prompt_templates
       SET name = $1,
           category = $2,
           description = $3,
           template = $4,
           ...
           status = $6,
           updated_at = NOW()
       WHERE id = $7`, [name, category, description, template, JSON.stringify(variables), status, id]);
...
async function deletePromptTemplate(req, res) {
    try {
        const { id } = req.params;
        await (0, db_1.query)(`DELETE FROM ai_prompt_templates WHERE id = $1`, [id]);
