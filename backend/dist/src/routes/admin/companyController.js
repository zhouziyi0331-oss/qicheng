...
async function getCompanyList(req, res) {
    try {
        const { page = 1, pageSize = 20, keyword, verificationStatus, sortBy = 'created_at', sortOrder = 'DESC' } = req.query;
       ...
       LIMIT $${paramIndex} OFFSET $${paramIndex + 1}`, params);
        ...
        logger_1.default.error('获取企业列表失败:', error);
...
async function getCompanyDetail(req, res) {
    try {
        const { id } = req.params;
        // 获取企业基本信息
        const companyInfo = await (0, db_1.query)(`SELECT
        cp.id,
        cp.user_id,
        cp.company_name,
        cp.industry,
       ...
       LIMIT 10`, [id]);
        ...
        logger_1.default.error('获取企业详情失败:', error);
...
async function verifyCompany(req, res) {
    try {
        ...
        // 获取企业信息
        const companyInfo = await (0, db_1.query)(`SELECT user_id, company_name FROM company_profiles WHERE id = $1`, [id]);
        if (companyInfo.length === 0) {
            return res.status(404).json({ message: '企业不存在' });
        }
        const { user_id, company_name } = companyInfo[0];
        if (approved) {
            await (0, db_1.query)(`UPDATE company_profiles
         SET verified_at = NOW(),
             updated_at = NOW()
         WHERE id = $1`, [id]);
            // 发送通知给企业
            await (0, db_1.query)(`INSERT INTO system_notifications (user_id, type, title, content, related_type, related_id)
         VALUES ($1, 'company_verified', '企业认证通过', $2, 'company', $3)`, [user_id, `恭喜！您的企业"${company_name}"已通过认证审核，现在可以发布任务了。`, id]);
        }
        else {
            // 拒绝认证
            await (0, db_1.query)(`UPDATE company_profiles
         ...
         WHERE id = $1`, [id]);
            // 发送通知给企业
            await (0, db_1.query)(`INSERT INTO system_notifications (user_id, type, title, content, related_type, related_id)
         VALUES ($1, 'company_rejected', '企业认证未通过', $2, 'company', $3)`, [user_id, `很抱歉，您的企业"${company_name}"认证未通过。原因：${reason || '不符合认证要求'}`, id]);
        }
        // 记录操作日志
        const adminId = req.user.userId;
        await (0, db_1.query)(`INSERT INTO admin_operation_logs (admin_id, operation_type, target_type, target_id, details)
       VALUES ($1, 'verify_company', 'company', $2, $3)`, [adminId, id, JSON.stringify({ approved, reason, companyName: company_name })]);
        res.json({ message: approved ? '认证通过' : '认证拒绝' });
    }
    catch (error) {
        logger_1.default.error('审核企业认证失败:', error);
...
async function toggleBlacklist(req, res) {
    try {
       ...
       SET is_blacklisted = $1,
           blacklist_reason = $2,
           updated_at = NOW()
       WHERE id = $3`, [isBlacklisted, reason || null, id]);
        res.json({ message: isBlacklisted ? '已加入黑名单' : '已移出黑名单' });
    }
    catch (error) {
        logger_1.default.error('操作黑名单失败:', error);
...
async function getPendingVerifications(req, res) {
    try {
       ...
       WHERE verified_at IS NULL`);
        ...
        logger_1.default.error('获取待审核企业列表失败:', error);
 ...
 * 获取企业发布的任务列表
 */
async function getCompanyTasks(req, res) {
    try {
        const { id } = req.params;
        const { page = '1', pageSize = '20' } = req.query;
        const offset = (Number(page) - 1) * Number(pageSize);
       ...
       WHERE company_id = $1`, [id]);
        ...
        logger_1.default.error('获取企业任务列表失败:', error);
