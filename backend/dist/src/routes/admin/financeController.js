...
async function getFinanceOverview(req, res) {
    try {
        // 总收入（所有账户的累计收入）
        const totalRevenue = await (0, db_1.query)(`SELECT COALESCE(SUM(total_earned), 0) as total
       FROM escrow_accounts`);
        // 待结算金额（所有账户的待结算余额）
        const pendingSettlement = await (0, db_1.query)(`SELECT COALESCE(SUM(pending_balance), 0) as total
       FROM escrow_accounts`);
        // 可提现余额（所有账户的可提现余额）
        const availableBalance = await (0, db_1.query)(`SELECT COALESCE(SUM(available_balance), 0) as total
       FROM escrow_accounts`);
        // 已提现金额
        const totalWithdrawn = await (0, db_1.query)(`SELECT COALESCE(SUM(total_withdrawn), 0) as total
       FROM escrow_accounts`);
        // 本月收入
        const monthlyRevenue = await (0, db_1.query)(`SELECT COALESCE(SUM(amount), 0) as total
       FROM income_records
...
async function approveWithdrawal(req, res) {
    try {
        ...
        // 获取提现申请详情
        const withdrawalRequest = await (0, db_1.query)(`SELECT user_id, amount, status
       ...
       WHERE id = $1`, [id]);
        ...
        if (approved) {
            ...
            try {
                // 1. 更新提现申请状态
                await (0, db_1.query)(`UPDATE withdrawal_requests
           SET status = 'approved',
               reviewed_at = NOW()
           WHERE id = $1`, [id]);
                // 2. 更新用户托管账户余额
                await (0, db_1.query)(`UPDATE escrow_accounts
           ...
           VALUES ($1, 'approve_withdrawal', 'withdrawal', $2, $3)`, [adminId, id, JSON.stringify({ amount, userId: user_id })]);
                // 提交事务
         ...
         WHERE id = $1`, [id, rejectReason]);
            ...
            const adminId = req.user.userId;
            await (0, db_1.query)(`INSERT INTO admin_operation_logs (admin_id, operation_type, target_type, target_id, details)
         VALUES ($1, 'reject_withdrawal', 'withdrawal', $2, $3)`, [adminId, id, JSON.stringify({ reason: rejectReason })]);
...
async function getCommissionConfig(req, res) {
    try {
        const config = await (0, db_1.query)(`SELECT key, value, description
       FROM system_configs
...
async function updateCommissionConfig(req, res) {
    try {
        ...
        await (0, db_1.query)(`UPDATE system_configs
       SET value = $2,
           updated_at = NOW()
