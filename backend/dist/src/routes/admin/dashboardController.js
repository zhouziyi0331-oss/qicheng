...
async function getDashboardStats(req, res) {
    try {
        // 今日数据
        const todayStats = await (0, db_1.query)(`
      SELECT
    ...
    `);
        // 总计数据
        const totalStats = await (0, db_1.query)(`
      SELECT
    ...
    `);
        // 近30天趋势数据
        const trendData = await (0, db_1.query)(`
      SELECT
    ...
    `);
        // 订单趋势
        const orderTrend = await (0, db_1.query)(`
      SELECT
        DATE(created_at) as date,
        COUNT(*) as orders,
    ...
    `);
        // 项目类型分布
        const taskTypeDistribution = await (0, db_1.query)(`
      SELECT
        track,
        COUNT(*) as count
      FROM tasks
    ...
    `);
        // 待处理事项
        const pendingItems = await (0, db_1.query)(`
      SELECT
...
async function getAlerts(req, res) {
    try {
        const alerts = [];
        // 超时订单
        const overdueOrders = await (0, db_1.query)(`
    ...
    `);
        ...
        // 待审核企业积压
        const pendingCompanies = await (0, db_1.query)(`
    ...
    `);
        ...
        // 待审核项目积压
        const pendingTasks = await (0, db_1.query)(`
      SELECT COUNT(*) as count
      FROM tasks
    ...
    `);
        ...
        // 待处理提现
        const pendingWithdrawals = await (0, db_1.query)(`
