...
async function getAdminList(req, res) {
    try {
       ...
       ${whereClause}`, params);
        ...
        const admins = await (0, db_1.query)(`SELECT
        a.id,
        a.username,
        a.name,
        a.email,
       ...
       LIMIT $${paramIndex} OFFSET $${paramIndex + 1}`, params);
        ...
        logger_1.default.error('获取管理员列表失败:', error);
...
async function createAdmin(req, res) {
    try {
        ...
        // 检查用户名是否已存在
        const existing = await (0, db_1.query)(`SELECT id FROM admin_users WHERE username = $1`, [username]);
        if (existing.length > 0) {
            return res.status(400).json({ message: '用户名已存在' });
        }
        // 加密密码
        const hashedPassword = await bcrypt_1.default.hash(password, 10);
        const result = await (0, db_1.query)(`INSERT INTO admin_users (username, password, name, email, role_id, status)
       VALUES ($1, $2, $3, $4, $5, 'active')
       RETURNING id`, [username, hashedPassword, name, email, role]);
        res.json({ id: result[0].id, message: '管理员创建成功' });
    }
    catch (error) {
        logger_1.default.error('创建管理员失败:', error);
...
async function updateAdmin(req, res) {
    try {
       ...
       SET name = $1,
           ...
           status = $4,
           updated_at = NOW()
       WHERE id = $5`, [name, email, role, status, id]);
        res.json({ message: '管理员更新成功' });
    }
    catch (error) {
        logger_1.default.error('更新管理员失败:', error);
...
async function resetAdminPassword(req, res) {
    try {
       ...
       WHERE id = $2`, [hashedPassword, id]);
        ...
        logger_1.default.error('重置密码失败:', error);
...
async function deleteAdmin(req, res) {
    try {
        const { id } = req.params;
        await (0, db_1.query)(`DELETE FROM admin_users WHERE id = $1`, [id]);
        res.json({ message: '管理员删除成功' });
    }
    catch (error) {
        logger_1.default.error('删除管理员失败:', error);
...
async function getOperationLogs(req, res) {
    try {
       ...
       LIMIT $${paramIndex} OFFSET $${paramIndex + 1}`, params);
        ...
        logger_1.default.error('获取操作日志失败:', error);
...
async function getSystemConfig(req, res) {
    try {
        const configs = await (0, db_1.query)(`SELECT
        key,
        value,
        description,
        updated_at
       ...
       ORDER BY key`);
        ...
        logger_1.default.error('获取系统配置失败:', error);
...
async function updateSystemConfig(req, res) {
    try {
       ...
       WHERE key = $2`, [value, key]);
        ...
        logger_1.default.error('更新系统配置失败:', error);
