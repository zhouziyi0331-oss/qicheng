...
router.post('/calculate-fee', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const fee = await talentLockService_1.default.calculateLockFee(lock_type, student_level, duration_months);
        res.json({
            success: true,
            data: fee,
        });
...
router.post('/apply', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const { student_id, lock_type, duration_months, monthly_fee, benefits, application_reason } = req.body;
        ...
        const application = await talentLockService_1.default.createLockApplication({
...
router.get('/applications', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const userRole = req.user.role;
        const applications = await talentLockService_1.default.getApplications(userId, userRole);
        res.json({
            success: true,
            data: {
                applications,
...
router.post('/applications/:id/respond', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const application = await talentLockService_1.default.respondToApplication(id, studentId, status, response);
        res.json({
            success: true,
            data: application,
...
router.get('/list', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        let locks;
        if (userRole === 'company') {
            locks = await talentLockService_1.default.getCompanyLocks(userId, status);
        }
        else if (userRole === 'student') {
            locks = await talentLockService_1.default.getStudentLocks(userId, status);
        }
        else {
            return res.status(403).json({
                success: false,
...
router.post('/:id/renew', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const lock = await talentLockService_1.default.renewLock(id, additional_months);
        res.json({
            success: true,
            data: lock,
            message: '续约成功',
...
router.post('/:id/cancel', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        await talentLockService_1.default.cancelLock(id, userId, reason);
        res.json({
            success: true,
            message: '锁定已取消',
        });
...
router.post('/:id/pause', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const { id } = req.params;
        const { reason } = req.body;
        const lock = await talentLockService_1.default.updateLockStatus(id, 'paused', userId, reason);
        res.json({
            success: true,
            data: lock,
            message: '锁定已暂停',
...
router.post('/:id/resume', auth_1.authenticateToken, async (req, res) => {
    try {
        const userId = req.user.id;
        const { id } = req.params;
        const lock = await talentLockService_1.default.updateLockStatus(id, 'active', userId);
        res.json({
            success: true,
            data: lock,
            message: '锁定已恢复',
...
router.get('/student/:studentId/status', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const companyId = req.user.id;
        const status = await talentLockService_1.default.isStudentLocked(studentId, companyId);
        res.json({
            success: true,
            data: status,
        });
...
router.get('/stats/overview', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const stats = await talentLockService_1.default.getLockStats(companyId);
        res.json({
            success: true,
            data: stats,
        });
...
router.get('/pricing/config', async (req, res) => {
    try {
        ...
        const pricing = await talentLockService_1.default.getPricing(lock_type, parseInt(student_level, 10));
        res.json({
            success: true,
            data: pricing,
        });
