 ...
 */
async function getTrackRecommendation(req, res) {
...
async function selectTrack(req, res) {
    ...
    try {
        ...
        // 1. 检查是否已经选择过赛道
        const userResult = await client.query('SELECT selected_track FROM users WHERE id = $1', [userId]);
 ...
 */
async function getTrackPaths(req, res) {
    try {
        // 返回两条赛道的完整路径信息
        const paths = {
            content: {
...
async function getMyTrack(req, res) {
    ...
    try {
       ...
       WHERE u.id = $1
       GROUP BY u.id, u.selected_track, u.track_selected_at, uap.track_analysis`, [userId]);
        if (result.rows.length === 0) {
            return res.status(404).json({
                success: false,
                message: '用户信息未找到'
...
//# sourceMappingURL=trackSelectionController.js.map