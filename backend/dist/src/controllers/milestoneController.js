 ...
 */
const handleSecondTaskComplete = async (req, res) => {
    const { userId } = req.body;
    try {
        // 1. 检查是否是第2单
        const taskCount = await db_1.default.query(`SELECT COUNT(*) as count FROM task_applications
       WHERE student_id = $1 AND status = 'completed'`, [userId]);
        ...
        // 2. 检查是否已推送过
        const notificationCheck = await db_1.default.query(`SELECT id FROM notifications
       WHERE user_id = $1 AND type = 'second_task_milestone'`, [userId]);
        if (notificationCheck.rows.length > 0) {
            return res.json({ success: true, message: '已推送过，跳过' });
        }
        // 3. 创建推送通知
        const message = '你现在可以独立接单了。平台不锁住你，但你的成长报告永远在这里，随时回来更新。';
        await db_1.default.query(`INSERT INTO notifications (user_id, type, title, content, created_at)
       VALUES ($1, 'second_task_milestone', '恭喜完成第2单', $2, NOW())`, [userId, message]);
        res.json({
            success: true,
            message: message
        });
...
exports.handleSecondTaskComplete = handleSecondTaskComplete;
/**
 * 获取OPC故事墙
 * GET /api/story-wall
 */
const getStoryWall = async (req, res) => {
    try {
        // 查询已经独立接单或创建OPC的学生
...
const submitStory = async (req, res) => {
    ...
    try {
        // 检查用户等级是否≥Lv.4
        const userResult = await db_1.default.query(`SELECT level FROM users WHERE id = $1`, [userId]);
        ...
        // 插入或更新故事
        await db_1.default.query(`INSERT INTO story_wall (student_id, story_text, current_status)
       VALUES ($1, $2, $3)
       ON CONFLICT (student_id)
       DO UPDATE SET story_text = $2, current_status = $3, updated_at = NOW()`, [userId, storyText, currentStatus]);
        res.json({
            success: true,
            message: '故事提交成功'
        });
