...
const handleSecondTaskComplete = async (req, res) => {
    ...
    try {
        // 1. 检查是否是第2单
        const taskCount = await (0, db_1.query)(`SELECT COUNT(*) as count FROM task_applications
       WHERE student_id = $1 AND status = 'completed'`, [userId]);
        ...
        // 2. 检查是否已推送过
        const notificationCheck = await (0, db_1.query)(`SELECT id FROM notifications
       WHERE user_id = $1 AND type = 'second_task_milestone'`, [userId]);
        ...
        await (0, db_1.query)(`INSERT INTO notifications (user_id, type, title, content, created_at)
...
const submitStory = async (req, res) => {
    ...
    try {
        // 检查用户等级是否≥Lv.4
        const userResult = await (0, db_1.query)(`SELECT level FROM users WHERE id = $1`, [userId]);
        ...
        // 插入或更新故事
        await (0, db_1.query)(`INSERT INTO story_wall (student_id, story_text, current_status)
       VALUES ($1, $2, $3)
