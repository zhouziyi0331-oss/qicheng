...
const upgradeLevel = async (req, res) => {
    ...
    try {
        // 1. 检查用户是否存在
        const userCheck = await (0, db_1.query)(`SELECT level FROM users WHERE id = $1`, [userId]);
        if (userCheck.length === 0) {
            return res.status(404).json({ error: '用户不存在' });
        }
        const oldLevel = userCheck[0].level;
        // 2. 获取学生的关键成长数据（用于生成个性化导师留言）
        const growthData = await (0, db_1.query)(`SELECT
        u.username,
       ...
       WHERE u.id = $1
       GROUP BY u.id, u.username, u.level`, [userId]);
        ...
        const mentorMessage = generateMentorMessage(studentData.username, studentData.old_level, newLevel, levelInfo.name, {
 ...
 * 生成个性化导师留言
 */
function generateMentorMessage(username, oldLevel, newLevel, newLevelName, growthData) {
    const messages = {
