...
exports.imgSecCheck = imgSecCheck;
exports.msgSecCheck = msgSecCheck;
 ...
 */
async function imgSecCheck(req, res) {
    try {
        ...
        // 调用微信安全检查
        const result = await (0, wechatService_1.checkImageSecurity)(imageBuffer);
        return res.json({
            success: true,
            data: result
        });
 ...
 */
async function msgSecCheck(req, res) {
    try {
        ...
        // 从数据库查询用户的openid
        const userQuery = await database_1.pool.query('SELECT wechat_openid FROM students WHERE id = $1', [userId]);
        ...
        // 调用微信安全检查
        const result = await (0, wechatService_1.checkTextSecurity)(content, wechatOpenid, sceneValue);
        return res.json({
            success: true,
            data: result
        });
