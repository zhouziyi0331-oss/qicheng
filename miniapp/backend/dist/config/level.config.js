...
exports.EXP_RULES = exports.LEVEL_SYSTEM = void 0;
...
exports.calculateProjectExp = calculateProjectExp;
/**
 * 等级体系：6个等级
 *
 * 理念："使命是河"
...
function getLevelConfig(level) {
    return exports.LEVEL_SYSTEM.find(l => l.level === level);
}
/**
 * 根据经验值获取等级
 */
function getLevelByExp(exp) {
    for (let i = exports.LEVEL_SYSTEM.length - 1; i >= 0; i--) {
        if (exp >= exports.LEVEL_SYSTEM[i].requiredExp) {
            return exports.LEVEL_SYSTEM[i];
        }
    }
    return exports.LEVEL_SYSTEM[0]; // 默认返回第一级
}
/**
 * 计算升级进度
 */
function calculateLevelProgress(exp) {
    const currentLevel = getLevelByExp(exp);
    const currentIndex = exports.LEVEL_SYSTEM.findIndex(l => l.level === currentLevel.level);
    const nextLevel = currentIndex < exports.LEVEL_SYSTEM.length - 1 ? exports.LEVEL_SYSTEM[currentIndex + 1] : null;
    if (!nextLevel) {
        // 已经是最高等级
        return {
            currentLevel,
 ...
 * 计算项目完成经验值
 */
function calculateProjectExp(difficulty, rating, isFirstProject) {
    let exp = exports.EXP_RULES.completeProject[difficulty];
    // 客户评价奖励
    if (rating && rating >= 3) {
        exp += exports.EXP_RULES.clientRating[rating];
    }
    // 首次完成奖励
    if (isFirstProject) {
        exp += exports.EXP_RULES.firstTime.firstProject;
    }
    return exp;
}
