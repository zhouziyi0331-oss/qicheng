...
const aiDecomposition_service_1 = require("../services/aiDecomposition.service");
...
class PracticeController {
    ...
    async getProjects(req, res) {
        try {
            ...
            const [projects, total] = await Promise.all([
                PracticeProject_1.PracticeProject.find(query)
                    .sort({ createdAt: -1 })
                    .skip(skip)
                    .limit(Number(limit))
                    .lean(),
                PracticeProject_1.PracticeProject.countDocuments(query)
            ]);
            // 转换为前端需要的格式
    ...
    async getReport(req, res) {
        try {
            ...
            const project = await PracticeProject_1.PracticeProject.findById(id);
            if (!project) {
            ...
            const report = await PracticeReport_1.PracticeReport.findOne({ projectId: id });
            if (!report) {
    ...
    async getStats(req, res) {
        try {
            ...
            const [completed, ongoing, projects] = await Promise.all([
                PracticeProject_1.PracticeProject.countDocuments({ userId, status: 'completed' }),
                PracticeProject_1.PracticeProject.countDocuments({ userId, status: 'ongoing' }),
                PracticeProject_1.PracticeProject.find({ userId, status: 'completed' }).select('budget scores').lean()
            ]);
     ...
     * 生成AI拆解报告
     */
    async generateDecomposition(req, res) {
        try {
            ...
            // 异步生成报告（可以改为后台任务队列）
            const preview = await aiDecomposition_service_1.aiDecompositionService.generateDecompositionReport(projectId, userId);
            res.json({
                success: true,
                message: '报告生成中',
                preview
     ...
     * 查询生成状态
     */
    async getDecompositionStatus(req, res) {
        try {
            const { reportId } = req.params;
            const userId = req.userId;
            const report = await DecompositionReport_1.DecompositionReport.findById(reportId);
            if (!report) {
                return res.status(404).json({ error: '报告不存在' });
            }
    ...
    async unlockDecomposition(req, res) {
        try {
            ...
            // 验证支付
            const hasPaid = await payment_service_1.paymentService.verifyPayment(userId, 'decomposition_report', reportId);
            if (!hasPaid) {
                return res.status(403).json({ error: '请先完成支付' });
            }
            const report = await aiDecomposition_service_1.aiDecompositionService.unlockReport(reportId, userId, paymentAmount);
            res.json({
                success: true,
                message: '解锁成功',
                report
     ...
     * 获取完整报告
     */
    async getDecomposition(req, res) {
        try {
            const { reportId } = req.params;
            const userId = req.userId;
            if (!userId) {
            ...
            const report = await aiDecomposition_service_1.aiDecompositionService.getFullReport(reportId, userId);
            res.json(report);
        }
        catch (error) {
            console.error('获取报告失败:', error);
