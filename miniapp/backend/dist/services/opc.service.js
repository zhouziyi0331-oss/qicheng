...
exports.opcService = exports.OPCService = void 0;
...
class OPCService {
     ...
     */
    async getQuestions() {
        const questions = await OPCQuestion_1.OPCQuestion.find().sort({ questionId: 1 });
        return questions;
    }
    /**
     * 计算各维度分数
     ...
     * 基于维度分数的算法
     */
    calculatePersonalityTag(dimensionScores) {
        const scores = {};
     ...
     * 生成优势和建议
     */
    generateStrengthsAndSuggestions(personalityTag, dimensionScores) {
        const strengthsMap = {
    ...
    async submitAssessment(userId, answers) {
        try {
            logger_1.log.info('开始处理OPC测评提交', { userId, answerCount: answers.length });
            // 1. 计算各维度分数
            ...
            // 3. 生成优势和建议
            const { strengths, suggestions } = this.generateStrengthsAndSuggestions(personalityTag, dimensionScores);
            // 4. 保存测评结果
            const opcResult = await OPCResult_1.OPCResult.create({
            ...
            logger_1.log.info('OPC测评处理成功', { userId, personalityTag });
            // 增加经验值
            ...
            logger_1.log.error('OPC测评提交失败', { userId, error: error.message });
            throw error;
        }
    }
    /**
    ...
    async getLatestResult(userId) {
        const result = await OPCResult_1.OPCResult.findOne({
            userId: new mongoose_1.default.Types.ObjectId(userId)
        }).sort({ completedAt: -1 });
        return result;
    }
    ...
    async getUserResults(userId) {
        const results = await OPCResult_1.OPCResult.find({
            userId: new mongoose_1.default.Types.ObjectId(userId)
        }).sort({ completedAt: -1 });
        return results;
    }
     ...
     * OPC测评完成时增加经验值
     */
    async addExpForOPC(userId) {
        try {
            const { levelService } = require('./level.service');
            await levelService.addExpForOPCCompletion(userId);
            logger_1.log.info('OPC测评经验值已添加', { userId });
        }
        catch (error) {
            logger_1.log.error('添加OPC测评经验值失败', { error: error.message });
            // 不影响主流程
        }
    }
}
exports.OPCService = OPCService;
exports.opcService = new OPCService();
