...
class AchievementService {
    ...
    async initializeUserAchievements(userId) {
        ...
        await Achievement_1.Achievement.insertMany(userAchievements);
    }
    /**
     * 检查并更新项目相关成就
     */
    async checkProjectAchievements(userId) {
        const unlockedAchievements = [];
        // 计算用户完成的项目总数
        const practiceCount = await PracticeProject_1.PracticeProject.countDocuments({
            userId: new mongoose_1.default.Types.ObjectId(userId),
            status: 'completed'
        });
        const realCount = await RealProject_1.RealProject.countDocuments({
            userId: new mongoose_1.default.Types.ObjectId(userId),
            status: 'completed'
        });
        const totalProjects = practiceCount + realCount;
        // 查找所有项目里程碑成就
        const projectAchievements = await Achievement_1.Achievement.find({
            userId: new mongoose_1.default.Types.ObjectId(userId),
            type: 'project_milestone',
            isUnlocked: false
        });
     ...
     * 检查并更新收入相关成就
     */
    async checkIncomeAchievements(userId) {
        const unlockedAchievements = [];
        // 计算用户总收入
        const totalIncome = await Income_1.Income.aggregate([
            {
                $match: {
                    userId: new mongoose_1.default.Types.ObjectId(userId),
                    status: 'confirmed'
        ...
        // 查找所有收入里程碑成就
        const incomeAchievements = await Achievement_1.Achievement.find({
            userId: new mongoose_1.default.Types.ObjectId(userId),
            type: 'income_milestone',
            isUnlocked: false
        });
     ...
     * 检查并更新能力成长相关成就
     */
    async checkAbilityAchievements(userId) {
        const unlockedAchievements = [];
        // 检查是否完成测评
        const assessmentCount = await Assessment_1.Assessment.countDocuments({
            userId: new mongoose_1.default.Types.ObjectId(userId)
        });
        if (assessmentCount > 0) {
            const assessmentAchievement = await Achievement_1.Achievement.findOne({
                userId: new mongoose_1.default.Types.ObjectId(userId),
                title: '能力觉醒',
                isUnlocked: false
            });
        ...
        if (latestRadar) {
            ...
            const allRounderAchievement = await Achievement_1.Achievement.findOne({
                userId: new mongoose_1.default.Types.ObjectId(userId),
                title: '全面提升',
                isUnlocked: false
            });
     ...
     * 获取用户的成就列表
     */
    async getUserAchievements(userId, filter) {
        ...
        return await Achievement_1.Achievement.find(query).sort({ isUnlocked: -1, unlockedAt: -1 });
    }
    /**
     * 获取成就统计
     */
    async getAchievementStats(userId) {
        const allAchievements = await Achievement_1.Achievement.find({
            userId: new mongoose_1.default.Types.ObjectId(userId)
        });
        ...
        // 最近解锁的成就
        const recentUnlocked = await Achievement_1.Achievement.find({
            userId: new mongoose_1.default.Types.ObjectId(userId),
            isUnlocked: true
        })
            .sort({ unlockedAt: -1 })
     ...
     * 切换成就展示状态
     */
    async toggleAchievementDisplay(userId, achievementId) {
        const achievement = await Achievement_1.Achievement.findOne({
            _id: new mongoose_1.default.Types.ObjectId(achievementId),
            userId: new mongoose_1.default.Types.ObjectId(userId)
        });
        if (!achievement) {
     ...
     * 授予经验值奖励（辅助方法）
     */
    async grantExpReward(userId, exp) {
