...
exports.aiDecompositionService = exports.AIDecompositionService = void 0;
...
class AIDecompositionService {
    /**
     * 生成AI实践拆解报告的主函数
     */
    async generateDecompositionReport(projectId, userId) {
        try {
            // 1. 获取项目数据
            const project = await PracticeProject_1.PracticeProject.findById(projectId);
            if (!project) {
            ...
            // 2. 检查是否已有报告
            let report = await DecompositionReport_1.DecompositionReport.findOne({ projectId });
            if (!report) {
                report = new DecompositionReport_1.DecompositionReport({
                    projectId,
                    userId,
                    status: 'generating'
                });
            ...
            // 4. 调用AI生成各模块
            const [abilityBreakdown, problemValue, targetCustomers, acquisitionChannels, growthPath] = await Promise.all([
            ...
            report.generationMetadata = {
                aiModel: openai_1.AI_CONFIG.model,
                promptVersion: '1.0',
     ...
     * 模块1: 能力拆解
     */
    async generateAbilityBreakdown(projectData) {
...
- 证据要从项目交付物和过程中提取，真实可信`;
        try {
            const response = await openai_1.openai.chat.completions.create({
                ...
                temperature: openai_1.AI_CONFIG.temperature,
                max_tokens: openai_1.AI_CONFIG.maxTokens,
     ...
     * 模块2: 问题价值分析
     */
    async generateProblemValue(projectData) {
...
- 关注商业影响，不只是技术问题`;
     ...
     * 模块3: 目标客户分析
     */
    async generateTargetCustomers(projectData) {
...
- 价格要符合市场行情`;
     ...
     * 模块4: 获客渠道推荐
     */
    async generateAcquisitionChannels(projectData) {
...
- 避免泛泛而谈（如"社交媒体营销"），要具体到平台和方法`;
     ...
     * 模块5: 成长路径规划
     */
    async generateGrowthPath(projectData) {
...
- 预期价值要量化（如收入提升、客单价提升等）`;
     ...
     * 解锁报告（付费后）
     */
    async unlockReport(reportId, userId, paymentAmount) {
...
exports.AIDecompositionService = AIDecompositionService;
exports.aiDecompositionService = new AIDecompositionService();
//# sourceMappingURL=aiDecomposition.service.js.map