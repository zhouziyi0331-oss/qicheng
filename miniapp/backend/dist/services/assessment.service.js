...
class AssessmentService {
     ...
     * 生成测评结果（AI分析）
     */
    async generateAssessmentResult(userId, answers) {
        try {
            logger_1.log.info('开始生成测评结果', { userId });
            // 获取用户已有测评次数
            const existingCount = await Assessment_1.Assessment.countDocuments({
...
返回JSON格式，严格遵循schema。`
            ...
            logger_1.log.info('测评结果生成成功', { userId, assessmentId: assessment._id });
            // 触发成就检查
            ...
            logger_1.log.error('生成测评结果失败', { error: error.message, userId });
     ...
     * 构建AI提示词
     */
    buildAssessmentPrompt(answers, assessmentNumber) {
        const answersText = answers.map(a => `问题${a.questionId}: ${JSON.stringify(a.answer)}`).join('\n');
...
返回JSON格式：
{
  ...
  "improvementAreas": ["待提升1", "待提升2"]  // 2个待提升领域
     ...
     * 生成能力雷达图快照
     */
    async generateAbilityRadarSnapshot(userId, assessmentId, abilityScores) {
        try {
            // 获取已有快照数量
            ...
            if (overallScore >= 80)
                ...
                rank = '进阶';
            await AbilityRadar_1.AbilityRadar.create({
                userId: new mongoose_1.default.Types.ObjectId(userId),
                snapshotNumber,
            ...
            logger_1.log.info('能力雷达图快照生成成功', { userId, snapshotNumber });
        }
        catch (error) {
            logger_1.log.error('生成能力雷达图快照失败', { error: error.message, userId });
            // 不抛出错误，允许测评继续
        }
    }
    /**
