...
const OPCResult_1 = require("../models/OPCResult");
...
class MatchService {
     ...
     */
    async matchProjects(userId, limit = 20) {
        try {
            logger_1.log.info('开始智能项目匹配', { userId });
            // 1. 获取用户信息和OPC测评结果
            const user = await User_1.User.findById(userId);
            if (!user) {
                throw new Error('用户不存在');
            }
            const opcResult = await OPCResult_1.OPCResult.findOne({
            ...
            for (const project of projects) {
                ...
                const reason = this.generateMatchReason(user.personalityTag || opcResult?.result.personalityTag, project, isStretch);
                matches.push({
                    project,
                    score,
                    isStretch,
            ...
            logger_1.log.error('项目匹配失败', { userId, error: error.message });
            throw error;
        }
    }
    /**
     * 计算项目匹配分数
     */
    calculateMatchScore(user, opcResult, project) {
        let score = 0; // 从0开始计算
        // 1. 难度匹配（40分）
        ...
        if (opcResult?.result?.dimensionScores) {
            const matchingScore = this.calculateAbilityMatch(opcResult.result.dimensionScores, project.requiredAbilities);
            score += matchingScore;
        }
        // 4. 完成项目经验加分（10分）
     ...
     * 获取项目适合的人格标签
     */
    getProjectSuitablePersonalities(project) {
        // 基于项目类别推断适合的人格标签
        const categoryMap = {
            // 内容运营类
            '内容运营': ['创意执行者', '混合型'],
     ...
     * 获取相关联的人格标签
     */
    getRelatedPersonalities(personalityTag) {
        const relationsMap = {
            '视觉叙事者': ['设计师思维', '创意执行者'],
            '系统构建者': ['逻辑拆解者', '架构思维'],
            '创意执行者': ['视觉叙事者', '表达引领者'],
     ...
     * 计算能力标签匹配度
     */
    calculateAbilityMatch(dimensionScores, requiredAbilities) {
        if (!requiredAbilities || requiredAbilities.length === 0) {
            return 10; // 没有明确要求，给基础分
        }
        // 能力关键词映射到OPC维度
        const abilityToDimensionMap = {
            '视觉': ['visual'],
            '设计': ['visual', 'creative'],
            '创意': ['creative'],
            '系统': ['systematic'],
     ...
     * 基于OPC人格标签生成个性化文案
     */
    generateMatchReason(personalityTag, project, isStretch) {
        // 如果是冒险项目，添加冒险标记
     ...
     * 获取项目详情（包含匹配理由）
     */
    async getProjectWithMatchReason(userId, projectId) {
        try {
            const user = await User_1.User.findById(userId);
            ...
            logger_1.log.error('获取项目匹配信息失败', { userId, projectId, error: error.message });
            throw error;
        }
    }
}
