...
export class PaymentController {
  ...
  async mockPayment(req: Request, res: Response) {
    try {
      ...
      const payment = await paymentService.mockPaymentSuccess(orderId)
  ...
  async wechatNotify(req: Request, res: Response) {
    try {
      ...
      // 3. 查找支付订单
      const payment = await paymentService.getPaymentByOrderId(out_trade_no)
      if (!payment) {
   ...
   * 调用微信支付统一下单API
   */
  private async requestWechatPayment(params: {
    outTradeNo: string
    amount: number
    description: string
    openId: string
    ...
    try {
      // TODO: 实现真实的微信支付调用
      const response = await axios.post('https://api.mch.weixin.qq.com/v3/pay/transactions/jsapi', {
        appid: appId,
        mchid: mchId,
   ...
   * 验证微信签名
   */
  private verifyWechatSignature(data: any): boolean {
    // TODO: 实现微信签名验证
