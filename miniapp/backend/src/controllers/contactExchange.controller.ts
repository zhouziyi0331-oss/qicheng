...
import { ContactExchange } from '../models/ContactExchange'
...
export class ContactExchangeController {
  ...
  async getPartners(req: Request, res: Response) {
    try {
      ...
      // 4. 查询交换状态
      const exchanges = await ContactExchange.find({
        $or: [
      ...
      const formattedPartners = await Promise.all(partners.map(async partner => {
        ...
        // 获取合作项目列表
        const partnerCollabs = await Collaboration.find({
          $or: [
  ...
  async requestExchange(req: Request, res: Response) {
    try {
      ...
      // 2. 检查是否已有交换记录
      let exchange = await ContactExchange.findOne({
        $or: [
      ...
      if (exchange.requesterConfirmed && exchange.partnerConfirmed) {
        ...
        const [requester, partner] = await Promise.all([
          User.findById(exchange.requesterId),
          User.findById(exchange.partnerId)
        ])
  ...
  async getExchangeStatus(req: Request, res: Response) {
    try {
      ...
      const exchange = await ContactExchange.findOne({
        $or: [
          { requesterId: userId, partnerId },
          { requesterId: partnerId, partnerId: userId }
        ]
   ...
   * 获取已交换的联系方式
   */
  async getExchangedContact(req: Request, res: Response) {
    try {
      const { partnerId } = req.params
      const userId = req.userId
  ...
  // 辅助方法
  private getExchangeStatusHelper(exchange: any, userId: string): string {
    if (!exchange) return 'available'
    if (exchange.status === 'confirmed') return 'confirmed'
    return 'pending'
  }

  private isMyConfirmed(exchange: any, userId: string): boolean {
  ...
  private isPartnerConfirmed(exchange: any, userId: string): boolean {
    if (!exchange) return false
    return exchange.requesterId === userId
      ? exchange.partnerConfirmed
      : exchange.requesterConfirmed
...
export const contactExchangeController = new ContactExchangeController()
