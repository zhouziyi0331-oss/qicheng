...
import { GraduationReport } from '../models/GraduationReport'
...
async function generatePersonalizedTestData() {
  try {
    ...
    await Promise.all([
      ...
      AbilityRadar.deleteMany({}),
      RealProject.deleteMany({}),
      Income.deleteMany({}),
      Withdrawal.deleteMany({}),
      ComparisonReport.deleteMany({}),
      DynamicGrowthPath.deleteMany({}),
      GraduationReport.deleteMany({})
    ])
    ...
    const graduationReport = await GraduationReport.create({
    ...
    console.log('生成的数据：')
    console.log(`  • OC测评: ${await Assessment.countDocuments({})} 条`)
    console.log(`  • 能力雷达图: ${await AbilityRadar.countDocuments({})} 个快照`)
    console.log(`  • 真实项目: ${await RealProject.countDocuments({})} 个`)
    console.log(`  • 收入记录: ${await Income.countDocuments({})} 条`)
    console.log(`  • 提现记录: ${await Withdrawal.countDocuments({})} 条`)
    console.log(`  • 对比报告: ${await ComparisonReport.countDocuments({})} 份`)
    console.log(`  • 成长路径: ${await DynamicGrowthPath.countDocuments({})} 份`)
    console.log(`  • 毕业报告: ${await GraduationReport.countDocuments({})} 份\n`)
...
async function createAssessment(userId: mongoose.Types.ObjectId, number: number, scores: any) {
...
async function createAbilityRadar(
  userId: mongoose.Types.ObjectId,
  number: number,
  triggerType: 'assessment' | 'project_completed',
  ...
  return await AbilityRadar.create({
    userId,
...
async function createWithdrawal(userId: mongoose.Types.ObjectId, amount: number) {
...
async function createComparisonReport(
  ...
  const dimensionChanges = afterRadar.dimensions.map((afterDim: any, index: number) => {
  ...
  return await ComparisonReport.create({
    userId,
...
async function createDynamicGrowthPath(
  userId: mongoose.Types.ObjectId,
  latestRadar: any,
  projectCount: number,
  totalEarnings: number
) {
  return await DynamicGrowthPath.create({
    userId,
    versionNumber: 1,
    currentState: {
...
function getRank(score: number): '新手' | '进阶' | '熟练' | '专家' | '大师' {
  if (score >= 90) return '大师'
  if (score >= 80) return '专家'
  if (score >= 70) return '熟练'
  if (score >= 50) return '进阶'
  return '新手'
}
