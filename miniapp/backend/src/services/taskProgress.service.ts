...
export class TaskProgressService {
  ...
  async generateTaskDecomposition(
    ...
    const project = projectType === 'practice'
      ? await PracticeProject.findById(projectId)
      : await RealProject.findById(projectId)
...
${projectType === 'real' && 'requirements' in project ? `项目需求：${(project as any).requirements}` : ''}
...
请根据以上信息，生成3-6个具体任务的详细拆解，包括思路、步骤、时间估算和建议。`
    ...
    const taskProgress = new TaskProgress({
      ...
      projectSnapshot: {
        ...
        difficulty: projectType === 'real' && 'difficulty' in project ? (project as any).difficulty : 'medium'
      },

      tasks: aiResult.tasks.map((task: any) => ({
        ...task,
        status: task.taskNumber === 1 ? 'pending' : 'pending',
        progress: 0,
        deliverables: task.deliverables.map((d: any) => ({
          ...d,
          completed: false
        }))
      })),
   ...
   * 获取项目的任务进度
   */
  async getTaskProgress(userId: string, projectId: string): Promise<ITaskProgress | null> {
    return await TaskProgress.findOne({
      userId: new mongoose.Types.ObjectId(userId),
      projectId: new mongoose.Types.ObjectId(projectId)
    })
  }
   ...
   * 生成项目完成总结（AI分析）
   */
  async generateProjectSummary(userId: string, progressId: string): Promise<ITaskProgress | null> {
    const taskProgress = await TaskProgress.findOne({
      _id: new mongoose.Types.ObjectId(progressId),
      userId: new mongoose.Types.ObjectId(userId)
    })
...
请生成项目总结，包括关键成就、提升的能力、下一步建议。`
   ...
   * 获取用户所有项目的任务进度列表
   */
  async getUserTaskProgressList(userId: string, status?: string): Promise<ITaskProgress[]> {
    ...
    return await TaskProgress.find(query).sort({ updatedAt: -1 })
  }
