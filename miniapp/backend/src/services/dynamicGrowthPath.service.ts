 ...
 */
export class DynamicGrowthPathService {
   ...
   * 3. 用户手动请求
   */
  async generateGrowthPath(userId: string) {
    try {
      log.info('开始生成成长路径', { userId })
      ...
      // 获取版本号
      const existingCount = await DynamicGrowthPath.countDocuments({
        userId: new mongoose.Types.ObjectId(userId)
      })
      const versionNumber = existingCount + 1

      // 保存成长路径
      const growthPath = await DynamicGrowthPath.create({
        userId: new mongoose.Types.ObjectId(userId),
        versionNumber,
      ...
      log.info('成长路径生成成功', { userId, versionNumber })

      return growthPath

    } catch (error: any) {
      log.error('生成成长路径失败', { error: error.message, userId })
  ...
  private async collectUserData(userId: string) {
    ...
    // 获取最近3个项目
    const recentProjects = await RealProject.find({
      userId: userObjectId,
      status: 'completed'
    })
      .sort({ completedAt: -1 })
   ...
   * AI生成成长路径
   */
  private async generateAIGrowthPath(userData: any) {
    try {
...
**能力详情：**
${userData.allDimensions.map((d: any) => `- ${d.name}: ${d.score}分 (${d.level})`).join('\n')}

**最近完成的项目：**
${userData.recentProjects.map((p: any, i: number) =>
  `${i + 1}. ${p.title} (${p.category}, ${p.difficulty})`
).join('\n')}
...
4. 预测要基于数据，不要过于乐观`
      ...
      log.error('AI生成成长路径失败', { error: error.message })
      throw error
    }
  }
   ...
   * 获取最新成长路径
   */
  async getLatestGrowthPath(userId: string) {
    const path = await DynamicGrowthPath.findOne({
      userId: new mongoose.Types.ObjectId(userId)
   ...
   * 获取成长路径历史
   */
  async getGrowthPathHistory(userId: string) {
    const paths = await DynamicGrowthPath.find({
      userId: new mongoose.Types.ObjectId(userId)
    }).sort({ versionNumber: 1 })
   ...
   * 更新里程碑状态
   */
  async updateMilestone(userId: string, milestoneTitle: string, completed: boolean) {
    ...
    log.info('里程碑状态更新', { userId, milestoneTitle, completed })

    return latestPath
  }
}

export const dynamicGrowthPathService = new DynamicGrowthPathService()
