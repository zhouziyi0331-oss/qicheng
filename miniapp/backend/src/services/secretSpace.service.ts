import { SecretSpace, ISecretSpace } from '../models/SecretSpace'
 ...
 */
export class SecretSpaceService {

  /**
   * 初始化用户的秘密空间（注册时调用）
   */
  async initializeSecretSpace(userId: string): Promise<ISecretSpace> {
    const secretSpace = new SecretSpace({
   ...
   * 获取用户的秘密空间
   */
  async getSecretSpace(userId: string): Promise<ISecretSpace | null> {
    const secretSpace = await SecretSpace.findOne({
      userId: new mongoose.Types.ObjectId(userId)
    })
  ...
  async updatePrivateNote(
    ...
    const note = secretSpace.privateNotes.find((n: any) => n._id.toString() === noteId)
    if (!note) {
   ...
   * 删除私密笔记
   */
  async deletePrivateNote(userId: string, noteId: string): Promise<ISecretSpace | null> {
    const secretSpace = await this.getSecretSpace(userId)
    if (!secretSpace) {
      throw new Error('秘密空间不存在')
    }

    secretSpace.privateNotes = secretSpace.privateNotes.filter(
   ...
   * 完成个人里程碑
   */
  async completeMilestone(userId: string, milestoneId: string): Promise<ISecretSpace | null> {
    const secretSpace = await this.getSecretSpace(userId)
    if (!secretSpace) {
      throw new Error('秘密空间不存在')
    }

    const milestone = secretSpace.personalMilestones.find((m: any) => m._id.toString() === milestoneId)
    if (!milestone) {
  ...
  async updateSettings(
    ...
    if (settings.theme) secretSpace.settings.theme = settings.theme
    if (settings.backgroundColor) secretSpace.settings.backgroundColor = settings.backgroundColor
    if (settings.isPublic !== undefined) secretSpace.settings.isPublic = settings.isPublic
   ...
   * 获取空间统计
   */
  async getSpaceStats(userId: string): Promise<{
...
export const secretSpaceService = new SecretSpaceService()
