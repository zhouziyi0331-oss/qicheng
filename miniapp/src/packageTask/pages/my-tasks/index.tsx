...
export default function MyTasks() {
  ...
  const loadTasks = async () => {
    // 先显示缓存数据
    if (tasksCache[activeTab]) {
      setTasks(tasksCache[activeTab])
    }
    ...
    try {
      ...
      setTasks(data)
      setTasksCache(prev => ({ ...prev, [activeTab]: data }))
    } catch (err: any) {
      Taro.showToast({
  ...
  const handleTaskClick = (task: Task) => {
    if (task.status === 'working') {
      Taro.navigateTo({ url: `/packageTask/pages/tasks/working?id=${task.id}` })
      ...
      Taro.navigateTo({ url: `/packageTask/pages/tasks/detail?id=${task.id}` })
    }
  }

  const handleContinue = (taskId: string) => {
    Taro.navigateTo({ url: `/packageTask/pages/tasks/working?id=${taskId}` })
  }

  const handleResubmit = (taskId: string) => {
    Taro.navigateTo({ url: `/packageTask/pages/tasks/submit?id=${taskId}` })
  }
  ...
  const handleViewPayment = (taskId: string) => {
    Taro.navigateTo({
      url: `/pages/payment-status/index?orderId=${taskId}`
    })
  }
      ...
      <ScrollView className="tasks-list" scrollY>
        {loading ? (
          ...
          <View className="empty-state">
            ...
            <Text className="empty-text">暂无{activeTab === 'working' ? '进行中' : activeTab === 'submitted' ? '待验收' : activeTab === 'completed' ? '已完成' : '被打回'}的任务</Text>
            {activeTab === 'working' && (
              <Button variant="primary" onClick={() => Taro.switchTab({ url: '/pages/tasks/index' })}>
                去接任务
              </Button>
            )}
          </View>
