...
import { companyRatingAPI } from '../../../services/api';
...
export default function RateTask() {
  ...
  const loadTaskInfo = async () => {
    try {
      const res = await companyRatingAPI.checkCanRate(taskId!);

      if (res.success && res.data.canRate) {
  ...
  const loadTagPresets = async () => {
    try {
      const res = await companyRatingAPI.getTagPresets();
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await companyRatingAPI.submitRating({
