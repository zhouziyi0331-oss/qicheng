...
import { taskAPI, taskFlowAPI } from '../../../services/api'
...
export default function TaskWorking() {
  ...
  const handleSubmit = () => {
    Taro.showModal({
      ...
      success: (res) => {
        if (res.confirm) {
          Taro.navigateTo({
            url: `/packageTask/pages/tasks/submit?id=${id}`
          })
        }
      }
    })
  ...
  const handleProgressSubmit = async () => {
    ...
    try {
      ...
      const res = await taskFlowAPI.updateProgress(id!, {
        progressPercent,
        note: progressNote
      })
      ...
      if (res.success) {
        ...
        Taro.showToast({ title: res.message || '更新失败', icon: 'none' })
      }
    } catch (err) {
      Taro.hideLoading()
