...
export default function Login() {
  ...
  const [accountType, setAccountType] = useState<'student' | 'enterprise' | null>(null)

  useEffect(() => {
    // 从URL参数获取account_type
    const params = Taro.getCurrentInstance().router?.params
    if (params?.account_type) {
      setAccountType(params.account_type as 'student' | 'enterprise')
    }
  }, [])
  ...
  const handleDevLogin = () => {
    ...
    Taro.setStorageSync('access_token', mockToken)
  ...
  const handleWechatLogin = async () => {
    ...
    try {
      ...
      const res = await Taro.request({
        url: getApiUrl('/api/v1/auth/wechat/login'),
        method: 'POST',
        data: {
          code: loginRes.code,
          userType: 'student',
