...
const CHALLENGE_TYPES = {
  skill_building: { name: '技能提升', icon: '🎯', color: '#3B82F6' },
  mindset_shift: { name: '思维转变', icon: '🧠', color: '#8B5CF6' },
  habit_formation: { name: '习惯养成', icon: '⚡', color: '#10B981' },
  fear_facing: { name: '面对恐惧', icon: '💪', color: '#EF4444' },
  exploration: { name: '探索尝试', icon: '🚀', color: '#F59E0B' }
}
...
export default function GrowthChallenges() {
  ...
  const handleAcceptChallenge = async (challengeId: string) => {
    try {
      ...
      // TODO: 调用接受挑战API
      Taro.showToast({ title: '挑战已接受', icon: 'success' })
      handleCloseDetail()
      loadChallenges()
    } catch (error: any) {
        ...
        <View className='detail-modal' onClick={handleCloseDetail}>
          <View className='detail-content' onClick={(e) => e.stopPropagation()}>
            ...
            <View className='detail-section'>
              <Text className='section-title'>📋 挑战描述</Text>
              <Text className='section-text'>{selectedChallenge.description}</Text>
            </View>

            <View className='detail-section'>
              <Text className='section-title'>💡 推荐理由</Text>
