...
const STATUS_CONFIG = {
  ideation: { label: '选主题', color: '#8B5CF6', icon: '想法' },
  planning: { label: '拆解问题', color: '#EC4899', icon: '▪' },
  executing: { label: '执行中', color: '#10B981', icon: '启动' },
...
export default function PBLProjectDetail() {
  ...
  const handleContinueProject = () => {
    if (project) {
      Taro.navigateTo({
        url: `/pages/mentor/index?projectId=${project.id}&projectTitle=${project.title}`
      })
    }
  }
        ...
        {activeTab === 'overview' && (
          <View className='overview-content'>
            ...
            <View className='section'>
              <Text className='section-title'>想法 初始问题</Text>
            ...
            {project.learningGoals && project.learningGoals.length > 0 && (
              <View className='section'>
                <Text className='section-title'>目标 学习目标</Text>
                <View className='section-card'>
              ...
              project.files.map(file => (
                <View key={file.id} className='file-item'>
                  <Text className='file-icon'>
                    {file.fileType === 'code' ? '●' : file.fileType === 'document' ? '笔记' : '●'}
                  </Text>
