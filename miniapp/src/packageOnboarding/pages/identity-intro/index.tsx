...
const IDENTITY_TYPES = [
  {
    ...
    icon: '◆',
    bg: 'linear-gradient(160deg, #1A0A04 0%, #3D1F10 40%, #6B3520 100%)',
    badgeGradient: 'linear-gradient(135deg, #BC6446, #D88760)',
  ...
  },
  {
    ...
    en: 'System Builder',
    icon: '▲',
    bg: 'linear-gradient(160deg, #0A1A10 0%, #1A3A20 40%, #2A5A30 100%)',
    badgeGradient: 'linear-gradient(135deg, #3A8A84, #5ABFB8)',
  ...
  },
  {
    ...
    icon: '○',
    bg: 'linear-gradient(160deg, #1A0A1A 0%, #3A1A3A 40%, #5A2A5A 100%)',
    badgeGradient: 'linear-gradient(135deg, #9B8EC4, #C4B8E8)',
  ...
  },
  {
    ...
    en: 'Logic Analyzer',
    icon: '◇',
    bg: 'linear-gradient(160deg, #0A0A1A 0%, #1A1A3A 40%, #2A2A5A 100%)',
    badgeGradient: 'linear-gradient(135deg, #5B8FAB, #93AEC1)',
  ...
  },
  {
    ...
    icon: '▼',
    bg: 'linear-gradient(160deg, #0A1A0A 0%, #1A3A1A 40%, #2A5A2A 100%)',
    badgeGradient: 'linear-gradient(135deg, #3A8A84, #BED7D1)',
  ...
  },
  {
    ...
    icon: '◈',
    bg: 'linear-gradient(160deg, #0A1A1A 0%, #1A3A3A 40%, #2A5A5A 100%)',
    badgeGradient: 'linear-gradient(135deg, #BF9E71, #F2CD78)',
  ...
  },
  {
    ...
    en: 'Hybrid Type',
    icon: '◐',
    bg: 'linear-gradient(160deg, #1A1008 0%, #3A2818 40%, #5A4028 100%)',
    badgeGradient: 'linear-gradient(135deg, #BC6446, #9B8EC4)',
    ...
    scores: [72, 70, 68, 72, 70, 68]
  }
]

export default function IdentityIntro() {
  // 这里应该从用户数据获取当前身份类型，暂时使用第一个
  const [currentIdentity] = useState(IDENTITY_TYPES[0])
  ...
  const handleViewRadar = () => {
    Taro.navigateTo({ url: '/packageOnboarding/pages/my-radar/index' })
  }

  return (
    <View className="identity-intro-page" style={{ background: currentIdentity.bg }}>
      {/* 顶部返回按钮 */}
      <View className="intro-header">
        <View className="back-btn" onClick={handleBack}>
          <Text className="back-icon">‹</Text>
        </View>
        <Text className="header-title">我的能力人格</Text>
      ...
      <ScrollView className="scroll-content" scrollY>
        ...
        <View className="hero-card">
          <View
            ...
            style={{ background: currentIdentity.badgeGradient }}
          >
            <Text className="hero-icon">{currentIdentity.icon}</Text>
          </View>
          <Text className="hero-name">{currentIdentity.name}</Text>
          <Text className="hero-en">{currentIdentity.en}</Text>
          <View className="trait-tags">
            {currentIdentity.traits.map((trait, index) => (
              <View key={index} className="trait-tag">
        ...
        <View className="desc-card">
          <Text className="desc-title">你是谁</Text>
          <Text className="desc-text">{currentIdentity.desc}</Text>
        </View>
        ...
        <View className="insight-card">
          ...
          <Text className="insight-text">{currentIdentity.insight}</Text>
        </View>

        {/* 查看完整雷达按钮 */}
        <View className="action-button" onClick={handleViewRadar}>
          <Text className="button-text">查看完整六维雷达 →</Text>
        </View>
