...
export default function Wallet() {
  ...
  const loadAccountInfo = async () => {
    try {
      ...
      const res = await Taro.request({
        url: getApiUrl('/api/v1/escrow/account'),
        method: 'GET',
        header: {
  ...
  const loadTransactions = async () => {
    try {
      const token = tokenManager.getAccessToken();
      const res = await Taro.request({
        url: getApiUrl('/api/v1/escrow/transactions'),
        method: 'GET',
        header: {
          Authorization: `Bearer ${token}`,
        },
  ...
  const handleViewPaymentStatus = () => {
    // 获取最近的订单ID，或跳转到我的任务页面
    Taro.navigateTo({
      url: '/pages/my-tasks/index?tab=completed',
    });
  };
  ...
  return (
    <View className="wallet-page">
      ...
      {/* 快捷入口 */}
      <View className={`quick-actions ${currentStep >= 3 ? 'stats-show' : ''}`}>
        <View className="action-item" onClick={handleViewPaymentStatus}>
          <Text className="action-icon">●</Text>
          <Text className="action-text">查看分阶段资金</Text>
          <Text className="action-arrow">›</Text>
        </View>
        <View className="action-item" onClick={() => Taro.navigateTo({ url: '/pages/wallet/withdraw-history/index' })}>
          <Text className="action-icon">▪</Text>
          <Text className="action-text">提现记录</Text>
          <Text className="action-arrow">›</Text>
        </View>
      </View>
